
kernel/kernel:     file format elf64-littleriscv


Disassembly of section .text:

0000000080000000 <_entry>:
_entry:
        # set up a stack for C.
        # stack0 is declared in start.c,
        # with a 4096-byte stack per CPU.
        # sp = stack0 + ((hartid + 1) * 4096)
        la sp, stack0
    80000000:	0000a117          	auipc	sp,0xa
    80000004:	2a813103          	ld	sp,680(sp) # 8000a2a8 <_GLOBAL_OFFSET_TABLE_+0x8>
        li a0, 1024*4
    80000008:	6505                	lui	a0,0x1
        csrr a1, mhartid
    8000000a:	f14025f3          	csrr	a1,mhartid
        addi a1, a1, 1
    8000000e:	0585                	addi	a1,a1,1
        mul a0, a0, a1
    80000010:	02b50533          	mul	a0,a0,a1
        add sp, sp, a0
    80000014:	912a                	add	sp,sp,a0
        # jump to start() in start.c
        call start
    80000016:	04a000ef          	jal	80000060 <start>

000000008000001a <spin>:
spin:
        j spin
    8000001a:	a001                	j	8000001a <spin>

000000008000001c <timerinit>:
}

// ask each hart to generate timer interrupts.
void
timerinit()
{
    8000001c:	1141                	addi	sp,sp,-16
    8000001e:	e422                	sd	s0,8(sp)
    80000020:	0800                	addi	s0,sp,16
#define MIE_STIE (1L << 5)  // supervisor timer
static inline uint64
r_mie()
{
  uint64 x;
  asm volatile("csrr %0, mie" : "=r" (x) );
    80000022:	304027f3          	csrr	a5,mie
  // enable supervisor-mode timer interrupts.
  w_mie(r_mie() | MIE_STIE);
    80000026:	0207e793          	ori	a5,a5,32
}

static inline void 
w_mie(uint64 x)
{
  asm volatile("csrw mie, %0" : : "r" (x));
    8000002a:	30479073          	csrw	mie,a5
static inline uint64
r_menvcfg()
{
  uint64 x;
  // asm volatile("csrr %0, menvcfg" : "=r" (x) );
  asm volatile("csrr %0, 0x30a" : "=r" (x) );
    8000002e:	30a027f3          	csrr	a5,0x30a
  
  // enable the sstc extension (i.e. stimecmp).
  w_menvcfg(r_menvcfg() | (1L << 63)); 
    80000032:	577d                	li	a4,-1
    80000034:	177e                	slli	a4,a4,0x3f
    80000036:	8fd9                	or	a5,a5,a4

static inline void 
w_menvcfg(uint64 x)
{
  // asm volatile("csrw menvcfg, %0" : : "r" (x));
  asm volatile("csrw 0x30a, %0" : : "r" (x));
    80000038:	30a79073          	csrw	0x30a,a5

static inline uint64
r_mcounteren()
{
  uint64 x;
  asm volatile("csrr %0, mcounteren" : "=r" (x) );
    8000003c:	306027f3          	csrr	a5,mcounteren
  
  // allow supervisor to use stimecmp and time.
  w_mcounteren(r_mcounteren() | 2);
    80000040:	0027e793          	ori	a5,a5,2
  asm volatile("csrw mcounteren, %0" : : "r" (x));
    80000044:	30679073          	csrw	mcounteren,a5
// machine-mode cycle counter
static inline uint64
r_time()
{
  uint64 x;
  asm volatile("csrr %0, time" : "=r" (x) );
    80000048:	c01027f3          	rdtime	a5
  
  // ask for the very first timer interrupt.
  w_stimecmp(r_time() + 1000000);
    8000004c:	000f4737          	lui	a4,0xf4
    80000050:	24070713          	addi	a4,a4,576 # f4240 <_entry-0x7ff0bdc0>
    80000054:	97ba                	add	a5,a5,a4
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    80000056:	14d79073          	csrw	stimecmp,a5
}
    8000005a:	6422                	ld	s0,8(sp)
    8000005c:	0141                	addi	sp,sp,16
    8000005e:	8082                	ret

0000000080000060 <start>:
{
    80000060:	1141                	addi	sp,sp,-16
    80000062:	e406                	sd	ra,8(sp)
    80000064:	e022                	sd	s0,0(sp)
    80000066:	0800                	addi	s0,sp,16
  asm volatile("csrr %0, mstatus" : "=r" (x) );
    80000068:	300027f3          	csrr	a5,mstatus
  x &= ~MSTATUS_MPP_MASK;
    8000006c:	7779                	lui	a4,0xffffe
    8000006e:	7ff70713          	addi	a4,a4,2047 # ffffffffffffe7ff <end+0xffffffff7ffda86f>
    80000072:	8ff9                	and	a5,a5,a4
  x |= MSTATUS_MPP_S;
    80000074:	6705                	lui	a4,0x1
    80000076:	80070713          	addi	a4,a4,-2048 # 800 <_entry-0x7ffff800>
    8000007a:	8fd9                	or	a5,a5,a4
  asm volatile("csrw mstatus, %0" : : "r" (x));
    8000007c:	30079073          	csrw	mstatus,a5
  asm volatile("csrw mepc, %0" : : "r" (x));
    80000080:	00001797          	auipc	a5,0x1
    80000084:	dbc78793          	addi	a5,a5,-580 # 80000e3c <main>
    80000088:	34179073          	csrw	mepc,a5
  asm volatile("csrw satp, %0" : : "r" (x));
    8000008c:	4781                	li	a5,0
    8000008e:	18079073          	csrw	satp,a5
  asm volatile("csrw medeleg, %0" : : "r" (x));
    80000092:	67c1                	lui	a5,0x10
    80000094:	17fd                	addi	a5,a5,-1 # ffff <_entry-0x7fff0001>
    80000096:	30279073          	csrw	medeleg,a5
  asm volatile("csrw mideleg, %0" : : "r" (x));
    8000009a:	30379073          	csrw	mideleg,a5
  asm volatile("csrr %0, sie" : "=r" (x) );
    8000009e:	104027f3          	csrr	a5,sie
  w_sie(r_sie() | SIE_SEIE | SIE_STIE);
    800000a2:	2207e793          	ori	a5,a5,544
  asm volatile("csrw sie, %0" : : "r" (x));
    800000a6:	10479073          	csrw	sie,a5
  asm volatile("csrw pmpaddr0, %0" : : "r" (x));
    800000aa:	57fd                	li	a5,-1
    800000ac:	83a9                	srli	a5,a5,0xa
    800000ae:	3b079073          	csrw	pmpaddr0,a5
  asm volatile("csrw pmpcfg0, %0" : : "r" (x));
    800000b2:	47bd                	li	a5,15
    800000b4:	3a079073          	csrw	pmpcfg0,a5
  timerinit();
    800000b8:	f65ff0ef          	jal	8000001c <timerinit>
  asm volatile("csrr %0, mhartid" : "=r" (x) );
    800000bc:	f14027f3          	csrr	a5,mhartid
  w_tp(id);
    800000c0:	2781                	sext.w	a5,a5
}

static inline void 
w_tp(uint64 x)
{
  asm volatile("mv tp, %0" : : "r" (x));
    800000c2:	823e                	mv	tp,a5
  asm volatile("mret");
    800000c4:	30200073          	mret
}
    800000c8:	60a2                	ld	ra,8(sp)
    800000ca:	6402                	ld	s0,0(sp)
    800000cc:	0141                	addi	sp,sp,16
    800000ce:	8082                	ret

00000000800000d0 <consolewrite>:
// user write() system calls to the console go here.
// uses sleep() and UART interrupts.
//
int
consolewrite(int user_src, uint64 src, int n)
{
    800000d0:	7119                	addi	sp,sp,-128
    800000d2:	fc86                	sd	ra,120(sp)
    800000d4:	f8a2                	sd	s0,112(sp)
    800000d6:	f4a6                	sd	s1,104(sp)
    800000d8:	0100                	addi	s0,sp,128
  char buf[32]; // move batches from user space to uart.
  int i = 0;

  while(i < n){
    800000da:	06c05a63          	blez	a2,8000014e <consolewrite+0x7e>
    800000de:	f0ca                	sd	s2,96(sp)
    800000e0:	ecce                	sd	s3,88(sp)
    800000e2:	e8d2                	sd	s4,80(sp)
    800000e4:	e4d6                	sd	s5,72(sp)
    800000e6:	e0da                	sd	s6,64(sp)
    800000e8:	fc5e                	sd	s7,56(sp)
    800000ea:	f862                	sd	s8,48(sp)
    800000ec:	f466                	sd	s9,40(sp)
    800000ee:	8aaa                	mv	s5,a0
    800000f0:	8b2e                	mv	s6,a1
    800000f2:	8a32                	mv	s4,a2
  int i = 0;
    800000f4:	4481                	li	s1,0
    int nn = sizeof(buf);
    if(nn > n - i)
    800000f6:	02000c13          	li	s8,32
    800000fa:	02000c93          	li	s9,32
      nn = n - i;
    if(either_copyin(buf, user_src, src+i, nn) == -1)
    800000fe:	5bfd                	li	s7,-1
    80000100:	a035                	j	8000012c <consolewrite+0x5c>
    if(nn > n - i)
    80000102:	0009099b          	sext.w	s3,s2
    if(either_copyin(buf, user_src, src+i, nn) == -1)
    80000106:	86ce                	mv	a3,s3
    80000108:	01648633          	add	a2,s1,s6
    8000010c:	85d6                	mv	a1,s5
    8000010e:	f8040513          	addi	a0,s0,-128
    80000112:	178020ef          	jal	8000228a <either_copyin>
    80000116:	03750e63          	beq	a0,s7,80000152 <consolewrite+0x82>
      break;
    uartwrite(buf, nn);
    8000011a:	85ce                	mv	a1,s3
    8000011c:	f8040513          	addi	a0,s0,-128
    80000120:	778000ef          	jal	80000898 <uartwrite>
    i += nn;
    80000124:	009904bb          	addw	s1,s2,s1
  while(i < n){
    80000128:	0144da63          	bge	s1,s4,8000013c <consolewrite+0x6c>
    if(nn > n - i)
    8000012c:	409a093b          	subw	s2,s4,s1
    80000130:	0009079b          	sext.w	a5,s2
    80000134:	fcfc57e3          	bge	s8,a5,80000102 <consolewrite+0x32>
    80000138:	8966                	mv	s2,s9
    8000013a:	b7e1                	j	80000102 <consolewrite+0x32>
    8000013c:	7906                	ld	s2,96(sp)
    8000013e:	69e6                	ld	s3,88(sp)
    80000140:	6a46                	ld	s4,80(sp)
    80000142:	6aa6                	ld	s5,72(sp)
    80000144:	6b06                	ld	s6,64(sp)
    80000146:	7be2                	ld	s7,56(sp)
    80000148:	7c42                	ld	s8,48(sp)
    8000014a:	7ca2                	ld	s9,40(sp)
    8000014c:	a819                	j	80000162 <consolewrite+0x92>
  int i = 0;
    8000014e:	4481                	li	s1,0
    80000150:	a809                	j	80000162 <consolewrite+0x92>
    80000152:	7906                	ld	s2,96(sp)
    80000154:	69e6                	ld	s3,88(sp)
    80000156:	6a46                	ld	s4,80(sp)
    80000158:	6aa6                	ld	s5,72(sp)
    8000015a:	6b06                	ld	s6,64(sp)
    8000015c:	7be2                	ld	s7,56(sp)
    8000015e:	7c42                	ld	s8,48(sp)
    80000160:	7ca2                	ld	s9,40(sp)
  }

  return i;
}
    80000162:	8526                	mv	a0,s1
    80000164:	70e6                	ld	ra,120(sp)
    80000166:	7446                	ld	s0,112(sp)
    80000168:	74a6                	ld	s1,104(sp)
    8000016a:	6109                	addi	sp,sp,128
    8000016c:	8082                	ret

000000008000016e <consoleread>:
// user_dst indicates whether dst is a user
// or kernel address.
//
int
consoleread(int user_dst, uint64 dst, int n)
{
    8000016e:	711d                	addi	sp,sp,-96
    80000170:	ec86                	sd	ra,88(sp)
    80000172:	e8a2                	sd	s0,80(sp)
    80000174:	e4a6                	sd	s1,72(sp)
    80000176:	e0ca                	sd	s2,64(sp)
    80000178:	fc4e                	sd	s3,56(sp)
    8000017a:	f852                	sd	s4,48(sp)
    8000017c:	f456                	sd	s5,40(sp)
    8000017e:	f05a                	sd	s6,32(sp)
    80000180:	1080                	addi	s0,sp,96
    80000182:	8aaa                	mv	s5,a0
    80000184:	8a2e                	mv	s4,a1
    80000186:	89b2                	mv	s3,a2
  uint target;
  int c;
  char cbuf;

  target = n;
    80000188:	00060b1b          	sext.w	s6,a2
  acquire(&cons.lock);
    8000018c:	00012517          	auipc	a0,0x12
    80000190:	16450513          	addi	a0,a0,356 # 800122f0 <cons>
    80000194:	23b000ef          	jal	80000bce <acquire>
  while(n > 0){
    // wait until interrupt handler has put some
    // input into cons.buffer.
    while(cons.r == cons.w){
    80000198:	00012497          	auipc	s1,0x12
    8000019c:	15848493          	addi	s1,s1,344 # 800122f0 <cons>
      if(killed(myproc())){
        release(&cons.lock);
        return -1;
      }
      sleep(&cons.r, &cons.lock);
    800001a0:	00012917          	auipc	s2,0x12
    800001a4:	1e890913          	addi	s2,s2,488 # 80012388 <cons+0x98>
  while(n > 0){
    800001a8:	0b305d63          	blez	s3,80000262 <consoleread+0xf4>
    while(cons.r == cons.w){
    800001ac:	0984a783          	lw	a5,152(s1)
    800001b0:	09c4a703          	lw	a4,156(s1)
    800001b4:	0af71263          	bne	a4,a5,80000258 <consoleread+0xea>
      if(killed(myproc())){
    800001b8:	722010ef          	jal	800018da <myproc>
    800001bc:	761010ef          	jal	8000211c <killed>
    800001c0:	e12d                	bnez	a0,80000222 <consoleread+0xb4>
      sleep(&cons.r, &cons.lock);
    800001c2:	85a6                	mv	a1,s1
    800001c4:	854a                	mv	a0,s2
    800001c6:	51f010ef          	jal	80001ee4 <sleep>
    while(cons.r == cons.w){
    800001ca:	0984a783          	lw	a5,152(s1)
    800001ce:	09c4a703          	lw	a4,156(s1)
    800001d2:	fef703e3          	beq	a4,a5,800001b8 <consoleread+0x4a>
    800001d6:	ec5e                	sd	s7,24(sp)
    }

    c = cons.buf[cons.r++ % INPUT_BUF_SIZE];
    800001d8:	00012717          	auipc	a4,0x12
    800001dc:	11870713          	addi	a4,a4,280 # 800122f0 <cons>
    800001e0:	0017869b          	addiw	a3,a5,1
    800001e4:	08d72c23          	sw	a3,152(a4)
    800001e8:	07f7f693          	andi	a3,a5,127
    800001ec:	9736                	add	a4,a4,a3
    800001ee:	01874703          	lbu	a4,24(a4)
    800001f2:	00070b9b          	sext.w	s7,a4

    if(c == C('D')){  // end-of-file
    800001f6:	4691                	li	a3,4
    800001f8:	04db8663          	beq	s7,a3,80000244 <consoleread+0xd6>
      }
      break;
    }

    // copy the input byte to the user-space buffer.
    cbuf = c;
    800001fc:	fae407a3          	sb	a4,-81(s0)
    if(either_copyout(user_dst, dst, &cbuf, 1) == -1)
    80000200:	4685                	li	a3,1
    80000202:	faf40613          	addi	a2,s0,-81
    80000206:	85d2                	mv	a1,s4
    80000208:	8556                	mv	a0,s5
    8000020a:	036020ef          	jal	80002240 <either_copyout>
    8000020e:	57fd                	li	a5,-1
    80000210:	04f50863          	beq	a0,a5,80000260 <consoleread+0xf2>
      break;

    dst++;
    80000214:	0a05                	addi	s4,s4,1
    --n;
    80000216:	39fd                	addiw	s3,s3,-1

    if(c == '\n'){
    80000218:	47a9                	li	a5,10
    8000021a:	04fb8d63          	beq	s7,a5,80000274 <consoleread+0x106>
    8000021e:	6be2                	ld	s7,24(sp)
    80000220:	b761                	j	800001a8 <consoleread+0x3a>
        release(&cons.lock);
    80000222:	00012517          	auipc	a0,0x12
    80000226:	0ce50513          	addi	a0,a0,206 # 800122f0 <cons>
    8000022a:	23d000ef          	jal	80000c66 <release>
        return -1;
    8000022e:	557d                	li	a0,-1
    }
  }
  release(&cons.lock);

  return target - n;
}
    80000230:	60e6                	ld	ra,88(sp)
    80000232:	6446                	ld	s0,80(sp)
    80000234:	64a6                	ld	s1,72(sp)
    80000236:	6906                	ld	s2,64(sp)
    80000238:	79e2                	ld	s3,56(sp)
    8000023a:	7a42                	ld	s4,48(sp)
    8000023c:	7aa2                	ld	s5,40(sp)
    8000023e:	7b02                	ld	s6,32(sp)
    80000240:	6125                	addi	sp,sp,96
    80000242:	8082                	ret
      if(n < target){
    80000244:	0009871b          	sext.w	a4,s3
    80000248:	01677a63          	bgeu	a4,s6,8000025c <consoleread+0xee>
        cons.r--;
    8000024c:	00012717          	auipc	a4,0x12
    80000250:	12f72e23          	sw	a5,316(a4) # 80012388 <cons+0x98>
    80000254:	6be2                	ld	s7,24(sp)
    80000256:	a031                	j	80000262 <consoleread+0xf4>
    80000258:	ec5e                	sd	s7,24(sp)
    8000025a:	bfbd                	j	800001d8 <consoleread+0x6a>
    8000025c:	6be2                	ld	s7,24(sp)
    8000025e:	a011                	j	80000262 <consoleread+0xf4>
    80000260:	6be2                	ld	s7,24(sp)
  release(&cons.lock);
    80000262:	00012517          	auipc	a0,0x12
    80000266:	08e50513          	addi	a0,a0,142 # 800122f0 <cons>
    8000026a:	1fd000ef          	jal	80000c66 <release>
  return target - n;
    8000026e:	413b053b          	subw	a0,s6,s3
    80000272:	bf7d                	j	80000230 <consoleread+0xc2>
    80000274:	6be2                	ld	s7,24(sp)
    80000276:	b7f5                	j	80000262 <consoleread+0xf4>

0000000080000278 <consputc>:
{
    80000278:	1141                	addi	sp,sp,-16
    8000027a:	e406                	sd	ra,8(sp)
    8000027c:	e022                	sd	s0,0(sp)
    8000027e:	0800                	addi	s0,sp,16
  if(c == BACKSPACE){
    80000280:	10000793          	li	a5,256
    80000284:	00f50863          	beq	a0,a5,80000294 <consputc+0x1c>
    uartputc_sync(c);
    80000288:	6a4000ef          	jal	8000092c <uartputc_sync>
}
    8000028c:	60a2                	ld	ra,8(sp)
    8000028e:	6402                	ld	s0,0(sp)
    80000290:	0141                	addi	sp,sp,16
    80000292:	8082                	ret
    uartputc_sync('\b'); uartputc_sync(' '); uartputc_sync('\b');
    80000294:	4521                	li	a0,8
    80000296:	696000ef          	jal	8000092c <uartputc_sync>
    8000029a:	02000513          	li	a0,32
    8000029e:	68e000ef          	jal	8000092c <uartputc_sync>
    800002a2:	4521                	li	a0,8
    800002a4:	688000ef          	jal	8000092c <uartputc_sync>
    800002a8:	b7d5                	j	8000028c <consputc+0x14>

00000000800002aa <consoleintr>:
// do erase/kill processing, append to cons.buf,
// wake up consoleread() if a whole line has arrived.
//
void
consoleintr(int c)
{
    800002aa:	1101                	addi	sp,sp,-32
    800002ac:	ec06                	sd	ra,24(sp)
    800002ae:	e822                	sd	s0,16(sp)
    800002b0:	e426                	sd	s1,8(sp)
    800002b2:	1000                	addi	s0,sp,32
    800002b4:	84aa                	mv	s1,a0
  acquire(&cons.lock);
    800002b6:	00012517          	auipc	a0,0x12
    800002ba:	03a50513          	addi	a0,a0,58 # 800122f0 <cons>
    800002be:	111000ef          	jal	80000bce <acquire>

  switch(c){
    800002c2:	47d5                	li	a5,21
    800002c4:	08f48f63          	beq	s1,a5,80000362 <consoleintr+0xb8>
    800002c8:	0297c563          	blt	a5,s1,800002f2 <consoleintr+0x48>
    800002cc:	47a1                	li	a5,8
    800002ce:	0ef48463          	beq	s1,a5,800003b6 <consoleintr+0x10c>
    800002d2:	47c1                	li	a5,16
    800002d4:	10f49563          	bne	s1,a5,800003de <consoleintr+0x134>
  case C('P'):  // Print process list.
    procdump();
    800002d8:	7fd010ef          	jal	800022d4 <procdump>
      }
    }
    break;
  }
  
  release(&cons.lock);
    800002dc:	00012517          	auipc	a0,0x12
    800002e0:	01450513          	addi	a0,a0,20 # 800122f0 <cons>
    800002e4:	183000ef          	jal	80000c66 <release>
}
    800002e8:	60e2                	ld	ra,24(sp)
    800002ea:	6442                	ld	s0,16(sp)
    800002ec:	64a2                	ld	s1,8(sp)
    800002ee:	6105                	addi	sp,sp,32
    800002f0:	8082                	ret
  switch(c){
    800002f2:	07f00793          	li	a5,127
    800002f6:	0cf48063          	beq	s1,a5,800003b6 <consoleintr+0x10c>
    if(c != 0 && cons.e-cons.r < INPUT_BUF_SIZE){
    800002fa:	00012717          	auipc	a4,0x12
    800002fe:	ff670713          	addi	a4,a4,-10 # 800122f0 <cons>
    80000302:	0a072783          	lw	a5,160(a4)
    80000306:	09872703          	lw	a4,152(a4)
    8000030a:	9f99                	subw	a5,a5,a4
    8000030c:	07f00713          	li	a4,127
    80000310:	fcf766e3          	bltu	a4,a5,800002dc <consoleintr+0x32>
      c = (c == '\r') ? '\n' : c;
    80000314:	47b5                	li	a5,13
    80000316:	0cf48763          	beq	s1,a5,800003e4 <consoleintr+0x13a>
      consputc(c);
    8000031a:	8526                	mv	a0,s1
    8000031c:	f5dff0ef          	jal	80000278 <consputc>
      cons.buf[cons.e++ % INPUT_BUF_SIZE] = c;
    80000320:	00012797          	auipc	a5,0x12
    80000324:	fd078793          	addi	a5,a5,-48 # 800122f0 <cons>
    80000328:	0a07a683          	lw	a3,160(a5)
    8000032c:	0016871b          	addiw	a4,a3,1
    80000330:	0007061b          	sext.w	a2,a4
    80000334:	0ae7a023          	sw	a4,160(a5)
    80000338:	07f6f693          	andi	a3,a3,127
    8000033c:	97b6                	add	a5,a5,a3
    8000033e:	00978c23          	sb	s1,24(a5)
      if(c == '\n' || c == C('D') || cons.e-cons.r == INPUT_BUF_SIZE){
    80000342:	47a9                	li	a5,10
    80000344:	0cf48563          	beq	s1,a5,8000040e <consoleintr+0x164>
    80000348:	4791                	li	a5,4
    8000034a:	0cf48263          	beq	s1,a5,8000040e <consoleintr+0x164>
    8000034e:	00012797          	auipc	a5,0x12
    80000352:	03a7a783          	lw	a5,58(a5) # 80012388 <cons+0x98>
    80000356:	9f1d                	subw	a4,a4,a5
    80000358:	08000793          	li	a5,128
    8000035c:	f8f710e3          	bne	a4,a5,800002dc <consoleintr+0x32>
    80000360:	a07d                	j	8000040e <consoleintr+0x164>
    80000362:	e04a                	sd	s2,0(sp)
    while(cons.e != cons.w &&
    80000364:	00012717          	auipc	a4,0x12
    80000368:	f8c70713          	addi	a4,a4,-116 # 800122f0 <cons>
    8000036c:	0a072783          	lw	a5,160(a4)
    80000370:	09c72703          	lw	a4,156(a4)
          cons.buf[(cons.e-1) % INPUT_BUF_SIZE] != '\n'){
    80000374:	00012497          	auipc	s1,0x12
    80000378:	f7c48493          	addi	s1,s1,-132 # 800122f0 <cons>
    while(cons.e != cons.w &&
    8000037c:	4929                	li	s2,10
    8000037e:	02f70863          	beq	a4,a5,800003ae <consoleintr+0x104>
          cons.buf[(cons.e-1) % INPUT_BUF_SIZE] != '\n'){
    80000382:	37fd                	addiw	a5,a5,-1
    80000384:	07f7f713          	andi	a4,a5,127
    80000388:	9726                	add	a4,a4,s1
    while(cons.e != cons.w &&
    8000038a:	01874703          	lbu	a4,24(a4)
    8000038e:	03270263          	beq	a4,s2,800003b2 <consoleintr+0x108>
      cons.e--;
    80000392:	0af4a023          	sw	a5,160(s1)
      consputc(BACKSPACE);
    80000396:	10000513          	li	a0,256
    8000039a:	edfff0ef          	jal	80000278 <consputc>
    while(cons.e != cons.w &&
    8000039e:	0a04a783          	lw	a5,160(s1)
    800003a2:	09c4a703          	lw	a4,156(s1)
    800003a6:	fcf71ee3          	bne	a4,a5,80000382 <consoleintr+0xd8>
    800003aa:	6902                	ld	s2,0(sp)
    800003ac:	bf05                	j	800002dc <consoleintr+0x32>
    800003ae:	6902                	ld	s2,0(sp)
    800003b0:	b735                	j	800002dc <consoleintr+0x32>
    800003b2:	6902                	ld	s2,0(sp)
    800003b4:	b725                	j	800002dc <consoleintr+0x32>
    if(cons.e != cons.w){
    800003b6:	00012717          	auipc	a4,0x12
    800003ba:	f3a70713          	addi	a4,a4,-198 # 800122f0 <cons>
    800003be:	0a072783          	lw	a5,160(a4)
    800003c2:	09c72703          	lw	a4,156(a4)
    800003c6:	f0f70be3          	beq	a4,a5,800002dc <consoleintr+0x32>
      cons.e--;
    800003ca:	37fd                	addiw	a5,a5,-1
    800003cc:	00012717          	auipc	a4,0x12
    800003d0:	fcf72223          	sw	a5,-60(a4) # 80012390 <cons+0xa0>
      consputc(BACKSPACE);
    800003d4:	10000513          	li	a0,256
    800003d8:	ea1ff0ef          	jal	80000278 <consputc>
    800003dc:	b701                	j	800002dc <consoleintr+0x32>
    if(c != 0 && cons.e-cons.r < INPUT_BUF_SIZE){
    800003de:	ee048fe3          	beqz	s1,800002dc <consoleintr+0x32>
    800003e2:	bf21                	j	800002fa <consoleintr+0x50>
      consputc(c);
    800003e4:	4529                	li	a0,10
    800003e6:	e93ff0ef          	jal	80000278 <consputc>
      cons.buf[cons.e++ % INPUT_BUF_SIZE] = c;
    800003ea:	00012797          	auipc	a5,0x12
    800003ee:	f0678793          	addi	a5,a5,-250 # 800122f0 <cons>
    800003f2:	0a07a703          	lw	a4,160(a5)
    800003f6:	0017069b          	addiw	a3,a4,1
    800003fa:	0006861b          	sext.w	a2,a3
    800003fe:	0ad7a023          	sw	a3,160(a5)
    80000402:	07f77713          	andi	a4,a4,127
    80000406:	97ba                	add	a5,a5,a4
    80000408:	4729                	li	a4,10
    8000040a:	00e78c23          	sb	a4,24(a5)
        cons.w = cons.e;
    8000040e:	00012797          	auipc	a5,0x12
    80000412:	f6c7af23          	sw	a2,-130(a5) # 8001238c <cons+0x9c>
        wakeup(&cons.r);
    80000416:	00012517          	auipc	a0,0x12
    8000041a:	f7250513          	addi	a0,a0,-142 # 80012388 <cons+0x98>
    8000041e:	313010ef          	jal	80001f30 <wakeup>
    80000422:	bd6d                	j	800002dc <consoleintr+0x32>

0000000080000424 <consoleinit>:

void
consoleinit(void)
{
    80000424:	1141                	addi	sp,sp,-16
    80000426:	e406                	sd	ra,8(sp)
    80000428:	e022                	sd	s0,0(sp)
    8000042a:	0800                	addi	s0,sp,16
  initlock(&cons.lock, "cons");
    8000042c:	00007597          	auipc	a1,0x7
    80000430:	bd458593          	addi	a1,a1,-1068 # 80007000 <etext>
    80000434:	00012517          	auipc	a0,0x12
    80000438:	ebc50513          	addi	a0,a0,-324 # 800122f0 <cons>
    8000043c:	712000ef          	jal	80000b4e <initlock>

  uartinit();
    80000440:	400000ef          	jal	80000840 <uartinit>

  // connect read and write system calls
  // to consoleread and consolewrite.
  devsw[CONSOLE].read = consoleread;
    80000444:	00022797          	auipc	a5,0x22
    80000448:	01c78793          	addi	a5,a5,28 # 80022460 <devsw>
    8000044c:	00000717          	auipc	a4,0x0
    80000450:	d2270713          	addi	a4,a4,-734 # 8000016e <consoleread>
    80000454:	eb98                	sd	a4,16(a5)
  devsw[CONSOLE].write = consolewrite;
    80000456:	00000717          	auipc	a4,0x0
    8000045a:	c7a70713          	addi	a4,a4,-902 # 800000d0 <consolewrite>
    8000045e:	ef98                	sd	a4,24(a5)
}
    80000460:	60a2                	ld	ra,8(sp)
    80000462:	6402                	ld	s0,0(sp)
    80000464:	0141                	addi	sp,sp,16
    80000466:	8082                	ret

0000000080000468 <printint>:

static char digits[] = "0123456789abcdef";

static void
printint(long long xx, int base, int sign)
{
    80000468:	7139                	addi	sp,sp,-64
    8000046a:	fc06                	sd	ra,56(sp)
    8000046c:	f822                	sd	s0,48(sp)
    8000046e:	0080                	addi	s0,sp,64
  char buf[20];
  int i;
  unsigned long long x;

  if(sign && (sign = (xx < 0)))
    80000470:	c219                	beqz	a2,80000476 <printint+0xe>
    80000472:	08054063          	bltz	a0,800004f2 <printint+0x8a>
    x = -xx;
  else
    x = xx;
    80000476:	4881                	li	a7,0
    80000478:	fc840693          	addi	a3,s0,-56

  i = 0;
    8000047c:	4781                	li	a5,0
  do {
    buf[i++] = digits[x % base];
    8000047e:	00007617          	auipc	a2,0x7
    80000482:	29a60613          	addi	a2,a2,666 # 80007718 <digits>
    80000486:	883e                	mv	a6,a5
    80000488:	2785                	addiw	a5,a5,1
    8000048a:	02b57733          	remu	a4,a0,a1
    8000048e:	9732                	add	a4,a4,a2
    80000490:	00074703          	lbu	a4,0(a4)
    80000494:	00e68023          	sb	a4,0(a3)
  } while((x /= base) != 0);
    80000498:	872a                	mv	a4,a0
    8000049a:	02b55533          	divu	a0,a0,a1
    8000049e:	0685                	addi	a3,a3,1
    800004a0:	feb773e3          	bgeu	a4,a1,80000486 <printint+0x1e>

  if(sign)
    800004a4:	00088a63          	beqz	a7,800004b8 <printint+0x50>
    buf[i++] = '-';
    800004a8:	1781                	addi	a5,a5,-32
    800004aa:	97a2                	add	a5,a5,s0
    800004ac:	02d00713          	li	a4,45
    800004b0:	fee78423          	sb	a4,-24(a5)
    800004b4:	0028079b          	addiw	a5,a6,2

  while(--i >= 0)
    800004b8:	02f05963          	blez	a5,800004ea <printint+0x82>
    800004bc:	f426                	sd	s1,40(sp)
    800004be:	f04a                	sd	s2,32(sp)
    800004c0:	fc840713          	addi	a4,s0,-56
    800004c4:	00f704b3          	add	s1,a4,a5
    800004c8:	fff70913          	addi	s2,a4,-1
    800004cc:	993e                	add	s2,s2,a5
    800004ce:	37fd                	addiw	a5,a5,-1
    800004d0:	1782                	slli	a5,a5,0x20
    800004d2:	9381                	srli	a5,a5,0x20
    800004d4:	40f90933          	sub	s2,s2,a5
    consputc(buf[i]);
    800004d8:	fff4c503          	lbu	a0,-1(s1)
    800004dc:	d9dff0ef          	jal	80000278 <consputc>
  while(--i >= 0)
    800004e0:	14fd                	addi	s1,s1,-1
    800004e2:	ff249be3          	bne	s1,s2,800004d8 <printint+0x70>
    800004e6:	74a2                	ld	s1,40(sp)
    800004e8:	7902                	ld	s2,32(sp)
}
    800004ea:	70e2                	ld	ra,56(sp)
    800004ec:	7442                	ld	s0,48(sp)
    800004ee:	6121                	addi	sp,sp,64
    800004f0:	8082                	ret
    x = -xx;
    800004f2:	40a00533          	neg	a0,a0
  if(sign && (sign = (xx < 0)))
    800004f6:	4885                	li	a7,1
    x = -xx;
    800004f8:	b741                	j	80000478 <printint+0x10>

00000000800004fa <printf>:
}

// Print to the console.
int
printf(char *fmt, ...)
{
    800004fa:	7131                	addi	sp,sp,-192
    800004fc:	fc86                	sd	ra,120(sp)
    800004fe:	f8a2                	sd	s0,112(sp)
    80000500:	e8d2                	sd	s4,80(sp)
    80000502:	0100                	addi	s0,sp,128
    80000504:	8a2a                	mv	s4,a0
    80000506:	e40c                	sd	a1,8(s0)
    80000508:	e810                	sd	a2,16(s0)
    8000050a:	ec14                	sd	a3,24(s0)
    8000050c:	f018                	sd	a4,32(s0)
    8000050e:	f41c                	sd	a5,40(s0)
    80000510:	03043823          	sd	a6,48(s0)
    80000514:	03143c23          	sd	a7,56(s0)
  va_list ap;
  int i, cx, c0, c1, c2;
  char *s;

  if(panicking == 0)
    80000518:	0000a797          	auipc	a5,0xa
    8000051c:	dac7a783          	lw	a5,-596(a5) # 8000a2c4 <panicking>
    80000520:	c3a1                	beqz	a5,80000560 <printf+0x66>
    acquire(&pr.lock);

  va_start(ap, fmt);
    80000522:	00840793          	addi	a5,s0,8
    80000526:	f8f43423          	sd	a5,-120(s0)
  for(i = 0; (cx = fmt[i] & 0xff) != 0; i++){
    8000052a:	000a4503          	lbu	a0,0(s4)
    8000052e:	28050763          	beqz	a0,800007bc <printf+0x2c2>
    80000532:	f4a6                	sd	s1,104(sp)
    80000534:	f0ca                	sd	s2,96(sp)
    80000536:	ecce                	sd	s3,88(sp)
    80000538:	e4d6                	sd	s5,72(sp)
    8000053a:	e0da                	sd	s6,64(sp)
    8000053c:	f862                	sd	s8,48(sp)
    8000053e:	f466                	sd	s9,40(sp)
    80000540:	f06a                	sd	s10,32(sp)
    80000542:	ec6e                	sd	s11,24(sp)
    80000544:	4981                	li	s3,0
    if(cx != '%'){
    80000546:	02500a93          	li	s5,37
    i++;
    c0 = fmt[i+0] & 0xff;
    c1 = c2 = 0;
    if(c0) c1 = fmt[i+1] & 0xff;
    if(c1) c2 = fmt[i+2] & 0xff;
    if(c0 == 'd'){
    8000054a:	06400b13          	li	s6,100
      printint(va_arg(ap, int), 10, 1);
    } else if(c0 == 'l' && c1 == 'd'){
    8000054e:	06c00c13          	li	s8,108
      printint(va_arg(ap, uint64), 10, 1);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
      printint(va_arg(ap, uint64), 10, 1);
      i += 2;
    } else if(c0 == 'u'){
    80000552:	07500c93          	li	s9,117
      printint(va_arg(ap, uint64), 10, 0);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'u'){
      printint(va_arg(ap, uint64), 10, 0);
      i += 2;
    } else if(c0 == 'x'){
    80000556:	07800d13          	li	s10,120
      printint(va_arg(ap, uint64), 16, 0);
      i += 1;
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'x'){
      printint(va_arg(ap, uint64), 16, 0);
      i += 2;
    } else if(c0 == 'p'){
    8000055a:	07000d93          	li	s11,112
    8000055e:	a01d                	j	80000584 <printf+0x8a>
    acquire(&pr.lock);
    80000560:	00012517          	auipc	a0,0x12
    80000564:	e3850513          	addi	a0,a0,-456 # 80012398 <pr>
    80000568:	666000ef          	jal	80000bce <acquire>
    8000056c:	bf5d                	j	80000522 <printf+0x28>
      consputc(cx);
    8000056e:	d0bff0ef          	jal	80000278 <consputc>
      continue;
    80000572:	84ce                	mv	s1,s3
  for(i = 0; (cx = fmt[i] & 0xff) != 0; i++){
    80000574:	0014899b          	addiw	s3,s1,1
    80000578:	013a07b3          	add	a5,s4,s3
    8000057c:	0007c503          	lbu	a0,0(a5)
    80000580:	20050b63          	beqz	a0,80000796 <printf+0x29c>
    if(cx != '%'){
    80000584:	ff5515e3          	bne	a0,s5,8000056e <printf+0x74>
    i++;
    80000588:	0019849b          	addiw	s1,s3,1
    c0 = fmt[i+0] & 0xff;
    8000058c:	009a07b3          	add	a5,s4,s1
    80000590:	0007c903          	lbu	s2,0(a5)
    if(c0) c1 = fmt[i+1] & 0xff;
    80000594:	20090b63          	beqz	s2,800007aa <printf+0x2b0>
    80000598:	0017c783          	lbu	a5,1(a5)
    c1 = c2 = 0;
    8000059c:	86be                	mv	a3,a5
    if(c1) c2 = fmt[i+2] & 0xff;
    8000059e:	c789                	beqz	a5,800005a8 <printf+0xae>
    800005a0:	009a0733          	add	a4,s4,s1
    800005a4:	00274683          	lbu	a3,2(a4)
    if(c0 == 'd'){
    800005a8:	03690963          	beq	s2,s6,800005da <printf+0xe0>
    } else if(c0 == 'l' && c1 == 'd'){
    800005ac:	05890363          	beq	s2,s8,800005f2 <printf+0xf8>
    } else if(c0 == 'u'){
    800005b0:	0d990663          	beq	s2,s9,8000067c <printf+0x182>
    } else if(c0 == 'x'){
    800005b4:	11a90d63          	beq	s2,s10,800006ce <printf+0x1d4>
    } else if(c0 == 'p'){
    800005b8:	15b90663          	beq	s2,s11,80000704 <printf+0x20a>
      printptr(va_arg(ap, uint64));
    } else if(c0 == 'c'){
    800005bc:	06300793          	li	a5,99
    800005c0:	18f90563          	beq	s2,a5,8000074a <printf+0x250>
      consputc(va_arg(ap, uint));
    } else if(c0 == 's'){
    800005c4:	07300793          	li	a5,115
    800005c8:	18f90b63          	beq	s2,a5,8000075e <printf+0x264>
      if((s = va_arg(ap, char*)) == 0)
        s = "(null)";
      for(; *s; s++)
        consputc(*s);
    } else if(c0 == '%'){
    800005cc:	03591b63          	bne	s2,s5,80000602 <printf+0x108>
      consputc('%');
    800005d0:	02500513          	li	a0,37
    800005d4:	ca5ff0ef          	jal	80000278 <consputc>
    800005d8:	bf71                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, int), 10, 1);
    800005da:	f8843783          	ld	a5,-120(s0)
    800005de:	00878713          	addi	a4,a5,8
    800005e2:	f8e43423          	sd	a4,-120(s0)
    800005e6:	4605                	li	a2,1
    800005e8:	45a9                	li	a1,10
    800005ea:	4388                	lw	a0,0(a5)
    800005ec:	e7dff0ef          	jal	80000468 <printint>
    800005f0:	b751                	j	80000574 <printf+0x7a>
    } else if(c0 == 'l' && c1 == 'd'){
    800005f2:	01678f63          	beq	a5,s6,80000610 <printf+0x116>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
    800005f6:	03878b63          	beq	a5,s8,8000062c <printf+0x132>
    } else if(c0 == 'l' && c1 == 'u'){
    800005fa:	09978e63          	beq	a5,s9,80000696 <printf+0x19c>
    } else if(c0 == 'l' && c1 == 'x'){
    800005fe:	0fa78563          	beq	a5,s10,800006e8 <printf+0x1ee>
    } else if(c0 == 0){
      break;
    } else {
      // Print unknown % sequence to draw attention.
      consputc('%');
    80000602:	8556                	mv	a0,s5
    80000604:	c75ff0ef          	jal	80000278 <consputc>
      consputc(c0);
    80000608:	854a                	mv	a0,s2
    8000060a:	c6fff0ef          	jal	80000278 <consputc>
    8000060e:	b79d                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 1);
    80000610:	f8843783          	ld	a5,-120(s0)
    80000614:	00878713          	addi	a4,a5,8
    80000618:	f8e43423          	sd	a4,-120(s0)
    8000061c:	4605                	li	a2,1
    8000061e:	45a9                	li	a1,10
    80000620:	6388                	ld	a0,0(a5)
    80000622:	e47ff0ef          	jal	80000468 <printint>
      i += 1;
    80000626:	0029849b          	addiw	s1,s3,2
    8000062a:	b7a9                	j	80000574 <printf+0x7a>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'd'){
    8000062c:	06400793          	li	a5,100
    80000630:	02f68863          	beq	a3,a5,80000660 <printf+0x166>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'u'){
    80000634:	07500793          	li	a5,117
    80000638:	06f68d63          	beq	a3,a5,800006b2 <printf+0x1b8>
    } else if(c0 == 'l' && c1 == 'l' && c2 == 'x'){
    8000063c:	07800793          	li	a5,120
    80000640:	fcf691e3          	bne	a3,a5,80000602 <printf+0x108>
      printint(va_arg(ap, uint64), 16, 0);
    80000644:	f8843783          	ld	a5,-120(s0)
    80000648:	00878713          	addi	a4,a5,8
    8000064c:	f8e43423          	sd	a4,-120(s0)
    80000650:	4601                	li	a2,0
    80000652:	45c1                	li	a1,16
    80000654:	6388                	ld	a0,0(a5)
    80000656:	e13ff0ef          	jal	80000468 <printint>
      i += 2;
    8000065a:	0039849b          	addiw	s1,s3,3
    8000065e:	bf19                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 1);
    80000660:	f8843783          	ld	a5,-120(s0)
    80000664:	00878713          	addi	a4,a5,8
    80000668:	f8e43423          	sd	a4,-120(s0)
    8000066c:	4605                	li	a2,1
    8000066e:	45a9                	li	a1,10
    80000670:	6388                	ld	a0,0(a5)
    80000672:	df7ff0ef          	jal	80000468 <printint>
      i += 2;
    80000676:	0039849b          	addiw	s1,s3,3
    8000067a:	bded                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint32), 10, 0);
    8000067c:	f8843783          	ld	a5,-120(s0)
    80000680:	00878713          	addi	a4,a5,8
    80000684:	f8e43423          	sd	a4,-120(s0)
    80000688:	4601                	li	a2,0
    8000068a:	45a9                	li	a1,10
    8000068c:	0007e503          	lwu	a0,0(a5)
    80000690:	dd9ff0ef          	jal	80000468 <printint>
    80000694:	b5c5                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 0);
    80000696:	f8843783          	ld	a5,-120(s0)
    8000069a:	00878713          	addi	a4,a5,8
    8000069e:	f8e43423          	sd	a4,-120(s0)
    800006a2:	4601                	li	a2,0
    800006a4:	45a9                	li	a1,10
    800006a6:	6388                	ld	a0,0(a5)
    800006a8:	dc1ff0ef          	jal	80000468 <printint>
      i += 1;
    800006ac:	0029849b          	addiw	s1,s3,2
    800006b0:	b5d1                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 10, 0);
    800006b2:	f8843783          	ld	a5,-120(s0)
    800006b6:	00878713          	addi	a4,a5,8
    800006ba:	f8e43423          	sd	a4,-120(s0)
    800006be:	4601                	li	a2,0
    800006c0:	45a9                	li	a1,10
    800006c2:	6388                	ld	a0,0(a5)
    800006c4:	da5ff0ef          	jal	80000468 <printint>
      i += 2;
    800006c8:	0039849b          	addiw	s1,s3,3
    800006cc:	b565                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint32), 16, 0);
    800006ce:	f8843783          	ld	a5,-120(s0)
    800006d2:	00878713          	addi	a4,a5,8
    800006d6:	f8e43423          	sd	a4,-120(s0)
    800006da:	4601                	li	a2,0
    800006dc:	45c1                	li	a1,16
    800006de:	0007e503          	lwu	a0,0(a5)
    800006e2:	d87ff0ef          	jal	80000468 <printint>
    800006e6:	b579                	j	80000574 <printf+0x7a>
      printint(va_arg(ap, uint64), 16, 0);
    800006e8:	f8843783          	ld	a5,-120(s0)
    800006ec:	00878713          	addi	a4,a5,8
    800006f0:	f8e43423          	sd	a4,-120(s0)
    800006f4:	4601                	li	a2,0
    800006f6:	45c1                	li	a1,16
    800006f8:	6388                	ld	a0,0(a5)
    800006fa:	d6fff0ef          	jal	80000468 <printint>
      i += 1;
    800006fe:	0029849b          	addiw	s1,s3,2
    80000702:	bd8d                	j	80000574 <printf+0x7a>
    80000704:	fc5e                	sd	s7,56(sp)
      printptr(va_arg(ap, uint64));
    80000706:	f8843783          	ld	a5,-120(s0)
    8000070a:	00878713          	addi	a4,a5,8
    8000070e:	f8e43423          	sd	a4,-120(s0)
    80000712:	0007b983          	ld	s3,0(a5)
  consputc('0');
    80000716:	03000513          	li	a0,48
    8000071a:	b5fff0ef          	jal	80000278 <consputc>
  consputc('x');
    8000071e:	07800513          	li	a0,120
    80000722:	b57ff0ef          	jal	80000278 <consputc>
    80000726:	4941                	li	s2,16
    consputc(digits[x >> (sizeof(uint64) * 8 - 4)]);
    80000728:	00007b97          	auipc	s7,0x7
    8000072c:	ff0b8b93          	addi	s7,s7,-16 # 80007718 <digits>
    80000730:	03c9d793          	srli	a5,s3,0x3c
    80000734:	97de                	add	a5,a5,s7
    80000736:	0007c503          	lbu	a0,0(a5)
    8000073a:	b3fff0ef          	jal	80000278 <consputc>
  for (i = 0; i < (sizeof(uint64) * 2); i++, x <<= 4)
    8000073e:	0992                	slli	s3,s3,0x4
    80000740:	397d                	addiw	s2,s2,-1
    80000742:	fe0917e3          	bnez	s2,80000730 <printf+0x236>
    80000746:	7be2                	ld	s7,56(sp)
    80000748:	b535                	j	80000574 <printf+0x7a>
      consputc(va_arg(ap, uint));
    8000074a:	f8843783          	ld	a5,-120(s0)
    8000074e:	00878713          	addi	a4,a5,8
    80000752:	f8e43423          	sd	a4,-120(s0)
    80000756:	4388                	lw	a0,0(a5)
    80000758:	b21ff0ef          	jal	80000278 <consputc>
    8000075c:	bd21                	j	80000574 <printf+0x7a>
      if((s = va_arg(ap, char*)) == 0)
    8000075e:	f8843783          	ld	a5,-120(s0)
    80000762:	00878713          	addi	a4,a5,8
    80000766:	f8e43423          	sd	a4,-120(s0)
    8000076a:	0007b903          	ld	s2,0(a5)
    8000076e:	00090d63          	beqz	s2,80000788 <printf+0x28e>
      for(; *s; s++)
    80000772:	00094503          	lbu	a0,0(s2)
    80000776:	de050fe3          	beqz	a0,80000574 <printf+0x7a>
        consputc(*s);
    8000077a:	affff0ef          	jal	80000278 <consputc>
      for(; *s; s++)
    8000077e:	0905                	addi	s2,s2,1
    80000780:	00094503          	lbu	a0,0(s2)
    80000784:	f97d                	bnez	a0,8000077a <printf+0x280>
    80000786:	b3fd                	j	80000574 <printf+0x7a>
        s = "(null)";
    80000788:	00007917          	auipc	s2,0x7
    8000078c:	88090913          	addi	s2,s2,-1920 # 80007008 <etext+0x8>
      for(; *s; s++)
    80000790:	02800513          	li	a0,40
    80000794:	b7dd                	j	8000077a <printf+0x280>
    80000796:	74a6                	ld	s1,104(sp)
    80000798:	7906                	ld	s2,96(sp)
    8000079a:	69e6                	ld	s3,88(sp)
    8000079c:	6aa6                	ld	s5,72(sp)
    8000079e:	6b06                	ld	s6,64(sp)
    800007a0:	7c42                	ld	s8,48(sp)
    800007a2:	7ca2                	ld	s9,40(sp)
    800007a4:	7d02                	ld	s10,32(sp)
    800007a6:	6de2                	ld	s11,24(sp)
    800007a8:	a811                	j	800007bc <printf+0x2c2>
    800007aa:	74a6                	ld	s1,104(sp)
    800007ac:	7906                	ld	s2,96(sp)
    800007ae:	69e6                	ld	s3,88(sp)
    800007b0:	6aa6                	ld	s5,72(sp)
    800007b2:	6b06                	ld	s6,64(sp)
    800007b4:	7c42                	ld	s8,48(sp)
    800007b6:	7ca2                	ld	s9,40(sp)
    800007b8:	7d02                	ld	s10,32(sp)
    800007ba:	6de2                	ld	s11,24(sp)
    }

  }
  va_end(ap);

  if(panicking == 0)
    800007bc:	0000a797          	auipc	a5,0xa
    800007c0:	b087a783          	lw	a5,-1272(a5) # 8000a2c4 <panicking>
    800007c4:	c799                	beqz	a5,800007d2 <printf+0x2d8>
    release(&pr.lock);

  return 0;
}
    800007c6:	4501                	li	a0,0
    800007c8:	70e6                	ld	ra,120(sp)
    800007ca:	7446                	ld	s0,112(sp)
    800007cc:	6a46                	ld	s4,80(sp)
    800007ce:	6129                	addi	sp,sp,192
    800007d0:	8082                	ret
    release(&pr.lock);
    800007d2:	00012517          	auipc	a0,0x12
    800007d6:	bc650513          	addi	a0,a0,-1082 # 80012398 <pr>
    800007da:	48c000ef          	jal	80000c66 <release>
  return 0;
    800007de:	b7e5                	j	800007c6 <printf+0x2cc>

00000000800007e0 <panic>:

void
panic(char *s)
{
    800007e0:	1101                	addi	sp,sp,-32
    800007e2:	ec06                	sd	ra,24(sp)
    800007e4:	e822                	sd	s0,16(sp)
    800007e6:	e426                	sd	s1,8(sp)
    800007e8:	e04a                	sd	s2,0(sp)
    800007ea:	1000                	addi	s0,sp,32
    800007ec:	84aa                	mv	s1,a0
  panicking = 1;
    800007ee:	4905                	li	s2,1
    800007f0:	0000a797          	auipc	a5,0xa
    800007f4:	ad27aa23          	sw	s2,-1324(a5) # 8000a2c4 <panicking>
  printf("panic: ");
    800007f8:	00007517          	auipc	a0,0x7
    800007fc:	82050513          	addi	a0,a0,-2016 # 80007018 <etext+0x18>
    80000800:	cfbff0ef          	jal	800004fa <printf>
  printf("%s\n", s);
    80000804:	85a6                	mv	a1,s1
    80000806:	00007517          	auipc	a0,0x7
    8000080a:	81a50513          	addi	a0,a0,-2022 # 80007020 <etext+0x20>
    8000080e:	cedff0ef          	jal	800004fa <printf>
  panicked = 1; // freeze uart output from other CPUs
    80000812:	0000a797          	auipc	a5,0xa
    80000816:	ab27a723          	sw	s2,-1362(a5) # 8000a2c0 <panicked>
  for(;;)
    8000081a:	a001                	j	8000081a <panic+0x3a>

000000008000081c <printfinit>:
    ;
}

void
printfinit(void)
{
    8000081c:	1141                	addi	sp,sp,-16
    8000081e:	e406                	sd	ra,8(sp)
    80000820:	e022                	sd	s0,0(sp)
    80000822:	0800                	addi	s0,sp,16
  initlock(&pr.lock, "pr");
    80000824:	00007597          	auipc	a1,0x7
    80000828:	80458593          	addi	a1,a1,-2044 # 80007028 <etext+0x28>
    8000082c:	00012517          	auipc	a0,0x12
    80000830:	b6c50513          	addi	a0,a0,-1172 # 80012398 <pr>
    80000834:	31a000ef          	jal	80000b4e <initlock>
}
    80000838:	60a2                	ld	ra,8(sp)
    8000083a:	6402                	ld	s0,0(sp)
    8000083c:	0141                	addi	sp,sp,16
    8000083e:	8082                	ret

0000000080000840 <uartinit>:
extern volatile int panicking; // from printf.c
extern volatile int panicked; // from printf.c

void
uartinit(void)
{
    80000840:	1141                	addi	sp,sp,-16
    80000842:	e406                	sd	ra,8(sp)
    80000844:	e022                	sd	s0,0(sp)
    80000846:	0800                	addi	s0,sp,16
  // disable interrupts.
  WriteReg(IER, 0x00);
    80000848:	100007b7          	lui	a5,0x10000
    8000084c:	000780a3          	sb	zero,1(a5) # 10000001 <_entry-0x6fffffff>

  // special mode to set baud rate.
  WriteReg(LCR, LCR_BAUD_LATCH);
    80000850:	10000737          	lui	a4,0x10000
    80000854:	f8000693          	li	a3,-128
    80000858:	00d701a3          	sb	a3,3(a4) # 10000003 <_entry-0x6ffffffd>

  // LSB for baud rate of 38.4K.
  WriteReg(0, 0x03);
    8000085c:	468d                	li	a3,3
    8000085e:	10000637          	lui	a2,0x10000
    80000862:	00d60023          	sb	a3,0(a2) # 10000000 <_entry-0x70000000>

  // MSB for baud rate of 38.4K.
  WriteReg(1, 0x00);
    80000866:	000780a3          	sb	zero,1(a5)

  // leave set-baud mode,
  // and set word length to 8 bits, no parity.
  WriteReg(LCR, LCR_EIGHT_BITS);
    8000086a:	00d701a3          	sb	a3,3(a4)

  // reset and enable FIFOs.
  WriteReg(FCR, FCR_FIFO_ENABLE | FCR_FIFO_CLEAR);
    8000086e:	10000737          	lui	a4,0x10000
    80000872:	461d                	li	a2,7
    80000874:	00c70123          	sb	a2,2(a4) # 10000002 <_entry-0x6ffffffe>

  // enable transmit and receive interrupts.
  WriteReg(IER, IER_TX_ENABLE | IER_RX_ENABLE);
    80000878:	00d780a3          	sb	a3,1(a5)

  initlock(&tx_lock, "uart");
    8000087c:	00006597          	auipc	a1,0x6
    80000880:	7b458593          	addi	a1,a1,1972 # 80007030 <etext+0x30>
    80000884:	00012517          	auipc	a0,0x12
    80000888:	b2c50513          	addi	a0,a0,-1236 # 800123b0 <tx_lock>
    8000088c:	2c2000ef          	jal	80000b4e <initlock>
}
    80000890:	60a2                	ld	ra,8(sp)
    80000892:	6402                	ld	s0,0(sp)
    80000894:	0141                	addi	sp,sp,16
    80000896:	8082                	ret

0000000080000898 <uartwrite>:
// transmit buf[] to the uart. it blocks if the
// uart is busy, so it cannot be called from
// interrupts, only from write() system calls.
void
uartwrite(char buf[], int n)
{
    80000898:	715d                	addi	sp,sp,-80
    8000089a:	e486                	sd	ra,72(sp)
    8000089c:	e0a2                	sd	s0,64(sp)
    8000089e:	fc26                	sd	s1,56(sp)
    800008a0:	ec56                	sd	s5,24(sp)
    800008a2:	0880                	addi	s0,sp,80
    800008a4:	8aaa                	mv	s5,a0
    800008a6:	84ae                	mv	s1,a1
  acquire(&tx_lock);
    800008a8:	00012517          	auipc	a0,0x12
    800008ac:	b0850513          	addi	a0,a0,-1272 # 800123b0 <tx_lock>
    800008b0:	31e000ef          	jal	80000bce <acquire>

  int i = 0;
  while(i < n){ 
    800008b4:	06905063          	blez	s1,80000914 <uartwrite+0x7c>
    800008b8:	f84a                	sd	s2,48(sp)
    800008ba:	f44e                	sd	s3,40(sp)
    800008bc:	f052                	sd	s4,32(sp)
    800008be:	e85a                	sd	s6,16(sp)
    800008c0:	e45e                	sd	s7,8(sp)
    800008c2:	8a56                	mv	s4,s5
    800008c4:	9aa6                	add	s5,s5,s1
    while(tx_busy != 0){
    800008c6:	0000a497          	auipc	s1,0xa
    800008ca:	a0648493          	addi	s1,s1,-1530 # 8000a2cc <tx_busy>
      // wait for a UART transmit-complete interrupt
      // to set tx_busy to 0.
      sleep(&tx_chan, &tx_lock);
    800008ce:	00012997          	auipc	s3,0x12
    800008d2:	ae298993          	addi	s3,s3,-1310 # 800123b0 <tx_lock>
    800008d6:	0000a917          	auipc	s2,0xa
    800008da:	9f290913          	addi	s2,s2,-1550 # 8000a2c8 <tx_chan>
    }   
      
    WriteReg(THR, buf[i]);
    800008de:	10000bb7          	lui	s7,0x10000
    i += 1;
    tx_busy = 1;
    800008e2:	4b05                	li	s6,1
    800008e4:	a005                	j	80000904 <uartwrite+0x6c>
      sleep(&tx_chan, &tx_lock);
    800008e6:	85ce                	mv	a1,s3
    800008e8:	854a                	mv	a0,s2
    800008ea:	5fa010ef          	jal	80001ee4 <sleep>
    while(tx_busy != 0){
    800008ee:	409c                	lw	a5,0(s1)
    800008f0:	fbfd                	bnez	a5,800008e6 <uartwrite+0x4e>
    WriteReg(THR, buf[i]);
    800008f2:	000a4783          	lbu	a5,0(s4)
    800008f6:	00fb8023          	sb	a5,0(s7) # 10000000 <_entry-0x70000000>
    tx_busy = 1;
    800008fa:	0164a023          	sw	s6,0(s1)
  while(i < n){ 
    800008fe:	0a05                	addi	s4,s4,1
    80000900:	015a0563          	beq	s4,s5,8000090a <uartwrite+0x72>
    while(tx_busy != 0){
    80000904:	409c                	lw	a5,0(s1)
    80000906:	f3e5                	bnez	a5,800008e6 <uartwrite+0x4e>
    80000908:	b7ed                	j	800008f2 <uartwrite+0x5a>
    8000090a:	7942                	ld	s2,48(sp)
    8000090c:	79a2                	ld	s3,40(sp)
    8000090e:	7a02                	ld	s4,32(sp)
    80000910:	6b42                	ld	s6,16(sp)
    80000912:	6ba2                	ld	s7,8(sp)
  }

  release(&tx_lock);
    80000914:	00012517          	auipc	a0,0x12
    80000918:	a9c50513          	addi	a0,a0,-1380 # 800123b0 <tx_lock>
    8000091c:	34a000ef          	jal	80000c66 <release>
}
    80000920:	60a6                	ld	ra,72(sp)
    80000922:	6406                	ld	s0,64(sp)
    80000924:	74e2                	ld	s1,56(sp)
    80000926:	6ae2                	ld	s5,24(sp)
    80000928:	6161                	addi	sp,sp,80
    8000092a:	8082                	ret

000000008000092c <uartputc_sync>:
// interrupts, for use by kernel printf() and
// to echo characters. it spins waiting for the uart's
// output register to be empty.
void
uartputc_sync(int c)
{
    8000092c:	1101                	addi	sp,sp,-32
    8000092e:	ec06                	sd	ra,24(sp)
    80000930:	e822                	sd	s0,16(sp)
    80000932:	e426                	sd	s1,8(sp)
    80000934:	1000                	addi	s0,sp,32
    80000936:	84aa                	mv	s1,a0
  if(panicking == 0)
    80000938:	0000a797          	auipc	a5,0xa
    8000093c:	98c7a783          	lw	a5,-1652(a5) # 8000a2c4 <panicking>
    80000940:	cf95                	beqz	a5,8000097c <uartputc_sync+0x50>
    push_off();

  if(panicked){
    80000942:	0000a797          	auipc	a5,0xa
    80000946:	97e7a783          	lw	a5,-1666(a5) # 8000a2c0 <panicked>
    8000094a:	ef85                	bnez	a5,80000982 <uartputc_sync+0x56>
    for(;;)
      ;
  }

  // wait for UART to set Transmit Holding Empty in LSR.
  while((ReadReg(LSR) & LSR_TX_IDLE) == 0)
    8000094c:	10000737          	lui	a4,0x10000
    80000950:	0715                	addi	a4,a4,5 # 10000005 <_entry-0x6ffffffb>
    80000952:	00074783          	lbu	a5,0(a4)
    80000956:	0207f793          	andi	a5,a5,32
    8000095a:	dfe5                	beqz	a5,80000952 <uartputc_sync+0x26>
    ;
  WriteReg(THR, c);
    8000095c:	0ff4f513          	zext.b	a0,s1
    80000960:	100007b7          	lui	a5,0x10000
    80000964:	00a78023          	sb	a0,0(a5) # 10000000 <_entry-0x70000000>

  if(panicking == 0)
    80000968:	0000a797          	auipc	a5,0xa
    8000096c:	95c7a783          	lw	a5,-1700(a5) # 8000a2c4 <panicking>
    80000970:	cb91                	beqz	a5,80000984 <uartputc_sync+0x58>
    pop_off();
}
    80000972:	60e2                	ld	ra,24(sp)
    80000974:	6442                	ld	s0,16(sp)
    80000976:	64a2                	ld	s1,8(sp)
    80000978:	6105                	addi	sp,sp,32
    8000097a:	8082                	ret
    push_off();
    8000097c:	212000ef          	jal	80000b8e <push_off>
    80000980:	b7c9                	j	80000942 <uartputc_sync+0x16>
    for(;;)
    80000982:	a001                	j	80000982 <uartputc_sync+0x56>
    pop_off();
    80000984:	28e000ef          	jal	80000c12 <pop_off>
}
    80000988:	b7ed                	j	80000972 <uartputc_sync+0x46>

000000008000098a <uartgetc>:

// try to read one input character from the UART.
// return -1 if none is waiting.
int
uartgetc(void)
{
    8000098a:	1141                	addi	sp,sp,-16
    8000098c:	e422                	sd	s0,8(sp)
    8000098e:	0800                	addi	s0,sp,16
  if(ReadReg(LSR) & LSR_RX_READY){
    80000990:	100007b7          	lui	a5,0x10000
    80000994:	0795                	addi	a5,a5,5 # 10000005 <_entry-0x6ffffffb>
    80000996:	0007c783          	lbu	a5,0(a5)
    8000099a:	8b85                	andi	a5,a5,1
    8000099c:	cb81                	beqz	a5,800009ac <uartgetc+0x22>
    // input data is ready.
    return ReadReg(RHR);
    8000099e:	100007b7          	lui	a5,0x10000
    800009a2:	0007c503          	lbu	a0,0(a5) # 10000000 <_entry-0x70000000>
  } else {
    return -1;
  }
}
    800009a6:	6422                	ld	s0,8(sp)
    800009a8:	0141                	addi	sp,sp,16
    800009aa:	8082                	ret
    return -1;
    800009ac:	557d                	li	a0,-1
    800009ae:	bfe5                	j	800009a6 <uartgetc+0x1c>

00000000800009b0 <uartintr>:
// handle a uart interrupt, raised because input has
// arrived, or the uart is ready for more output, or
// both. called from devintr().
void
uartintr(void)
{
    800009b0:	1101                	addi	sp,sp,-32
    800009b2:	ec06                	sd	ra,24(sp)
    800009b4:	e822                	sd	s0,16(sp)
    800009b6:	e426                	sd	s1,8(sp)
    800009b8:	1000                	addi	s0,sp,32
  ReadReg(ISR); // acknowledge the interrupt
    800009ba:	100007b7          	lui	a5,0x10000
    800009be:	0789                	addi	a5,a5,2 # 10000002 <_entry-0x6ffffffe>
    800009c0:	0007c783          	lbu	a5,0(a5)

  acquire(&tx_lock);
    800009c4:	00012517          	auipc	a0,0x12
    800009c8:	9ec50513          	addi	a0,a0,-1556 # 800123b0 <tx_lock>
    800009cc:	202000ef          	jal	80000bce <acquire>
  if(ReadReg(LSR) & LSR_TX_IDLE){
    800009d0:	100007b7          	lui	a5,0x10000
    800009d4:	0795                	addi	a5,a5,5 # 10000005 <_entry-0x6ffffffb>
    800009d6:	0007c783          	lbu	a5,0(a5)
    800009da:	0207f793          	andi	a5,a5,32
    800009de:	eb89                	bnez	a5,800009f0 <uartintr+0x40>
    // UART finished transmitting; wake up sending thread.
    tx_busy = 0;
    wakeup(&tx_chan);
  }
  release(&tx_lock);
    800009e0:	00012517          	auipc	a0,0x12
    800009e4:	9d050513          	addi	a0,a0,-1584 # 800123b0 <tx_lock>
    800009e8:	27e000ef          	jal	80000c66 <release>

  // read and process incoming characters, if any.
  while(1){
    int c = uartgetc();
    if(c == -1)
    800009ec:	54fd                	li	s1,-1
    800009ee:	a831                	j	80000a0a <uartintr+0x5a>
    tx_busy = 0;
    800009f0:	0000a797          	auipc	a5,0xa
    800009f4:	8c07ae23          	sw	zero,-1828(a5) # 8000a2cc <tx_busy>
    wakeup(&tx_chan);
    800009f8:	0000a517          	auipc	a0,0xa
    800009fc:	8d050513          	addi	a0,a0,-1840 # 8000a2c8 <tx_chan>
    80000a00:	530010ef          	jal	80001f30 <wakeup>
    80000a04:	bff1                	j	800009e0 <uartintr+0x30>
      break;
    consoleintr(c);
    80000a06:	8a5ff0ef          	jal	800002aa <consoleintr>
    int c = uartgetc();
    80000a0a:	f81ff0ef          	jal	8000098a <uartgetc>
    if(c == -1)
    80000a0e:	fe951ce3          	bne	a0,s1,80000a06 <uartintr+0x56>
  }
}
    80000a12:	60e2                	ld	ra,24(sp)
    80000a14:	6442                	ld	s0,16(sp)
    80000a16:	64a2                	ld	s1,8(sp)
    80000a18:	6105                	addi	sp,sp,32
    80000a1a:	8082                	ret

0000000080000a1c <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(void *pa)
{
    80000a1c:	1101                	addi	sp,sp,-32
    80000a1e:	ec06                	sd	ra,24(sp)
    80000a20:	e822                	sd	s0,16(sp)
    80000a22:	e426                	sd	s1,8(sp)
    80000a24:	e04a                	sd	s2,0(sp)
    80000a26:	1000                	addi	s0,sp,32
  struct run *r;

  if(((uint64)pa % PGSIZE) != 0 || (char*)pa < end || (uint64)pa >= PHYSTOP)
    80000a28:	03451793          	slli	a5,a0,0x34
    80000a2c:	e7a9                	bnez	a5,80000a76 <kfree+0x5a>
    80000a2e:	84aa                	mv	s1,a0
    80000a30:	00023797          	auipc	a5,0x23
    80000a34:	56078793          	addi	a5,a5,1376 # 80023f90 <end>
    80000a38:	02f56f63          	bltu	a0,a5,80000a76 <kfree+0x5a>
    80000a3c:	47c5                	li	a5,17
    80000a3e:	07ee                	slli	a5,a5,0x1b
    80000a40:	02f57b63          	bgeu	a0,a5,80000a76 <kfree+0x5a>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(pa, 1, PGSIZE);
    80000a44:	6605                	lui	a2,0x1
    80000a46:	4585                	li	a1,1
    80000a48:	25a000ef          	jal	80000ca2 <memset>

  r = (struct run*)pa;

  acquire(&kmem.lock);
    80000a4c:	00012917          	auipc	s2,0x12
    80000a50:	97c90913          	addi	s2,s2,-1668 # 800123c8 <kmem>
    80000a54:	854a                	mv	a0,s2
    80000a56:	178000ef          	jal	80000bce <acquire>
  r->next = kmem.freelist;
    80000a5a:	01893783          	ld	a5,24(s2)
    80000a5e:	e09c                	sd	a5,0(s1)
  kmem.freelist = r;
    80000a60:	00993c23          	sd	s1,24(s2)
  release(&kmem.lock);
    80000a64:	854a                	mv	a0,s2
    80000a66:	200000ef          	jal	80000c66 <release>
}
    80000a6a:	60e2                	ld	ra,24(sp)
    80000a6c:	6442                	ld	s0,16(sp)
    80000a6e:	64a2                	ld	s1,8(sp)
    80000a70:	6902                	ld	s2,0(sp)
    80000a72:	6105                	addi	sp,sp,32
    80000a74:	8082                	ret
    panic("kfree");
    80000a76:	00006517          	auipc	a0,0x6
    80000a7a:	5c250513          	addi	a0,a0,1474 # 80007038 <etext+0x38>
    80000a7e:	d63ff0ef          	jal	800007e0 <panic>

0000000080000a82 <freerange>:
{
    80000a82:	7179                	addi	sp,sp,-48
    80000a84:	f406                	sd	ra,40(sp)
    80000a86:	f022                	sd	s0,32(sp)
    80000a88:	ec26                	sd	s1,24(sp)
    80000a8a:	1800                	addi	s0,sp,48
  p = (char*)PGROUNDUP((uint64)pa_start);
    80000a8c:	6785                	lui	a5,0x1
    80000a8e:	fff78713          	addi	a4,a5,-1 # fff <_entry-0x7ffff001>
    80000a92:	00e504b3          	add	s1,a0,a4
    80000a96:	777d                	lui	a4,0xfffff
    80000a98:	8cf9                	and	s1,s1,a4
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000a9a:	94be                	add	s1,s1,a5
    80000a9c:	0295e263          	bltu	a1,s1,80000ac0 <freerange+0x3e>
    80000aa0:	e84a                	sd	s2,16(sp)
    80000aa2:	e44e                	sd	s3,8(sp)
    80000aa4:	e052                	sd	s4,0(sp)
    80000aa6:	892e                	mv	s2,a1
    kfree(p);
    80000aa8:	7a7d                	lui	s4,0xfffff
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000aaa:	6985                	lui	s3,0x1
    kfree(p);
    80000aac:	01448533          	add	a0,s1,s4
    80000ab0:	f6dff0ef          	jal	80000a1c <kfree>
  for(; p + PGSIZE <= (char*)pa_end; p += PGSIZE)
    80000ab4:	94ce                	add	s1,s1,s3
    80000ab6:	fe997be3          	bgeu	s2,s1,80000aac <freerange+0x2a>
    80000aba:	6942                	ld	s2,16(sp)
    80000abc:	69a2                	ld	s3,8(sp)
    80000abe:	6a02                	ld	s4,0(sp)
}
    80000ac0:	70a2                	ld	ra,40(sp)
    80000ac2:	7402                	ld	s0,32(sp)
    80000ac4:	64e2                	ld	s1,24(sp)
    80000ac6:	6145                	addi	sp,sp,48
    80000ac8:	8082                	ret

0000000080000aca <kinit>:
{
    80000aca:	1141                	addi	sp,sp,-16
    80000acc:	e406                	sd	ra,8(sp)
    80000ace:	e022                	sd	s0,0(sp)
    80000ad0:	0800                	addi	s0,sp,16
  initlock(&kmem.lock, "kmem");
    80000ad2:	00006597          	auipc	a1,0x6
    80000ad6:	56e58593          	addi	a1,a1,1390 # 80007040 <etext+0x40>
    80000ada:	00012517          	auipc	a0,0x12
    80000ade:	8ee50513          	addi	a0,a0,-1810 # 800123c8 <kmem>
    80000ae2:	06c000ef          	jal	80000b4e <initlock>
  freerange(end, (void*)PHYSTOP);
    80000ae6:	45c5                	li	a1,17
    80000ae8:	05ee                	slli	a1,a1,0x1b
    80000aea:	00023517          	auipc	a0,0x23
    80000aee:	4a650513          	addi	a0,a0,1190 # 80023f90 <end>
    80000af2:	f91ff0ef          	jal	80000a82 <freerange>
}
    80000af6:	60a2                	ld	ra,8(sp)
    80000af8:	6402                	ld	s0,0(sp)
    80000afa:	0141                	addi	sp,sp,16
    80000afc:	8082                	ret

0000000080000afe <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
void *
kalloc(void)
{
    80000afe:	1101                	addi	sp,sp,-32
    80000b00:	ec06                	sd	ra,24(sp)
    80000b02:	e822                	sd	s0,16(sp)
    80000b04:	e426                	sd	s1,8(sp)
    80000b06:	1000                	addi	s0,sp,32
  struct run *r;

  acquire(&kmem.lock);
    80000b08:	00012497          	auipc	s1,0x12
    80000b0c:	8c048493          	addi	s1,s1,-1856 # 800123c8 <kmem>
    80000b10:	8526                	mv	a0,s1
    80000b12:	0bc000ef          	jal	80000bce <acquire>
  r = kmem.freelist;
    80000b16:	6c84                	ld	s1,24(s1)
  if(r)
    80000b18:	c485                	beqz	s1,80000b40 <kalloc+0x42>
    kmem.freelist = r->next;
    80000b1a:	609c                	ld	a5,0(s1)
    80000b1c:	00012517          	auipc	a0,0x12
    80000b20:	8ac50513          	addi	a0,a0,-1876 # 800123c8 <kmem>
    80000b24:	ed1c                	sd	a5,24(a0)
  release(&kmem.lock);
    80000b26:	140000ef          	jal	80000c66 <release>

  if(r)
    memset((char*)r, 5, PGSIZE); // fill with junk
    80000b2a:	6605                	lui	a2,0x1
    80000b2c:	4595                	li	a1,5
    80000b2e:	8526                	mv	a0,s1
    80000b30:	172000ef          	jal	80000ca2 <memset>
  return (void*)r;
}
    80000b34:	8526                	mv	a0,s1
    80000b36:	60e2                	ld	ra,24(sp)
    80000b38:	6442                	ld	s0,16(sp)
    80000b3a:	64a2                	ld	s1,8(sp)
    80000b3c:	6105                	addi	sp,sp,32
    80000b3e:	8082                	ret
  release(&kmem.lock);
    80000b40:	00012517          	auipc	a0,0x12
    80000b44:	88850513          	addi	a0,a0,-1912 # 800123c8 <kmem>
    80000b48:	11e000ef          	jal	80000c66 <release>
  if(r)
    80000b4c:	b7e5                	j	80000b34 <kalloc+0x36>

0000000080000b4e <initlock>:
#include "proc.h"
#include "defs.h"

void
initlock(struct spinlock *lk, char *name)
{
    80000b4e:	1141                	addi	sp,sp,-16
    80000b50:	e422                	sd	s0,8(sp)
    80000b52:	0800                	addi	s0,sp,16
  lk->name = name;
    80000b54:	e50c                	sd	a1,8(a0)
  lk->locked = 0;
    80000b56:	00052023          	sw	zero,0(a0)
  lk->cpu = 0;
    80000b5a:	00053823          	sd	zero,16(a0)
}
    80000b5e:	6422                	ld	s0,8(sp)
    80000b60:	0141                	addi	sp,sp,16
    80000b62:	8082                	ret

0000000080000b64 <holding>:
// Interrupts must be off.
int
holding(struct spinlock *lk)
{
  int r;
  r = (lk->locked && lk->cpu == mycpu());
    80000b64:	411c                	lw	a5,0(a0)
    80000b66:	e399                	bnez	a5,80000b6c <holding+0x8>
    80000b68:	4501                	li	a0,0
  return r;
}
    80000b6a:	8082                	ret
{
    80000b6c:	1101                	addi	sp,sp,-32
    80000b6e:	ec06                	sd	ra,24(sp)
    80000b70:	e822                	sd	s0,16(sp)
    80000b72:	e426                	sd	s1,8(sp)
    80000b74:	1000                	addi	s0,sp,32
  r = (lk->locked && lk->cpu == mycpu());
    80000b76:	6904                	ld	s1,16(a0)
    80000b78:	547000ef          	jal	800018be <mycpu>
    80000b7c:	40a48533          	sub	a0,s1,a0
    80000b80:	00153513          	seqz	a0,a0
}
    80000b84:	60e2                	ld	ra,24(sp)
    80000b86:	6442                	ld	s0,16(sp)
    80000b88:	64a2                	ld	s1,8(sp)
    80000b8a:	6105                	addi	sp,sp,32
    80000b8c:	8082                	ret

0000000080000b8e <push_off>:
// it takes two pop_off()s to undo two push_off()s.  Also, if interrupts
// are initially off, then push_off, pop_off leaves them off.

void
push_off(void)
{
    80000b8e:	1101                	addi	sp,sp,-32
    80000b90:	ec06                	sd	ra,24(sp)
    80000b92:	e822                	sd	s0,16(sp)
    80000b94:	e426                	sd	s1,8(sp)
    80000b96:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000b98:	100024f3          	csrr	s1,sstatus
    80000b9c:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80000ba0:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000ba2:	10079073          	csrw	sstatus,a5

  // disable interrupts to prevent an involuntary context
  // switch while using mycpu().
  intr_off();

  if(mycpu()->noff == 0)
    80000ba6:	519000ef          	jal	800018be <mycpu>
    80000baa:	5d3c                	lw	a5,120(a0)
    80000bac:	cb99                	beqz	a5,80000bc2 <push_off+0x34>
    mycpu()->intena = old;
  mycpu()->noff += 1;
    80000bae:	511000ef          	jal	800018be <mycpu>
    80000bb2:	5d3c                	lw	a5,120(a0)
    80000bb4:	2785                	addiw	a5,a5,1
    80000bb6:	dd3c                	sw	a5,120(a0)
}
    80000bb8:	60e2                	ld	ra,24(sp)
    80000bba:	6442                	ld	s0,16(sp)
    80000bbc:	64a2                	ld	s1,8(sp)
    80000bbe:	6105                	addi	sp,sp,32
    80000bc0:	8082                	ret
    mycpu()->intena = old;
    80000bc2:	4fd000ef          	jal	800018be <mycpu>
  return (x & SSTATUS_SIE) != 0;
    80000bc6:	8085                	srli	s1,s1,0x1
    80000bc8:	8885                	andi	s1,s1,1
    80000bca:	dd64                	sw	s1,124(a0)
    80000bcc:	b7cd                	j	80000bae <push_off+0x20>

0000000080000bce <acquire>:
{
    80000bce:	1101                	addi	sp,sp,-32
    80000bd0:	ec06                	sd	ra,24(sp)
    80000bd2:	e822                	sd	s0,16(sp)
    80000bd4:	e426                	sd	s1,8(sp)
    80000bd6:	1000                	addi	s0,sp,32
    80000bd8:	84aa                	mv	s1,a0
  push_off(); // disable interrupts to avoid deadlock.
    80000bda:	fb5ff0ef          	jal	80000b8e <push_off>
  if(holding(lk))
    80000bde:	8526                	mv	a0,s1
    80000be0:	f85ff0ef          	jal	80000b64 <holding>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000be4:	4705                	li	a4,1
  if(holding(lk))
    80000be6:	e105                	bnez	a0,80000c06 <acquire+0x38>
  while(__sync_lock_test_and_set(&lk->locked, 1) != 0)
    80000be8:	87ba                	mv	a5,a4
    80000bea:	0cf4a7af          	amoswap.w.aq	a5,a5,(s1)
    80000bee:	2781                	sext.w	a5,a5
    80000bf0:	ffe5                	bnez	a5,80000be8 <acquire+0x1a>
  __sync_synchronize();
    80000bf2:	0330000f          	fence	rw,rw
  lk->cpu = mycpu();
    80000bf6:	4c9000ef          	jal	800018be <mycpu>
    80000bfa:	e888                	sd	a0,16(s1)
}
    80000bfc:	60e2                	ld	ra,24(sp)
    80000bfe:	6442                	ld	s0,16(sp)
    80000c00:	64a2                	ld	s1,8(sp)
    80000c02:	6105                	addi	sp,sp,32
    80000c04:	8082                	ret
    panic("acquire");
    80000c06:	00006517          	auipc	a0,0x6
    80000c0a:	44250513          	addi	a0,a0,1090 # 80007048 <etext+0x48>
    80000c0e:	bd3ff0ef          	jal	800007e0 <panic>

0000000080000c12 <pop_off>:

void
pop_off(void)
{
    80000c12:	1141                	addi	sp,sp,-16
    80000c14:	e406                	sd	ra,8(sp)
    80000c16:	e022                	sd	s0,0(sp)
    80000c18:	0800                	addi	s0,sp,16
  struct cpu *c = mycpu();
    80000c1a:	4a5000ef          	jal	800018be <mycpu>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c1e:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80000c22:	8b89                	andi	a5,a5,2
  if(intr_get())
    80000c24:	e78d                	bnez	a5,80000c4e <pop_off+0x3c>
    panic("pop_off - interruptible");
  if(c->noff < 1)
    80000c26:	5d3c                	lw	a5,120(a0)
    80000c28:	02f05963          	blez	a5,80000c5a <pop_off+0x48>
    panic("pop_off");
  c->noff -= 1;
    80000c2c:	37fd                	addiw	a5,a5,-1
    80000c2e:	0007871b          	sext.w	a4,a5
    80000c32:	dd3c                	sw	a5,120(a0)
  if(c->noff == 0 && c->intena)
    80000c34:	eb09                	bnez	a4,80000c46 <pop_off+0x34>
    80000c36:	5d7c                	lw	a5,124(a0)
    80000c38:	c799                	beqz	a5,80000c46 <pop_off+0x34>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80000c3a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80000c3e:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80000c42:	10079073          	csrw	sstatus,a5
    intr_on();
}
    80000c46:	60a2                	ld	ra,8(sp)
    80000c48:	6402                	ld	s0,0(sp)
    80000c4a:	0141                	addi	sp,sp,16
    80000c4c:	8082                	ret
    panic("pop_off - interruptible");
    80000c4e:	00006517          	auipc	a0,0x6
    80000c52:	40250513          	addi	a0,a0,1026 # 80007050 <etext+0x50>
    80000c56:	b8bff0ef          	jal	800007e0 <panic>
    panic("pop_off");
    80000c5a:	00006517          	auipc	a0,0x6
    80000c5e:	40e50513          	addi	a0,a0,1038 # 80007068 <etext+0x68>
    80000c62:	b7fff0ef          	jal	800007e0 <panic>

0000000080000c66 <release>:
{
    80000c66:	1101                	addi	sp,sp,-32
    80000c68:	ec06                	sd	ra,24(sp)
    80000c6a:	e822                	sd	s0,16(sp)
    80000c6c:	e426                	sd	s1,8(sp)
    80000c6e:	1000                	addi	s0,sp,32
    80000c70:	84aa                	mv	s1,a0
  if(!holding(lk))
    80000c72:	ef3ff0ef          	jal	80000b64 <holding>
    80000c76:	c105                	beqz	a0,80000c96 <release+0x30>
  lk->cpu = 0;
    80000c78:	0004b823          	sd	zero,16(s1)
  __sync_synchronize();
    80000c7c:	0330000f          	fence	rw,rw
  __sync_lock_release(&lk->locked);
    80000c80:	0310000f          	fence	rw,w
    80000c84:	0004a023          	sw	zero,0(s1)
  pop_off();
    80000c88:	f8bff0ef          	jal	80000c12 <pop_off>
}
    80000c8c:	60e2                	ld	ra,24(sp)
    80000c8e:	6442                	ld	s0,16(sp)
    80000c90:	64a2                	ld	s1,8(sp)
    80000c92:	6105                	addi	sp,sp,32
    80000c94:	8082                	ret
    panic("release");
    80000c96:	00006517          	auipc	a0,0x6
    80000c9a:	3da50513          	addi	a0,a0,986 # 80007070 <etext+0x70>
    80000c9e:	b43ff0ef          	jal	800007e0 <panic>

0000000080000ca2 <memset>:
#include "types.h"

void*
memset(void *dst, int c, uint n)
{
    80000ca2:	1141                	addi	sp,sp,-16
    80000ca4:	e422                	sd	s0,8(sp)
    80000ca6:	0800                	addi	s0,sp,16
  char *cdst = (char *) dst;
  int i;
  for(i = 0; i < n; i++){
    80000ca8:	ca19                	beqz	a2,80000cbe <memset+0x1c>
    80000caa:	87aa                	mv	a5,a0
    80000cac:	1602                	slli	a2,a2,0x20
    80000cae:	9201                	srli	a2,a2,0x20
    80000cb0:	00a60733          	add	a4,a2,a0
    cdst[i] = c;
    80000cb4:	00b78023          	sb	a1,0(a5)
  for(i = 0; i < n; i++){
    80000cb8:	0785                	addi	a5,a5,1
    80000cba:	fee79de3          	bne	a5,a4,80000cb4 <memset+0x12>
  }
  return dst;
}
    80000cbe:	6422                	ld	s0,8(sp)
    80000cc0:	0141                	addi	sp,sp,16
    80000cc2:	8082                	ret

0000000080000cc4 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
    80000cc4:	1141                	addi	sp,sp,-16
    80000cc6:	e422                	sd	s0,8(sp)
    80000cc8:	0800                	addi	s0,sp,16
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
    80000cca:	ca05                	beqz	a2,80000cfa <memcmp+0x36>
    80000ccc:	fff6069b          	addiw	a3,a2,-1 # fff <_entry-0x7ffff001>
    80000cd0:	1682                	slli	a3,a3,0x20
    80000cd2:	9281                	srli	a3,a3,0x20
    80000cd4:	0685                	addi	a3,a3,1
    80000cd6:	96aa                	add	a3,a3,a0
    if(*s1 != *s2)
    80000cd8:	00054783          	lbu	a5,0(a0)
    80000cdc:	0005c703          	lbu	a4,0(a1)
    80000ce0:	00e79863          	bne	a5,a4,80000cf0 <memcmp+0x2c>
      return *s1 - *s2;
    s1++, s2++;
    80000ce4:	0505                	addi	a0,a0,1
    80000ce6:	0585                	addi	a1,a1,1
  while(n-- > 0){
    80000ce8:	fed518e3          	bne	a0,a3,80000cd8 <memcmp+0x14>
  }

  return 0;
    80000cec:	4501                	li	a0,0
    80000cee:	a019                	j	80000cf4 <memcmp+0x30>
      return *s1 - *s2;
    80000cf0:	40e7853b          	subw	a0,a5,a4
}
    80000cf4:	6422                	ld	s0,8(sp)
    80000cf6:	0141                	addi	sp,sp,16
    80000cf8:	8082                	ret
  return 0;
    80000cfa:	4501                	li	a0,0
    80000cfc:	bfe5                	j	80000cf4 <memcmp+0x30>

0000000080000cfe <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
    80000cfe:	1141                	addi	sp,sp,-16
    80000d00:	e422                	sd	s0,8(sp)
    80000d02:	0800                	addi	s0,sp,16
  const char *s;
  char *d;

  if(n == 0)
    80000d04:	c205                	beqz	a2,80000d24 <memmove+0x26>
    return dst;
  
  s = src;
  d = dst;
  if(s < d && s + n > d){
    80000d06:	02a5e263          	bltu	a1,a0,80000d2a <memmove+0x2c>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
    80000d0a:	1602                	slli	a2,a2,0x20
    80000d0c:	9201                	srli	a2,a2,0x20
    80000d0e:	00c587b3          	add	a5,a1,a2
{
    80000d12:	872a                	mv	a4,a0
      *d++ = *s++;
    80000d14:	0585                	addi	a1,a1,1
    80000d16:	0705                	addi	a4,a4,1 # fffffffffffff001 <end+0xffffffff7ffdb071>
    80000d18:	fff5c683          	lbu	a3,-1(a1)
    80000d1c:	fed70fa3          	sb	a3,-1(a4)
    while(n-- > 0)
    80000d20:	feb79ae3          	bne	a5,a1,80000d14 <memmove+0x16>

  return dst;
}
    80000d24:	6422                	ld	s0,8(sp)
    80000d26:	0141                	addi	sp,sp,16
    80000d28:	8082                	ret
  if(s < d && s + n > d){
    80000d2a:	02061693          	slli	a3,a2,0x20
    80000d2e:	9281                	srli	a3,a3,0x20
    80000d30:	00d58733          	add	a4,a1,a3
    80000d34:	fce57be3          	bgeu	a0,a4,80000d0a <memmove+0xc>
    d += n;
    80000d38:	96aa                	add	a3,a3,a0
    while(n-- > 0)
    80000d3a:	fff6079b          	addiw	a5,a2,-1
    80000d3e:	1782                	slli	a5,a5,0x20
    80000d40:	9381                	srli	a5,a5,0x20
    80000d42:	fff7c793          	not	a5,a5
    80000d46:	97ba                	add	a5,a5,a4
      *--d = *--s;
    80000d48:	177d                	addi	a4,a4,-1
    80000d4a:	16fd                	addi	a3,a3,-1
    80000d4c:	00074603          	lbu	a2,0(a4)
    80000d50:	00c68023          	sb	a2,0(a3)
    while(n-- > 0)
    80000d54:	fef71ae3          	bne	a4,a5,80000d48 <memmove+0x4a>
    80000d58:	b7f1                	j	80000d24 <memmove+0x26>

0000000080000d5a <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
    80000d5a:	1141                	addi	sp,sp,-16
    80000d5c:	e406                	sd	ra,8(sp)
    80000d5e:	e022                	sd	s0,0(sp)
    80000d60:	0800                	addi	s0,sp,16
  return memmove(dst, src, n);
    80000d62:	f9dff0ef          	jal	80000cfe <memmove>
}
    80000d66:	60a2                	ld	ra,8(sp)
    80000d68:	6402                	ld	s0,0(sp)
    80000d6a:	0141                	addi	sp,sp,16
    80000d6c:	8082                	ret

0000000080000d6e <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
    80000d6e:	1141                	addi	sp,sp,-16
    80000d70:	e422                	sd	s0,8(sp)
    80000d72:	0800                	addi	s0,sp,16
  while(n > 0 && *p && *p == *q)
    80000d74:	ce11                	beqz	a2,80000d90 <strncmp+0x22>
    80000d76:	00054783          	lbu	a5,0(a0)
    80000d7a:	cf89                	beqz	a5,80000d94 <strncmp+0x26>
    80000d7c:	0005c703          	lbu	a4,0(a1)
    80000d80:	00f71a63          	bne	a4,a5,80000d94 <strncmp+0x26>
    n--, p++, q++;
    80000d84:	367d                	addiw	a2,a2,-1
    80000d86:	0505                	addi	a0,a0,1
    80000d88:	0585                	addi	a1,a1,1
  while(n > 0 && *p && *p == *q)
    80000d8a:	f675                	bnez	a2,80000d76 <strncmp+0x8>
  if(n == 0)
    return 0;
    80000d8c:	4501                	li	a0,0
    80000d8e:	a801                	j	80000d9e <strncmp+0x30>
    80000d90:	4501                	li	a0,0
    80000d92:	a031                	j	80000d9e <strncmp+0x30>
  return (uchar)*p - (uchar)*q;
    80000d94:	00054503          	lbu	a0,0(a0)
    80000d98:	0005c783          	lbu	a5,0(a1)
    80000d9c:	9d1d                	subw	a0,a0,a5
}
    80000d9e:	6422                	ld	s0,8(sp)
    80000da0:	0141                	addi	sp,sp,16
    80000da2:	8082                	ret

0000000080000da4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
    80000da4:	1141                	addi	sp,sp,-16
    80000da6:	e422                	sd	s0,8(sp)
    80000da8:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    80000daa:	87aa                	mv	a5,a0
    80000dac:	86b2                	mv	a3,a2
    80000dae:	367d                	addiw	a2,a2,-1
    80000db0:	02d05563          	blez	a3,80000dda <strncpy+0x36>
    80000db4:	0785                	addi	a5,a5,1
    80000db6:	0005c703          	lbu	a4,0(a1)
    80000dba:	fee78fa3          	sb	a4,-1(a5)
    80000dbe:	0585                	addi	a1,a1,1
    80000dc0:	f775                	bnez	a4,80000dac <strncpy+0x8>
    ;
  while(n-- > 0)
    80000dc2:	873e                	mv	a4,a5
    80000dc4:	9fb5                	addw	a5,a5,a3
    80000dc6:	37fd                	addiw	a5,a5,-1
    80000dc8:	00c05963          	blez	a2,80000dda <strncpy+0x36>
    *s++ = 0;
    80000dcc:	0705                	addi	a4,a4,1
    80000dce:	fe070fa3          	sb	zero,-1(a4)
  while(n-- > 0)
    80000dd2:	40e786bb          	subw	a3,a5,a4
    80000dd6:	fed04be3          	bgtz	a3,80000dcc <strncpy+0x28>
  return os;
}
    80000dda:	6422                	ld	s0,8(sp)
    80000ddc:	0141                	addi	sp,sp,16
    80000dde:	8082                	ret

0000000080000de0 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
    80000de0:	1141                	addi	sp,sp,-16
    80000de2:	e422                	sd	s0,8(sp)
    80000de4:	0800                	addi	s0,sp,16
  char *os;

  os = s;
  if(n <= 0)
    80000de6:	02c05363          	blez	a2,80000e0c <safestrcpy+0x2c>
    80000dea:	fff6069b          	addiw	a3,a2,-1
    80000dee:	1682                	slli	a3,a3,0x20
    80000df0:	9281                	srli	a3,a3,0x20
    80000df2:	96ae                	add	a3,a3,a1
    80000df4:	87aa                	mv	a5,a0
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
    80000df6:	00d58963          	beq	a1,a3,80000e08 <safestrcpy+0x28>
    80000dfa:	0585                	addi	a1,a1,1
    80000dfc:	0785                	addi	a5,a5,1
    80000dfe:	fff5c703          	lbu	a4,-1(a1)
    80000e02:	fee78fa3          	sb	a4,-1(a5)
    80000e06:	fb65                	bnez	a4,80000df6 <safestrcpy+0x16>
    ;
  *s = 0;
    80000e08:	00078023          	sb	zero,0(a5)
  return os;
}
    80000e0c:	6422                	ld	s0,8(sp)
    80000e0e:	0141                	addi	sp,sp,16
    80000e10:	8082                	ret

0000000080000e12 <strlen>:

int
strlen(const char *s)
{
    80000e12:	1141                	addi	sp,sp,-16
    80000e14:	e422                	sd	s0,8(sp)
    80000e16:	0800                	addi	s0,sp,16
  int n;

  for(n = 0; s[n]; n++)
    80000e18:	00054783          	lbu	a5,0(a0)
    80000e1c:	cf91                	beqz	a5,80000e38 <strlen+0x26>
    80000e1e:	0505                	addi	a0,a0,1
    80000e20:	87aa                	mv	a5,a0
    80000e22:	86be                	mv	a3,a5
    80000e24:	0785                	addi	a5,a5,1
    80000e26:	fff7c703          	lbu	a4,-1(a5)
    80000e2a:	ff65                	bnez	a4,80000e22 <strlen+0x10>
    80000e2c:	40a6853b          	subw	a0,a3,a0
    80000e30:	2505                	addiw	a0,a0,1
    ;
  return n;
}
    80000e32:	6422                	ld	s0,8(sp)
    80000e34:	0141                	addi	sp,sp,16
    80000e36:	8082                	ret
  for(n = 0; s[n]; n++)
    80000e38:	4501                	li	a0,0
    80000e3a:	bfe5                	j	80000e32 <strlen+0x20>

0000000080000e3c <main>:
volatile static int started = 0;

// start() jumps here in supervisor mode on all CPUs.
void
main()
{
    80000e3c:	1141                	addi	sp,sp,-16
    80000e3e:	e406                	sd	ra,8(sp)
    80000e40:	e022                	sd	s0,0(sp)
    80000e42:	0800                	addi	s0,sp,16
  if(cpuid() == 0){
    80000e44:	26b000ef          	jal	800018ae <cpuid>
    virtio_disk_init(); // emulated hard disk
    userinit();      // first user process
    __sync_synchronize();
    started = 1;
  } else {
    while(started == 0)
    80000e48:	00009717          	auipc	a4,0x9
    80000e4c:	48870713          	addi	a4,a4,1160 # 8000a2d0 <started>
  if(cpuid() == 0){
    80000e50:	c51d                	beqz	a0,80000e7e <main+0x42>
    while(started == 0)
    80000e52:	431c                	lw	a5,0(a4)
    80000e54:	2781                	sext.w	a5,a5
    80000e56:	dff5                	beqz	a5,80000e52 <main+0x16>
      ;
    __sync_synchronize();
    80000e58:	0330000f          	fence	rw,rw
    printf("hart %d starting\n", cpuid());
    80000e5c:	253000ef          	jal	800018ae <cpuid>
    80000e60:	85aa                	mv	a1,a0
    80000e62:	00006517          	auipc	a0,0x6
    80000e66:	23650513          	addi	a0,a0,566 # 80007098 <etext+0x98>
    80000e6a:	e90ff0ef          	jal	800004fa <printf>
    kvminithart();    // turn on paging
    80000e6e:	08c000ef          	jal	80000efa <kvminithart>
    trapinithart();   // install kernel trap vector
    80000e72:	594010ef          	jal	80002406 <trapinithart>
    plicinithart();   // ask PLIC for device interrupts
    80000e76:	5d2040ef          	jal	80005448 <plicinithart>
  }

  scheduler();        
    80000e7a:	6d3000ef          	jal	80001d4c <scheduler>
    consoleinit();
    80000e7e:	da6ff0ef          	jal	80000424 <consoleinit>
    printfinit();
    80000e82:	99bff0ef          	jal	8000081c <printfinit>
    printf("\n");
    80000e86:	00006517          	auipc	a0,0x6
    80000e8a:	1f250513          	addi	a0,a0,498 # 80007078 <etext+0x78>
    80000e8e:	e6cff0ef          	jal	800004fa <printf>
    printf("xv6 kernel is booting\n");
    80000e92:	00006517          	auipc	a0,0x6
    80000e96:	1ee50513          	addi	a0,a0,494 # 80007080 <etext+0x80>
    80000e9a:	e60ff0ef          	jal	800004fa <printf>
    printf("\n");
    80000e9e:	00006517          	auipc	a0,0x6
    80000ea2:	1da50513          	addi	a0,a0,474 # 80007078 <etext+0x78>
    80000ea6:	e54ff0ef          	jal	800004fa <printf>
    kinit();         // physical page allocator
    80000eaa:	c21ff0ef          	jal	80000aca <kinit>
    kvminit();       // create kernel page table
    80000eae:	2d6000ef          	jal	80001184 <kvminit>
    kvminithart();   // turn on paging
    80000eb2:	048000ef          	jal	80000efa <kvminithart>
    procinit();      // process table
    80000eb6:	143000ef          	jal	800017f8 <procinit>
    trapinit();      // trap vectors
    80000eba:	528010ef          	jal	800023e2 <trapinit>
    trapinithart();  // install kernel trap vector
    80000ebe:	548010ef          	jal	80002406 <trapinithart>
    plicinit();      // set up interrupt controller
    80000ec2:	56c040ef          	jal	8000542e <plicinit>
    plicinithart();  // ask PLIC for device interrupts
    80000ec6:	582040ef          	jal	80005448 <plicinithart>
    binit();         // buffer cache
    80000eca:	3d3010ef          	jal	80002a9c <binit>
    iinit();         // inode table
    80000ece:	158020ef          	jal	80003026 <iinit>
    fileinit();      // file table
    80000ed2:	04a030ef          	jal	80003f1c <fileinit>
    fmlockinit();
    80000ed6:	5ec040ef          	jal	800054c2 <fmlockinit>
    virtio_disk_init();
    80000eda:	0fb040ef          	jal	800057d4 <virtio_disk_init>
    userinit();
    80000ede:	4c3000ef          	jal	80001ba0 <userinit>
    virtio_disk_init(); // emulated hard disk
    80000ee2:	0f3040ef          	jal	800057d4 <virtio_disk_init>
    userinit();      // first user process
    80000ee6:	4bb000ef          	jal	80001ba0 <userinit>
    __sync_synchronize();
    80000eea:	0330000f          	fence	rw,rw
    started = 1;
    80000eee:	4785                	li	a5,1
    80000ef0:	00009717          	auipc	a4,0x9
    80000ef4:	3ef72023          	sw	a5,992(a4) # 8000a2d0 <started>
    80000ef8:	b749                	j	80000e7a <main+0x3e>

0000000080000efa <kvminithart>:

// Switch the current CPU's h/w page table register to
// the kernel's page table, and enable paging.
void
kvminithart()
{
    80000efa:	1141                	addi	sp,sp,-16
    80000efc:	e422                	sd	s0,8(sp)
    80000efe:	0800                	addi	s0,sp,16
// flush the TLB.
static inline void
sfence_vma()
{
  // the zero, zero means flush all TLB entries.
  asm volatile("sfence.vma zero, zero");
    80000f00:	12000073          	sfence.vma
  // wait for any previous writes to the page table memory to finish.
  sfence_vma();

  w_satp(MAKE_SATP(kernel_pagetable));
    80000f04:	00009797          	auipc	a5,0x9
    80000f08:	3d47b783          	ld	a5,980(a5) # 8000a2d8 <kernel_pagetable>
    80000f0c:	83b1                	srli	a5,a5,0xc
    80000f0e:	577d                	li	a4,-1
    80000f10:	177e                	slli	a4,a4,0x3f
    80000f12:	8fd9                	or	a5,a5,a4
  asm volatile("csrw satp, %0" : : "r" (x));
    80000f14:	18079073          	csrw	satp,a5
  asm volatile("sfence.vma zero, zero");
    80000f18:	12000073          	sfence.vma

  // flush stale entries from the TLB.
  sfence_vma();
}
    80000f1c:	6422                	ld	s0,8(sp)
    80000f1e:	0141                	addi	sp,sp,16
    80000f20:	8082                	ret

0000000080000f22 <walk>:
//   21..29 -- 9 bits of level-1 index.
//   12..20 -- 9 bits of level-0 index.
//    0..11 -- 12 bits of byte offset within the page.
pte_t *
walk(pagetable_t pagetable, uint64 va, int alloc)
{
    80000f22:	7139                	addi	sp,sp,-64
    80000f24:	fc06                	sd	ra,56(sp)
    80000f26:	f822                	sd	s0,48(sp)
    80000f28:	f426                	sd	s1,40(sp)
    80000f2a:	f04a                	sd	s2,32(sp)
    80000f2c:	ec4e                	sd	s3,24(sp)
    80000f2e:	e852                	sd	s4,16(sp)
    80000f30:	e456                	sd	s5,8(sp)
    80000f32:	e05a                	sd	s6,0(sp)
    80000f34:	0080                	addi	s0,sp,64
    80000f36:	84aa                	mv	s1,a0
    80000f38:	89ae                	mv	s3,a1
    80000f3a:	8ab2                	mv	s5,a2
  if(va >= MAXVA)
    80000f3c:	57fd                	li	a5,-1
    80000f3e:	83e9                	srli	a5,a5,0x1a
    80000f40:	4a79                	li	s4,30
    panic("walk");

  for(int level = 2; level > 0; level--) {
    80000f42:	4b31                	li	s6,12
  if(va >= MAXVA)
    80000f44:	02b7fc63          	bgeu	a5,a1,80000f7c <walk+0x5a>
    panic("walk");
    80000f48:	00006517          	auipc	a0,0x6
    80000f4c:	16850513          	addi	a0,a0,360 # 800070b0 <etext+0xb0>
    80000f50:	891ff0ef          	jal	800007e0 <panic>
    pte_t *pte = &pagetable[PX(level, va)];
    if(*pte & PTE_V) {
      pagetable = (pagetable_t)PTE2PA(*pte);
    } else {
      if(!alloc || (pagetable = (pde_t*)kalloc()) == 0)
    80000f54:	060a8263          	beqz	s5,80000fb8 <walk+0x96>
    80000f58:	ba7ff0ef          	jal	80000afe <kalloc>
    80000f5c:	84aa                	mv	s1,a0
    80000f5e:	c139                	beqz	a0,80000fa4 <walk+0x82>
        return 0;
      memset(pagetable, 0, PGSIZE);
    80000f60:	6605                	lui	a2,0x1
    80000f62:	4581                	li	a1,0
    80000f64:	d3fff0ef          	jal	80000ca2 <memset>
      *pte = PA2PTE(pagetable) | PTE_V;
    80000f68:	00c4d793          	srli	a5,s1,0xc
    80000f6c:	07aa                	slli	a5,a5,0xa
    80000f6e:	0017e793          	ori	a5,a5,1
    80000f72:	00f93023          	sd	a5,0(s2)
  for(int level = 2; level > 0; level--) {
    80000f76:	3a5d                	addiw	s4,s4,-9 # ffffffffffffeff7 <end+0xffffffff7ffdb067>
    80000f78:	036a0063          	beq	s4,s6,80000f98 <walk+0x76>
    pte_t *pte = &pagetable[PX(level, va)];
    80000f7c:	0149d933          	srl	s2,s3,s4
    80000f80:	1ff97913          	andi	s2,s2,511
    80000f84:	090e                	slli	s2,s2,0x3
    80000f86:	9926                	add	s2,s2,s1
    if(*pte & PTE_V) {
    80000f88:	00093483          	ld	s1,0(s2)
    80000f8c:	0014f793          	andi	a5,s1,1
    80000f90:	d3f1                	beqz	a5,80000f54 <walk+0x32>
      pagetable = (pagetable_t)PTE2PA(*pte);
    80000f92:	80a9                	srli	s1,s1,0xa
    80000f94:	04b2                	slli	s1,s1,0xc
    80000f96:	b7c5                	j	80000f76 <walk+0x54>
    }
  }
  return &pagetable[PX(0, va)];
    80000f98:	00c9d513          	srli	a0,s3,0xc
    80000f9c:	1ff57513          	andi	a0,a0,511
    80000fa0:	050e                	slli	a0,a0,0x3
    80000fa2:	9526                	add	a0,a0,s1
}
    80000fa4:	70e2                	ld	ra,56(sp)
    80000fa6:	7442                	ld	s0,48(sp)
    80000fa8:	74a2                	ld	s1,40(sp)
    80000faa:	7902                	ld	s2,32(sp)
    80000fac:	69e2                	ld	s3,24(sp)
    80000fae:	6a42                	ld	s4,16(sp)
    80000fb0:	6aa2                	ld	s5,8(sp)
    80000fb2:	6b02                	ld	s6,0(sp)
    80000fb4:	6121                	addi	sp,sp,64
    80000fb6:	8082                	ret
        return 0;
    80000fb8:	4501                	li	a0,0
    80000fba:	b7ed                	j	80000fa4 <walk+0x82>

0000000080000fbc <walkaddr>:
walkaddr(pagetable_t pagetable, uint64 va)
{
  pte_t *pte;
  uint64 pa;

  if(va >= MAXVA)
    80000fbc:	57fd                	li	a5,-1
    80000fbe:	83e9                	srli	a5,a5,0x1a
    80000fc0:	00b7f463          	bgeu	a5,a1,80000fc8 <walkaddr+0xc>
    return 0;
    80000fc4:	4501                	li	a0,0
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  pa = PTE2PA(*pte);
  return pa;
}
    80000fc6:	8082                	ret
{
    80000fc8:	1141                	addi	sp,sp,-16
    80000fca:	e406                	sd	ra,8(sp)
    80000fcc:	e022                	sd	s0,0(sp)
    80000fce:	0800                	addi	s0,sp,16
  pte = walk(pagetable, va, 0);
    80000fd0:	4601                	li	a2,0
    80000fd2:	f51ff0ef          	jal	80000f22 <walk>
  if(pte == 0)
    80000fd6:	c105                	beqz	a0,80000ff6 <walkaddr+0x3a>
  if((*pte & PTE_V) == 0)
    80000fd8:	611c                	ld	a5,0(a0)
  if((*pte & PTE_U) == 0)
    80000fda:	0117f693          	andi	a3,a5,17
    80000fde:	4745                	li	a4,17
    return 0;
    80000fe0:	4501                	li	a0,0
  if((*pte & PTE_U) == 0)
    80000fe2:	00e68663          	beq	a3,a4,80000fee <walkaddr+0x32>
}
    80000fe6:	60a2                	ld	ra,8(sp)
    80000fe8:	6402                	ld	s0,0(sp)
    80000fea:	0141                	addi	sp,sp,16
    80000fec:	8082                	ret
  pa = PTE2PA(*pte);
    80000fee:	83a9                	srli	a5,a5,0xa
    80000ff0:	00c79513          	slli	a0,a5,0xc
  return pa;
    80000ff4:	bfcd                	j	80000fe6 <walkaddr+0x2a>
    return 0;
    80000ff6:	4501                	li	a0,0
    80000ff8:	b7fd                	j	80000fe6 <walkaddr+0x2a>

0000000080000ffa <mappages>:
// va and size MUST be page-aligned.
// Returns 0 on success, -1 if walk() couldn't
// allocate a needed page-table page.
int
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
    80000ffa:	715d                	addi	sp,sp,-80
    80000ffc:	e486                	sd	ra,72(sp)
    80000ffe:	e0a2                	sd	s0,64(sp)
    80001000:	fc26                	sd	s1,56(sp)
    80001002:	f84a                	sd	s2,48(sp)
    80001004:	f44e                	sd	s3,40(sp)
    80001006:	f052                	sd	s4,32(sp)
    80001008:	ec56                	sd	s5,24(sp)
    8000100a:	e85a                	sd	s6,16(sp)
    8000100c:	e45e                	sd	s7,8(sp)
    8000100e:	0880                	addi	s0,sp,80
  uint64 a, last;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    80001010:	03459793          	slli	a5,a1,0x34
    80001014:	e7a9                	bnez	a5,8000105e <mappages+0x64>
    80001016:	8aaa                	mv	s5,a0
    80001018:	8b3a                	mv	s6,a4
    panic("mappages: va not aligned");

  if((size % PGSIZE) != 0)
    8000101a:	03461793          	slli	a5,a2,0x34
    8000101e:	e7b1                	bnez	a5,8000106a <mappages+0x70>
    panic("mappages: size not aligned");

  if(size == 0)
    80001020:	ca39                	beqz	a2,80001076 <mappages+0x7c>
    panic("mappages: size");
  
  a = va;
  last = va + size - PGSIZE;
    80001022:	77fd                	lui	a5,0xfffff
    80001024:	963e                	add	a2,a2,a5
    80001026:	00b609b3          	add	s3,a2,a1
  a = va;
    8000102a:	892e                	mv	s2,a1
    8000102c:	40b68a33          	sub	s4,a3,a1
    if(*pte & PTE_V)
      panic("mappages: remap");
    *pte = PA2PTE(pa) | perm | PTE_V;
    if(a == last)
      break;
    a += PGSIZE;
    80001030:	6b85                	lui	s7,0x1
    80001032:	014904b3          	add	s1,s2,s4
    if((pte = walk(pagetable, a, 1)) == 0)
    80001036:	4605                	li	a2,1
    80001038:	85ca                	mv	a1,s2
    8000103a:	8556                	mv	a0,s5
    8000103c:	ee7ff0ef          	jal	80000f22 <walk>
    80001040:	c539                	beqz	a0,8000108e <mappages+0x94>
    if(*pte & PTE_V)
    80001042:	611c                	ld	a5,0(a0)
    80001044:	8b85                	andi	a5,a5,1
    80001046:	ef95                	bnez	a5,80001082 <mappages+0x88>
    *pte = PA2PTE(pa) | perm | PTE_V;
    80001048:	80b1                	srli	s1,s1,0xc
    8000104a:	04aa                	slli	s1,s1,0xa
    8000104c:	0164e4b3          	or	s1,s1,s6
    80001050:	0014e493          	ori	s1,s1,1
    80001054:	e104                	sd	s1,0(a0)
    if(a == last)
    80001056:	05390863          	beq	s2,s3,800010a6 <mappages+0xac>
    a += PGSIZE;
    8000105a:	995e                	add	s2,s2,s7
    if((pte = walk(pagetable, a, 1)) == 0)
    8000105c:	bfd9                	j	80001032 <mappages+0x38>
    panic("mappages: va not aligned");
    8000105e:	00006517          	auipc	a0,0x6
    80001062:	05a50513          	addi	a0,a0,90 # 800070b8 <etext+0xb8>
    80001066:	f7aff0ef          	jal	800007e0 <panic>
    panic("mappages: size not aligned");
    8000106a:	00006517          	auipc	a0,0x6
    8000106e:	06e50513          	addi	a0,a0,110 # 800070d8 <etext+0xd8>
    80001072:	f6eff0ef          	jal	800007e0 <panic>
    panic("mappages: size");
    80001076:	00006517          	auipc	a0,0x6
    8000107a:	08250513          	addi	a0,a0,130 # 800070f8 <etext+0xf8>
    8000107e:	f62ff0ef          	jal	800007e0 <panic>
      panic("mappages: remap");
    80001082:	00006517          	auipc	a0,0x6
    80001086:	08650513          	addi	a0,a0,134 # 80007108 <etext+0x108>
    8000108a:	f56ff0ef          	jal	800007e0 <panic>
      return -1;
    8000108e:	557d                	li	a0,-1
    pa += PGSIZE;
  }
  return 0;
}
    80001090:	60a6                	ld	ra,72(sp)
    80001092:	6406                	ld	s0,64(sp)
    80001094:	74e2                	ld	s1,56(sp)
    80001096:	7942                	ld	s2,48(sp)
    80001098:	79a2                	ld	s3,40(sp)
    8000109a:	7a02                	ld	s4,32(sp)
    8000109c:	6ae2                	ld	s5,24(sp)
    8000109e:	6b42                	ld	s6,16(sp)
    800010a0:	6ba2                	ld	s7,8(sp)
    800010a2:	6161                	addi	sp,sp,80
    800010a4:	8082                	ret
  return 0;
    800010a6:	4501                	li	a0,0
    800010a8:	b7e5                	j	80001090 <mappages+0x96>

00000000800010aa <kvmmap>:
{
    800010aa:	1141                	addi	sp,sp,-16
    800010ac:	e406                	sd	ra,8(sp)
    800010ae:	e022                	sd	s0,0(sp)
    800010b0:	0800                	addi	s0,sp,16
    800010b2:	87b6                	mv	a5,a3
  if(mappages(kpgtbl, va, sz, pa, perm) != 0)
    800010b4:	86b2                	mv	a3,a2
    800010b6:	863e                	mv	a2,a5
    800010b8:	f43ff0ef          	jal	80000ffa <mappages>
    800010bc:	e509                	bnez	a0,800010c6 <kvmmap+0x1c>
}
    800010be:	60a2                	ld	ra,8(sp)
    800010c0:	6402                	ld	s0,0(sp)
    800010c2:	0141                	addi	sp,sp,16
    800010c4:	8082                	ret
    panic("kvmmap");
    800010c6:	00006517          	auipc	a0,0x6
    800010ca:	05250513          	addi	a0,a0,82 # 80007118 <etext+0x118>
    800010ce:	f12ff0ef          	jal	800007e0 <panic>

00000000800010d2 <kvmmake>:
{
    800010d2:	1101                	addi	sp,sp,-32
    800010d4:	ec06                	sd	ra,24(sp)
    800010d6:	e822                	sd	s0,16(sp)
    800010d8:	e426                	sd	s1,8(sp)
    800010da:	e04a                	sd	s2,0(sp)
    800010dc:	1000                	addi	s0,sp,32
  kpgtbl = (pagetable_t) kalloc();
    800010de:	a21ff0ef          	jal	80000afe <kalloc>
    800010e2:	84aa                	mv	s1,a0
  memset(kpgtbl, 0, PGSIZE);
    800010e4:	6605                	lui	a2,0x1
    800010e6:	4581                	li	a1,0
    800010e8:	bbbff0ef          	jal	80000ca2 <memset>
  kvmmap(kpgtbl, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    800010ec:	4719                	li	a4,6
    800010ee:	6685                	lui	a3,0x1
    800010f0:	10000637          	lui	a2,0x10000
    800010f4:	100005b7          	lui	a1,0x10000
    800010f8:	8526                	mv	a0,s1
    800010fa:	fb1ff0ef          	jal	800010aa <kvmmap>
  kvmmap(kpgtbl, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    800010fe:	4719                	li	a4,6
    80001100:	6685                	lui	a3,0x1
    80001102:	10001637          	lui	a2,0x10001
    80001106:	100015b7          	lui	a1,0x10001
    8000110a:	8526                	mv	a0,s1
    8000110c:	f9fff0ef          	jal	800010aa <kvmmap>
  kvmmap(kpgtbl, PLIC, PLIC, 0x4000000, PTE_R | PTE_W);
    80001110:	4719                	li	a4,6
    80001112:	040006b7          	lui	a3,0x4000
    80001116:	0c000637          	lui	a2,0xc000
    8000111a:	0c0005b7          	lui	a1,0xc000
    8000111e:	8526                	mv	a0,s1
    80001120:	f8bff0ef          	jal	800010aa <kvmmap>
  kvmmap(kpgtbl, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
    80001124:	00006917          	auipc	s2,0x6
    80001128:	edc90913          	addi	s2,s2,-292 # 80007000 <etext>
    8000112c:	4729                	li	a4,10
    8000112e:	80006697          	auipc	a3,0x80006
    80001132:	ed268693          	addi	a3,a3,-302 # 7000 <_entry-0x7fff9000>
    80001136:	4605                	li	a2,1
    80001138:	067e                	slli	a2,a2,0x1f
    8000113a:	85b2                	mv	a1,a2
    8000113c:	8526                	mv	a0,s1
    8000113e:	f6dff0ef          	jal	800010aa <kvmmap>
  kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
    80001142:	46c5                	li	a3,17
    80001144:	06ee                	slli	a3,a3,0x1b
    80001146:	4719                	li	a4,6
    80001148:	412686b3          	sub	a3,a3,s2
    8000114c:	864a                	mv	a2,s2
    8000114e:	85ca                	mv	a1,s2
    80001150:	8526                	mv	a0,s1
    80001152:	f59ff0ef          	jal	800010aa <kvmmap>
  kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    80001156:	4729                	li	a4,10
    80001158:	6685                	lui	a3,0x1
    8000115a:	00005617          	auipc	a2,0x5
    8000115e:	ea660613          	addi	a2,a2,-346 # 80006000 <_trampoline>
    80001162:	040005b7          	lui	a1,0x4000
    80001166:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001168:	05b2                	slli	a1,a1,0xc
    8000116a:	8526                	mv	a0,s1
    8000116c:	f3fff0ef          	jal	800010aa <kvmmap>
  proc_mapstacks(kpgtbl);
    80001170:	8526                	mv	a0,s1
    80001172:	5ee000ef          	jal	80001760 <proc_mapstacks>
}
    80001176:	8526                	mv	a0,s1
    80001178:	60e2                	ld	ra,24(sp)
    8000117a:	6442                	ld	s0,16(sp)
    8000117c:	64a2                	ld	s1,8(sp)
    8000117e:	6902                	ld	s2,0(sp)
    80001180:	6105                	addi	sp,sp,32
    80001182:	8082                	ret

0000000080001184 <kvminit>:
{
    80001184:	1141                	addi	sp,sp,-16
    80001186:	e406                	sd	ra,8(sp)
    80001188:	e022                	sd	s0,0(sp)
    8000118a:	0800                	addi	s0,sp,16
  kernel_pagetable = kvmmake();
    8000118c:	f47ff0ef          	jal	800010d2 <kvmmake>
    80001190:	00009797          	auipc	a5,0x9
    80001194:	14a7b423          	sd	a0,328(a5) # 8000a2d8 <kernel_pagetable>
}
    80001198:	60a2                	ld	ra,8(sp)
    8000119a:	6402                	ld	s0,0(sp)
    8000119c:	0141                	addi	sp,sp,16
    8000119e:	8082                	ret

00000000800011a0 <uvmcreate>:

// create an empty user page table.
// returns 0 if out of memory.
pagetable_t
uvmcreate()
{
    800011a0:	1101                	addi	sp,sp,-32
    800011a2:	ec06                	sd	ra,24(sp)
    800011a4:	e822                	sd	s0,16(sp)
    800011a6:	e426                	sd	s1,8(sp)
    800011a8:	1000                	addi	s0,sp,32
  pagetable_t pagetable;
  pagetable = (pagetable_t) kalloc();
    800011aa:	955ff0ef          	jal	80000afe <kalloc>
    800011ae:	84aa                	mv	s1,a0
  if(pagetable == 0)
    800011b0:	c509                	beqz	a0,800011ba <uvmcreate+0x1a>
    return 0;
  memset(pagetable, 0, PGSIZE);
    800011b2:	6605                	lui	a2,0x1
    800011b4:	4581                	li	a1,0
    800011b6:	aedff0ef          	jal	80000ca2 <memset>
  return pagetable;
}
    800011ba:	8526                	mv	a0,s1
    800011bc:	60e2                	ld	ra,24(sp)
    800011be:	6442                	ld	s0,16(sp)
    800011c0:	64a2                	ld	s1,8(sp)
    800011c2:	6105                	addi	sp,sp,32
    800011c4:	8082                	ret

00000000800011c6 <uvmunmap>:
// Remove npages of mappings starting from va. va must be
// page-aligned. It's OK if the mappings don't exist.
// Optionally free the physical memory.
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
    800011c6:	7139                	addi	sp,sp,-64
    800011c8:	fc06                	sd	ra,56(sp)
    800011ca:	f822                	sd	s0,48(sp)
    800011cc:	0080                	addi	s0,sp,64
  uint64 a;
  pte_t *pte;

  if((va % PGSIZE) != 0)
    800011ce:	03459793          	slli	a5,a1,0x34
    800011d2:	e38d                	bnez	a5,800011f4 <uvmunmap+0x2e>
    800011d4:	f04a                	sd	s2,32(sp)
    800011d6:	ec4e                	sd	s3,24(sp)
    800011d8:	e852                	sd	s4,16(sp)
    800011da:	e456                	sd	s5,8(sp)
    800011dc:	e05a                	sd	s6,0(sp)
    800011de:	8a2a                	mv	s4,a0
    800011e0:	892e                	mv	s2,a1
    800011e2:	8ab6                	mv	s5,a3
    panic("uvmunmap: not aligned");

  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    800011e4:	0632                	slli	a2,a2,0xc
    800011e6:	00b609b3          	add	s3,a2,a1
    800011ea:	6b05                	lui	s6,0x1
    800011ec:	0535f963          	bgeu	a1,s3,8000123e <uvmunmap+0x78>
    800011f0:	f426                	sd	s1,40(sp)
    800011f2:	a015                	j	80001216 <uvmunmap+0x50>
    800011f4:	f426                	sd	s1,40(sp)
    800011f6:	f04a                	sd	s2,32(sp)
    800011f8:	ec4e                	sd	s3,24(sp)
    800011fa:	e852                	sd	s4,16(sp)
    800011fc:	e456                	sd	s5,8(sp)
    800011fe:	e05a                	sd	s6,0(sp)
    panic("uvmunmap: not aligned");
    80001200:	00006517          	auipc	a0,0x6
    80001204:	f2050513          	addi	a0,a0,-224 # 80007120 <etext+0x120>
    80001208:	dd8ff0ef          	jal	800007e0 <panic>
      continue;
    if(do_free){
      uint64 pa = PTE2PA(*pte);
      kfree((void*)pa);
    }
    *pte = 0;
    8000120c:	0004b023          	sd	zero,0(s1)
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    80001210:	995a                	add	s2,s2,s6
    80001212:	03397563          	bgeu	s2,s3,8000123c <uvmunmap+0x76>
    if((pte = walk(pagetable, a, 0)) == 0) // leaf page table entry allocated?
    80001216:	4601                	li	a2,0
    80001218:	85ca                	mv	a1,s2
    8000121a:	8552                	mv	a0,s4
    8000121c:	d07ff0ef          	jal	80000f22 <walk>
    80001220:	84aa                	mv	s1,a0
    80001222:	d57d                	beqz	a0,80001210 <uvmunmap+0x4a>
    if((*pte & PTE_V) == 0)  // has physical page been allocated?
    80001224:	611c                	ld	a5,0(a0)
    80001226:	0017f713          	andi	a4,a5,1
    8000122a:	d37d                	beqz	a4,80001210 <uvmunmap+0x4a>
    if(do_free){
    8000122c:	fe0a80e3          	beqz	s5,8000120c <uvmunmap+0x46>
      uint64 pa = PTE2PA(*pte);
    80001230:	83a9                	srli	a5,a5,0xa
      kfree((void*)pa);
    80001232:	00c79513          	slli	a0,a5,0xc
    80001236:	fe6ff0ef          	jal	80000a1c <kfree>
    8000123a:	bfc9                	j	8000120c <uvmunmap+0x46>
    8000123c:	74a2                	ld	s1,40(sp)
    8000123e:	7902                	ld	s2,32(sp)
    80001240:	69e2                	ld	s3,24(sp)
    80001242:	6a42                	ld	s4,16(sp)
    80001244:	6aa2                	ld	s5,8(sp)
    80001246:	6b02                	ld	s6,0(sp)
  }
}
    80001248:	70e2                	ld	ra,56(sp)
    8000124a:	7442                	ld	s0,48(sp)
    8000124c:	6121                	addi	sp,sp,64
    8000124e:	8082                	ret

0000000080001250 <uvmdealloc>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
uint64
uvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    80001250:	1101                	addi	sp,sp,-32
    80001252:	ec06                	sd	ra,24(sp)
    80001254:	e822                	sd	s0,16(sp)
    80001256:	e426                	sd	s1,8(sp)
    80001258:	1000                	addi	s0,sp,32
  if(newsz >= oldsz)
    return oldsz;
    8000125a:	84ae                	mv	s1,a1
  if(newsz >= oldsz)
    8000125c:	00b67d63          	bgeu	a2,a1,80001276 <uvmdealloc+0x26>
    80001260:	84b2                	mv	s1,a2

  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    80001262:	6785                	lui	a5,0x1
    80001264:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    80001266:	00f60733          	add	a4,a2,a5
    8000126a:	76fd                	lui	a3,0xfffff
    8000126c:	8f75                	and	a4,a4,a3
    8000126e:	97ae                	add	a5,a5,a1
    80001270:	8ff5                	and	a5,a5,a3
    80001272:	00f76863          	bltu	a4,a5,80001282 <uvmdealloc+0x32>
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
  }

  return newsz;
}
    80001276:	8526                	mv	a0,s1
    80001278:	60e2                	ld	ra,24(sp)
    8000127a:	6442                	ld	s0,16(sp)
    8000127c:	64a2                	ld	s1,8(sp)
    8000127e:	6105                	addi	sp,sp,32
    80001280:	8082                	ret
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    80001282:	8f99                	sub	a5,a5,a4
    80001284:	83b1                	srli	a5,a5,0xc
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 1);
    80001286:	4685                	li	a3,1
    80001288:	0007861b          	sext.w	a2,a5
    8000128c:	85ba                	mv	a1,a4
    8000128e:	f39ff0ef          	jal	800011c6 <uvmunmap>
    80001292:	b7d5                	j	80001276 <uvmdealloc+0x26>

0000000080001294 <uvmalloc>:
  if(newsz < oldsz)
    80001294:	08b66f63          	bltu	a2,a1,80001332 <uvmalloc+0x9e>
{
    80001298:	7139                	addi	sp,sp,-64
    8000129a:	fc06                	sd	ra,56(sp)
    8000129c:	f822                	sd	s0,48(sp)
    8000129e:	ec4e                	sd	s3,24(sp)
    800012a0:	e852                	sd	s4,16(sp)
    800012a2:	e456                	sd	s5,8(sp)
    800012a4:	0080                	addi	s0,sp,64
    800012a6:	8aaa                	mv	s5,a0
    800012a8:	8a32                	mv	s4,a2
  oldsz = PGROUNDUP(oldsz);
    800012aa:	6785                	lui	a5,0x1
    800012ac:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    800012ae:	95be                	add	a1,a1,a5
    800012b0:	77fd                	lui	a5,0xfffff
    800012b2:	00f5f9b3          	and	s3,a1,a5
  for(a = oldsz; a < newsz; a += PGSIZE){
    800012b6:	08c9f063          	bgeu	s3,a2,80001336 <uvmalloc+0xa2>
    800012ba:	f426                	sd	s1,40(sp)
    800012bc:	f04a                	sd	s2,32(sp)
    800012be:	e05a                	sd	s6,0(sp)
    800012c0:	894e                	mv	s2,s3
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
    800012c2:	0126eb13          	ori	s6,a3,18
    mem = kalloc();
    800012c6:	839ff0ef          	jal	80000afe <kalloc>
    800012ca:	84aa                	mv	s1,a0
    if(mem == 0){
    800012cc:	c515                	beqz	a0,800012f8 <uvmalloc+0x64>
    memset(mem, 0, PGSIZE);
    800012ce:	6605                	lui	a2,0x1
    800012d0:	4581                	li	a1,0
    800012d2:	9d1ff0ef          	jal	80000ca2 <memset>
    if(mappages(pagetable, a, PGSIZE, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
    800012d6:	875a                	mv	a4,s6
    800012d8:	86a6                	mv	a3,s1
    800012da:	6605                	lui	a2,0x1
    800012dc:	85ca                	mv	a1,s2
    800012de:	8556                	mv	a0,s5
    800012e0:	d1bff0ef          	jal	80000ffa <mappages>
    800012e4:	e915                	bnez	a0,80001318 <uvmalloc+0x84>
  for(a = oldsz; a < newsz; a += PGSIZE){
    800012e6:	6785                	lui	a5,0x1
    800012e8:	993e                	add	s2,s2,a5
    800012ea:	fd496ee3          	bltu	s2,s4,800012c6 <uvmalloc+0x32>
  return newsz;
    800012ee:	8552                	mv	a0,s4
    800012f0:	74a2                	ld	s1,40(sp)
    800012f2:	7902                	ld	s2,32(sp)
    800012f4:	6b02                	ld	s6,0(sp)
    800012f6:	a811                	j	8000130a <uvmalloc+0x76>
      uvmdealloc(pagetable, a, oldsz);
    800012f8:	864e                	mv	a2,s3
    800012fa:	85ca                	mv	a1,s2
    800012fc:	8556                	mv	a0,s5
    800012fe:	f53ff0ef          	jal	80001250 <uvmdealloc>
      return 0;
    80001302:	4501                	li	a0,0
    80001304:	74a2                	ld	s1,40(sp)
    80001306:	7902                	ld	s2,32(sp)
    80001308:	6b02                	ld	s6,0(sp)
}
    8000130a:	70e2                	ld	ra,56(sp)
    8000130c:	7442                	ld	s0,48(sp)
    8000130e:	69e2                	ld	s3,24(sp)
    80001310:	6a42                	ld	s4,16(sp)
    80001312:	6aa2                	ld	s5,8(sp)
    80001314:	6121                	addi	sp,sp,64
    80001316:	8082                	ret
      kfree(mem);
    80001318:	8526                	mv	a0,s1
    8000131a:	f02ff0ef          	jal	80000a1c <kfree>
      uvmdealloc(pagetable, a, oldsz);
    8000131e:	864e                	mv	a2,s3
    80001320:	85ca                	mv	a1,s2
    80001322:	8556                	mv	a0,s5
    80001324:	f2dff0ef          	jal	80001250 <uvmdealloc>
      return 0;
    80001328:	4501                	li	a0,0
    8000132a:	74a2                	ld	s1,40(sp)
    8000132c:	7902                	ld	s2,32(sp)
    8000132e:	6b02                	ld	s6,0(sp)
    80001330:	bfe9                	j	8000130a <uvmalloc+0x76>
    return oldsz;
    80001332:	852e                	mv	a0,a1
}
    80001334:	8082                	ret
  return newsz;
    80001336:	8532                	mv	a0,a2
    80001338:	bfc9                	j	8000130a <uvmalloc+0x76>

000000008000133a <freewalk>:

// Recursively free page-table pages.
// All leaf mappings must already have been removed.
void
freewalk(pagetable_t pagetable)
{
    8000133a:	7179                	addi	sp,sp,-48
    8000133c:	f406                	sd	ra,40(sp)
    8000133e:	f022                	sd	s0,32(sp)
    80001340:	ec26                	sd	s1,24(sp)
    80001342:	e84a                	sd	s2,16(sp)
    80001344:	e44e                	sd	s3,8(sp)
    80001346:	e052                	sd	s4,0(sp)
    80001348:	1800                	addi	s0,sp,48
    8000134a:	8a2a                	mv	s4,a0
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){
    8000134c:	84aa                	mv	s1,a0
    8000134e:	6905                	lui	s2,0x1
    80001350:	992a                	add	s2,s2,a0
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    80001352:	4985                	li	s3,1
    80001354:	a819                	j	8000136a <freewalk+0x30>
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
    80001356:	83a9                	srli	a5,a5,0xa
      freewalk((pagetable_t)child);
    80001358:	00c79513          	slli	a0,a5,0xc
    8000135c:	fdfff0ef          	jal	8000133a <freewalk>
      pagetable[i] = 0;
    80001360:	0004b023          	sd	zero,0(s1)
  for(int i = 0; i < 512; i++){
    80001364:	04a1                	addi	s1,s1,8
    80001366:	01248f63          	beq	s1,s2,80001384 <freewalk+0x4a>
    pte_t pte = pagetable[i];
    8000136a:	609c                	ld	a5,0(s1)
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
    8000136c:	00f7f713          	andi	a4,a5,15
    80001370:	ff3703e3          	beq	a4,s3,80001356 <freewalk+0x1c>
    } else if(pte & PTE_V){
    80001374:	8b85                	andi	a5,a5,1
    80001376:	d7fd                	beqz	a5,80001364 <freewalk+0x2a>
      panic("freewalk: leaf");
    80001378:	00006517          	auipc	a0,0x6
    8000137c:	dc050513          	addi	a0,a0,-576 # 80007138 <etext+0x138>
    80001380:	c60ff0ef          	jal	800007e0 <panic>
    }
  }
  kfree((void*)pagetable);
    80001384:	8552                	mv	a0,s4
    80001386:	e96ff0ef          	jal	80000a1c <kfree>
}
    8000138a:	70a2                	ld	ra,40(sp)
    8000138c:	7402                	ld	s0,32(sp)
    8000138e:	64e2                	ld	s1,24(sp)
    80001390:	6942                	ld	s2,16(sp)
    80001392:	69a2                	ld	s3,8(sp)
    80001394:	6a02                	ld	s4,0(sp)
    80001396:	6145                	addi	sp,sp,48
    80001398:	8082                	ret

000000008000139a <uvmfree>:

// Free user memory pages,
// then free page-table pages.
void
uvmfree(pagetable_t pagetable, uint64 sz)
{
    8000139a:	1101                	addi	sp,sp,-32
    8000139c:	ec06                	sd	ra,24(sp)
    8000139e:	e822                	sd	s0,16(sp)
    800013a0:	e426                	sd	s1,8(sp)
    800013a2:	1000                	addi	s0,sp,32
    800013a4:	84aa                	mv	s1,a0
  if(sz > 0)
    800013a6:	e989                	bnez	a1,800013b8 <uvmfree+0x1e>
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  freewalk(pagetable);
    800013a8:	8526                	mv	a0,s1
    800013aa:	f91ff0ef          	jal	8000133a <freewalk>
}
    800013ae:	60e2                	ld	ra,24(sp)
    800013b0:	6442                	ld	s0,16(sp)
    800013b2:	64a2                	ld	s1,8(sp)
    800013b4:	6105                	addi	sp,sp,32
    800013b6:	8082                	ret
    uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
    800013b8:	6785                	lui	a5,0x1
    800013ba:	17fd                	addi	a5,a5,-1 # fff <_entry-0x7ffff001>
    800013bc:	95be                	add	a1,a1,a5
    800013be:	4685                	li	a3,1
    800013c0:	00c5d613          	srli	a2,a1,0xc
    800013c4:	4581                	li	a1,0
    800013c6:	e01ff0ef          	jal	800011c6 <uvmunmap>
    800013ca:	bff9                	j	800013a8 <uvmfree+0xe>

00000000800013cc <uvmcopy>:
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  char *mem;

  for(i = 0; i < sz; i += PGSIZE){
    800013cc:	ce49                	beqz	a2,80001466 <uvmcopy+0x9a>
{
    800013ce:	715d                	addi	sp,sp,-80
    800013d0:	e486                	sd	ra,72(sp)
    800013d2:	e0a2                	sd	s0,64(sp)
    800013d4:	fc26                	sd	s1,56(sp)
    800013d6:	f84a                	sd	s2,48(sp)
    800013d8:	f44e                	sd	s3,40(sp)
    800013da:	f052                	sd	s4,32(sp)
    800013dc:	ec56                	sd	s5,24(sp)
    800013de:	e85a                	sd	s6,16(sp)
    800013e0:	e45e                	sd	s7,8(sp)
    800013e2:	0880                	addi	s0,sp,80
    800013e4:	8aaa                	mv	s5,a0
    800013e6:	8b2e                	mv	s6,a1
    800013e8:	8a32                	mv	s4,a2
  for(i = 0; i < sz; i += PGSIZE){
    800013ea:	4481                	li	s1,0
    800013ec:	a029                	j	800013f6 <uvmcopy+0x2a>
    800013ee:	6785                	lui	a5,0x1
    800013f0:	94be                	add	s1,s1,a5
    800013f2:	0544fe63          	bgeu	s1,s4,8000144e <uvmcopy+0x82>
    if((pte = walk(old, i, 0)) == 0)
    800013f6:	4601                	li	a2,0
    800013f8:	85a6                	mv	a1,s1
    800013fa:	8556                	mv	a0,s5
    800013fc:	b27ff0ef          	jal	80000f22 <walk>
    80001400:	d57d                	beqz	a0,800013ee <uvmcopy+0x22>
      continue;   // page table entry hasn't been allocated
    if((*pte & PTE_V) == 0)
    80001402:	6118                	ld	a4,0(a0)
    80001404:	00177793          	andi	a5,a4,1
    80001408:	d3fd                	beqz	a5,800013ee <uvmcopy+0x22>
      continue;   // physical page hasn't been allocated
    pa = PTE2PA(*pte);
    8000140a:	00a75593          	srli	a1,a4,0xa
    8000140e:	00c59b93          	slli	s7,a1,0xc
    flags = PTE_FLAGS(*pte);
    80001412:	3ff77913          	andi	s2,a4,1023
    if((mem = kalloc()) == 0)
    80001416:	ee8ff0ef          	jal	80000afe <kalloc>
    8000141a:	89aa                	mv	s3,a0
    8000141c:	c105                	beqz	a0,8000143c <uvmcopy+0x70>
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    8000141e:	6605                	lui	a2,0x1
    80001420:	85de                	mv	a1,s7
    80001422:	8ddff0ef          	jal	80000cfe <memmove>
    if(mappages(new, i, PGSIZE, (uint64)mem, flags) != 0){
    80001426:	874a                	mv	a4,s2
    80001428:	86ce                	mv	a3,s3
    8000142a:	6605                	lui	a2,0x1
    8000142c:	85a6                	mv	a1,s1
    8000142e:	855a                	mv	a0,s6
    80001430:	bcbff0ef          	jal	80000ffa <mappages>
    80001434:	dd4d                	beqz	a0,800013ee <uvmcopy+0x22>
      kfree(mem);
    80001436:	854e                	mv	a0,s3
    80001438:	de4ff0ef          	jal	80000a1c <kfree>
    }
  }
  return 0;

 err:
  uvmunmap(new, 0, i / PGSIZE, 1);
    8000143c:	4685                	li	a3,1
    8000143e:	00c4d613          	srli	a2,s1,0xc
    80001442:	4581                	li	a1,0
    80001444:	855a                	mv	a0,s6
    80001446:	d81ff0ef          	jal	800011c6 <uvmunmap>
  return -1;
    8000144a:	557d                	li	a0,-1
    8000144c:	a011                	j	80001450 <uvmcopy+0x84>
  return 0;
    8000144e:	4501                	li	a0,0
}
    80001450:	60a6                	ld	ra,72(sp)
    80001452:	6406                	ld	s0,64(sp)
    80001454:	74e2                	ld	s1,56(sp)
    80001456:	7942                	ld	s2,48(sp)
    80001458:	79a2                	ld	s3,40(sp)
    8000145a:	7a02                	ld	s4,32(sp)
    8000145c:	6ae2                	ld	s5,24(sp)
    8000145e:	6b42                	ld	s6,16(sp)
    80001460:	6ba2                	ld	s7,8(sp)
    80001462:	6161                	addi	sp,sp,80
    80001464:	8082                	ret
  return 0;
    80001466:	4501                	li	a0,0
}
    80001468:	8082                	ret

000000008000146a <uvmclear>:

// mark a PTE invalid for user access.
// used by exec for the user stack guard page.
void
uvmclear(pagetable_t pagetable, uint64 va)
{
    8000146a:	1141                	addi	sp,sp,-16
    8000146c:	e406                	sd	ra,8(sp)
    8000146e:	e022                	sd	s0,0(sp)
    80001470:	0800                	addi	s0,sp,16
  pte_t *pte;
  
  pte = walk(pagetable, va, 0);
    80001472:	4601                	li	a2,0
    80001474:	aafff0ef          	jal	80000f22 <walk>
  if(pte == 0)
    80001478:	c901                	beqz	a0,80001488 <uvmclear+0x1e>
    panic("uvmclear");
  *pte &= ~PTE_U;
    8000147a:	611c                	ld	a5,0(a0)
    8000147c:	9bbd                	andi	a5,a5,-17
    8000147e:	e11c                	sd	a5,0(a0)
}
    80001480:	60a2                	ld	ra,8(sp)
    80001482:	6402                	ld	s0,0(sp)
    80001484:	0141                	addi	sp,sp,16
    80001486:	8082                	ret
    panic("uvmclear");
    80001488:	00006517          	auipc	a0,0x6
    8000148c:	cc050513          	addi	a0,a0,-832 # 80007148 <etext+0x148>
    80001490:	b50ff0ef          	jal	800007e0 <panic>

0000000080001494 <copyinstr>:
copyinstr(pagetable_t pagetable, char *dst, uint64 srcva, uint64 max)
{
  uint64 n, va0, pa0;
  int got_null = 0;

  while(got_null == 0 && max > 0){
    80001494:	c6dd                	beqz	a3,80001542 <copyinstr+0xae>
{
    80001496:	715d                	addi	sp,sp,-80
    80001498:	e486                	sd	ra,72(sp)
    8000149a:	e0a2                	sd	s0,64(sp)
    8000149c:	fc26                	sd	s1,56(sp)
    8000149e:	f84a                	sd	s2,48(sp)
    800014a0:	f44e                	sd	s3,40(sp)
    800014a2:	f052                	sd	s4,32(sp)
    800014a4:	ec56                	sd	s5,24(sp)
    800014a6:	e85a                	sd	s6,16(sp)
    800014a8:	e45e                	sd	s7,8(sp)
    800014aa:	0880                	addi	s0,sp,80
    800014ac:	8a2a                	mv	s4,a0
    800014ae:	8b2e                	mv	s6,a1
    800014b0:	8bb2                	mv	s7,a2
    800014b2:	8936                	mv	s2,a3
    va0 = PGROUNDDOWN(srcva);
    800014b4:	7afd                	lui	s5,0xfffff
    pa0 = walkaddr(pagetable, va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (srcva - va0);
    800014b6:	6985                	lui	s3,0x1
    800014b8:	a825                	j	800014f0 <copyinstr+0x5c>
      n = max;

    char *p = (char *) (pa0 + (srcva - va0));
    while(n > 0){
      if(*p == '\0'){
        *dst = '\0';
    800014ba:	00078023          	sb	zero,0(a5) # 1000 <_entry-0x7ffff000>
    800014be:	4785                	li	a5,1
      dst++;
    }

    srcva = va0 + PGSIZE;
  }
  if(got_null){
    800014c0:	37fd                	addiw	a5,a5,-1
    800014c2:	0007851b          	sext.w	a0,a5
    return 0;
  } else {
    return -1;
  }
}
    800014c6:	60a6                	ld	ra,72(sp)
    800014c8:	6406                	ld	s0,64(sp)
    800014ca:	74e2                	ld	s1,56(sp)
    800014cc:	7942                	ld	s2,48(sp)
    800014ce:	79a2                	ld	s3,40(sp)
    800014d0:	7a02                	ld	s4,32(sp)
    800014d2:	6ae2                	ld	s5,24(sp)
    800014d4:	6b42                	ld	s6,16(sp)
    800014d6:	6ba2                	ld	s7,8(sp)
    800014d8:	6161                	addi	sp,sp,80
    800014da:	8082                	ret
    800014dc:	fff90713          	addi	a4,s2,-1 # fff <_entry-0x7ffff001>
    800014e0:	9742                	add	a4,a4,a6
      --max;
    800014e2:	40b70933          	sub	s2,a4,a1
    srcva = va0 + PGSIZE;
    800014e6:	01348bb3          	add	s7,s1,s3
  while(got_null == 0 && max > 0){
    800014ea:	04e58463          	beq	a1,a4,80001532 <copyinstr+0x9e>
{
    800014ee:	8b3e                	mv	s6,a5
    va0 = PGROUNDDOWN(srcva);
    800014f0:	015bf4b3          	and	s1,s7,s5
    pa0 = walkaddr(pagetable, va0);
    800014f4:	85a6                	mv	a1,s1
    800014f6:	8552                	mv	a0,s4
    800014f8:	ac5ff0ef          	jal	80000fbc <walkaddr>
    if(pa0 == 0)
    800014fc:	cd0d                	beqz	a0,80001536 <copyinstr+0xa2>
    n = PGSIZE - (srcva - va0);
    800014fe:	417486b3          	sub	a3,s1,s7
    80001502:	96ce                	add	a3,a3,s3
    if(n > max)
    80001504:	00d97363          	bgeu	s2,a3,8000150a <copyinstr+0x76>
    80001508:	86ca                	mv	a3,s2
    char *p = (char *) (pa0 + (srcva - va0));
    8000150a:	955e                	add	a0,a0,s7
    8000150c:	8d05                	sub	a0,a0,s1
    while(n > 0){
    8000150e:	c695                	beqz	a3,8000153a <copyinstr+0xa6>
    80001510:	87da                	mv	a5,s6
    80001512:	885a                	mv	a6,s6
      if(*p == '\0'){
    80001514:	41650633          	sub	a2,a0,s6
    while(n > 0){
    80001518:	96da                	add	a3,a3,s6
    8000151a:	85be                	mv	a1,a5
      if(*p == '\0'){
    8000151c:	00f60733          	add	a4,a2,a5
    80001520:	00074703          	lbu	a4,0(a4)
    80001524:	db59                	beqz	a4,800014ba <copyinstr+0x26>
        *dst = *p;
    80001526:	00e78023          	sb	a4,0(a5)
      dst++;
    8000152a:	0785                	addi	a5,a5,1
    while(n > 0){
    8000152c:	fed797e3          	bne	a5,a3,8000151a <copyinstr+0x86>
    80001530:	b775                	j	800014dc <copyinstr+0x48>
    80001532:	4781                	li	a5,0
    80001534:	b771                	j	800014c0 <copyinstr+0x2c>
      return -1;
    80001536:	557d                	li	a0,-1
    80001538:	b779                	j	800014c6 <copyinstr+0x32>
    srcva = va0 + PGSIZE;
    8000153a:	6b85                	lui	s7,0x1
    8000153c:	9ba6                	add	s7,s7,s1
    8000153e:	87da                	mv	a5,s6
    80001540:	b77d                	j	800014ee <copyinstr+0x5a>
  int got_null = 0;
    80001542:	4781                	li	a5,0
  if(got_null){
    80001544:	37fd                	addiw	a5,a5,-1
    80001546:	0007851b          	sext.w	a0,a5
}
    8000154a:	8082                	ret

000000008000154c <ismapped>:
  return mem;
}

int
ismapped(pagetable_t pagetable, uint64 va)
{
    8000154c:	1141                	addi	sp,sp,-16
    8000154e:	e406                	sd	ra,8(sp)
    80001550:	e022                	sd	s0,0(sp)
    80001552:	0800                	addi	s0,sp,16
  pte_t *pte = walk(pagetable, va, 0);
    80001554:	4601                	li	a2,0
    80001556:	9cdff0ef          	jal	80000f22 <walk>
  if (pte == 0) {
    8000155a:	c519                	beqz	a0,80001568 <ismapped+0x1c>
    return 0;
  }
  if (*pte & PTE_V){
    8000155c:	6108                	ld	a0,0(a0)
    8000155e:	8905                	andi	a0,a0,1
    return 1;
  }
  return 0;
}
    80001560:	60a2                	ld	ra,8(sp)
    80001562:	6402                	ld	s0,0(sp)
    80001564:	0141                	addi	sp,sp,16
    80001566:	8082                	ret
    return 0;
    80001568:	4501                	li	a0,0
    8000156a:	bfdd                	j	80001560 <ismapped+0x14>

000000008000156c <vmfault>:
{
    8000156c:	7179                	addi	sp,sp,-48
    8000156e:	f406                	sd	ra,40(sp)
    80001570:	f022                	sd	s0,32(sp)
    80001572:	ec26                	sd	s1,24(sp)
    80001574:	e44e                	sd	s3,8(sp)
    80001576:	1800                	addi	s0,sp,48
    80001578:	89aa                	mv	s3,a0
    8000157a:	84ae                	mv	s1,a1
  struct proc *p = myproc();
    8000157c:	35e000ef          	jal	800018da <myproc>
  if (va >= p->sz)
    80001580:	653c                	ld	a5,72(a0)
    80001582:	00f4ea63          	bltu	s1,a5,80001596 <vmfault+0x2a>
    return 0;
    80001586:	4981                	li	s3,0
}
    80001588:	854e                	mv	a0,s3
    8000158a:	70a2                	ld	ra,40(sp)
    8000158c:	7402                	ld	s0,32(sp)
    8000158e:	64e2                	ld	s1,24(sp)
    80001590:	69a2                	ld	s3,8(sp)
    80001592:	6145                	addi	sp,sp,48
    80001594:	8082                	ret
    80001596:	e84a                	sd	s2,16(sp)
    80001598:	892a                	mv	s2,a0
  va = PGROUNDDOWN(va);
    8000159a:	77fd                	lui	a5,0xfffff
    8000159c:	8cfd                	and	s1,s1,a5
  if(ismapped(pagetable, va)) {
    8000159e:	85a6                	mv	a1,s1
    800015a0:	854e                	mv	a0,s3
    800015a2:	fabff0ef          	jal	8000154c <ismapped>
    return 0;
    800015a6:	4981                	li	s3,0
  if(ismapped(pagetable, va)) {
    800015a8:	c119                	beqz	a0,800015ae <vmfault+0x42>
    800015aa:	6942                	ld	s2,16(sp)
    800015ac:	bff1                	j	80001588 <vmfault+0x1c>
    800015ae:	e052                	sd	s4,0(sp)
  mem = (uint64) kalloc();
    800015b0:	d4eff0ef          	jal	80000afe <kalloc>
    800015b4:	8a2a                	mv	s4,a0
  if(mem == 0)
    800015b6:	c90d                	beqz	a0,800015e8 <vmfault+0x7c>
  mem = (uint64) kalloc();
    800015b8:	89aa                	mv	s3,a0
  memset((void *) mem, 0, PGSIZE);
    800015ba:	6605                	lui	a2,0x1
    800015bc:	4581                	li	a1,0
    800015be:	ee4ff0ef          	jal	80000ca2 <memset>
  if (mappages(p->pagetable, va, PGSIZE, mem, PTE_W|PTE_U|PTE_R) != 0) {
    800015c2:	4759                	li	a4,22
    800015c4:	86d2                	mv	a3,s4
    800015c6:	6605                	lui	a2,0x1
    800015c8:	85a6                	mv	a1,s1
    800015ca:	05093503          	ld	a0,80(s2)
    800015ce:	a2dff0ef          	jal	80000ffa <mappages>
    800015d2:	e501                	bnez	a0,800015da <vmfault+0x6e>
    800015d4:	6942                	ld	s2,16(sp)
    800015d6:	6a02                	ld	s4,0(sp)
    800015d8:	bf45                	j	80001588 <vmfault+0x1c>
    kfree((void *)mem);
    800015da:	8552                	mv	a0,s4
    800015dc:	c40ff0ef          	jal	80000a1c <kfree>
    return 0;
    800015e0:	4981                	li	s3,0
    800015e2:	6942                	ld	s2,16(sp)
    800015e4:	6a02                	ld	s4,0(sp)
    800015e6:	b74d                	j	80001588 <vmfault+0x1c>
    800015e8:	6942                	ld	s2,16(sp)
    800015ea:	6a02                	ld	s4,0(sp)
    800015ec:	bf71                	j	80001588 <vmfault+0x1c>

00000000800015ee <copyout>:
  while(len > 0){
    800015ee:	c2cd                	beqz	a3,80001690 <copyout+0xa2>
{
    800015f0:	711d                	addi	sp,sp,-96
    800015f2:	ec86                	sd	ra,88(sp)
    800015f4:	e8a2                	sd	s0,80(sp)
    800015f6:	e4a6                	sd	s1,72(sp)
    800015f8:	f852                	sd	s4,48(sp)
    800015fa:	f05a                	sd	s6,32(sp)
    800015fc:	ec5e                	sd	s7,24(sp)
    800015fe:	e862                	sd	s8,16(sp)
    80001600:	1080                	addi	s0,sp,96
    80001602:	8c2a                	mv	s8,a0
    80001604:	8b2e                	mv	s6,a1
    80001606:	8bb2                	mv	s7,a2
    80001608:	8a36                	mv	s4,a3
    va0 = PGROUNDDOWN(dstva);
    8000160a:	74fd                	lui	s1,0xfffff
    8000160c:	8ced                	and	s1,s1,a1
    if(va0 >= MAXVA)
    8000160e:	57fd                	li	a5,-1
    80001610:	83e9                	srli	a5,a5,0x1a
    80001612:	0897e163          	bltu	a5,s1,80001694 <copyout+0xa6>
    80001616:	e0ca                	sd	s2,64(sp)
    80001618:	fc4e                	sd	s3,56(sp)
    8000161a:	f456                	sd	s5,40(sp)
    8000161c:	e466                	sd	s9,8(sp)
    8000161e:	e06a                	sd	s10,0(sp)
    80001620:	6d05                	lui	s10,0x1
    80001622:	8cbe                	mv	s9,a5
    80001624:	a015                	j	80001648 <copyout+0x5a>
    memmove((void *)(pa0 + (dstva - va0)), src, n);
    80001626:	409b0533          	sub	a0,s6,s1
    8000162a:	0009861b          	sext.w	a2,s3
    8000162e:	85de                	mv	a1,s7
    80001630:	954a                	add	a0,a0,s2
    80001632:	eccff0ef          	jal	80000cfe <memmove>
    len -= n;
    80001636:	413a0a33          	sub	s4,s4,s3
    src += n;
    8000163a:	9bce                	add	s7,s7,s3
  while(len > 0){
    8000163c:	040a0363          	beqz	s4,80001682 <copyout+0x94>
    if(va0 >= MAXVA)
    80001640:	055cec63          	bltu	s9,s5,80001698 <copyout+0xaa>
    80001644:	84d6                	mv	s1,s5
    80001646:	8b56                	mv	s6,s5
    pa0 = walkaddr(pagetable, va0);
    80001648:	85a6                	mv	a1,s1
    8000164a:	8562                	mv	a0,s8
    8000164c:	971ff0ef          	jal	80000fbc <walkaddr>
    80001650:	892a                	mv	s2,a0
    if(pa0 == 0) {
    80001652:	e901                	bnez	a0,80001662 <copyout+0x74>
      if((pa0 = vmfault(pagetable, va0, 0)) == 0) {
    80001654:	4601                	li	a2,0
    80001656:	85a6                	mv	a1,s1
    80001658:	8562                	mv	a0,s8
    8000165a:	f13ff0ef          	jal	8000156c <vmfault>
    8000165e:	892a                	mv	s2,a0
    80001660:	c139                	beqz	a0,800016a6 <copyout+0xb8>
    pte = walk(pagetable, va0, 0);
    80001662:	4601                	li	a2,0
    80001664:	85a6                	mv	a1,s1
    80001666:	8562                	mv	a0,s8
    80001668:	8bbff0ef          	jal	80000f22 <walk>
    if((*pte & PTE_W) == 0)
    8000166c:	611c                	ld	a5,0(a0)
    8000166e:	8b91                	andi	a5,a5,4
    80001670:	c3b1                	beqz	a5,800016b4 <copyout+0xc6>
    n = PGSIZE - (dstva - va0);
    80001672:	01a48ab3          	add	s5,s1,s10
    80001676:	416a89b3          	sub	s3,s5,s6
    if(n > len)
    8000167a:	fb3a76e3          	bgeu	s4,s3,80001626 <copyout+0x38>
    8000167e:	89d2                	mv	s3,s4
    80001680:	b75d                	j	80001626 <copyout+0x38>
  return 0;
    80001682:	4501                	li	a0,0
    80001684:	6906                	ld	s2,64(sp)
    80001686:	79e2                	ld	s3,56(sp)
    80001688:	7aa2                	ld	s5,40(sp)
    8000168a:	6ca2                	ld	s9,8(sp)
    8000168c:	6d02                	ld	s10,0(sp)
    8000168e:	a80d                	j	800016c0 <copyout+0xd2>
    80001690:	4501                	li	a0,0
}
    80001692:	8082                	ret
      return -1;
    80001694:	557d                	li	a0,-1
    80001696:	a02d                	j	800016c0 <copyout+0xd2>
    80001698:	557d                	li	a0,-1
    8000169a:	6906                	ld	s2,64(sp)
    8000169c:	79e2                	ld	s3,56(sp)
    8000169e:	7aa2                	ld	s5,40(sp)
    800016a0:	6ca2                	ld	s9,8(sp)
    800016a2:	6d02                	ld	s10,0(sp)
    800016a4:	a831                	j	800016c0 <copyout+0xd2>
        return -1;
    800016a6:	557d                	li	a0,-1
    800016a8:	6906                	ld	s2,64(sp)
    800016aa:	79e2                	ld	s3,56(sp)
    800016ac:	7aa2                	ld	s5,40(sp)
    800016ae:	6ca2                	ld	s9,8(sp)
    800016b0:	6d02                	ld	s10,0(sp)
    800016b2:	a039                	j	800016c0 <copyout+0xd2>
      return -1;
    800016b4:	557d                	li	a0,-1
    800016b6:	6906                	ld	s2,64(sp)
    800016b8:	79e2                	ld	s3,56(sp)
    800016ba:	7aa2                	ld	s5,40(sp)
    800016bc:	6ca2                	ld	s9,8(sp)
    800016be:	6d02                	ld	s10,0(sp)
}
    800016c0:	60e6                	ld	ra,88(sp)
    800016c2:	6446                	ld	s0,80(sp)
    800016c4:	64a6                	ld	s1,72(sp)
    800016c6:	7a42                	ld	s4,48(sp)
    800016c8:	7b02                	ld	s6,32(sp)
    800016ca:	6be2                	ld	s7,24(sp)
    800016cc:	6c42                	ld	s8,16(sp)
    800016ce:	6125                	addi	sp,sp,96
    800016d0:	8082                	ret

00000000800016d2 <copyin>:
  while(len > 0){
    800016d2:	c6c9                	beqz	a3,8000175c <copyin+0x8a>
{
    800016d4:	715d                	addi	sp,sp,-80
    800016d6:	e486                	sd	ra,72(sp)
    800016d8:	e0a2                	sd	s0,64(sp)
    800016da:	fc26                	sd	s1,56(sp)
    800016dc:	f84a                	sd	s2,48(sp)
    800016de:	f44e                	sd	s3,40(sp)
    800016e0:	f052                	sd	s4,32(sp)
    800016e2:	ec56                	sd	s5,24(sp)
    800016e4:	e85a                	sd	s6,16(sp)
    800016e6:	e45e                	sd	s7,8(sp)
    800016e8:	e062                	sd	s8,0(sp)
    800016ea:	0880                	addi	s0,sp,80
    800016ec:	8baa                	mv	s7,a0
    800016ee:	8aae                	mv	s5,a1
    800016f0:	8932                	mv	s2,a2
    800016f2:	8a36                	mv	s4,a3
    va0 = PGROUNDDOWN(srcva);
    800016f4:	7c7d                	lui	s8,0xfffff
    n = PGSIZE - (srcva - va0);
    800016f6:	6b05                	lui	s6,0x1
    800016f8:	a035                	j	80001724 <copyin+0x52>
    800016fa:	412984b3          	sub	s1,s3,s2
    800016fe:	94da                	add	s1,s1,s6
    if(n > len)
    80001700:	009a7363          	bgeu	s4,s1,80001706 <copyin+0x34>
    80001704:	84d2                	mv	s1,s4
    memmove(dst, (void *)(pa0 + (srcva - va0)), n);
    80001706:	413905b3          	sub	a1,s2,s3
    8000170a:	0004861b          	sext.w	a2,s1
    8000170e:	95aa                	add	a1,a1,a0
    80001710:	8556                	mv	a0,s5
    80001712:	decff0ef          	jal	80000cfe <memmove>
    len -= n;
    80001716:	409a0a33          	sub	s4,s4,s1
    dst += n;
    8000171a:	9aa6                	add	s5,s5,s1
    srcva = va0 + PGSIZE;
    8000171c:	01698933          	add	s2,s3,s6
  while(len > 0){
    80001720:	020a0163          	beqz	s4,80001742 <copyin+0x70>
    va0 = PGROUNDDOWN(srcva);
    80001724:	018979b3          	and	s3,s2,s8
    pa0 = walkaddr(pagetable, va0);
    80001728:	85ce                	mv	a1,s3
    8000172a:	855e                	mv	a0,s7
    8000172c:	891ff0ef          	jal	80000fbc <walkaddr>
    if(pa0 == 0) {
    80001730:	f569                	bnez	a0,800016fa <copyin+0x28>
      if((pa0 = vmfault(pagetable, va0, 0)) == 0) {
    80001732:	4601                	li	a2,0
    80001734:	85ce                	mv	a1,s3
    80001736:	855e                	mv	a0,s7
    80001738:	e35ff0ef          	jal	8000156c <vmfault>
    8000173c:	fd5d                	bnez	a0,800016fa <copyin+0x28>
        return -1;
    8000173e:	557d                	li	a0,-1
    80001740:	a011                	j	80001744 <copyin+0x72>
  return 0;
    80001742:	4501                	li	a0,0
}
    80001744:	60a6                	ld	ra,72(sp)
    80001746:	6406                	ld	s0,64(sp)
    80001748:	74e2                	ld	s1,56(sp)
    8000174a:	7942                	ld	s2,48(sp)
    8000174c:	79a2                	ld	s3,40(sp)
    8000174e:	7a02                	ld	s4,32(sp)
    80001750:	6ae2                	ld	s5,24(sp)
    80001752:	6b42                	ld	s6,16(sp)
    80001754:	6ba2                	ld	s7,8(sp)
    80001756:	6c02                	ld	s8,0(sp)
    80001758:	6161                	addi	sp,sp,80
    8000175a:	8082                	ret
  return 0;
    8000175c:	4501                	li	a0,0
}
    8000175e:	8082                	ret

0000000080001760 <proc_mapstacks>:
// Allocate a page for each process's kernel stack.
// Map it high in memory, followed by an invalid
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl)
{
    80001760:	7139                	addi	sp,sp,-64
    80001762:	fc06                	sd	ra,56(sp)
    80001764:	f822                	sd	s0,48(sp)
    80001766:	f426                	sd	s1,40(sp)
    80001768:	f04a                	sd	s2,32(sp)
    8000176a:	ec4e                	sd	s3,24(sp)
    8000176c:	e852                	sd	s4,16(sp)
    8000176e:	e456                	sd	s5,8(sp)
    80001770:	e05a                	sd	s6,0(sp)
    80001772:	0080                	addi	s0,sp,64
    80001774:	8a2a                	mv	s4,a0
  struct proc *p;
  
  for(p = proc; p < &proc[NPROC]; p++) {
    80001776:	00011497          	auipc	s1,0x11
    8000177a:	0a248493          	addi	s1,s1,162 # 80012818 <proc>
    char *pa = kalloc();
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));
    8000177e:	8b26                	mv	s6,s1
    80001780:	04fa5937          	lui	s2,0x4fa5
    80001784:	fa590913          	addi	s2,s2,-91 # 4fa4fa5 <_entry-0x7b05b05b>
    80001788:	0932                	slli	s2,s2,0xc
    8000178a:	fa590913          	addi	s2,s2,-91
    8000178e:	0932                	slli	s2,s2,0xc
    80001790:	fa590913          	addi	s2,s2,-91
    80001794:	0932                	slli	s2,s2,0xc
    80001796:	fa590913          	addi	s2,s2,-91
    8000179a:	040009b7          	lui	s3,0x4000
    8000179e:	19fd                	addi	s3,s3,-1 # 3ffffff <_entry-0x7c000001>
    800017a0:	09b2                	slli	s3,s3,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    800017a2:	00017a97          	auipc	s5,0x17
    800017a6:	a76a8a93          	addi	s5,s5,-1418 # 80018218 <tickslock>
    char *pa = kalloc();
    800017aa:	b54ff0ef          	jal	80000afe <kalloc>
    800017ae:	862a                	mv	a2,a0
    if(pa == 0)
    800017b0:	cd15                	beqz	a0,800017ec <proc_mapstacks+0x8c>
    uint64 va = KSTACK((int) (p - proc));
    800017b2:	416485b3          	sub	a1,s1,s6
    800017b6:	858d                	srai	a1,a1,0x3
    800017b8:	032585b3          	mul	a1,a1,s2
    800017bc:	2585                	addiw	a1,a1,1
    800017be:	00d5959b          	slliw	a1,a1,0xd
    kvmmap(kpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
    800017c2:	4719                	li	a4,6
    800017c4:	6685                	lui	a3,0x1
    800017c6:	40b985b3          	sub	a1,s3,a1
    800017ca:	8552                	mv	a0,s4
    800017cc:	8dfff0ef          	jal	800010aa <kvmmap>
  for(p = proc; p < &proc[NPROC]; p++) {
    800017d0:	16848493          	addi	s1,s1,360
    800017d4:	fd549be3          	bne	s1,s5,800017aa <proc_mapstacks+0x4a>
  }
}
    800017d8:	70e2                	ld	ra,56(sp)
    800017da:	7442                	ld	s0,48(sp)
    800017dc:	74a2                	ld	s1,40(sp)
    800017de:	7902                	ld	s2,32(sp)
    800017e0:	69e2                	ld	s3,24(sp)
    800017e2:	6a42                	ld	s4,16(sp)
    800017e4:	6aa2                	ld	s5,8(sp)
    800017e6:	6b02                	ld	s6,0(sp)
    800017e8:	6121                	addi	sp,sp,64
    800017ea:	8082                	ret
      panic("kalloc");
    800017ec:	00006517          	auipc	a0,0x6
    800017f0:	96c50513          	addi	a0,a0,-1684 # 80007158 <etext+0x158>
    800017f4:	fedfe0ef          	jal	800007e0 <panic>

00000000800017f8 <procinit>:

// initialize the proc table.
void
procinit(void)
{
    800017f8:	7139                	addi	sp,sp,-64
    800017fa:	fc06                	sd	ra,56(sp)
    800017fc:	f822                	sd	s0,48(sp)
    800017fe:	f426                	sd	s1,40(sp)
    80001800:	f04a                	sd	s2,32(sp)
    80001802:	ec4e                	sd	s3,24(sp)
    80001804:	e852                	sd	s4,16(sp)
    80001806:	e456                	sd	s5,8(sp)
    80001808:	e05a                	sd	s6,0(sp)
    8000180a:	0080                	addi	s0,sp,64
  struct proc *p;
  
  initlock(&pid_lock, "nextpid");
    8000180c:	00006597          	auipc	a1,0x6
    80001810:	95458593          	addi	a1,a1,-1708 # 80007160 <etext+0x160>
    80001814:	00011517          	auipc	a0,0x11
    80001818:	bd450513          	addi	a0,a0,-1068 # 800123e8 <pid_lock>
    8000181c:	b32ff0ef          	jal	80000b4e <initlock>
  initlock(&wait_lock, "wait_lock");
    80001820:	00006597          	auipc	a1,0x6
    80001824:	94858593          	addi	a1,a1,-1720 # 80007168 <etext+0x168>
    80001828:	00011517          	auipc	a0,0x11
    8000182c:	bd850513          	addi	a0,a0,-1064 # 80012400 <wait_lock>
    80001830:	b1eff0ef          	jal	80000b4e <initlock>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001834:	00011497          	auipc	s1,0x11
    80001838:	fe448493          	addi	s1,s1,-28 # 80012818 <proc>
      initlock(&p->lock, "proc");
    8000183c:	00006b17          	auipc	s6,0x6
    80001840:	93cb0b13          	addi	s6,s6,-1732 # 80007178 <etext+0x178>
      p->state = UNUSED;
      p->kstack = KSTACK((int) (p - proc));
    80001844:	8aa6                	mv	s5,s1
    80001846:	04fa5937          	lui	s2,0x4fa5
    8000184a:	fa590913          	addi	s2,s2,-91 # 4fa4fa5 <_entry-0x7b05b05b>
    8000184e:	0932                	slli	s2,s2,0xc
    80001850:	fa590913          	addi	s2,s2,-91
    80001854:	0932                	slli	s2,s2,0xc
    80001856:	fa590913          	addi	s2,s2,-91
    8000185a:	0932                	slli	s2,s2,0xc
    8000185c:	fa590913          	addi	s2,s2,-91
    80001860:	040009b7          	lui	s3,0x4000
    80001864:	19fd                	addi	s3,s3,-1 # 3ffffff <_entry-0x7c000001>
    80001866:	09b2                	slli	s3,s3,0xc
  for(p = proc; p < &proc[NPROC]; p++) {
    80001868:	00017a17          	auipc	s4,0x17
    8000186c:	9b0a0a13          	addi	s4,s4,-1616 # 80018218 <tickslock>
      initlock(&p->lock, "proc");
    80001870:	85da                	mv	a1,s6
    80001872:	8526                	mv	a0,s1
    80001874:	adaff0ef          	jal	80000b4e <initlock>
      p->state = UNUSED;
    80001878:	0004ac23          	sw	zero,24(s1)
      p->kstack = KSTACK((int) (p - proc));
    8000187c:	415487b3          	sub	a5,s1,s5
    80001880:	878d                	srai	a5,a5,0x3
    80001882:	032787b3          	mul	a5,a5,s2
    80001886:	2785                	addiw	a5,a5,1 # fffffffffffff001 <end+0xffffffff7ffdb071>
    80001888:	00d7979b          	slliw	a5,a5,0xd
    8000188c:	40f987b3          	sub	a5,s3,a5
    80001890:	e0bc                	sd	a5,64(s1)
  for(p = proc; p < &proc[NPROC]; p++) {
    80001892:	16848493          	addi	s1,s1,360
    80001896:	fd449de3          	bne	s1,s4,80001870 <procinit+0x78>
  }
}
    8000189a:	70e2                	ld	ra,56(sp)
    8000189c:	7442                	ld	s0,48(sp)
    8000189e:	74a2                	ld	s1,40(sp)
    800018a0:	7902                	ld	s2,32(sp)
    800018a2:	69e2                	ld	s3,24(sp)
    800018a4:	6a42                	ld	s4,16(sp)
    800018a6:	6aa2                	ld	s5,8(sp)
    800018a8:	6b02                	ld	s6,0(sp)
    800018aa:	6121                	addi	sp,sp,64
    800018ac:	8082                	ret

00000000800018ae <cpuid>:
// Must be called with interrupts disabled,
// to prevent race with process being moved
// to a different CPU.
int
cpuid()
{
    800018ae:	1141                	addi	sp,sp,-16
    800018b0:	e422                	sd	s0,8(sp)
    800018b2:	0800                	addi	s0,sp,16
  asm volatile("mv %0, tp" : "=r" (x) );
    800018b4:	8512                	mv	a0,tp
  int id = r_tp();
  return id;
}
    800018b6:	2501                	sext.w	a0,a0
    800018b8:	6422                	ld	s0,8(sp)
    800018ba:	0141                	addi	sp,sp,16
    800018bc:	8082                	ret

00000000800018be <mycpu>:

// Return this CPU's cpu struct.
// Interrupts must be disabled.
struct cpu*
mycpu(void)
{
    800018be:	1141                	addi	sp,sp,-16
    800018c0:	e422                	sd	s0,8(sp)
    800018c2:	0800                	addi	s0,sp,16
    800018c4:	8792                	mv	a5,tp
  int id = cpuid();
  struct cpu *c = &cpus[id];
    800018c6:	2781                	sext.w	a5,a5
    800018c8:	079e                	slli	a5,a5,0x7
  return c;
}
    800018ca:	00011517          	auipc	a0,0x11
    800018ce:	b4e50513          	addi	a0,a0,-1202 # 80012418 <cpus>
    800018d2:	953e                	add	a0,a0,a5
    800018d4:	6422                	ld	s0,8(sp)
    800018d6:	0141                	addi	sp,sp,16
    800018d8:	8082                	ret

00000000800018da <myproc>:

// Return the current struct proc *, or zero if none.
struct proc*
myproc(void)
{
    800018da:	1101                	addi	sp,sp,-32
    800018dc:	ec06                	sd	ra,24(sp)
    800018de:	e822                	sd	s0,16(sp)
    800018e0:	e426                	sd	s1,8(sp)
    800018e2:	1000                	addi	s0,sp,32
  push_off();
    800018e4:	aaaff0ef          	jal	80000b8e <push_off>
    800018e8:	8792                	mv	a5,tp
  struct cpu *c = mycpu();
  struct proc *p = c->proc;
    800018ea:	2781                	sext.w	a5,a5
    800018ec:	079e                	slli	a5,a5,0x7
    800018ee:	00011717          	auipc	a4,0x11
    800018f2:	afa70713          	addi	a4,a4,-1286 # 800123e8 <pid_lock>
    800018f6:	97ba                	add	a5,a5,a4
    800018f8:	7b84                	ld	s1,48(a5)
  pop_off();
    800018fa:	b18ff0ef          	jal	80000c12 <pop_off>
  return p;
}
    800018fe:	8526                	mv	a0,s1
    80001900:	60e2                	ld	ra,24(sp)
    80001902:	6442                	ld	s0,16(sp)
    80001904:	64a2                	ld	s1,8(sp)
    80001906:	6105                	addi	sp,sp,32
    80001908:	8082                	ret

000000008000190a <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch to forkret.
void
forkret(void)
{
    8000190a:	7179                	addi	sp,sp,-48
    8000190c:	f406                	sd	ra,40(sp)
    8000190e:	f022                	sd	s0,32(sp)
    80001910:	ec26                	sd	s1,24(sp)
    80001912:	1800                	addi	s0,sp,48
  extern char userret[];
  static int first = 1;
  struct proc *p = myproc();
    80001914:	fc7ff0ef          	jal	800018da <myproc>
    80001918:	84aa                	mv	s1,a0

  // Still holding p->lock from scheduler.
  release(&p->lock);
    8000191a:	b4cff0ef          	jal	80000c66 <release>

  if (first) {
    8000191e:	00009797          	auipc	a5,0x9
    80001922:	9727a783          	lw	a5,-1678(a5) # 8000a290 <first.1>
    80001926:	cf8d                	beqz	a5,80001960 <forkret+0x56>
    // File system initialization must be run in the context of a
    // regular process (e.g., because it calls sleep), and thus cannot
    // be run from main().
    fsinit(ROOTDEV);
    80001928:	4505                	li	a0,1
    8000192a:	3b9010ef          	jal	800034e2 <fsinit>

    first = 0;
    8000192e:	00009797          	auipc	a5,0x9
    80001932:	9607a123          	sw	zero,-1694(a5) # 8000a290 <first.1>
    // ensure other cores see first=0.
    __sync_synchronize();
    80001936:	0330000f          	fence	rw,rw

    // We can invoke kexec() now that file system is initialized.
    // Put the return value (argc) of kexec into a0.
    p->trapframe->a0 = kexec("/init", (char *[]){ "/init", 0 });
    8000193a:	00006517          	auipc	a0,0x6
    8000193e:	84650513          	addi	a0,a0,-1978 # 80007180 <etext+0x180>
    80001942:	fca43823          	sd	a0,-48(s0)
    80001946:	fc043c23          	sd	zero,-40(s0)
    8000194a:	fd040593          	addi	a1,s0,-48
    8000194e:	49f020ef          	jal	800045ec <kexec>
    80001952:	6cbc                	ld	a5,88(s1)
    80001954:	fba8                	sd	a0,112(a5)
    if (p->trapframe->a0 == -1) {
    80001956:	6cbc                	ld	a5,88(s1)
    80001958:	7bb8                	ld	a4,112(a5)
    8000195a:	57fd                	li	a5,-1
    8000195c:	02f70d63          	beq	a4,a5,80001996 <forkret+0x8c>
      panic("exec");
    }
  }

  // return to user space, mimicing usertrap()'s return.
  prepare_return();
    80001960:	2bf000ef          	jal	8000241e <prepare_return>
  uint64 satp = MAKE_SATP(p->pagetable);
    80001964:	68a8                	ld	a0,80(s1)
    80001966:	8131                	srli	a0,a0,0xc
  uint64 trampoline_userret = TRAMPOLINE + (userret - trampoline);
    80001968:	04000737          	lui	a4,0x4000
    8000196c:	177d                	addi	a4,a4,-1 # 3ffffff <_entry-0x7c000001>
    8000196e:	0732                	slli	a4,a4,0xc
    80001970:	00004797          	auipc	a5,0x4
    80001974:	72c78793          	addi	a5,a5,1836 # 8000609c <userret>
    80001978:	00004697          	auipc	a3,0x4
    8000197c:	68868693          	addi	a3,a3,1672 # 80006000 <_trampoline>
    80001980:	8f95                	sub	a5,a5,a3
    80001982:	97ba                	add	a5,a5,a4
  ((void (*)(uint64))trampoline_userret)(satp);
    80001984:	577d                	li	a4,-1
    80001986:	177e                	slli	a4,a4,0x3f
    80001988:	8d59                	or	a0,a0,a4
    8000198a:	9782                	jalr	a5
}
    8000198c:	70a2                	ld	ra,40(sp)
    8000198e:	7402                	ld	s0,32(sp)
    80001990:	64e2                	ld	s1,24(sp)
    80001992:	6145                	addi	sp,sp,48
    80001994:	8082                	ret
      panic("exec");
    80001996:	00005517          	auipc	a0,0x5
    8000199a:	7f250513          	addi	a0,a0,2034 # 80007188 <etext+0x188>
    8000199e:	e43fe0ef          	jal	800007e0 <panic>

00000000800019a2 <allocpid>:
{
    800019a2:	1101                	addi	sp,sp,-32
    800019a4:	ec06                	sd	ra,24(sp)
    800019a6:	e822                	sd	s0,16(sp)
    800019a8:	e426                	sd	s1,8(sp)
    800019aa:	e04a                	sd	s2,0(sp)
    800019ac:	1000                	addi	s0,sp,32
  acquire(&pid_lock);
    800019ae:	00011917          	auipc	s2,0x11
    800019b2:	a3a90913          	addi	s2,s2,-1478 # 800123e8 <pid_lock>
    800019b6:	854a                	mv	a0,s2
    800019b8:	a16ff0ef          	jal	80000bce <acquire>
  pid = nextpid;
    800019bc:	00009797          	auipc	a5,0x9
    800019c0:	8d878793          	addi	a5,a5,-1832 # 8000a294 <nextpid>
    800019c4:	4384                	lw	s1,0(a5)
  nextpid = nextpid + 1;
    800019c6:	0014871b          	addiw	a4,s1,1
    800019ca:	c398                	sw	a4,0(a5)
  release(&pid_lock);
    800019cc:	854a                	mv	a0,s2
    800019ce:	a98ff0ef          	jal	80000c66 <release>
}
    800019d2:	8526                	mv	a0,s1
    800019d4:	60e2                	ld	ra,24(sp)
    800019d6:	6442                	ld	s0,16(sp)
    800019d8:	64a2                	ld	s1,8(sp)
    800019da:	6902                	ld	s2,0(sp)
    800019dc:	6105                	addi	sp,sp,32
    800019de:	8082                	ret

00000000800019e0 <proc_pagetable>:
{
    800019e0:	1101                	addi	sp,sp,-32
    800019e2:	ec06                	sd	ra,24(sp)
    800019e4:	e822                	sd	s0,16(sp)
    800019e6:	e426                	sd	s1,8(sp)
    800019e8:	e04a                	sd	s2,0(sp)
    800019ea:	1000                	addi	s0,sp,32
    800019ec:	892a                	mv	s2,a0
  pagetable = uvmcreate();
    800019ee:	fb2ff0ef          	jal	800011a0 <uvmcreate>
    800019f2:	84aa                	mv	s1,a0
  if(pagetable == 0)
    800019f4:	cd05                	beqz	a0,80001a2c <proc_pagetable+0x4c>
  if(mappages(pagetable, TRAMPOLINE, PGSIZE,
    800019f6:	4729                	li	a4,10
    800019f8:	00004697          	auipc	a3,0x4
    800019fc:	60868693          	addi	a3,a3,1544 # 80006000 <_trampoline>
    80001a00:	6605                	lui	a2,0x1
    80001a02:	040005b7          	lui	a1,0x4000
    80001a06:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001a08:	05b2                	slli	a1,a1,0xc
    80001a0a:	df0ff0ef          	jal	80000ffa <mappages>
    80001a0e:	02054663          	bltz	a0,80001a3a <proc_pagetable+0x5a>
  if(mappages(pagetable, TRAPFRAME, PGSIZE,
    80001a12:	4719                	li	a4,6
    80001a14:	05893683          	ld	a3,88(s2)
    80001a18:	6605                	lui	a2,0x1
    80001a1a:	020005b7          	lui	a1,0x2000
    80001a1e:	15fd                	addi	a1,a1,-1 # 1ffffff <_entry-0x7e000001>
    80001a20:	05b6                	slli	a1,a1,0xd
    80001a22:	8526                	mv	a0,s1
    80001a24:	dd6ff0ef          	jal	80000ffa <mappages>
    80001a28:	00054f63          	bltz	a0,80001a46 <proc_pagetable+0x66>
}
    80001a2c:	8526                	mv	a0,s1
    80001a2e:	60e2                	ld	ra,24(sp)
    80001a30:	6442                	ld	s0,16(sp)
    80001a32:	64a2                	ld	s1,8(sp)
    80001a34:	6902                	ld	s2,0(sp)
    80001a36:	6105                	addi	sp,sp,32
    80001a38:	8082                	ret
    uvmfree(pagetable, 0);
    80001a3a:	4581                	li	a1,0
    80001a3c:	8526                	mv	a0,s1
    80001a3e:	95dff0ef          	jal	8000139a <uvmfree>
    return 0;
    80001a42:	4481                	li	s1,0
    80001a44:	b7e5                	j	80001a2c <proc_pagetable+0x4c>
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001a46:	4681                	li	a3,0
    80001a48:	4605                	li	a2,1
    80001a4a:	040005b7          	lui	a1,0x4000
    80001a4e:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001a50:	05b2                	slli	a1,a1,0xc
    80001a52:	8526                	mv	a0,s1
    80001a54:	f72ff0ef          	jal	800011c6 <uvmunmap>
    uvmfree(pagetable, 0);
    80001a58:	4581                	li	a1,0
    80001a5a:	8526                	mv	a0,s1
    80001a5c:	93fff0ef          	jal	8000139a <uvmfree>
    return 0;
    80001a60:	4481                	li	s1,0
    80001a62:	b7e9                	j	80001a2c <proc_pagetable+0x4c>

0000000080001a64 <proc_freepagetable>:
{
    80001a64:	1101                	addi	sp,sp,-32
    80001a66:	ec06                	sd	ra,24(sp)
    80001a68:	e822                	sd	s0,16(sp)
    80001a6a:	e426                	sd	s1,8(sp)
    80001a6c:	e04a                	sd	s2,0(sp)
    80001a6e:	1000                	addi	s0,sp,32
    80001a70:	84aa                	mv	s1,a0
    80001a72:	892e                	mv	s2,a1
  uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    80001a74:	4681                	li	a3,0
    80001a76:	4605                	li	a2,1
    80001a78:	040005b7          	lui	a1,0x4000
    80001a7c:	15fd                	addi	a1,a1,-1 # 3ffffff <_entry-0x7c000001>
    80001a7e:	05b2                	slli	a1,a1,0xc
    80001a80:	f46ff0ef          	jal	800011c6 <uvmunmap>
  uvmunmap(pagetable, TRAPFRAME, 1, 0);
    80001a84:	4681                	li	a3,0
    80001a86:	4605                	li	a2,1
    80001a88:	020005b7          	lui	a1,0x2000
    80001a8c:	15fd                	addi	a1,a1,-1 # 1ffffff <_entry-0x7e000001>
    80001a8e:	05b6                	slli	a1,a1,0xd
    80001a90:	8526                	mv	a0,s1
    80001a92:	f34ff0ef          	jal	800011c6 <uvmunmap>
  uvmfree(pagetable, sz);
    80001a96:	85ca                	mv	a1,s2
    80001a98:	8526                	mv	a0,s1
    80001a9a:	901ff0ef          	jal	8000139a <uvmfree>
}
    80001a9e:	60e2                	ld	ra,24(sp)
    80001aa0:	6442                	ld	s0,16(sp)
    80001aa2:	64a2                	ld	s1,8(sp)
    80001aa4:	6902                	ld	s2,0(sp)
    80001aa6:	6105                	addi	sp,sp,32
    80001aa8:	8082                	ret

0000000080001aaa <freeproc>:
{
    80001aaa:	1101                	addi	sp,sp,-32
    80001aac:	ec06                	sd	ra,24(sp)
    80001aae:	e822                	sd	s0,16(sp)
    80001ab0:	e426                	sd	s1,8(sp)
    80001ab2:	1000                	addi	s0,sp,32
    80001ab4:	84aa                	mv	s1,a0
  if(p->trapframe)
    80001ab6:	6d28                	ld	a0,88(a0)
    80001ab8:	c119                	beqz	a0,80001abe <freeproc+0x14>
    kfree((void*)p->trapframe);
    80001aba:	f63fe0ef          	jal	80000a1c <kfree>
  p->trapframe = 0;
    80001abe:	0404bc23          	sd	zero,88(s1)
  if(p->pagetable)
    80001ac2:	68a8                	ld	a0,80(s1)
    80001ac4:	c501                	beqz	a0,80001acc <freeproc+0x22>
    proc_freepagetable(p->pagetable, p->sz);
    80001ac6:	64ac                	ld	a1,72(s1)
    80001ac8:	f9dff0ef          	jal	80001a64 <proc_freepagetable>
  p->pagetable = 0;
    80001acc:	0404b823          	sd	zero,80(s1)
  p->sz = 0;
    80001ad0:	0404b423          	sd	zero,72(s1)
  p->pid = 0;
    80001ad4:	0204a823          	sw	zero,48(s1)
  p->parent = 0;
    80001ad8:	0204bc23          	sd	zero,56(s1)
  p->name[0] = 0;
    80001adc:	14048c23          	sb	zero,344(s1)
  p->chan = 0;
    80001ae0:	0204b023          	sd	zero,32(s1)
  p->killed = 0;
    80001ae4:	0204a423          	sw	zero,40(s1)
  p->xstate = 0;
    80001ae8:	0204a623          	sw	zero,44(s1)
  p->state = UNUSED;
    80001aec:	0004ac23          	sw	zero,24(s1)
}
    80001af0:	60e2                	ld	ra,24(sp)
    80001af2:	6442                	ld	s0,16(sp)
    80001af4:	64a2                	ld	s1,8(sp)
    80001af6:	6105                	addi	sp,sp,32
    80001af8:	8082                	ret

0000000080001afa <allocproc>:
{
    80001afa:	1101                	addi	sp,sp,-32
    80001afc:	ec06                	sd	ra,24(sp)
    80001afe:	e822                	sd	s0,16(sp)
    80001b00:	e426                	sd	s1,8(sp)
    80001b02:	e04a                	sd	s2,0(sp)
    80001b04:	1000                	addi	s0,sp,32
  for(p = proc; p < &proc[NPROC]; p++) {
    80001b06:	00011497          	auipc	s1,0x11
    80001b0a:	d1248493          	addi	s1,s1,-750 # 80012818 <proc>
    80001b0e:	00016917          	auipc	s2,0x16
    80001b12:	70a90913          	addi	s2,s2,1802 # 80018218 <tickslock>
    acquire(&p->lock);
    80001b16:	8526                	mv	a0,s1
    80001b18:	8b6ff0ef          	jal	80000bce <acquire>
    if(p->state == UNUSED) {
    80001b1c:	4c9c                	lw	a5,24(s1)
    80001b1e:	cb91                	beqz	a5,80001b32 <allocproc+0x38>
      release(&p->lock);
    80001b20:	8526                	mv	a0,s1
    80001b22:	944ff0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001b26:	16848493          	addi	s1,s1,360
    80001b2a:	ff2496e3          	bne	s1,s2,80001b16 <allocproc+0x1c>
  return 0;
    80001b2e:	4481                	li	s1,0
    80001b30:	a089                	j	80001b72 <allocproc+0x78>
  p->pid = allocpid();
    80001b32:	e71ff0ef          	jal	800019a2 <allocpid>
    80001b36:	d888                	sw	a0,48(s1)
  p->state = USED;
    80001b38:	4785                	li	a5,1
    80001b3a:	cc9c                	sw	a5,24(s1)
  if((p->trapframe = (struct trapframe *)kalloc()) == 0){
    80001b3c:	fc3fe0ef          	jal	80000afe <kalloc>
    80001b40:	892a                	mv	s2,a0
    80001b42:	eca8                	sd	a0,88(s1)
    80001b44:	cd15                	beqz	a0,80001b80 <allocproc+0x86>
  p->pagetable = proc_pagetable(p);
    80001b46:	8526                	mv	a0,s1
    80001b48:	e99ff0ef          	jal	800019e0 <proc_pagetable>
    80001b4c:	892a                	mv	s2,a0
    80001b4e:	e8a8                	sd	a0,80(s1)
  if(p->pagetable == 0){
    80001b50:	c121                	beqz	a0,80001b90 <allocproc+0x96>
  memset(&p->context, 0, sizeof(p->context));
    80001b52:	07000613          	li	a2,112
    80001b56:	4581                	li	a1,0
    80001b58:	06048513          	addi	a0,s1,96
    80001b5c:	946ff0ef          	jal	80000ca2 <memset>
  p->context.ra = (uint64)forkret;
    80001b60:	00000797          	auipc	a5,0x0
    80001b64:	daa78793          	addi	a5,a5,-598 # 8000190a <forkret>
    80001b68:	f0bc                	sd	a5,96(s1)
  p->context.sp = p->kstack + PGSIZE;
    80001b6a:	60bc                	ld	a5,64(s1)
    80001b6c:	6705                	lui	a4,0x1
    80001b6e:	97ba                	add	a5,a5,a4
    80001b70:	f4bc                	sd	a5,104(s1)
}
    80001b72:	8526                	mv	a0,s1
    80001b74:	60e2                	ld	ra,24(sp)
    80001b76:	6442                	ld	s0,16(sp)
    80001b78:	64a2                	ld	s1,8(sp)
    80001b7a:	6902                	ld	s2,0(sp)
    80001b7c:	6105                	addi	sp,sp,32
    80001b7e:	8082                	ret
    freeproc(p);
    80001b80:	8526                	mv	a0,s1
    80001b82:	f29ff0ef          	jal	80001aaa <freeproc>
    release(&p->lock);
    80001b86:	8526                	mv	a0,s1
    80001b88:	8deff0ef          	jal	80000c66 <release>
    return 0;
    80001b8c:	84ca                	mv	s1,s2
    80001b8e:	b7d5                	j	80001b72 <allocproc+0x78>
    freeproc(p);
    80001b90:	8526                	mv	a0,s1
    80001b92:	f19ff0ef          	jal	80001aaa <freeproc>
    release(&p->lock);
    80001b96:	8526                	mv	a0,s1
    80001b98:	8ceff0ef          	jal	80000c66 <release>
    return 0;
    80001b9c:	84ca                	mv	s1,s2
    80001b9e:	bfd1                	j	80001b72 <allocproc+0x78>

0000000080001ba0 <userinit>:
{
    80001ba0:	1101                	addi	sp,sp,-32
    80001ba2:	ec06                	sd	ra,24(sp)
    80001ba4:	e822                	sd	s0,16(sp)
    80001ba6:	e426                	sd	s1,8(sp)
    80001ba8:	1000                	addi	s0,sp,32
  p = allocproc();
    80001baa:	f51ff0ef          	jal	80001afa <allocproc>
    80001bae:	84aa                	mv	s1,a0
  initproc = p;
    80001bb0:	00008797          	auipc	a5,0x8
    80001bb4:	72a7b823          	sd	a0,1840(a5) # 8000a2e0 <initproc>
  p->cwd = namei("/");
    80001bb8:	00005517          	auipc	a0,0x5
    80001bbc:	5d850513          	addi	a0,a0,1496 # 80007190 <etext+0x190>
    80001bc0:	645010ef          	jal	80003a04 <namei>
    80001bc4:	14a4b823          	sd	a0,336(s1)
  p->state = RUNNABLE;
    80001bc8:	478d                	li	a5,3
    80001bca:	cc9c                	sw	a5,24(s1)
  release(&p->lock);
    80001bcc:	8526                	mv	a0,s1
    80001bce:	898ff0ef          	jal	80000c66 <release>
}
    80001bd2:	60e2                	ld	ra,24(sp)
    80001bd4:	6442                	ld	s0,16(sp)
    80001bd6:	64a2                	ld	s1,8(sp)
    80001bd8:	6105                	addi	sp,sp,32
    80001bda:	8082                	ret

0000000080001bdc <growproc>:
{
    80001bdc:	1101                	addi	sp,sp,-32
    80001bde:	ec06                	sd	ra,24(sp)
    80001be0:	e822                	sd	s0,16(sp)
    80001be2:	e426                	sd	s1,8(sp)
    80001be4:	e04a                	sd	s2,0(sp)
    80001be6:	1000                	addi	s0,sp,32
    80001be8:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    80001bea:	cf1ff0ef          	jal	800018da <myproc>
    80001bee:	892a                	mv	s2,a0
  sz = p->sz;
    80001bf0:	652c                	ld	a1,72(a0)
  if(n > 0){
    80001bf2:	02905963          	blez	s1,80001c24 <growproc+0x48>
    if(sz + n > TRAPFRAME) {
    80001bf6:	00b48633          	add	a2,s1,a1
    80001bfa:	020007b7          	lui	a5,0x2000
    80001bfe:	17fd                	addi	a5,a5,-1 # 1ffffff <_entry-0x7e000001>
    80001c00:	07b6                	slli	a5,a5,0xd
    80001c02:	02c7ea63          	bltu	a5,a2,80001c36 <growproc+0x5a>
    if((sz = uvmalloc(p->pagetable, sz, sz + n, PTE_W)) == 0) {
    80001c06:	4691                	li	a3,4
    80001c08:	6928                	ld	a0,80(a0)
    80001c0a:	e8aff0ef          	jal	80001294 <uvmalloc>
    80001c0e:	85aa                	mv	a1,a0
    80001c10:	c50d                	beqz	a0,80001c3a <growproc+0x5e>
  p->sz = sz;
    80001c12:	04b93423          	sd	a1,72(s2)
  return 0;
    80001c16:	4501                	li	a0,0
}
    80001c18:	60e2                	ld	ra,24(sp)
    80001c1a:	6442                	ld	s0,16(sp)
    80001c1c:	64a2                	ld	s1,8(sp)
    80001c1e:	6902                	ld	s2,0(sp)
    80001c20:	6105                	addi	sp,sp,32
    80001c22:	8082                	ret
  } else if(n < 0){
    80001c24:	fe04d7e3          	bgez	s1,80001c12 <growproc+0x36>
    sz = uvmdealloc(p->pagetable, sz, sz + n);
    80001c28:	00b48633          	add	a2,s1,a1
    80001c2c:	6928                	ld	a0,80(a0)
    80001c2e:	e22ff0ef          	jal	80001250 <uvmdealloc>
    80001c32:	85aa                	mv	a1,a0
    80001c34:	bff9                	j	80001c12 <growproc+0x36>
      return -1;
    80001c36:	557d                	li	a0,-1
    80001c38:	b7c5                	j	80001c18 <growproc+0x3c>
      return -1;
    80001c3a:	557d                	li	a0,-1
    80001c3c:	bff1                	j	80001c18 <growproc+0x3c>

0000000080001c3e <kfork>:
{
    80001c3e:	7139                	addi	sp,sp,-64
    80001c40:	fc06                	sd	ra,56(sp)
    80001c42:	f822                	sd	s0,48(sp)
    80001c44:	f04a                	sd	s2,32(sp)
    80001c46:	e456                	sd	s5,8(sp)
    80001c48:	0080                	addi	s0,sp,64
  struct proc *p = myproc();
    80001c4a:	c91ff0ef          	jal	800018da <myproc>
    80001c4e:	8aaa                	mv	s5,a0
  if((np = allocproc()) == 0){
    80001c50:	eabff0ef          	jal	80001afa <allocproc>
    80001c54:	0e050a63          	beqz	a0,80001d48 <kfork+0x10a>
    80001c58:	e852                	sd	s4,16(sp)
    80001c5a:	8a2a                	mv	s4,a0
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0){
    80001c5c:	048ab603          	ld	a2,72(s5)
    80001c60:	692c                	ld	a1,80(a0)
    80001c62:	050ab503          	ld	a0,80(s5)
    80001c66:	f66ff0ef          	jal	800013cc <uvmcopy>
    80001c6a:	04054a63          	bltz	a0,80001cbe <kfork+0x80>
    80001c6e:	f426                	sd	s1,40(sp)
    80001c70:	ec4e                	sd	s3,24(sp)
  np->sz = p->sz;
    80001c72:	048ab783          	ld	a5,72(s5)
    80001c76:	04fa3423          	sd	a5,72(s4)
  *(np->trapframe) = *(p->trapframe);
    80001c7a:	058ab683          	ld	a3,88(s5)
    80001c7e:	87b6                	mv	a5,a3
    80001c80:	058a3703          	ld	a4,88(s4)
    80001c84:	12068693          	addi	a3,a3,288
    80001c88:	0007b803          	ld	a6,0(a5)
    80001c8c:	6788                	ld	a0,8(a5)
    80001c8e:	6b8c                	ld	a1,16(a5)
    80001c90:	6f90                	ld	a2,24(a5)
    80001c92:	01073023          	sd	a6,0(a4) # 1000 <_entry-0x7ffff000>
    80001c96:	e708                	sd	a0,8(a4)
    80001c98:	eb0c                	sd	a1,16(a4)
    80001c9a:	ef10                	sd	a2,24(a4)
    80001c9c:	02078793          	addi	a5,a5,32
    80001ca0:	02070713          	addi	a4,a4,32
    80001ca4:	fed792e3          	bne	a5,a3,80001c88 <kfork+0x4a>
  np->trapframe->a0 = 0;
    80001ca8:	058a3783          	ld	a5,88(s4)
    80001cac:	0607b823          	sd	zero,112(a5)
  for(i = 0; i < NOFILE; i++)
    80001cb0:	0d0a8493          	addi	s1,s5,208
    80001cb4:	0d0a0913          	addi	s2,s4,208
    80001cb8:	150a8993          	addi	s3,s5,336
    80001cbc:	a831                	j	80001cd8 <kfork+0x9a>
    freeproc(np);
    80001cbe:	8552                	mv	a0,s4
    80001cc0:	debff0ef          	jal	80001aaa <freeproc>
    release(&np->lock);
    80001cc4:	8552                	mv	a0,s4
    80001cc6:	fa1fe0ef          	jal	80000c66 <release>
    return -1;
    80001cca:	597d                	li	s2,-1
    80001ccc:	6a42                	ld	s4,16(sp)
    80001cce:	a0b5                	j	80001d3a <kfork+0xfc>
  for(i = 0; i < NOFILE; i++)
    80001cd0:	04a1                	addi	s1,s1,8
    80001cd2:	0921                	addi	s2,s2,8
    80001cd4:	01348963          	beq	s1,s3,80001ce6 <kfork+0xa8>
    if(p->ofile[i])
    80001cd8:	6088                	ld	a0,0(s1)
    80001cda:	d97d                	beqz	a0,80001cd0 <kfork+0x92>
      np->ofile[i] = filedup(p->ofile[i]);
    80001cdc:	2c2020ef          	jal	80003f9e <filedup>
    80001ce0:	00a93023          	sd	a0,0(s2)
    80001ce4:	b7f5                	j	80001cd0 <kfork+0x92>
  np->cwd = idup(p->cwd);
    80001ce6:	150ab503          	ld	a0,336(s5)
    80001cea:	4ce010ef          	jal	800031b8 <idup>
    80001cee:	14aa3823          	sd	a0,336(s4)
  safestrcpy(np->name, p->name, sizeof(p->name));
    80001cf2:	4641                	li	a2,16
    80001cf4:	158a8593          	addi	a1,s5,344
    80001cf8:	158a0513          	addi	a0,s4,344
    80001cfc:	8e4ff0ef          	jal	80000de0 <safestrcpy>
  pid = np->pid;
    80001d00:	030a2903          	lw	s2,48(s4)
  release(&np->lock);
    80001d04:	8552                	mv	a0,s4
    80001d06:	f61fe0ef          	jal	80000c66 <release>
  acquire(&wait_lock);
    80001d0a:	00010497          	auipc	s1,0x10
    80001d0e:	6f648493          	addi	s1,s1,1782 # 80012400 <wait_lock>
    80001d12:	8526                	mv	a0,s1
    80001d14:	ebbfe0ef          	jal	80000bce <acquire>
  np->parent = p;
    80001d18:	035a3c23          	sd	s5,56(s4)
  release(&wait_lock);
    80001d1c:	8526                	mv	a0,s1
    80001d1e:	f49fe0ef          	jal	80000c66 <release>
  acquire(&np->lock);
    80001d22:	8552                	mv	a0,s4
    80001d24:	eabfe0ef          	jal	80000bce <acquire>
  np->state = RUNNABLE;
    80001d28:	478d                	li	a5,3
    80001d2a:	00fa2c23          	sw	a5,24(s4)
  release(&np->lock);
    80001d2e:	8552                	mv	a0,s4
    80001d30:	f37fe0ef          	jal	80000c66 <release>
  return pid;
    80001d34:	74a2                	ld	s1,40(sp)
    80001d36:	69e2                	ld	s3,24(sp)
    80001d38:	6a42                	ld	s4,16(sp)
}
    80001d3a:	854a                	mv	a0,s2
    80001d3c:	70e2                	ld	ra,56(sp)
    80001d3e:	7442                	ld	s0,48(sp)
    80001d40:	7902                	ld	s2,32(sp)
    80001d42:	6aa2                	ld	s5,8(sp)
    80001d44:	6121                	addi	sp,sp,64
    80001d46:	8082                	ret
    return -1;
    80001d48:	597d                	li	s2,-1
    80001d4a:	bfc5                	j	80001d3a <kfork+0xfc>

0000000080001d4c <scheduler>:
{
    80001d4c:	715d                	addi	sp,sp,-80
    80001d4e:	e486                	sd	ra,72(sp)
    80001d50:	e0a2                	sd	s0,64(sp)
    80001d52:	fc26                	sd	s1,56(sp)
    80001d54:	f84a                	sd	s2,48(sp)
    80001d56:	f44e                	sd	s3,40(sp)
    80001d58:	f052                	sd	s4,32(sp)
    80001d5a:	ec56                	sd	s5,24(sp)
    80001d5c:	e85a                	sd	s6,16(sp)
    80001d5e:	e45e                	sd	s7,8(sp)
    80001d60:	e062                	sd	s8,0(sp)
    80001d62:	0880                	addi	s0,sp,80
    80001d64:	8792                	mv	a5,tp
  int id = r_tp();
    80001d66:	2781                	sext.w	a5,a5
  c->proc = 0;
    80001d68:	00779b13          	slli	s6,a5,0x7
    80001d6c:	00010717          	auipc	a4,0x10
    80001d70:	67c70713          	addi	a4,a4,1660 # 800123e8 <pid_lock>
    80001d74:	975a                	add	a4,a4,s6
    80001d76:	02073823          	sd	zero,48(a4)
        swtch(&c->context, &p->context);
    80001d7a:	00010717          	auipc	a4,0x10
    80001d7e:	6a670713          	addi	a4,a4,1702 # 80012420 <cpus+0x8>
    80001d82:	9b3a                	add	s6,s6,a4
        p->state = RUNNING;
    80001d84:	4c11                	li	s8,4
        c->proc = p;
    80001d86:	079e                	slli	a5,a5,0x7
    80001d88:	00010a17          	auipc	s4,0x10
    80001d8c:	660a0a13          	addi	s4,s4,1632 # 800123e8 <pid_lock>
    80001d90:	9a3e                	add	s4,s4,a5
        found = 1;
    80001d92:	4b85                	li	s7,1
    for(p = proc; p < &proc[NPROC]; p++) {
    80001d94:	00016997          	auipc	s3,0x16
    80001d98:	48498993          	addi	s3,s3,1156 # 80018218 <tickslock>
    80001d9c:	a83d                	j	80001dda <scheduler+0x8e>
      release(&p->lock);
    80001d9e:	8526                	mv	a0,s1
    80001da0:	ec7fe0ef          	jal	80000c66 <release>
    for(p = proc; p < &proc[NPROC]; p++) {
    80001da4:	16848493          	addi	s1,s1,360
    80001da8:	03348563          	beq	s1,s3,80001dd2 <scheduler+0x86>
      acquire(&p->lock);
    80001dac:	8526                	mv	a0,s1
    80001dae:	e21fe0ef          	jal	80000bce <acquire>
      if(p->state == RUNNABLE) {
    80001db2:	4c9c                	lw	a5,24(s1)
    80001db4:	ff2795e3          	bne	a5,s2,80001d9e <scheduler+0x52>
        p->state = RUNNING;
    80001db8:	0184ac23          	sw	s8,24(s1)
        c->proc = p;
    80001dbc:	029a3823          	sd	s1,48(s4)
        swtch(&c->context, &p->context);
    80001dc0:	06048593          	addi	a1,s1,96
    80001dc4:	855a                	mv	a0,s6
    80001dc6:	5b2000ef          	jal	80002378 <swtch>
        c->proc = 0;
    80001dca:	020a3823          	sd	zero,48(s4)
        found = 1;
    80001dce:	8ade                	mv	s5,s7
    80001dd0:	b7f9                	j	80001d9e <scheduler+0x52>
    if(found == 0) {
    80001dd2:	000a9463          	bnez	s5,80001dda <scheduler+0x8e>
      asm volatile("wfi");
    80001dd6:	10500073          	wfi
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001dda:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80001dde:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80001de2:	10079073          	csrw	sstatus,a5
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001de6:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    80001dea:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80001dec:	10079073          	csrw	sstatus,a5
    int found = 0;
    80001df0:	4a81                	li	s5,0
    for(p = proc; p < &proc[NPROC]; p++) {
    80001df2:	00011497          	auipc	s1,0x11
    80001df6:	a2648493          	addi	s1,s1,-1498 # 80012818 <proc>
      if(p->state == RUNNABLE) {
    80001dfa:	490d                	li	s2,3
    80001dfc:	bf45                	j	80001dac <scheduler+0x60>

0000000080001dfe <sched>:
{
    80001dfe:	7179                	addi	sp,sp,-48
    80001e00:	f406                	sd	ra,40(sp)
    80001e02:	f022                	sd	s0,32(sp)
    80001e04:	ec26                	sd	s1,24(sp)
    80001e06:	e84a                	sd	s2,16(sp)
    80001e08:	e44e                	sd	s3,8(sp)
    80001e0a:	1800                	addi	s0,sp,48
  struct proc *p = myproc();
    80001e0c:	acfff0ef          	jal	800018da <myproc>
    80001e10:	84aa                	mv	s1,a0
  if(!holding(&p->lock))
    80001e12:	d53fe0ef          	jal	80000b64 <holding>
    80001e16:	c92d                	beqz	a0,80001e88 <sched+0x8a>
  asm volatile("mv %0, tp" : "=r" (x) );
    80001e18:	8792                	mv	a5,tp
  if(mycpu()->noff != 1)
    80001e1a:	2781                	sext.w	a5,a5
    80001e1c:	079e                	slli	a5,a5,0x7
    80001e1e:	00010717          	auipc	a4,0x10
    80001e22:	5ca70713          	addi	a4,a4,1482 # 800123e8 <pid_lock>
    80001e26:	97ba                	add	a5,a5,a4
    80001e28:	0a87a703          	lw	a4,168(a5)
    80001e2c:	4785                	li	a5,1
    80001e2e:	06f71363          	bne	a4,a5,80001e94 <sched+0x96>
  if(p->state == RUNNING)
    80001e32:	4c98                	lw	a4,24(s1)
    80001e34:	4791                	li	a5,4
    80001e36:	06f70563          	beq	a4,a5,80001ea0 <sched+0xa2>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80001e3a:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80001e3e:	8b89                	andi	a5,a5,2
  if(intr_get())
    80001e40:	e7b5                	bnez	a5,80001eac <sched+0xae>
  asm volatile("mv %0, tp" : "=r" (x) );
    80001e42:	8792                	mv	a5,tp
  intena = mycpu()->intena;
    80001e44:	00010917          	auipc	s2,0x10
    80001e48:	5a490913          	addi	s2,s2,1444 # 800123e8 <pid_lock>
    80001e4c:	2781                	sext.w	a5,a5
    80001e4e:	079e                	slli	a5,a5,0x7
    80001e50:	97ca                	add	a5,a5,s2
    80001e52:	0ac7a983          	lw	s3,172(a5)
    80001e56:	8792                	mv	a5,tp
  swtch(&p->context, &mycpu()->context);
    80001e58:	2781                	sext.w	a5,a5
    80001e5a:	079e                	slli	a5,a5,0x7
    80001e5c:	00010597          	auipc	a1,0x10
    80001e60:	5c458593          	addi	a1,a1,1476 # 80012420 <cpus+0x8>
    80001e64:	95be                	add	a1,a1,a5
    80001e66:	06048513          	addi	a0,s1,96
    80001e6a:	50e000ef          	jal	80002378 <swtch>
    80001e6e:	8792                	mv	a5,tp
  mycpu()->intena = intena;
    80001e70:	2781                	sext.w	a5,a5
    80001e72:	079e                	slli	a5,a5,0x7
    80001e74:	993e                	add	s2,s2,a5
    80001e76:	0b392623          	sw	s3,172(s2)
}
    80001e7a:	70a2                	ld	ra,40(sp)
    80001e7c:	7402                	ld	s0,32(sp)
    80001e7e:	64e2                	ld	s1,24(sp)
    80001e80:	6942                	ld	s2,16(sp)
    80001e82:	69a2                	ld	s3,8(sp)
    80001e84:	6145                	addi	sp,sp,48
    80001e86:	8082                	ret
    panic("sched p->lock");
    80001e88:	00005517          	auipc	a0,0x5
    80001e8c:	31050513          	addi	a0,a0,784 # 80007198 <etext+0x198>
    80001e90:	951fe0ef          	jal	800007e0 <panic>
    panic("sched locks");
    80001e94:	00005517          	auipc	a0,0x5
    80001e98:	31450513          	addi	a0,a0,788 # 800071a8 <etext+0x1a8>
    80001e9c:	945fe0ef          	jal	800007e0 <panic>
    panic("sched RUNNING");
    80001ea0:	00005517          	auipc	a0,0x5
    80001ea4:	31850513          	addi	a0,a0,792 # 800071b8 <etext+0x1b8>
    80001ea8:	939fe0ef          	jal	800007e0 <panic>
    panic("sched interruptible");
    80001eac:	00005517          	auipc	a0,0x5
    80001eb0:	31c50513          	addi	a0,a0,796 # 800071c8 <etext+0x1c8>
    80001eb4:	92dfe0ef          	jal	800007e0 <panic>

0000000080001eb8 <yield>:
{
    80001eb8:	1101                	addi	sp,sp,-32
    80001eba:	ec06                	sd	ra,24(sp)
    80001ebc:	e822                	sd	s0,16(sp)
    80001ebe:	e426                	sd	s1,8(sp)
    80001ec0:	1000                	addi	s0,sp,32
  struct proc *p = myproc();
    80001ec2:	a19ff0ef          	jal	800018da <myproc>
    80001ec6:	84aa                	mv	s1,a0
  acquire(&p->lock);
    80001ec8:	d07fe0ef          	jal	80000bce <acquire>
  p->state = RUNNABLE;
    80001ecc:	478d                	li	a5,3
    80001ece:	cc9c                	sw	a5,24(s1)
  sched();
    80001ed0:	f2fff0ef          	jal	80001dfe <sched>
  release(&p->lock);
    80001ed4:	8526                	mv	a0,s1
    80001ed6:	d91fe0ef          	jal	80000c66 <release>
}
    80001eda:	60e2                	ld	ra,24(sp)
    80001edc:	6442                	ld	s0,16(sp)
    80001ede:	64a2                	ld	s1,8(sp)
    80001ee0:	6105                	addi	sp,sp,32
    80001ee2:	8082                	ret

0000000080001ee4 <sleep>:

// Sleep on channel chan, releasing condition lock lk.
// Re-acquires lk when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
    80001ee4:	7179                	addi	sp,sp,-48
    80001ee6:	f406                	sd	ra,40(sp)
    80001ee8:	f022                	sd	s0,32(sp)
    80001eea:	ec26                	sd	s1,24(sp)
    80001eec:	e84a                	sd	s2,16(sp)
    80001eee:	e44e                	sd	s3,8(sp)
    80001ef0:	1800                	addi	s0,sp,48
    80001ef2:	89aa                	mv	s3,a0
    80001ef4:	892e                	mv	s2,a1
  struct proc *p = myproc();
    80001ef6:	9e5ff0ef          	jal	800018da <myproc>
    80001efa:	84aa                	mv	s1,a0
  // Once we hold p->lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup locks p->lock),
  // so it's okay to release lk.

  acquire(&p->lock);  //DOC: sleeplock1
    80001efc:	cd3fe0ef          	jal	80000bce <acquire>
  release(lk);
    80001f00:	854a                	mv	a0,s2
    80001f02:	d65fe0ef          	jal	80000c66 <release>

  // Go to sleep.
  p->chan = chan;
    80001f06:	0334b023          	sd	s3,32(s1)
  p->state = SLEEPING;
    80001f0a:	4789                	li	a5,2
    80001f0c:	cc9c                	sw	a5,24(s1)

  sched();
    80001f0e:	ef1ff0ef          	jal	80001dfe <sched>

  // Tidy up.
  p->chan = 0;
    80001f12:	0204b023          	sd	zero,32(s1)

  // Reacquire original lock.
  release(&p->lock);
    80001f16:	8526                	mv	a0,s1
    80001f18:	d4ffe0ef          	jal	80000c66 <release>
  acquire(lk);
    80001f1c:	854a                	mv	a0,s2
    80001f1e:	cb1fe0ef          	jal	80000bce <acquire>
}
    80001f22:	70a2                	ld	ra,40(sp)
    80001f24:	7402                	ld	s0,32(sp)
    80001f26:	64e2                	ld	s1,24(sp)
    80001f28:	6942                	ld	s2,16(sp)
    80001f2a:	69a2                	ld	s3,8(sp)
    80001f2c:	6145                	addi	sp,sp,48
    80001f2e:	8082                	ret

0000000080001f30 <wakeup>:

// Wake up all processes sleeping on channel chan.
// Caller should hold the condition lock.
void
wakeup(void *chan)
{
    80001f30:	7139                	addi	sp,sp,-64
    80001f32:	fc06                	sd	ra,56(sp)
    80001f34:	f822                	sd	s0,48(sp)
    80001f36:	f426                	sd	s1,40(sp)
    80001f38:	f04a                	sd	s2,32(sp)
    80001f3a:	ec4e                	sd	s3,24(sp)
    80001f3c:	e852                	sd	s4,16(sp)
    80001f3e:	e456                	sd	s5,8(sp)
    80001f40:	0080                	addi	s0,sp,64
    80001f42:	8a2a                	mv	s4,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++) {
    80001f44:	00011497          	auipc	s1,0x11
    80001f48:	8d448493          	addi	s1,s1,-1836 # 80012818 <proc>
    if(p != myproc()){
      acquire(&p->lock);
      if(p->state == SLEEPING && p->chan == chan) {
    80001f4c:	4989                	li	s3,2
        p->state = RUNNABLE;
    80001f4e:	4a8d                	li	s5,3
  for(p = proc; p < &proc[NPROC]; p++) {
    80001f50:	00016917          	auipc	s2,0x16
    80001f54:	2c890913          	addi	s2,s2,712 # 80018218 <tickslock>
    80001f58:	a801                	j	80001f68 <wakeup+0x38>
      }
      release(&p->lock);
    80001f5a:	8526                	mv	a0,s1
    80001f5c:	d0bfe0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++) {
    80001f60:	16848493          	addi	s1,s1,360
    80001f64:	03248263          	beq	s1,s2,80001f88 <wakeup+0x58>
    if(p != myproc()){
    80001f68:	973ff0ef          	jal	800018da <myproc>
    80001f6c:	fea48ae3          	beq	s1,a0,80001f60 <wakeup+0x30>
      acquire(&p->lock);
    80001f70:	8526                	mv	a0,s1
    80001f72:	c5dfe0ef          	jal	80000bce <acquire>
      if(p->state == SLEEPING && p->chan == chan) {
    80001f76:	4c9c                	lw	a5,24(s1)
    80001f78:	ff3791e3          	bne	a5,s3,80001f5a <wakeup+0x2a>
    80001f7c:	709c                	ld	a5,32(s1)
    80001f7e:	fd479ee3          	bne	a5,s4,80001f5a <wakeup+0x2a>
        p->state = RUNNABLE;
    80001f82:	0154ac23          	sw	s5,24(s1)
    80001f86:	bfd1                	j	80001f5a <wakeup+0x2a>
    }
  }
}
    80001f88:	70e2                	ld	ra,56(sp)
    80001f8a:	7442                	ld	s0,48(sp)
    80001f8c:	74a2                	ld	s1,40(sp)
    80001f8e:	7902                	ld	s2,32(sp)
    80001f90:	69e2                	ld	s3,24(sp)
    80001f92:	6a42                	ld	s4,16(sp)
    80001f94:	6aa2                	ld	s5,8(sp)
    80001f96:	6121                	addi	sp,sp,64
    80001f98:	8082                	ret

0000000080001f9a <reparent>:
{
    80001f9a:	7179                	addi	sp,sp,-48
    80001f9c:	f406                	sd	ra,40(sp)
    80001f9e:	f022                	sd	s0,32(sp)
    80001fa0:	ec26                	sd	s1,24(sp)
    80001fa2:	e84a                	sd	s2,16(sp)
    80001fa4:	e44e                	sd	s3,8(sp)
    80001fa6:	e052                	sd	s4,0(sp)
    80001fa8:	1800                	addi	s0,sp,48
    80001faa:	892a                	mv	s2,a0
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001fac:	00011497          	auipc	s1,0x11
    80001fb0:	86c48493          	addi	s1,s1,-1940 # 80012818 <proc>
      pp->parent = initproc;
    80001fb4:	00008a17          	auipc	s4,0x8
    80001fb8:	32ca0a13          	addi	s4,s4,812 # 8000a2e0 <initproc>
  for(pp = proc; pp < &proc[NPROC]; pp++){
    80001fbc:	00016997          	auipc	s3,0x16
    80001fc0:	25c98993          	addi	s3,s3,604 # 80018218 <tickslock>
    80001fc4:	a029                	j	80001fce <reparent+0x34>
    80001fc6:	16848493          	addi	s1,s1,360
    80001fca:	01348b63          	beq	s1,s3,80001fe0 <reparent+0x46>
    if(pp->parent == p){
    80001fce:	7c9c                	ld	a5,56(s1)
    80001fd0:	ff279be3          	bne	a5,s2,80001fc6 <reparent+0x2c>
      pp->parent = initproc;
    80001fd4:	000a3503          	ld	a0,0(s4)
    80001fd8:	fc88                	sd	a0,56(s1)
      wakeup(initproc);
    80001fda:	f57ff0ef          	jal	80001f30 <wakeup>
    80001fde:	b7e5                	j	80001fc6 <reparent+0x2c>
}
    80001fe0:	70a2                	ld	ra,40(sp)
    80001fe2:	7402                	ld	s0,32(sp)
    80001fe4:	64e2                	ld	s1,24(sp)
    80001fe6:	6942                	ld	s2,16(sp)
    80001fe8:	69a2                	ld	s3,8(sp)
    80001fea:	6a02                	ld	s4,0(sp)
    80001fec:	6145                	addi	sp,sp,48
    80001fee:	8082                	ret

0000000080001ff0 <kexit>:
{
    80001ff0:	7179                	addi	sp,sp,-48
    80001ff2:	f406                	sd	ra,40(sp)
    80001ff4:	f022                	sd	s0,32(sp)
    80001ff6:	ec26                	sd	s1,24(sp)
    80001ff8:	e84a                	sd	s2,16(sp)
    80001ffa:	e44e                	sd	s3,8(sp)
    80001ffc:	e052                	sd	s4,0(sp)
    80001ffe:	1800                	addi	s0,sp,48
    80002000:	8a2a                	mv	s4,a0
  struct proc *p = myproc();
    80002002:	8d9ff0ef          	jal	800018da <myproc>
    80002006:	89aa                	mv	s3,a0
  if(p == initproc)
    80002008:	00008797          	auipc	a5,0x8
    8000200c:	2d87b783          	ld	a5,728(a5) # 8000a2e0 <initproc>
    80002010:	0d050493          	addi	s1,a0,208
    80002014:	15050913          	addi	s2,a0,336
    80002018:	00a79f63          	bne	a5,a0,80002036 <kexit+0x46>
    panic("init exiting");
    8000201c:	00005517          	auipc	a0,0x5
    80002020:	1c450513          	addi	a0,a0,452 # 800071e0 <etext+0x1e0>
    80002024:	fbcfe0ef          	jal	800007e0 <panic>
      fileclose(f);
    80002028:	7bd010ef          	jal	80003fe4 <fileclose>
      p->ofile[fd] = 0;
    8000202c:	0004b023          	sd	zero,0(s1)
  for(int fd = 0; fd < NOFILE; fd++){
    80002030:	04a1                	addi	s1,s1,8
    80002032:	01248563          	beq	s1,s2,8000203c <kexit+0x4c>
    if(p->ofile[fd]){
    80002036:	6088                	ld	a0,0(s1)
    80002038:	f965                	bnez	a0,80002028 <kexit+0x38>
    8000203a:	bfdd                	j	80002030 <kexit+0x40>
  begin_op();
    8000203c:	39d010ef          	jal	80003bd8 <begin_op>
  iput(p->cwd);
    80002040:	1509b503          	ld	a0,336(s3)
    80002044:	32c010ef          	jal	80003370 <iput>
  end_op();
    80002048:	3fb010ef          	jal	80003c42 <end_op>
  p->cwd = 0;
    8000204c:	1409b823          	sd	zero,336(s3)
  acquire(&wait_lock);
    80002050:	00010497          	auipc	s1,0x10
    80002054:	3b048493          	addi	s1,s1,944 # 80012400 <wait_lock>
    80002058:	8526                	mv	a0,s1
    8000205a:	b75fe0ef          	jal	80000bce <acquire>
  reparent(p);
    8000205e:	854e                	mv	a0,s3
    80002060:	f3bff0ef          	jal	80001f9a <reparent>
  wakeup(p->parent);
    80002064:	0389b503          	ld	a0,56(s3)
    80002068:	ec9ff0ef          	jal	80001f30 <wakeup>
  acquire(&p->lock);
    8000206c:	854e                	mv	a0,s3
    8000206e:	b61fe0ef          	jal	80000bce <acquire>
  p->xstate = status;
    80002072:	0349a623          	sw	s4,44(s3)
  p->state = ZOMBIE;
    80002076:	4795                	li	a5,5
    80002078:	00f9ac23          	sw	a5,24(s3)
  release(&wait_lock);
    8000207c:	8526                	mv	a0,s1
    8000207e:	be9fe0ef          	jal	80000c66 <release>
  sched();
    80002082:	d7dff0ef          	jal	80001dfe <sched>
  panic("zombie exit");
    80002086:	00005517          	auipc	a0,0x5
    8000208a:	16a50513          	addi	a0,a0,362 # 800071f0 <etext+0x1f0>
    8000208e:	f52fe0ef          	jal	800007e0 <panic>

0000000080002092 <kkill>:
// Kill the process with the given pid.
// The victim won't exit until it tries to return
// to user space (see usertrap() in trap.c).
int
kkill(int pid)
{
    80002092:	7179                	addi	sp,sp,-48
    80002094:	f406                	sd	ra,40(sp)
    80002096:	f022                	sd	s0,32(sp)
    80002098:	ec26                	sd	s1,24(sp)
    8000209a:	e84a                	sd	s2,16(sp)
    8000209c:	e44e                	sd	s3,8(sp)
    8000209e:	1800                	addi	s0,sp,48
    800020a0:	892a                	mv	s2,a0
  struct proc *p;

  for(p = proc; p < &proc[NPROC]; p++){
    800020a2:	00010497          	auipc	s1,0x10
    800020a6:	77648493          	addi	s1,s1,1910 # 80012818 <proc>
    800020aa:	00016997          	auipc	s3,0x16
    800020ae:	16e98993          	addi	s3,s3,366 # 80018218 <tickslock>
    acquire(&p->lock);
    800020b2:	8526                	mv	a0,s1
    800020b4:	b1bfe0ef          	jal	80000bce <acquire>
    if(p->pid == pid){
    800020b8:	589c                	lw	a5,48(s1)
    800020ba:	01278b63          	beq	a5,s2,800020d0 <kkill+0x3e>
        p->state = RUNNABLE;
      }
      release(&p->lock);
      return 0;
    }
    release(&p->lock);
    800020be:	8526                	mv	a0,s1
    800020c0:	ba7fe0ef          	jal	80000c66 <release>
  for(p = proc; p < &proc[NPROC]; p++){
    800020c4:	16848493          	addi	s1,s1,360
    800020c8:	ff3495e3          	bne	s1,s3,800020b2 <kkill+0x20>
  }
  return -1;
    800020cc:	557d                	li	a0,-1
    800020ce:	a819                	j	800020e4 <kkill+0x52>
      p->killed = 1;
    800020d0:	4785                	li	a5,1
    800020d2:	d49c                	sw	a5,40(s1)
      if(p->state == SLEEPING){
    800020d4:	4c98                	lw	a4,24(s1)
    800020d6:	4789                	li	a5,2
    800020d8:	00f70d63          	beq	a4,a5,800020f2 <kkill+0x60>
      release(&p->lock);
    800020dc:	8526                	mv	a0,s1
    800020de:	b89fe0ef          	jal	80000c66 <release>
      return 0;
    800020e2:	4501                	li	a0,0
}
    800020e4:	70a2                	ld	ra,40(sp)
    800020e6:	7402                	ld	s0,32(sp)
    800020e8:	64e2                	ld	s1,24(sp)
    800020ea:	6942                	ld	s2,16(sp)
    800020ec:	69a2                	ld	s3,8(sp)
    800020ee:	6145                	addi	sp,sp,48
    800020f0:	8082                	ret
        p->state = RUNNABLE;
    800020f2:	478d                	li	a5,3
    800020f4:	cc9c                	sw	a5,24(s1)
    800020f6:	b7dd                	j	800020dc <kkill+0x4a>

00000000800020f8 <setkilled>:

void
setkilled(struct proc *p)
{
    800020f8:	1101                	addi	sp,sp,-32
    800020fa:	ec06                	sd	ra,24(sp)
    800020fc:	e822                	sd	s0,16(sp)
    800020fe:	e426                	sd	s1,8(sp)
    80002100:	1000                	addi	s0,sp,32
    80002102:	84aa                	mv	s1,a0
  acquire(&p->lock);
    80002104:	acbfe0ef          	jal	80000bce <acquire>
  p->killed = 1;
    80002108:	4785                	li	a5,1
    8000210a:	d49c                	sw	a5,40(s1)
  release(&p->lock);
    8000210c:	8526                	mv	a0,s1
    8000210e:	b59fe0ef          	jal	80000c66 <release>
}
    80002112:	60e2                	ld	ra,24(sp)
    80002114:	6442                	ld	s0,16(sp)
    80002116:	64a2                	ld	s1,8(sp)
    80002118:	6105                	addi	sp,sp,32
    8000211a:	8082                	ret

000000008000211c <killed>:

int
killed(struct proc *p)
{
    8000211c:	1101                	addi	sp,sp,-32
    8000211e:	ec06                	sd	ra,24(sp)
    80002120:	e822                	sd	s0,16(sp)
    80002122:	e426                	sd	s1,8(sp)
    80002124:	e04a                	sd	s2,0(sp)
    80002126:	1000                	addi	s0,sp,32
    80002128:	84aa                	mv	s1,a0
  int k;
  
  acquire(&p->lock);
    8000212a:	aa5fe0ef          	jal	80000bce <acquire>
  k = p->killed;
    8000212e:	0284a903          	lw	s2,40(s1)
  release(&p->lock);
    80002132:	8526                	mv	a0,s1
    80002134:	b33fe0ef          	jal	80000c66 <release>
  return k;
}
    80002138:	854a                	mv	a0,s2
    8000213a:	60e2                	ld	ra,24(sp)
    8000213c:	6442                	ld	s0,16(sp)
    8000213e:	64a2                	ld	s1,8(sp)
    80002140:	6902                	ld	s2,0(sp)
    80002142:	6105                	addi	sp,sp,32
    80002144:	8082                	ret

0000000080002146 <kwait>:
{
    80002146:	715d                	addi	sp,sp,-80
    80002148:	e486                	sd	ra,72(sp)
    8000214a:	e0a2                	sd	s0,64(sp)
    8000214c:	fc26                	sd	s1,56(sp)
    8000214e:	f84a                	sd	s2,48(sp)
    80002150:	f44e                	sd	s3,40(sp)
    80002152:	f052                	sd	s4,32(sp)
    80002154:	ec56                	sd	s5,24(sp)
    80002156:	e85a                	sd	s6,16(sp)
    80002158:	e45e                	sd	s7,8(sp)
    8000215a:	e062                	sd	s8,0(sp)
    8000215c:	0880                	addi	s0,sp,80
    8000215e:	8b2a                	mv	s6,a0
  struct proc *p = myproc();
    80002160:	f7aff0ef          	jal	800018da <myproc>
    80002164:	892a                	mv	s2,a0
  acquire(&wait_lock);
    80002166:	00010517          	auipc	a0,0x10
    8000216a:	29a50513          	addi	a0,a0,666 # 80012400 <wait_lock>
    8000216e:	a61fe0ef          	jal	80000bce <acquire>
    havekids = 0;
    80002172:	4b81                	li	s7,0
        if(pp->state == ZOMBIE){
    80002174:	4a15                	li	s4,5
        havekids = 1;
    80002176:	4a85                	li	s5,1
    for(pp = proc; pp < &proc[NPROC]; pp++){
    80002178:	00016997          	auipc	s3,0x16
    8000217c:	0a098993          	addi	s3,s3,160 # 80018218 <tickslock>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    80002180:	00010c17          	auipc	s8,0x10
    80002184:	280c0c13          	addi	s8,s8,640 # 80012400 <wait_lock>
    80002188:	a871                	j	80002224 <kwait+0xde>
          pid = pp->pid;
    8000218a:	0304a983          	lw	s3,48(s1)
          if(addr != 0 && copyout(p->pagetable, addr, (char *)&pp->xstate,
    8000218e:	000b0c63          	beqz	s6,800021a6 <kwait+0x60>
    80002192:	4691                	li	a3,4
    80002194:	02c48613          	addi	a2,s1,44
    80002198:	85da                	mv	a1,s6
    8000219a:	05093503          	ld	a0,80(s2)
    8000219e:	c50ff0ef          	jal	800015ee <copyout>
    800021a2:	02054b63          	bltz	a0,800021d8 <kwait+0x92>
          freeproc(pp);
    800021a6:	8526                	mv	a0,s1
    800021a8:	903ff0ef          	jal	80001aaa <freeproc>
          release(&pp->lock);
    800021ac:	8526                	mv	a0,s1
    800021ae:	ab9fe0ef          	jal	80000c66 <release>
          release(&wait_lock);
    800021b2:	00010517          	auipc	a0,0x10
    800021b6:	24e50513          	addi	a0,a0,590 # 80012400 <wait_lock>
    800021ba:	aadfe0ef          	jal	80000c66 <release>
}
    800021be:	854e                	mv	a0,s3
    800021c0:	60a6                	ld	ra,72(sp)
    800021c2:	6406                	ld	s0,64(sp)
    800021c4:	74e2                	ld	s1,56(sp)
    800021c6:	7942                	ld	s2,48(sp)
    800021c8:	79a2                	ld	s3,40(sp)
    800021ca:	7a02                	ld	s4,32(sp)
    800021cc:	6ae2                	ld	s5,24(sp)
    800021ce:	6b42                	ld	s6,16(sp)
    800021d0:	6ba2                	ld	s7,8(sp)
    800021d2:	6c02                	ld	s8,0(sp)
    800021d4:	6161                	addi	sp,sp,80
    800021d6:	8082                	ret
            release(&pp->lock);
    800021d8:	8526                	mv	a0,s1
    800021da:	a8dfe0ef          	jal	80000c66 <release>
            release(&wait_lock);
    800021de:	00010517          	auipc	a0,0x10
    800021e2:	22250513          	addi	a0,a0,546 # 80012400 <wait_lock>
    800021e6:	a81fe0ef          	jal	80000c66 <release>
            return -1;
    800021ea:	59fd                	li	s3,-1
    800021ec:	bfc9                	j	800021be <kwait+0x78>
    for(pp = proc; pp < &proc[NPROC]; pp++){
    800021ee:	16848493          	addi	s1,s1,360
    800021f2:	03348063          	beq	s1,s3,80002212 <kwait+0xcc>
      if(pp->parent == p){
    800021f6:	7c9c                	ld	a5,56(s1)
    800021f8:	ff279be3          	bne	a5,s2,800021ee <kwait+0xa8>
        acquire(&pp->lock);
    800021fc:	8526                	mv	a0,s1
    800021fe:	9d1fe0ef          	jal	80000bce <acquire>
        if(pp->state == ZOMBIE){
    80002202:	4c9c                	lw	a5,24(s1)
    80002204:	f94783e3          	beq	a5,s4,8000218a <kwait+0x44>
        release(&pp->lock);
    80002208:	8526                	mv	a0,s1
    8000220a:	a5dfe0ef          	jal	80000c66 <release>
        havekids = 1;
    8000220e:	8756                	mv	a4,s5
    80002210:	bff9                	j	800021ee <kwait+0xa8>
    if(!havekids || killed(p)){
    80002212:	cf19                	beqz	a4,80002230 <kwait+0xea>
    80002214:	854a                	mv	a0,s2
    80002216:	f07ff0ef          	jal	8000211c <killed>
    8000221a:	e919                	bnez	a0,80002230 <kwait+0xea>
    sleep(p, &wait_lock);  //DOC: wait-sleep
    8000221c:	85e2                	mv	a1,s8
    8000221e:	854a                	mv	a0,s2
    80002220:	cc5ff0ef          	jal	80001ee4 <sleep>
    havekids = 0;
    80002224:	875e                	mv	a4,s7
    for(pp = proc; pp < &proc[NPROC]; pp++){
    80002226:	00010497          	auipc	s1,0x10
    8000222a:	5f248493          	addi	s1,s1,1522 # 80012818 <proc>
    8000222e:	b7e1                	j	800021f6 <kwait+0xb0>
      release(&wait_lock);
    80002230:	00010517          	auipc	a0,0x10
    80002234:	1d050513          	addi	a0,a0,464 # 80012400 <wait_lock>
    80002238:	a2ffe0ef          	jal	80000c66 <release>
      return -1;
    8000223c:	59fd                	li	s3,-1
    8000223e:	b741                	j	800021be <kwait+0x78>

0000000080002240 <either_copyout>:
// Copy to either a user address, or kernel address,
// depending on usr_dst.
// Returns 0 on success, -1 on error.
int
either_copyout(int user_dst, uint64 dst, void *src, uint64 len)
{
    80002240:	7179                	addi	sp,sp,-48
    80002242:	f406                	sd	ra,40(sp)
    80002244:	f022                	sd	s0,32(sp)
    80002246:	ec26                	sd	s1,24(sp)
    80002248:	e84a                	sd	s2,16(sp)
    8000224a:	e44e                	sd	s3,8(sp)
    8000224c:	e052                	sd	s4,0(sp)
    8000224e:	1800                	addi	s0,sp,48
    80002250:	84aa                	mv	s1,a0
    80002252:	892e                	mv	s2,a1
    80002254:	89b2                	mv	s3,a2
    80002256:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    80002258:	e82ff0ef          	jal	800018da <myproc>
  if(user_dst){
    8000225c:	cc99                	beqz	s1,8000227a <either_copyout+0x3a>
    return copyout(p->pagetable, dst, src, len);
    8000225e:	86d2                	mv	a3,s4
    80002260:	864e                	mv	a2,s3
    80002262:	85ca                	mv	a1,s2
    80002264:	6928                	ld	a0,80(a0)
    80002266:	b88ff0ef          	jal	800015ee <copyout>
  } else {
    memmove((char *)dst, src, len);
    return 0;
  }
}
    8000226a:	70a2                	ld	ra,40(sp)
    8000226c:	7402                	ld	s0,32(sp)
    8000226e:	64e2                	ld	s1,24(sp)
    80002270:	6942                	ld	s2,16(sp)
    80002272:	69a2                	ld	s3,8(sp)
    80002274:	6a02                	ld	s4,0(sp)
    80002276:	6145                	addi	sp,sp,48
    80002278:	8082                	ret
    memmove((char *)dst, src, len);
    8000227a:	000a061b          	sext.w	a2,s4
    8000227e:	85ce                	mv	a1,s3
    80002280:	854a                	mv	a0,s2
    80002282:	a7dfe0ef          	jal	80000cfe <memmove>
    return 0;
    80002286:	8526                	mv	a0,s1
    80002288:	b7cd                	j	8000226a <either_copyout+0x2a>

000000008000228a <either_copyin>:
// Copy from either a user address, or kernel address,
// depending on usr_src.
// Returns 0 on success, -1 on error.
int
either_copyin(void *dst, int user_src, uint64 src, uint64 len)
{
    8000228a:	7179                	addi	sp,sp,-48
    8000228c:	f406                	sd	ra,40(sp)
    8000228e:	f022                	sd	s0,32(sp)
    80002290:	ec26                	sd	s1,24(sp)
    80002292:	e84a                	sd	s2,16(sp)
    80002294:	e44e                	sd	s3,8(sp)
    80002296:	e052                	sd	s4,0(sp)
    80002298:	1800                	addi	s0,sp,48
    8000229a:	892a                	mv	s2,a0
    8000229c:	84ae                	mv	s1,a1
    8000229e:	89b2                	mv	s3,a2
    800022a0:	8a36                	mv	s4,a3
  struct proc *p = myproc();
    800022a2:	e38ff0ef          	jal	800018da <myproc>
  if(user_src){
    800022a6:	cc99                	beqz	s1,800022c4 <either_copyin+0x3a>
    return copyin(p->pagetable, dst, src, len);
    800022a8:	86d2                	mv	a3,s4
    800022aa:	864e                	mv	a2,s3
    800022ac:	85ca                	mv	a1,s2
    800022ae:	6928                	ld	a0,80(a0)
    800022b0:	c22ff0ef          	jal	800016d2 <copyin>
  } else {
    memmove(dst, (char*)src, len);
    return 0;
  }
}
    800022b4:	70a2                	ld	ra,40(sp)
    800022b6:	7402                	ld	s0,32(sp)
    800022b8:	64e2                	ld	s1,24(sp)
    800022ba:	6942                	ld	s2,16(sp)
    800022bc:	69a2                	ld	s3,8(sp)
    800022be:	6a02                	ld	s4,0(sp)
    800022c0:	6145                	addi	sp,sp,48
    800022c2:	8082                	ret
    memmove(dst, (char*)src, len);
    800022c4:	000a061b          	sext.w	a2,s4
    800022c8:	85ce                	mv	a1,s3
    800022ca:	854a                	mv	a0,s2
    800022cc:	a33fe0ef          	jal	80000cfe <memmove>
    return 0;
    800022d0:	8526                	mv	a0,s1
    800022d2:	b7cd                	j	800022b4 <either_copyin+0x2a>

00000000800022d4 <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
    800022d4:	715d                	addi	sp,sp,-80
    800022d6:	e486                	sd	ra,72(sp)
    800022d8:	e0a2                	sd	s0,64(sp)
    800022da:	fc26                	sd	s1,56(sp)
    800022dc:	f84a                	sd	s2,48(sp)
    800022de:	f44e                	sd	s3,40(sp)
    800022e0:	f052                	sd	s4,32(sp)
    800022e2:	ec56                	sd	s5,24(sp)
    800022e4:	e85a                	sd	s6,16(sp)
    800022e6:	e45e                	sd	s7,8(sp)
    800022e8:	0880                	addi	s0,sp,80
  [ZOMBIE]    "zombie"
  };
  struct proc *p;
  char *state;

  printf("\n");
    800022ea:	00005517          	auipc	a0,0x5
    800022ee:	d8e50513          	addi	a0,a0,-626 # 80007078 <etext+0x78>
    800022f2:	a08fe0ef          	jal	800004fa <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    800022f6:	00010497          	auipc	s1,0x10
    800022fa:	67a48493          	addi	s1,s1,1658 # 80012970 <proc+0x158>
    800022fe:	00016917          	auipc	s2,0x16
    80002302:	07290913          	addi	s2,s2,114 # 80018370 <bcache+0x140>
    if(p->state == UNUSED)
      continue;
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002306:	4b15                	li	s6,5
      state = states[p->state];
    else
      state = "???";
    80002308:	00005997          	auipc	s3,0x5
    8000230c:	ef898993          	addi	s3,s3,-264 # 80007200 <etext+0x200>
    printf("%d %s %s", p->pid, state, p->name);
    80002310:	00005a97          	auipc	s5,0x5
    80002314:	ef8a8a93          	addi	s5,s5,-264 # 80007208 <etext+0x208>
    printf("\n");
    80002318:	00005a17          	auipc	s4,0x5
    8000231c:	d60a0a13          	addi	s4,s4,-672 # 80007078 <etext+0x78>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    80002320:	00005b97          	auipc	s7,0x5
    80002324:	410b8b93          	addi	s7,s7,1040 # 80007730 <states.0>
    80002328:	a829                	j	80002342 <procdump+0x6e>
    printf("%d %s %s", p->pid, state, p->name);
    8000232a:	ed86a583          	lw	a1,-296(a3)
    8000232e:	8556                	mv	a0,s5
    80002330:	9cafe0ef          	jal	800004fa <printf>
    printf("\n");
    80002334:	8552                	mv	a0,s4
    80002336:	9c4fe0ef          	jal	800004fa <printf>
  for(p = proc; p < &proc[NPROC]; p++){
    8000233a:	16848493          	addi	s1,s1,360
    8000233e:	03248263          	beq	s1,s2,80002362 <procdump+0x8e>
    if(p->state == UNUSED)
    80002342:	86a6                	mv	a3,s1
    80002344:	ec04a783          	lw	a5,-320(s1)
    80002348:	dbed                	beqz	a5,8000233a <procdump+0x66>
      state = "???";
    8000234a:	864e                	mv	a2,s3
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
    8000234c:	fcfb6fe3          	bltu	s6,a5,8000232a <procdump+0x56>
    80002350:	02079713          	slli	a4,a5,0x20
    80002354:	01d75793          	srli	a5,a4,0x1d
    80002358:	97de                	add	a5,a5,s7
    8000235a:	6390                	ld	a2,0(a5)
    8000235c:	f679                	bnez	a2,8000232a <procdump+0x56>
      state = "???";
    8000235e:	864e                	mv	a2,s3
    80002360:	b7e9                	j	8000232a <procdump+0x56>
  }
}
    80002362:	60a6                	ld	ra,72(sp)
    80002364:	6406                	ld	s0,64(sp)
    80002366:	74e2                	ld	s1,56(sp)
    80002368:	7942                	ld	s2,48(sp)
    8000236a:	79a2                	ld	s3,40(sp)
    8000236c:	7a02                	ld	s4,32(sp)
    8000236e:	6ae2                	ld	s5,24(sp)
    80002370:	6b42                	ld	s6,16(sp)
    80002372:	6ba2                	ld	s7,8(sp)
    80002374:	6161                	addi	sp,sp,80
    80002376:	8082                	ret

0000000080002378 <swtch>:
# Save current registers in old. Load from new.	


.globl swtch
swtch:
        sd ra, 0(a0)
    80002378:	00153023          	sd	ra,0(a0)
        sd sp, 8(a0)
    8000237c:	00253423          	sd	sp,8(a0)
        sd s0, 16(a0)
    80002380:	e900                	sd	s0,16(a0)
        sd s1, 24(a0)
    80002382:	ed04                	sd	s1,24(a0)
        sd s2, 32(a0)
    80002384:	03253023          	sd	s2,32(a0)
        sd s3, 40(a0)
    80002388:	03353423          	sd	s3,40(a0)
        sd s4, 48(a0)
    8000238c:	03453823          	sd	s4,48(a0)
        sd s5, 56(a0)
    80002390:	03553c23          	sd	s5,56(a0)
        sd s6, 64(a0)
    80002394:	05653023          	sd	s6,64(a0)
        sd s7, 72(a0)
    80002398:	05753423          	sd	s7,72(a0)
        sd s8, 80(a0)
    8000239c:	05853823          	sd	s8,80(a0)
        sd s9, 88(a0)
    800023a0:	05953c23          	sd	s9,88(a0)
        sd s10, 96(a0)
    800023a4:	07a53023          	sd	s10,96(a0)
        sd s11, 104(a0)
    800023a8:	07b53423          	sd	s11,104(a0)

        ld ra, 0(a1)
    800023ac:	0005b083          	ld	ra,0(a1)
        ld sp, 8(a1)
    800023b0:	0085b103          	ld	sp,8(a1)
        ld s0, 16(a1)
    800023b4:	6980                	ld	s0,16(a1)
        ld s1, 24(a1)
    800023b6:	6d84                	ld	s1,24(a1)
        ld s2, 32(a1)
    800023b8:	0205b903          	ld	s2,32(a1)
        ld s3, 40(a1)
    800023bc:	0285b983          	ld	s3,40(a1)
        ld s4, 48(a1)
    800023c0:	0305ba03          	ld	s4,48(a1)
        ld s5, 56(a1)
    800023c4:	0385ba83          	ld	s5,56(a1)
        ld s6, 64(a1)
    800023c8:	0405bb03          	ld	s6,64(a1)
        ld s7, 72(a1)
    800023cc:	0485bb83          	ld	s7,72(a1)
        ld s8, 80(a1)
    800023d0:	0505bc03          	ld	s8,80(a1)
        ld s9, 88(a1)
    800023d4:	0585bc83          	ld	s9,88(a1)
        ld s10, 96(a1)
    800023d8:	0605bd03          	ld	s10,96(a1)
        ld s11, 104(a1)
    800023dc:	0685bd83          	ld	s11,104(a1)
        
        ret
    800023e0:	8082                	ret

00000000800023e2 <trapinit>:

extern int devintr();

void
trapinit(void)
{
    800023e2:	1141                	addi	sp,sp,-16
    800023e4:	e406                	sd	ra,8(sp)
    800023e6:	e022                	sd	s0,0(sp)
    800023e8:	0800                	addi	s0,sp,16
  initlock(&tickslock, "time");
    800023ea:	00005597          	auipc	a1,0x5
    800023ee:	e5e58593          	addi	a1,a1,-418 # 80007248 <etext+0x248>
    800023f2:	00016517          	auipc	a0,0x16
    800023f6:	e2650513          	addi	a0,a0,-474 # 80018218 <tickslock>
    800023fa:	f54fe0ef          	jal	80000b4e <initlock>
}
    800023fe:	60a2                	ld	ra,8(sp)
    80002400:	6402                	ld	s0,0(sp)
    80002402:	0141                	addi	sp,sp,16
    80002404:	8082                	ret

0000000080002406 <trapinithart>:

// set up to take exceptions and traps while in the kernel.
void
trapinithart(void)
{
    80002406:	1141                	addi	sp,sp,-16
    80002408:	e422                	sd	s0,8(sp)
    8000240a:	0800                	addi	s0,sp,16
  asm volatile("csrw stvec, %0" : : "r" (x));
    8000240c:	00003797          	auipc	a5,0x3
    80002410:	fc478793          	addi	a5,a5,-60 # 800053d0 <kernelvec>
    80002414:	10579073          	csrw	stvec,a5
  w_stvec((uint64)kernelvec);
}
    80002418:	6422                	ld	s0,8(sp)
    8000241a:	0141                	addi	sp,sp,16
    8000241c:	8082                	ret

000000008000241e <prepare_return>:
//
// set up trapframe and control registers for a return to user space
//
void
prepare_return(void)
{
    8000241e:	1141                	addi	sp,sp,-16
    80002420:	e406                	sd	ra,8(sp)
    80002422:	e022                	sd	s0,0(sp)
    80002424:	0800                	addi	s0,sp,16
  struct proc *p = myproc();
    80002426:	cb4ff0ef          	jal	800018da <myproc>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000242a:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() & ~SSTATUS_SIE);
    8000242e:	9bf5                	andi	a5,a5,-3
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002430:	10079073          	csrw	sstatus,a5
  // kerneltrap() to usertrap(). because a trap from kernel
  // code to usertrap would be a disaster, turn off interrupts.
  intr_off();

  // send syscalls, interrupts, and exceptions to uservec in trampoline.S
  uint64 trampoline_uservec = TRAMPOLINE + (uservec - trampoline);
    80002434:	04000737          	lui	a4,0x4000
    80002438:	177d                	addi	a4,a4,-1 # 3ffffff <_entry-0x7c000001>
    8000243a:	0732                	slli	a4,a4,0xc
    8000243c:	00004797          	auipc	a5,0x4
    80002440:	bc478793          	addi	a5,a5,-1084 # 80006000 <_trampoline>
    80002444:	00004697          	auipc	a3,0x4
    80002448:	bbc68693          	addi	a3,a3,-1092 # 80006000 <_trampoline>
    8000244c:	8f95                	sub	a5,a5,a3
    8000244e:	97ba                	add	a5,a5,a4
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002450:	10579073          	csrw	stvec,a5
  w_stvec(trampoline_uservec);

  // set up trapframe values that uservec will need when
  // the process next traps into the kernel.
  p->trapframe->kernel_satp = r_satp();         // kernel page table
    80002454:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, satp" : "=r" (x) );
    80002456:	18002773          	csrr	a4,satp
    8000245a:	e398                	sd	a4,0(a5)
  p->trapframe->kernel_sp = p->kstack + PGSIZE; // process's kernel stack
    8000245c:	6d38                	ld	a4,88(a0)
    8000245e:	613c                	ld	a5,64(a0)
    80002460:	6685                	lui	a3,0x1
    80002462:	97b6                	add	a5,a5,a3
    80002464:	e71c                	sd	a5,8(a4)
  p->trapframe->kernel_trap = (uint64)usertrap;
    80002466:	6d3c                	ld	a5,88(a0)
    80002468:	00000717          	auipc	a4,0x0
    8000246c:	0f870713          	addi	a4,a4,248 # 80002560 <usertrap>
    80002470:	eb98                	sd	a4,16(a5)
  p->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
    80002472:	6d3c                	ld	a5,88(a0)
  asm volatile("mv %0, tp" : "=r" (x) );
    80002474:	8712                	mv	a4,tp
    80002476:	f398                	sd	a4,32(a5)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002478:	100027f3          	csrr	a5,sstatus
  // set up the registers that trampoline.S's sret will use
  // to get to user space.
  
  // set S Previous Privilege mode to User.
  unsigned long x = r_sstatus();
  x &= ~SSTATUS_SPP; // clear SPP to 0 for user mode
    8000247c:	eff7f793          	andi	a5,a5,-257
  x |= SSTATUS_SPIE; // enable interrupts in user mode
    80002480:	0207e793          	ori	a5,a5,32
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002484:	10079073          	csrw	sstatus,a5
  w_sstatus(x);

  // set S Exception Program Counter to the saved user pc.
  w_sepc(p->trapframe->epc);
    80002488:	6d3c                	ld	a5,88(a0)
  asm volatile("csrw sepc, %0" : : "r" (x));
    8000248a:	6f9c                	ld	a5,24(a5)
    8000248c:	14179073          	csrw	sepc,a5
}
    80002490:	60a2                	ld	ra,8(sp)
    80002492:	6402                	ld	s0,0(sp)
    80002494:	0141                	addi	sp,sp,16
    80002496:	8082                	ret

0000000080002498 <clockintr>:
  w_sstatus(sstatus);
}

void
clockintr()
{
    80002498:	1101                	addi	sp,sp,-32
    8000249a:	ec06                	sd	ra,24(sp)
    8000249c:	e822                	sd	s0,16(sp)
    8000249e:	1000                	addi	s0,sp,32
  if(cpuid() == 0){
    800024a0:	c0eff0ef          	jal	800018ae <cpuid>
    800024a4:	cd11                	beqz	a0,800024c0 <clockintr+0x28>
  asm volatile("csrr %0, time" : "=r" (x) );
    800024a6:	c01027f3          	rdtime	a5
  }

  // ask for the next timer interrupt. this also clears
  // the interrupt request. 1000000 is about a tenth
  // of a second.
  w_stimecmp(r_time() + 1000000);
    800024aa:	000f4737          	lui	a4,0xf4
    800024ae:	24070713          	addi	a4,a4,576 # f4240 <_entry-0x7ff0bdc0>
    800024b2:	97ba                	add	a5,a5,a4
  asm volatile("csrw 0x14d, %0" : : "r" (x));
    800024b4:	14d79073          	csrw	stimecmp,a5
}
    800024b8:	60e2                	ld	ra,24(sp)
    800024ba:	6442                	ld	s0,16(sp)
    800024bc:	6105                	addi	sp,sp,32
    800024be:	8082                	ret
    800024c0:	e426                	sd	s1,8(sp)
    acquire(&tickslock);
    800024c2:	00016497          	auipc	s1,0x16
    800024c6:	d5648493          	addi	s1,s1,-682 # 80018218 <tickslock>
    800024ca:	8526                	mv	a0,s1
    800024cc:	f02fe0ef          	jal	80000bce <acquire>
    ticks++;
    800024d0:	00008517          	auipc	a0,0x8
    800024d4:	e1850513          	addi	a0,a0,-488 # 8000a2e8 <ticks>
    800024d8:	411c                	lw	a5,0(a0)
    800024da:	2785                	addiw	a5,a5,1
    800024dc:	c11c                	sw	a5,0(a0)
    wakeup(&ticks);
    800024de:	a53ff0ef          	jal	80001f30 <wakeup>
    release(&tickslock);
    800024e2:	8526                	mv	a0,s1
    800024e4:	f82fe0ef          	jal	80000c66 <release>
    800024e8:	64a2                	ld	s1,8(sp)
    800024ea:	bf75                	j	800024a6 <clockintr+0xe>

00000000800024ec <devintr>:
// returns 2 if timer interrupt,
// 1 if other device,
// 0 if not recognized.
int
devintr()
{
    800024ec:	1101                	addi	sp,sp,-32
    800024ee:	ec06                	sd	ra,24(sp)
    800024f0:	e822                	sd	s0,16(sp)
    800024f2:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, scause" : "=r" (x) );
    800024f4:	14202773          	csrr	a4,scause
  uint64 scause = r_scause();

  if(scause == 0x8000000000000009L){
    800024f8:	57fd                	li	a5,-1
    800024fa:	17fe                	slli	a5,a5,0x3f
    800024fc:	07a5                	addi	a5,a5,9
    800024fe:	00f70c63          	beq	a4,a5,80002516 <devintr+0x2a>
    // now allowed to interrupt again.
    if(irq)
      plic_complete(irq);

    return 1;
  } else if(scause == 0x8000000000000005L){
    80002502:	57fd                	li	a5,-1
    80002504:	17fe                	slli	a5,a5,0x3f
    80002506:	0795                	addi	a5,a5,5
    // timer interrupt.
    clockintr();
    return 2;
  } else {
    return 0;
    80002508:	4501                	li	a0,0
  } else if(scause == 0x8000000000000005L){
    8000250a:	04f70763          	beq	a4,a5,80002558 <devintr+0x6c>
  }
}
    8000250e:	60e2                	ld	ra,24(sp)
    80002510:	6442                	ld	s0,16(sp)
    80002512:	6105                	addi	sp,sp,32
    80002514:	8082                	ret
    80002516:	e426                	sd	s1,8(sp)
    int irq = plic_claim();
    80002518:	765020ef          	jal	8000547c <plic_claim>
    8000251c:	84aa                	mv	s1,a0
    if(irq == UART0_IRQ){
    8000251e:	47a9                	li	a5,10
    80002520:	00f50963          	beq	a0,a5,80002532 <devintr+0x46>
    } else if(irq == VIRTIO0_IRQ){
    80002524:	4785                	li	a5,1
    80002526:	00f50963          	beq	a0,a5,80002538 <devintr+0x4c>
    return 1;
    8000252a:	4505                	li	a0,1
    } else if(irq){
    8000252c:	e889                	bnez	s1,8000253e <devintr+0x52>
    8000252e:	64a2                	ld	s1,8(sp)
    80002530:	bff9                	j	8000250e <devintr+0x22>
      uartintr();
    80002532:	c7efe0ef          	jal	800009b0 <uartintr>
    if(irq)
    80002536:	a819                	j	8000254c <devintr+0x60>
      virtio_disk_intr();
    80002538:	6a6030ef          	jal	80005bde <virtio_disk_intr>
    if(irq)
    8000253c:	a801                	j	8000254c <devintr+0x60>
      printf("unexpected interrupt irq=%d\n", irq);
    8000253e:	85a6                	mv	a1,s1
    80002540:	00005517          	auipc	a0,0x5
    80002544:	d1050513          	addi	a0,a0,-752 # 80007250 <etext+0x250>
    80002548:	fb3fd0ef          	jal	800004fa <printf>
      plic_complete(irq);
    8000254c:	8526                	mv	a0,s1
    8000254e:	74f020ef          	jal	8000549c <plic_complete>
    return 1;
    80002552:	4505                	li	a0,1
    80002554:	64a2                	ld	s1,8(sp)
    80002556:	bf65                	j	8000250e <devintr+0x22>
    clockintr();
    80002558:	f41ff0ef          	jal	80002498 <clockintr>
    return 2;
    8000255c:	4509                	li	a0,2
    8000255e:	bf45                	j	8000250e <devintr+0x22>

0000000080002560 <usertrap>:
{
    80002560:	1101                	addi	sp,sp,-32
    80002562:	ec06                	sd	ra,24(sp)
    80002564:	e822                	sd	s0,16(sp)
    80002566:	e426                	sd	s1,8(sp)
    80002568:	e04a                	sd	s2,0(sp)
    8000256a:	1000                	addi	s0,sp,32
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000256c:	100027f3          	csrr	a5,sstatus
  if((r_sstatus() & SSTATUS_SPP) != 0)
    80002570:	1007f793          	andi	a5,a5,256
    80002574:	eba5                	bnez	a5,800025e4 <usertrap+0x84>
  asm volatile("csrw stvec, %0" : : "r" (x));
    80002576:	00003797          	auipc	a5,0x3
    8000257a:	e5a78793          	addi	a5,a5,-422 # 800053d0 <kernelvec>
    8000257e:	10579073          	csrw	stvec,a5
  struct proc *p = myproc();
    80002582:	b58ff0ef          	jal	800018da <myproc>
    80002586:	84aa                	mv	s1,a0
  p->trapframe->epc = r_sepc();
    80002588:	6d3c                	ld	a5,88(a0)
  asm volatile("csrr %0, sepc" : "=r" (x) );
    8000258a:	14102773          	csrr	a4,sepc
    8000258e:	ef98                	sd	a4,24(a5)
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002590:	14202773          	csrr	a4,scause
  if(r_scause() == 8){
    80002594:	47a1                	li	a5,8
    80002596:	04f70d63          	beq	a4,a5,800025f0 <usertrap+0x90>
  } else if((which_dev = devintr()) != 0){
    8000259a:	f53ff0ef          	jal	800024ec <devintr>
    8000259e:	892a                	mv	s2,a0
    800025a0:	e945                	bnez	a0,80002650 <usertrap+0xf0>
    800025a2:	14202773          	csrr	a4,scause
  } else if((r_scause() == 15 || r_scause() == 13) &&
    800025a6:	47bd                	li	a5,15
    800025a8:	08f70863          	beq	a4,a5,80002638 <usertrap+0xd8>
    800025ac:	14202773          	csrr	a4,scause
    800025b0:	47b5                	li	a5,13
    800025b2:	08f70363          	beq	a4,a5,80002638 <usertrap+0xd8>
    800025b6:	142025f3          	csrr	a1,scause
    printf("usertrap(): unexpected scause 0x%lx pid=%d\n", r_scause(), p->pid);
    800025ba:	5890                	lw	a2,48(s1)
    800025bc:	00005517          	auipc	a0,0x5
    800025c0:	cd450513          	addi	a0,a0,-812 # 80007290 <etext+0x290>
    800025c4:	f37fd0ef          	jal	800004fa <printf>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    800025c8:	141025f3          	csrr	a1,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    800025cc:	14302673          	csrr	a2,stval
    printf("            sepc=0x%lx stval=0x%lx\n", r_sepc(), r_stval());
    800025d0:	00005517          	auipc	a0,0x5
    800025d4:	cf050513          	addi	a0,a0,-784 # 800072c0 <etext+0x2c0>
    800025d8:	f23fd0ef          	jal	800004fa <printf>
    setkilled(p);
    800025dc:	8526                	mv	a0,s1
    800025de:	b1bff0ef          	jal	800020f8 <setkilled>
    800025e2:	a035                	j	8000260e <usertrap+0xae>
    panic("usertrap: not from user mode");
    800025e4:	00005517          	auipc	a0,0x5
    800025e8:	c8c50513          	addi	a0,a0,-884 # 80007270 <etext+0x270>
    800025ec:	9f4fe0ef          	jal	800007e0 <panic>
    if(killed(p))
    800025f0:	b2dff0ef          	jal	8000211c <killed>
    800025f4:	ed15                	bnez	a0,80002630 <usertrap+0xd0>
    p->trapframe->epc += 4;
    800025f6:	6cb8                	ld	a4,88(s1)
    800025f8:	6f1c                	ld	a5,24(a4)
    800025fa:	0791                	addi	a5,a5,4
    800025fc:	ef1c                	sd	a5,24(a4)
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    800025fe:	100027f3          	csrr	a5,sstatus
  w_sstatus(r_sstatus() | SSTATUS_SIE);
    80002602:	0027e793          	ori	a5,a5,2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    80002606:	10079073          	csrw	sstatus,a5
    syscall();
    8000260a:	246000ef          	jal	80002850 <syscall>
  if(killed(p))
    8000260e:	8526                	mv	a0,s1
    80002610:	b0dff0ef          	jal	8000211c <killed>
    80002614:	e139                	bnez	a0,8000265a <usertrap+0xfa>
  prepare_return();
    80002616:	e09ff0ef          	jal	8000241e <prepare_return>
  uint64 satp = MAKE_SATP(p->pagetable);
    8000261a:	68a8                	ld	a0,80(s1)
    8000261c:	8131                	srli	a0,a0,0xc
    8000261e:	57fd                	li	a5,-1
    80002620:	17fe                	slli	a5,a5,0x3f
    80002622:	8d5d                	or	a0,a0,a5
}
    80002624:	60e2                	ld	ra,24(sp)
    80002626:	6442                	ld	s0,16(sp)
    80002628:	64a2                	ld	s1,8(sp)
    8000262a:	6902                	ld	s2,0(sp)
    8000262c:	6105                	addi	sp,sp,32
    8000262e:	8082                	ret
      kexit(-1);
    80002630:	557d                	li	a0,-1
    80002632:	9bfff0ef          	jal	80001ff0 <kexit>
    80002636:	b7c1                	j	800025f6 <usertrap+0x96>
  asm volatile("csrr %0, stval" : "=r" (x) );
    80002638:	143025f3          	csrr	a1,stval
  asm volatile("csrr %0, scause" : "=r" (x) );
    8000263c:	14202673          	csrr	a2,scause
            vmfault(p->pagetable, r_stval(), (r_scause() == 13)? 1 : 0) != 0) {
    80002640:	164d                	addi	a2,a2,-13 # ff3 <_entry-0x7ffff00d>
    80002642:	00163613          	seqz	a2,a2
    80002646:	68a8                	ld	a0,80(s1)
    80002648:	f25fe0ef          	jal	8000156c <vmfault>
  } else if((r_scause() == 15 || r_scause() == 13) &&
    8000264c:	f169                	bnez	a0,8000260e <usertrap+0xae>
    8000264e:	b7a5                	j	800025b6 <usertrap+0x56>
  if(killed(p))
    80002650:	8526                	mv	a0,s1
    80002652:	acbff0ef          	jal	8000211c <killed>
    80002656:	c511                	beqz	a0,80002662 <usertrap+0x102>
    80002658:	a011                	j	8000265c <usertrap+0xfc>
    8000265a:	4901                	li	s2,0
    kexit(-1);
    8000265c:	557d                	li	a0,-1
    8000265e:	993ff0ef          	jal	80001ff0 <kexit>
  if(which_dev == 2)
    80002662:	4789                	li	a5,2
    80002664:	faf919e3          	bne	s2,a5,80002616 <usertrap+0xb6>
    yield();
    80002668:	851ff0ef          	jal	80001eb8 <yield>
    8000266c:	b76d                	j	80002616 <usertrap+0xb6>

000000008000266e <kerneltrap>:
{
    8000266e:	7179                	addi	sp,sp,-48
    80002670:	f406                	sd	ra,40(sp)
    80002672:	f022                	sd	s0,32(sp)
    80002674:	ec26                	sd	s1,24(sp)
    80002676:	e84a                	sd	s2,16(sp)
    80002678:	e44e                	sd	s3,8(sp)
    8000267a:	1800                	addi	s0,sp,48
  asm volatile("csrr %0, sepc" : "=r" (x) );
    8000267c:	14102973          	csrr	s2,sepc
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    80002680:	100024f3          	csrr	s1,sstatus
  asm volatile("csrr %0, scause" : "=r" (x) );
    80002684:	142029f3          	csrr	s3,scause
  if((sstatus & SSTATUS_SPP) == 0)
    80002688:	1004f793          	andi	a5,s1,256
    8000268c:	c795                	beqz	a5,800026b8 <kerneltrap+0x4a>
  asm volatile("csrr %0, sstatus" : "=r" (x) );
    8000268e:	100027f3          	csrr	a5,sstatus
  return (x & SSTATUS_SIE) != 0;
    80002692:	8b89                	andi	a5,a5,2
  if(intr_get() != 0)
    80002694:	eb85                	bnez	a5,800026c4 <kerneltrap+0x56>
  if((which_dev = devintr()) == 0){
    80002696:	e57ff0ef          	jal	800024ec <devintr>
    8000269a:	c91d                	beqz	a0,800026d0 <kerneltrap+0x62>
  if(which_dev == 2 && myproc() != 0)
    8000269c:	4789                	li	a5,2
    8000269e:	04f50a63          	beq	a0,a5,800026f2 <kerneltrap+0x84>
  asm volatile("csrw sepc, %0" : : "r" (x));
    800026a2:	14191073          	csrw	sepc,s2
  asm volatile("csrw sstatus, %0" : : "r" (x));
    800026a6:	10049073          	csrw	sstatus,s1
}
    800026aa:	70a2                	ld	ra,40(sp)
    800026ac:	7402                	ld	s0,32(sp)
    800026ae:	64e2                	ld	s1,24(sp)
    800026b0:	6942                	ld	s2,16(sp)
    800026b2:	69a2                	ld	s3,8(sp)
    800026b4:	6145                	addi	sp,sp,48
    800026b6:	8082                	ret
    panic("kerneltrap: not from supervisor mode");
    800026b8:	00005517          	auipc	a0,0x5
    800026bc:	c3050513          	addi	a0,a0,-976 # 800072e8 <etext+0x2e8>
    800026c0:	920fe0ef          	jal	800007e0 <panic>
    panic("kerneltrap: interrupts enabled");
    800026c4:	00005517          	auipc	a0,0x5
    800026c8:	c4c50513          	addi	a0,a0,-948 # 80007310 <etext+0x310>
    800026cc:	914fe0ef          	jal	800007e0 <panic>
  asm volatile("csrr %0, sepc" : "=r" (x) );
    800026d0:	14102673          	csrr	a2,sepc
  asm volatile("csrr %0, stval" : "=r" (x) );
    800026d4:	143026f3          	csrr	a3,stval
    printf("scause=0x%lx sepc=0x%lx stval=0x%lx\n", scause, r_sepc(), r_stval());
    800026d8:	85ce                	mv	a1,s3
    800026da:	00005517          	auipc	a0,0x5
    800026de:	c5650513          	addi	a0,a0,-938 # 80007330 <etext+0x330>
    800026e2:	e19fd0ef          	jal	800004fa <printf>
    panic("kerneltrap");
    800026e6:	00005517          	auipc	a0,0x5
    800026ea:	c7250513          	addi	a0,a0,-910 # 80007358 <etext+0x358>
    800026ee:	8f2fe0ef          	jal	800007e0 <panic>
  if(which_dev == 2 && myproc() != 0)
    800026f2:	9e8ff0ef          	jal	800018da <myproc>
    800026f6:	d555                	beqz	a0,800026a2 <kerneltrap+0x34>
    yield();
    800026f8:	fc0ff0ef          	jal	80001eb8 <yield>
    800026fc:	b75d                	j	800026a2 <kerneltrap+0x34>

00000000800026fe <argraw>:
  return strlen(buf);
}

static uint64
argraw(int n)
{
    800026fe:	1101                	addi	sp,sp,-32
    80002700:	ec06                	sd	ra,24(sp)
    80002702:	e822                	sd	s0,16(sp)
    80002704:	e426                	sd	s1,8(sp)
    80002706:	1000                	addi	s0,sp,32
    80002708:	84aa                	mv	s1,a0
  struct proc *p = myproc();
    8000270a:	9d0ff0ef          	jal	800018da <myproc>
  switch (n) {
    8000270e:	4795                	li	a5,5
    80002710:	0497e163          	bltu	a5,s1,80002752 <argraw+0x54>
    80002714:	048a                	slli	s1,s1,0x2
    80002716:	00005717          	auipc	a4,0x5
    8000271a:	04a70713          	addi	a4,a4,74 # 80007760 <states.0+0x30>
    8000271e:	94ba                	add	s1,s1,a4
    80002720:	409c                	lw	a5,0(s1)
    80002722:	97ba                	add	a5,a5,a4
    80002724:	8782                	jr	a5
  case 0:
    return p->trapframe->a0;
    80002726:	6d3c                	ld	a5,88(a0)
    80002728:	7ba8                	ld	a0,112(a5)
  case 5:
    return p->trapframe->a5;
  }
  panic("argraw");
  return -1;
}
    8000272a:	60e2                	ld	ra,24(sp)
    8000272c:	6442                	ld	s0,16(sp)
    8000272e:	64a2                	ld	s1,8(sp)
    80002730:	6105                	addi	sp,sp,32
    80002732:	8082                	ret
    return p->trapframe->a1;
    80002734:	6d3c                	ld	a5,88(a0)
    80002736:	7fa8                	ld	a0,120(a5)
    80002738:	bfcd                	j	8000272a <argraw+0x2c>
    return p->trapframe->a2;
    8000273a:	6d3c                	ld	a5,88(a0)
    8000273c:	63c8                	ld	a0,128(a5)
    8000273e:	b7f5                	j	8000272a <argraw+0x2c>
    return p->trapframe->a3;
    80002740:	6d3c                	ld	a5,88(a0)
    80002742:	67c8                	ld	a0,136(a5)
    80002744:	b7dd                	j	8000272a <argraw+0x2c>
    return p->trapframe->a4;
    80002746:	6d3c                	ld	a5,88(a0)
    80002748:	6bc8                	ld	a0,144(a5)
    8000274a:	b7c5                	j	8000272a <argraw+0x2c>
    return p->trapframe->a5;
    8000274c:	6d3c                	ld	a5,88(a0)
    8000274e:	6fc8                	ld	a0,152(a5)
    80002750:	bfe9                	j	8000272a <argraw+0x2c>
  panic("argraw");
    80002752:	00005517          	auipc	a0,0x5
    80002756:	c1650513          	addi	a0,a0,-1002 # 80007368 <etext+0x368>
    8000275a:	886fe0ef          	jal	800007e0 <panic>

000000008000275e <fetchaddr>:
{
    8000275e:	1101                	addi	sp,sp,-32
    80002760:	ec06                	sd	ra,24(sp)
    80002762:	e822                	sd	s0,16(sp)
    80002764:	e426                	sd	s1,8(sp)
    80002766:	e04a                	sd	s2,0(sp)
    80002768:	1000                	addi	s0,sp,32
    8000276a:	84aa                	mv	s1,a0
    8000276c:	892e                	mv	s2,a1
  struct proc *p = myproc();
    8000276e:	96cff0ef          	jal	800018da <myproc>
  if(addr >= p->sz || addr+sizeof(uint64) > p->sz) // both tests needed, in case of overflow
    80002772:	653c                	ld	a5,72(a0)
    80002774:	02f4f663          	bgeu	s1,a5,800027a0 <fetchaddr+0x42>
    80002778:	00848713          	addi	a4,s1,8
    8000277c:	02e7e463          	bltu	a5,a4,800027a4 <fetchaddr+0x46>
  if(copyin(p->pagetable, (char *)ip, addr, sizeof(*ip)) != 0)
    80002780:	46a1                	li	a3,8
    80002782:	8626                	mv	a2,s1
    80002784:	85ca                	mv	a1,s2
    80002786:	6928                	ld	a0,80(a0)
    80002788:	f4bfe0ef          	jal	800016d2 <copyin>
    8000278c:	00a03533          	snez	a0,a0
    80002790:	40a00533          	neg	a0,a0
}
    80002794:	60e2                	ld	ra,24(sp)
    80002796:	6442                	ld	s0,16(sp)
    80002798:	64a2                	ld	s1,8(sp)
    8000279a:	6902                	ld	s2,0(sp)
    8000279c:	6105                	addi	sp,sp,32
    8000279e:	8082                	ret
    return -1;
    800027a0:	557d                	li	a0,-1
    800027a2:	bfcd                	j	80002794 <fetchaddr+0x36>
    800027a4:	557d                	li	a0,-1
    800027a6:	b7fd                	j	80002794 <fetchaddr+0x36>

00000000800027a8 <fetchstr>:
{
    800027a8:	7179                	addi	sp,sp,-48
    800027aa:	f406                	sd	ra,40(sp)
    800027ac:	f022                	sd	s0,32(sp)
    800027ae:	ec26                	sd	s1,24(sp)
    800027b0:	e84a                	sd	s2,16(sp)
    800027b2:	e44e                	sd	s3,8(sp)
    800027b4:	1800                	addi	s0,sp,48
    800027b6:	892a                	mv	s2,a0
    800027b8:	84ae                	mv	s1,a1
    800027ba:	89b2                	mv	s3,a2
  struct proc *p = myproc();
    800027bc:	91eff0ef          	jal	800018da <myproc>
  if(copyinstr(p->pagetable, buf, addr, max) < 0)
    800027c0:	86ce                	mv	a3,s3
    800027c2:	864a                	mv	a2,s2
    800027c4:	85a6                	mv	a1,s1
    800027c6:	6928                	ld	a0,80(a0)
    800027c8:	ccdfe0ef          	jal	80001494 <copyinstr>
    800027cc:	00054c63          	bltz	a0,800027e4 <fetchstr+0x3c>
  return strlen(buf);
    800027d0:	8526                	mv	a0,s1
    800027d2:	e40fe0ef          	jal	80000e12 <strlen>
}
    800027d6:	70a2                	ld	ra,40(sp)
    800027d8:	7402                	ld	s0,32(sp)
    800027da:	64e2                	ld	s1,24(sp)
    800027dc:	6942                	ld	s2,16(sp)
    800027de:	69a2                	ld	s3,8(sp)
    800027e0:	6145                	addi	sp,sp,48
    800027e2:	8082                	ret
    return -1;
    800027e4:	557d                	li	a0,-1
    800027e6:	bfc5                	j	800027d6 <fetchstr+0x2e>

00000000800027e8 <argint>:

// Fetch the nth 32-bit system call argument.
void
argint(int n, int *ip)
{
    800027e8:	1101                	addi	sp,sp,-32
    800027ea:	ec06                	sd	ra,24(sp)
    800027ec:	e822                	sd	s0,16(sp)
    800027ee:	e426                	sd	s1,8(sp)
    800027f0:	1000                	addi	s0,sp,32
    800027f2:	84ae                	mv	s1,a1
  *ip = argraw(n);
    800027f4:	f0bff0ef          	jal	800026fe <argraw>
    800027f8:	c088                	sw	a0,0(s1)
}
    800027fa:	60e2                	ld	ra,24(sp)
    800027fc:	6442                	ld	s0,16(sp)
    800027fe:	64a2                	ld	s1,8(sp)
    80002800:	6105                	addi	sp,sp,32
    80002802:	8082                	ret

0000000080002804 <argaddr>:
// Retrieve an argument as a pointer.
// Doesn't check for legality, since
// copyin/copyout will do that.
void
argaddr(int n, uint64 *ip)
{
    80002804:	1101                	addi	sp,sp,-32
    80002806:	ec06                	sd	ra,24(sp)
    80002808:	e822                	sd	s0,16(sp)
    8000280a:	e426                	sd	s1,8(sp)
    8000280c:	1000                	addi	s0,sp,32
    8000280e:	84ae                	mv	s1,a1
  *ip = argraw(n);
    80002810:	eefff0ef          	jal	800026fe <argraw>
    80002814:	e088                	sd	a0,0(s1)
}
    80002816:	60e2                	ld	ra,24(sp)
    80002818:	6442                	ld	s0,16(sp)
    8000281a:	64a2                	ld	s1,8(sp)
    8000281c:	6105                	addi	sp,sp,32
    8000281e:	8082                	ret

0000000080002820 <argstr>:
// Fetch the nth word-sized system call argument as a null-terminated string.
// Copies into buf, at most max.
// Returns string length if OK (including nul), -1 if error.
int
argstr(int n, char *buf, int max)
{
    80002820:	7179                	addi	sp,sp,-48
    80002822:	f406                	sd	ra,40(sp)
    80002824:	f022                	sd	s0,32(sp)
    80002826:	ec26                	sd	s1,24(sp)
    80002828:	e84a                	sd	s2,16(sp)
    8000282a:	1800                	addi	s0,sp,48
    8000282c:	84ae                	mv	s1,a1
    8000282e:	8932                	mv	s2,a2
  uint64 addr;
  argaddr(n, &addr);
    80002830:	fd840593          	addi	a1,s0,-40
    80002834:	fd1ff0ef          	jal	80002804 <argaddr>
  return fetchstr(addr, buf, max);
    80002838:	864a                	mv	a2,s2
    8000283a:	85a6                	mv	a1,s1
    8000283c:	fd843503          	ld	a0,-40(s0)
    80002840:	f69ff0ef          	jal	800027a8 <fetchstr>
}
    80002844:	70a2                	ld	ra,40(sp)
    80002846:	7402                	ld	s0,32(sp)
    80002848:	64e2                	ld	s1,24(sp)
    8000284a:	6942                	ld	s2,16(sp)
    8000284c:	6145                	addi	sp,sp,48
    8000284e:	8082                	ret

0000000080002850 <syscall>:
[SYS_fmunlock] sys_fmunlock,
};

void
syscall(void)
{
    80002850:	1101                	addi	sp,sp,-32
    80002852:	ec06                	sd	ra,24(sp)
    80002854:	e822                	sd	s0,16(sp)
    80002856:	e426                	sd	s1,8(sp)
    80002858:	e04a                	sd	s2,0(sp)
    8000285a:	1000                	addi	s0,sp,32
  int num;
  struct proc *p = myproc();
    8000285c:	87eff0ef          	jal	800018da <myproc>
    80002860:	84aa                	mv	s1,a0

  num = p->trapframe->a7;
    80002862:	05853903          	ld	s2,88(a0)
    80002866:	0a893783          	ld	a5,168(s2)
    8000286a:	0007869b          	sext.w	a3,a5
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    8000286e:	37fd                	addiw	a5,a5,-1
    80002870:	4759                	li	a4,22
    80002872:	00f76f63          	bltu	a4,a5,80002890 <syscall+0x40>
    80002876:	00369713          	slli	a4,a3,0x3
    8000287a:	00005797          	auipc	a5,0x5
    8000287e:	efe78793          	addi	a5,a5,-258 # 80007778 <syscalls>
    80002882:	97ba                	add	a5,a5,a4
    80002884:	639c                	ld	a5,0(a5)
    80002886:	c789                	beqz	a5,80002890 <syscall+0x40>
    // Use num to lookup the system call function for num, call it,
    // and store its return value in p->trapframe->a0
    p->trapframe->a0 = syscalls[num]();
    80002888:	9782                	jalr	a5
    8000288a:	06a93823          	sd	a0,112(s2)
    8000288e:	a829                	j	800028a8 <syscall+0x58>
  } else {
    printf("%d %s: unknown sys call %d\n",
    80002890:	15848613          	addi	a2,s1,344
    80002894:	588c                	lw	a1,48(s1)
    80002896:	00005517          	auipc	a0,0x5
    8000289a:	ada50513          	addi	a0,a0,-1318 # 80007370 <etext+0x370>
    8000289e:	c5dfd0ef          	jal	800004fa <printf>
            p->pid, p->name, num);
    p->trapframe->a0 = -1;
    800028a2:	6cbc                	ld	a5,88(s1)
    800028a4:	577d                	li	a4,-1
    800028a6:	fbb8                	sd	a4,112(a5)
  }
}
    800028a8:	60e2                	ld	ra,24(sp)
    800028aa:	6442                	ld	s0,16(sp)
    800028ac:	64a2                	ld	s1,8(sp)
    800028ae:	6902                	ld	s2,0(sp)
    800028b0:	6105                	addi	sp,sp,32
    800028b2:	8082                	ret

00000000800028b4 <sys_exit>:
#include "proc.h"
#include "vm.h"

uint64
sys_exit(void)
{
    800028b4:	1101                	addi	sp,sp,-32
    800028b6:	ec06                	sd	ra,24(sp)
    800028b8:	e822                	sd	s0,16(sp)
    800028ba:	1000                	addi	s0,sp,32
  int n;
  argint(0, &n);
    800028bc:	fec40593          	addi	a1,s0,-20
    800028c0:	4501                	li	a0,0
    800028c2:	f27ff0ef          	jal	800027e8 <argint>
  kexit(n);
    800028c6:	fec42503          	lw	a0,-20(s0)
    800028ca:	f26ff0ef          	jal	80001ff0 <kexit>
  return 0;  // not reached
}
    800028ce:	4501                	li	a0,0
    800028d0:	60e2                	ld	ra,24(sp)
    800028d2:	6442                	ld	s0,16(sp)
    800028d4:	6105                	addi	sp,sp,32
    800028d6:	8082                	ret

00000000800028d8 <sys_getpid>:

uint64
sys_getpid(void)
{
    800028d8:	1141                	addi	sp,sp,-16
    800028da:	e406                	sd	ra,8(sp)
    800028dc:	e022                	sd	s0,0(sp)
    800028de:	0800                	addi	s0,sp,16
  return myproc()->pid;
    800028e0:	ffbfe0ef          	jal	800018da <myproc>
}
    800028e4:	5908                	lw	a0,48(a0)
    800028e6:	60a2                	ld	ra,8(sp)
    800028e8:	6402                	ld	s0,0(sp)
    800028ea:	0141                	addi	sp,sp,16
    800028ec:	8082                	ret

00000000800028ee <sys_fork>:

uint64
sys_fork(void)
{
    800028ee:	1141                	addi	sp,sp,-16
    800028f0:	e406                	sd	ra,8(sp)
    800028f2:	e022                	sd	s0,0(sp)
    800028f4:	0800                	addi	s0,sp,16
  return kfork();
    800028f6:	b48ff0ef          	jal	80001c3e <kfork>
}
    800028fa:	60a2                	ld	ra,8(sp)
    800028fc:	6402                	ld	s0,0(sp)
    800028fe:	0141                	addi	sp,sp,16
    80002900:	8082                	ret

0000000080002902 <sys_wait>:

uint64
sys_wait(void)
{
    80002902:	1101                	addi	sp,sp,-32
    80002904:	ec06                	sd	ra,24(sp)
    80002906:	e822                	sd	s0,16(sp)
    80002908:	1000                	addi	s0,sp,32
  uint64 p;
  argaddr(0, &p);
    8000290a:	fe840593          	addi	a1,s0,-24
    8000290e:	4501                	li	a0,0
    80002910:	ef5ff0ef          	jal	80002804 <argaddr>
  return kwait(p);
    80002914:	fe843503          	ld	a0,-24(s0)
    80002918:	82fff0ef          	jal	80002146 <kwait>
}
    8000291c:	60e2                	ld	ra,24(sp)
    8000291e:	6442                	ld	s0,16(sp)
    80002920:	6105                	addi	sp,sp,32
    80002922:	8082                	ret

0000000080002924 <sys_sbrk>:

uint64
sys_sbrk(void)
{
    80002924:	7179                	addi	sp,sp,-48
    80002926:	f406                	sd	ra,40(sp)
    80002928:	f022                	sd	s0,32(sp)
    8000292a:	ec26                	sd	s1,24(sp)
    8000292c:	1800                	addi	s0,sp,48
  uint64 addr;
  int t;
  int n;

  argint(0, &n);
    8000292e:	fd840593          	addi	a1,s0,-40
    80002932:	4501                	li	a0,0
    80002934:	eb5ff0ef          	jal	800027e8 <argint>
  argint(1, &t);
    80002938:	fdc40593          	addi	a1,s0,-36
    8000293c:	4505                	li	a0,1
    8000293e:	eabff0ef          	jal	800027e8 <argint>
  addr = myproc()->sz;
    80002942:	f99fe0ef          	jal	800018da <myproc>
    80002946:	6524                	ld	s1,72(a0)

  if(t == SBRK_EAGER || n < 0) {
    80002948:	fdc42703          	lw	a4,-36(s0)
    8000294c:	4785                	li	a5,1
    8000294e:	02f70763          	beq	a4,a5,8000297c <sys_sbrk+0x58>
    80002952:	fd842783          	lw	a5,-40(s0)
    80002956:	0207c363          	bltz	a5,8000297c <sys_sbrk+0x58>
    }
  } else {
    // Lazily allocate memory for this process: increase its memory
    // size but don't allocate memory. If the processes uses the
    // memory, vmfault() will allocate it.
    if(addr + n < addr)
    8000295a:	97a6                	add	a5,a5,s1
    8000295c:	0297ee63          	bltu	a5,s1,80002998 <sys_sbrk+0x74>
      return -1;
    if(addr + n > TRAPFRAME)
    80002960:	02000737          	lui	a4,0x2000
    80002964:	177d                	addi	a4,a4,-1 # 1ffffff <_entry-0x7e000001>
    80002966:	0736                	slli	a4,a4,0xd
    80002968:	02f76a63          	bltu	a4,a5,8000299c <sys_sbrk+0x78>
      return -1;
    myproc()->sz += n;
    8000296c:	f6ffe0ef          	jal	800018da <myproc>
    80002970:	fd842703          	lw	a4,-40(s0)
    80002974:	653c                	ld	a5,72(a0)
    80002976:	97ba                	add	a5,a5,a4
    80002978:	e53c                	sd	a5,72(a0)
    8000297a:	a039                	j	80002988 <sys_sbrk+0x64>
    if(growproc(n) < 0) {
    8000297c:	fd842503          	lw	a0,-40(s0)
    80002980:	a5cff0ef          	jal	80001bdc <growproc>
    80002984:	00054863          	bltz	a0,80002994 <sys_sbrk+0x70>
  }
  return addr;
}
    80002988:	8526                	mv	a0,s1
    8000298a:	70a2                	ld	ra,40(sp)
    8000298c:	7402                	ld	s0,32(sp)
    8000298e:	64e2                	ld	s1,24(sp)
    80002990:	6145                	addi	sp,sp,48
    80002992:	8082                	ret
      return -1;
    80002994:	54fd                	li	s1,-1
    80002996:	bfcd                	j	80002988 <sys_sbrk+0x64>
      return -1;
    80002998:	54fd                	li	s1,-1
    8000299a:	b7fd                	j	80002988 <sys_sbrk+0x64>
      return -1;
    8000299c:	54fd                	li	s1,-1
    8000299e:	b7ed                	j	80002988 <sys_sbrk+0x64>

00000000800029a0 <sys_pause>:

uint64
sys_pause(void)
{
    800029a0:	7139                	addi	sp,sp,-64
    800029a2:	fc06                	sd	ra,56(sp)
    800029a4:	f822                	sd	s0,48(sp)
    800029a6:	f04a                	sd	s2,32(sp)
    800029a8:	0080                	addi	s0,sp,64
  int n;
  uint ticks0;

  argint(0, &n);
    800029aa:	fcc40593          	addi	a1,s0,-52
    800029ae:	4501                	li	a0,0
    800029b0:	e39ff0ef          	jal	800027e8 <argint>
  if(n < 0)
    800029b4:	fcc42783          	lw	a5,-52(s0)
    800029b8:	0607c763          	bltz	a5,80002a26 <sys_pause+0x86>
    n = 0;
  acquire(&tickslock);
    800029bc:	00016517          	auipc	a0,0x16
    800029c0:	85c50513          	addi	a0,a0,-1956 # 80018218 <tickslock>
    800029c4:	a0afe0ef          	jal	80000bce <acquire>
  ticks0 = ticks;
    800029c8:	00008917          	auipc	s2,0x8
    800029cc:	92092903          	lw	s2,-1760(s2) # 8000a2e8 <ticks>
  while(ticks - ticks0 < n){
    800029d0:	fcc42783          	lw	a5,-52(s0)
    800029d4:	cf8d                	beqz	a5,80002a0e <sys_pause+0x6e>
    800029d6:	f426                	sd	s1,40(sp)
    800029d8:	ec4e                	sd	s3,24(sp)
    if(killed(myproc())){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
    800029da:	00016997          	auipc	s3,0x16
    800029de:	83e98993          	addi	s3,s3,-1986 # 80018218 <tickslock>
    800029e2:	00008497          	auipc	s1,0x8
    800029e6:	90648493          	addi	s1,s1,-1786 # 8000a2e8 <ticks>
    if(killed(myproc())){
    800029ea:	ef1fe0ef          	jal	800018da <myproc>
    800029ee:	f2eff0ef          	jal	8000211c <killed>
    800029f2:	ed0d                	bnez	a0,80002a2c <sys_pause+0x8c>
    sleep(&ticks, &tickslock);
    800029f4:	85ce                	mv	a1,s3
    800029f6:	8526                	mv	a0,s1
    800029f8:	cecff0ef          	jal	80001ee4 <sleep>
  while(ticks - ticks0 < n){
    800029fc:	409c                	lw	a5,0(s1)
    800029fe:	412787bb          	subw	a5,a5,s2
    80002a02:	fcc42703          	lw	a4,-52(s0)
    80002a06:	fee7e2e3          	bltu	a5,a4,800029ea <sys_pause+0x4a>
    80002a0a:	74a2                	ld	s1,40(sp)
    80002a0c:	69e2                	ld	s3,24(sp)
  }
  release(&tickslock);
    80002a0e:	00016517          	auipc	a0,0x16
    80002a12:	80a50513          	addi	a0,a0,-2038 # 80018218 <tickslock>
    80002a16:	a50fe0ef          	jal	80000c66 <release>
  return 0;
    80002a1a:	4501                	li	a0,0
}
    80002a1c:	70e2                	ld	ra,56(sp)
    80002a1e:	7442                	ld	s0,48(sp)
    80002a20:	7902                	ld	s2,32(sp)
    80002a22:	6121                	addi	sp,sp,64
    80002a24:	8082                	ret
    n = 0;
    80002a26:	fc042623          	sw	zero,-52(s0)
    80002a2a:	bf49                	j	800029bc <sys_pause+0x1c>
      release(&tickslock);
    80002a2c:	00015517          	auipc	a0,0x15
    80002a30:	7ec50513          	addi	a0,a0,2028 # 80018218 <tickslock>
    80002a34:	a32fe0ef          	jal	80000c66 <release>
      return -1;
    80002a38:	557d                	li	a0,-1
    80002a3a:	74a2                	ld	s1,40(sp)
    80002a3c:	69e2                	ld	s3,24(sp)
    80002a3e:	bff9                	j	80002a1c <sys_pause+0x7c>

0000000080002a40 <sys_kill>:

uint64
sys_kill(void)
{
    80002a40:	1101                	addi	sp,sp,-32
    80002a42:	ec06                	sd	ra,24(sp)
    80002a44:	e822                	sd	s0,16(sp)
    80002a46:	1000                	addi	s0,sp,32
  int pid;

  argint(0, &pid);
    80002a48:	fec40593          	addi	a1,s0,-20
    80002a4c:	4501                	li	a0,0
    80002a4e:	d9bff0ef          	jal	800027e8 <argint>
  return kkill(pid);
    80002a52:	fec42503          	lw	a0,-20(s0)
    80002a56:	e3cff0ef          	jal	80002092 <kkill>
}
    80002a5a:	60e2                	ld	ra,24(sp)
    80002a5c:	6442                	ld	s0,16(sp)
    80002a5e:	6105                	addi	sp,sp,32
    80002a60:	8082                	ret

0000000080002a62 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
uint64
sys_uptime(void)
{
    80002a62:	1101                	addi	sp,sp,-32
    80002a64:	ec06                	sd	ra,24(sp)
    80002a66:	e822                	sd	s0,16(sp)
    80002a68:	e426                	sd	s1,8(sp)
    80002a6a:	1000                	addi	s0,sp,32
  uint xticks;

  acquire(&tickslock);
    80002a6c:	00015517          	auipc	a0,0x15
    80002a70:	7ac50513          	addi	a0,a0,1964 # 80018218 <tickslock>
    80002a74:	95afe0ef          	jal	80000bce <acquire>
  xticks = ticks;
    80002a78:	00008497          	auipc	s1,0x8
    80002a7c:	8704a483          	lw	s1,-1936(s1) # 8000a2e8 <ticks>
  release(&tickslock);
    80002a80:	00015517          	auipc	a0,0x15
    80002a84:	79850513          	addi	a0,a0,1944 # 80018218 <tickslock>
    80002a88:	9defe0ef          	jal	80000c66 <release>
  return xticks;
}
    80002a8c:	02049513          	slli	a0,s1,0x20
    80002a90:	9101                	srli	a0,a0,0x20
    80002a92:	60e2                	ld	ra,24(sp)
    80002a94:	6442                	ld	s0,16(sp)
    80002a96:	64a2                	ld	s1,8(sp)
    80002a98:	6105                	addi	sp,sp,32
    80002a9a:	8082                	ret

0000000080002a9c <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
    80002a9c:	7179                	addi	sp,sp,-48
    80002a9e:	f406                	sd	ra,40(sp)
    80002aa0:	f022                	sd	s0,32(sp)
    80002aa2:	ec26                	sd	s1,24(sp)
    80002aa4:	e84a                	sd	s2,16(sp)
    80002aa6:	e44e                	sd	s3,8(sp)
    80002aa8:	e052                	sd	s4,0(sp)
    80002aaa:	1800                	addi	s0,sp,48
  struct buf *b;

  initlock(&bcache.lock, "bcache");
    80002aac:	00005597          	auipc	a1,0x5
    80002ab0:	8e458593          	addi	a1,a1,-1820 # 80007390 <etext+0x390>
    80002ab4:	00015517          	auipc	a0,0x15
    80002ab8:	77c50513          	addi	a0,a0,1916 # 80018230 <bcache>
    80002abc:	892fe0ef          	jal	80000b4e <initlock>

  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
    80002ac0:	0001d797          	auipc	a5,0x1d
    80002ac4:	77078793          	addi	a5,a5,1904 # 80020230 <bcache+0x8000>
    80002ac8:	0001e717          	auipc	a4,0x1e
    80002acc:	9d070713          	addi	a4,a4,-1584 # 80020498 <bcache+0x8268>
    80002ad0:	2ae7b823          	sd	a4,688(a5)
  bcache.head.next = &bcache.head;
    80002ad4:	2ae7bc23          	sd	a4,696(a5)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002ad8:	00015497          	auipc	s1,0x15
    80002adc:	77048493          	addi	s1,s1,1904 # 80018248 <bcache+0x18>
    b->next = bcache.head.next;
    80002ae0:	893e                	mv	s2,a5
    b->prev = &bcache.head;
    80002ae2:	89ba                	mv	s3,a4
    initsleeplock(&b->lock, "buffer");
    80002ae4:	00005a17          	auipc	s4,0x5
    80002ae8:	8b4a0a13          	addi	s4,s4,-1868 # 80007398 <etext+0x398>
    b->next = bcache.head.next;
    80002aec:	2b893783          	ld	a5,696(s2)
    80002af0:	e8bc                	sd	a5,80(s1)
    b->prev = &bcache.head;
    80002af2:	0534b423          	sd	s3,72(s1)
    initsleeplock(&b->lock, "buffer");
    80002af6:	85d2                	mv	a1,s4
    80002af8:	01048513          	addi	a0,s1,16
    80002afc:	322010ef          	jal	80003e1e <initsleeplock>
    bcache.head.next->prev = b;
    80002b00:	2b893783          	ld	a5,696(s2)
    80002b04:	e7a4                	sd	s1,72(a5)
    bcache.head.next = b;
    80002b06:	2a993c23          	sd	s1,696(s2)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
    80002b0a:	45848493          	addi	s1,s1,1112
    80002b0e:	fd349fe3          	bne	s1,s3,80002aec <binit+0x50>
  }
}
    80002b12:	70a2                	ld	ra,40(sp)
    80002b14:	7402                	ld	s0,32(sp)
    80002b16:	64e2                	ld	s1,24(sp)
    80002b18:	6942                	ld	s2,16(sp)
    80002b1a:	69a2                	ld	s3,8(sp)
    80002b1c:	6a02                	ld	s4,0(sp)
    80002b1e:	6145                	addi	sp,sp,48
    80002b20:	8082                	ret

0000000080002b22 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
    80002b22:	7179                	addi	sp,sp,-48
    80002b24:	f406                	sd	ra,40(sp)
    80002b26:	f022                	sd	s0,32(sp)
    80002b28:	ec26                	sd	s1,24(sp)
    80002b2a:	e84a                	sd	s2,16(sp)
    80002b2c:	e44e                	sd	s3,8(sp)
    80002b2e:	1800                	addi	s0,sp,48
    80002b30:	892a                	mv	s2,a0
    80002b32:	89ae                	mv	s3,a1
  acquire(&bcache.lock);
    80002b34:	00015517          	auipc	a0,0x15
    80002b38:	6fc50513          	addi	a0,a0,1788 # 80018230 <bcache>
    80002b3c:	892fe0ef          	jal	80000bce <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
    80002b40:	0001e497          	auipc	s1,0x1e
    80002b44:	9a84b483          	ld	s1,-1624(s1) # 800204e8 <bcache+0x82b8>
    80002b48:	0001e797          	auipc	a5,0x1e
    80002b4c:	95078793          	addi	a5,a5,-1712 # 80020498 <bcache+0x8268>
    80002b50:	02f48b63          	beq	s1,a5,80002b86 <bread+0x64>
    80002b54:	873e                	mv	a4,a5
    80002b56:	a021                	j	80002b5e <bread+0x3c>
    80002b58:	68a4                	ld	s1,80(s1)
    80002b5a:	02e48663          	beq	s1,a4,80002b86 <bread+0x64>
    if(b->dev == dev && b->blockno == blockno){
    80002b5e:	449c                	lw	a5,8(s1)
    80002b60:	ff279ce3          	bne	a5,s2,80002b58 <bread+0x36>
    80002b64:	44dc                	lw	a5,12(s1)
    80002b66:	ff3799e3          	bne	a5,s3,80002b58 <bread+0x36>
      b->refcnt++;
    80002b6a:	40bc                	lw	a5,64(s1)
    80002b6c:	2785                	addiw	a5,a5,1
    80002b6e:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002b70:	00015517          	auipc	a0,0x15
    80002b74:	6c050513          	addi	a0,a0,1728 # 80018230 <bcache>
    80002b78:	8eefe0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002b7c:	01048513          	addi	a0,s1,16
    80002b80:	2d4010ef          	jal	80003e54 <acquiresleep>
      return b;
    80002b84:	a889                	j	80002bd6 <bread+0xb4>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002b86:	0001e497          	auipc	s1,0x1e
    80002b8a:	95a4b483          	ld	s1,-1702(s1) # 800204e0 <bcache+0x82b0>
    80002b8e:	0001e797          	auipc	a5,0x1e
    80002b92:	90a78793          	addi	a5,a5,-1782 # 80020498 <bcache+0x8268>
    80002b96:	00f48863          	beq	s1,a5,80002ba6 <bread+0x84>
    80002b9a:	873e                	mv	a4,a5
    if(b->refcnt == 0) {
    80002b9c:	40bc                	lw	a5,64(s1)
    80002b9e:	cb91                	beqz	a5,80002bb2 <bread+0x90>
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
    80002ba0:	64a4                	ld	s1,72(s1)
    80002ba2:	fee49de3          	bne	s1,a4,80002b9c <bread+0x7a>
  panic("bget: no buffers");
    80002ba6:	00004517          	auipc	a0,0x4
    80002baa:	7fa50513          	addi	a0,a0,2042 # 800073a0 <etext+0x3a0>
    80002bae:	c33fd0ef          	jal	800007e0 <panic>
      b->dev = dev;
    80002bb2:	0124a423          	sw	s2,8(s1)
      b->blockno = blockno;
    80002bb6:	0134a623          	sw	s3,12(s1)
      b->valid = 0;
    80002bba:	0004a023          	sw	zero,0(s1)
      b->refcnt = 1;
    80002bbe:	4785                	li	a5,1
    80002bc0:	c0bc                	sw	a5,64(s1)
      release(&bcache.lock);
    80002bc2:	00015517          	auipc	a0,0x15
    80002bc6:	66e50513          	addi	a0,a0,1646 # 80018230 <bcache>
    80002bca:	89cfe0ef          	jal	80000c66 <release>
      acquiresleep(&b->lock);
    80002bce:	01048513          	addi	a0,s1,16
    80002bd2:	282010ef          	jal	80003e54 <acquiresleep>
  struct buf *b;

  b = bget(dev, blockno);
  if(!b->valid) {
    80002bd6:	409c                	lw	a5,0(s1)
    80002bd8:	cb89                	beqz	a5,80002bea <bread+0xc8>
    virtio_disk_rw(b, 0);
    b->valid = 1;
  }
  return b;
}
    80002bda:	8526                	mv	a0,s1
    80002bdc:	70a2                	ld	ra,40(sp)
    80002bde:	7402                	ld	s0,32(sp)
    80002be0:	64e2                	ld	s1,24(sp)
    80002be2:	6942                	ld	s2,16(sp)
    80002be4:	69a2                	ld	s3,8(sp)
    80002be6:	6145                	addi	sp,sp,48
    80002be8:	8082                	ret
    virtio_disk_rw(b, 0);
    80002bea:	4581                	li	a1,0
    80002bec:	8526                	mv	a0,s1
    80002bee:	5df020ef          	jal	800059cc <virtio_disk_rw>
    b->valid = 1;
    80002bf2:	4785                	li	a5,1
    80002bf4:	c09c                	sw	a5,0(s1)
  return b;
    80002bf6:	b7d5                	j	80002bda <bread+0xb8>

0000000080002bf8 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
    80002bf8:	1101                	addi	sp,sp,-32
    80002bfa:	ec06                	sd	ra,24(sp)
    80002bfc:	e822                	sd	s0,16(sp)
    80002bfe:	e426                	sd	s1,8(sp)
    80002c00:	1000                	addi	s0,sp,32
    80002c02:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80002c04:	0541                	addi	a0,a0,16
    80002c06:	2cc010ef          	jal	80003ed2 <holdingsleep>
    80002c0a:	c911                	beqz	a0,80002c1e <bwrite+0x26>
    panic("bwrite");
  virtio_disk_rw(b, 1);
    80002c0c:	4585                	li	a1,1
    80002c0e:	8526                	mv	a0,s1
    80002c10:	5bd020ef          	jal	800059cc <virtio_disk_rw>
}
    80002c14:	60e2                	ld	ra,24(sp)
    80002c16:	6442                	ld	s0,16(sp)
    80002c18:	64a2                	ld	s1,8(sp)
    80002c1a:	6105                	addi	sp,sp,32
    80002c1c:	8082                	ret
    panic("bwrite");
    80002c1e:	00004517          	auipc	a0,0x4
    80002c22:	79a50513          	addi	a0,a0,1946 # 800073b8 <etext+0x3b8>
    80002c26:	bbbfd0ef          	jal	800007e0 <panic>

0000000080002c2a <brelse>:

// Release a locked buffer.
// Move to the head of the most-recently-used list.
void
brelse(struct buf *b)
{
    80002c2a:	1101                	addi	sp,sp,-32
    80002c2c:	ec06                	sd	ra,24(sp)
    80002c2e:	e822                	sd	s0,16(sp)
    80002c30:	e426                	sd	s1,8(sp)
    80002c32:	e04a                	sd	s2,0(sp)
    80002c34:	1000                	addi	s0,sp,32
    80002c36:	84aa                	mv	s1,a0
  if(!holdingsleep(&b->lock))
    80002c38:	01050913          	addi	s2,a0,16
    80002c3c:	854a                	mv	a0,s2
    80002c3e:	294010ef          	jal	80003ed2 <holdingsleep>
    80002c42:	c135                	beqz	a0,80002ca6 <brelse+0x7c>
    panic("brelse");

  releasesleep(&b->lock);
    80002c44:	854a                	mv	a0,s2
    80002c46:	254010ef          	jal	80003e9a <releasesleep>

  acquire(&bcache.lock);
    80002c4a:	00015517          	auipc	a0,0x15
    80002c4e:	5e650513          	addi	a0,a0,1510 # 80018230 <bcache>
    80002c52:	f7dfd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002c56:	40bc                	lw	a5,64(s1)
    80002c58:	37fd                	addiw	a5,a5,-1
    80002c5a:	0007871b          	sext.w	a4,a5
    80002c5e:	c0bc                	sw	a5,64(s1)
  if (b->refcnt == 0) {
    80002c60:	e71d                	bnez	a4,80002c8e <brelse+0x64>
    // no one is waiting for it.
    b->next->prev = b->prev;
    80002c62:	68b8                	ld	a4,80(s1)
    80002c64:	64bc                	ld	a5,72(s1)
    80002c66:	e73c                	sd	a5,72(a4)
    b->prev->next = b->next;
    80002c68:	68b8                	ld	a4,80(s1)
    80002c6a:	ebb8                	sd	a4,80(a5)
    b->next = bcache.head.next;
    80002c6c:	0001d797          	auipc	a5,0x1d
    80002c70:	5c478793          	addi	a5,a5,1476 # 80020230 <bcache+0x8000>
    80002c74:	2b87b703          	ld	a4,696(a5)
    80002c78:	e8b8                	sd	a4,80(s1)
    b->prev = &bcache.head;
    80002c7a:	0001e717          	auipc	a4,0x1e
    80002c7e:	81e70713          	addi	a4,a4,-2018 # 80020498 <bcache+0x8268>
    80002c82:	e4b8                	sd	a4,72(s1)
    bcache.head.next->prev = b;
    80002c84:	2b87b703          	ld	a4,696(a5)
    80002c88:	e724                	sd	s1,72(a4)
    bcache.head.next = b;
    80002c8a:	2a97bc23          	sd	s1,696(a5)
  }
  
  release(&bcache.lock);
    80002c8e:	00015517          	auipc	a0,0x15
    80002c92:	5a250513          	addi	a0,a0,1442 # 80018230 <bcache>
    80002c96:	fd1fd0ef          	jal	80000c66 <release>
}
    80002c9a:	60e2                	ld	ra,24(sp)
    80002c9c:	6442                	ld	s0,16(sp)
    80002c9e:	64a2                	ld	s1,8(sp)
    80002ca0:	6902                	ld	s2,0(sp)
    80002ca2:	6105                	addi	sp,sp,32
    80002ca4:	8082                	ret
    panic("brelse");
    80002ca6:	00004517          	auipc	a0,0x4
    80002caa:	71a50513          	addi	a0,a0,1818 # 800073c0 <etext+0x3c0>
    80002cae:	b33fd0ef          	jal	800007e0 <panic>

0000000080002cb2 <bpin>:

void
bpin(struct buf *b) {
    80002cb2:	1101                	addi	sp,sp,-32
    80002cb4:	ec06                	sd	ra,24(sp)
    80002cb6:	e822                	sd	s0,16(sp)
    80002cb8:	e426                	sd	s1,8(sp)
    80002cba:	1000                	addi	s0,sp,32
    80002cbc:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80002cbe:	00015517          	auipc	a0,0x15
    80002cc2:	57250513          	addi	a0,a0,1394 # 80018230 <bcache>
    80002cc6:	f09fd0ef          	jal	80000bce <acquire>
  b->refcnt++;
    80002cca:	40bc                	lw	a5,64(s1)
    80002ccc:	2785                	addiw	a5,a5,1
    80002cce:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002cd0:	00015517          	auipc	a0,0x15
    80002cd4:	56050513          	addi	a0,a0,1376 # 80018230 <bcache>
    80002cd8:	f8ffd0ef          	jal	80000c66 <release>
}
    80002cdc:	60e2                	ld	ra,24(sp)
    80002cde:	6442                	ld	s0,16(sp)
    80002ce0:	64a2                	ld	s1,8(sp)
    80002ce2:	6105                	addi	sp,sp,32
    80002ce4:	8082                	ret

0000000080002ce6 <bunpin>:

void
bunpin(struct buf *b) {
    80002ce6:	1101                	addi	sp,sp,-32
    80002ce8:	ec06                	sd	ra,24(sp)
    80002cea:	e822                	sd	s0,16(sp)
    80002cec:	e426                	sd	s1,8(sp)
    80002cee:	1000                	addi	s0,sp,32
    80002cf0:	84aa                	mv	s1,a0
  acquire(&bcache.lock);
    80002cf2:	00015517          	auipc	a0,0x15
    80002cf6:	53e50513          	addi	a0,a0,1342 # 80018230 <bcache>
    80002cfa:	ed5fd0ef          	jal	80000bce <acquire>
  b->refcnt--;
    80002cfe:	40bc                	lw	a5,64(s1)
    80002d00:	37fd                	addiw	a5,a5,-1
    80002d02:	c0bc                	sw	a5,64(s1)
  release(&bcache.lock);
    80002d04:	00015517          	auipc	a0,0x15
    80002d08:	52c50513          	addi	a0,a0,1324 # 80018230 <bcache>
    80002d0c:	f5bfd0ef          	jal	80000c66 <release>
}
    80002d10:	60e2                	ld	ra,24(sp)
    80002d12:	6442                	ld	s0,16(sp)
    80002d14:	64a2                	ld	s1,8(sp)
    80002d16:	6105                	addi	sp,sp,32
    80002d18:	8082                	ret

0000000080002d1a <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
    80002d1a:	1101                	addi	sp,sp,-32
    80002d1c:	ec06                	sd	ra,24(sp)
    80002d1e:	e822                	sd	s0,16(sp)
    80002d20:	e426                	sd	s1,8(sp)
    80002d22:	e04a                	sd	s2,0(sp)
    80002d24:	1000                	addi	s0,sp,32
    80002d26:	84ae                	mv	s1,a1
  struct buf *bp;
  int bi, m;

  bp = bread(dev, BBLOCK(b, sb));
    80002d28:	00d5d59b          	srliw	a1,a1,0xd
    80002d2c:	0001e797          	auipc	a5,0x1e
    80002d30:	be07a783          	lw	a5,-1056(a5) # 8002090c <sb+0x1c>
    80002d34:	9dbd                	addw	a1,a1,a5
    80002d36:	dedff0ef          	jal	80002b22 <bread>
  bi = b % BPB;
  m = 1 << (bi % 8);
    80002d3a:	0074f713          	andi	a4,s1,7
    80002d3e:	4785                	li	a5,1
    80002d40:	00e797bb          	sllw	a5,a5,a4
  if((bp->data[bi/8] & m) == 0)
    80002d44:	14ce                	slli	s1,s1,0x33
    80002d46:	90d9                	srli	s1,s1,0x36
    80002d48:	00950733          	add	a4,a0,s1
    80002d4c:	05874703          	lbu	a4,88(a4)
    80002d50:	00e7f6b3          	and	a3,a5,a4
    80002d54:	c29d                	beqz	a3,80002d7a <bfree+0x60>
    80002d56:	892a                	mv	s2,a0
    panic("freeing free block");
  bp->data[bi/8] &= ~m;
    80002d58:	94aa                	add	s1,s1,a0
    80002d5a:	fff7c793          	not	a5,a5
    80002d5e:	8f7d                	and	a4,a4,a5
    80002d60:	04e48c23          	sb	a4,88(s1)
  log_write(bp);
    80002d64:	7f9000ef          	jal	80003d5c <log_write>
  brelse(bp);
    80002d68:	854a                	mv	a0,s2
    80002d6a:	ec1ff0ef          	jal	80002c2a <brelse>
}
    80002d6e:	60e2                	ld	ra,24(sp)
    80002d70:	6442                	ld	s0,16(sp)
    80002d72:	64a2                	ld	s1,8(sp)
    80002d74:	6902                	ld	s2,0(sp)
    80002d76:	6105                	addi	sp,sp,32
    80002d78:	8082                	ret
    panic("freeing free block");
    80002d7a:	00004517          	auipc	a0,0x4
    80002d7e:	64e50513          	addi	a0,a0,1614 # 800073c8 <etext+0x3c8>
    80002d82:	a5ffd0ef          	jal	800007e0 <panic>

0000000080002d86 <balloc>:
{
    80002d86:	711d                	addi	sp,sp,-96
    80002d88:	ec86                	sd	ra,88(sp)
    80002d8a:	e8a2                	sd	s0,80(sp)
    80002d8c:	e4a6                	sd	s1,72(sp)
    80002d8e:	1080                	addi	s0,sp,96
  for(b = 0; b < sb.size; b += BPB){
    80002d90:	0001e797          	auipc	a5,0x1e
    80002d94:	b647a783          	lw	a5,-1180(a5) # 800208f4 <sb+0x4>
    80002d98:	0e078f63          	beqz	a5,80002e96 <balloc+0x110>
    80002d9c:	e0ca                	sd	s2,64(sp)
    80002d9e:	fc4e                	sd	s3,56(sp)
    80002da0:	f852                	sd	s4,48(sp)
    80002da2:	f456                	sd	s5,40(sp)
    80002da4:	f05a                	sd	s6,32(sp)
    80002da6:	ec5e                	sd	s7,24(sp)
    80002da8:	e862                	sd	s8,16(sp)
    80002daa:	e466                	sd	s9,8(sp)
    80002dac:	8baa                	mv	s7,a0
    80002dae:	4a81                	li	s5,0
    bp = bread(dev, BBLOCK(b, sb));
    80002db0:	0001eb17          	auipc	s6,0x1e
    80002db4:	b40b0b13          	addi	s6,s6,-1216 # 800208f0 <sb>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002db8:	4c01                	li	s8,0
      m = 1 << (bi % 8);
    80002dba:	4985                	li	s3,1
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002dbc:	6a09                	lui	s4,0x2
  for(b = 0; b < sb.size; b += BPB){
    80002dbe:	6c89                	lui	s9,0x2
    80002dc0:	a0b5                	j	80002e2c <balloc+0xa6>
        bp->data[bi/8] |= m;  // Mark block in use.
    80002dc2:	97ca                	add	a5,a5,s2
    80002dc4:	8e55                	or	a2,a2,a3
    80002dc6:	04c78c23          	sb	a2,88(a5)
        log_write(bp);
    80002dca:	854a                	mv	a0,s2
    80002dcc:	791000ef          	jal	80003d5c <log_write>
        brelse(bp);
    80002dd0:	854a                	mv	a0,s2
    80002dd2:	e59ff0ef          	jal	80002c2a <brelse>
  bp = bread(dev, bno);
    80002dd6:	85a6                	mv	a1,s1
    80002dd8:	855e                	mv	a0,s7
    80002dda:	d49ff0ef          	jal	80002b22 <bread>
    80002dde:	892a                	mv	s2,a0
  memset(bp->data, 0, BSIZE);
    80002de0:	40000613          	li	a2,1024
    80002de4:	4581                	li	a1,0
    80002de6:	05850513          	addi	a0,a0,88
    80002dea:	eb9fd0ef          	jal	80000ca2 <memset>
  log_write(bp);
    80002dee:	854a                	mv	a0,s2
    80002df0:	76d000ef          	jal	80003d5c <log_write>
  brelse(bp);
    80002df4:	854a                	mv	a0,s2
    80002df6:	e35ff0ef          	jal	80002c2a <brelse>
}
    80002dfa:	6906                	ld	s2,64(sp)
    80002dfc:	79e2                	ld	s3,56(sp)
    80002dfe:	7a42                	ld	s4,48(sp)
    80002e00:	7aa2                	ld	s5,40(sp)
    80002e02:	7b02                	ld	s6,32(sp)
    80002e04:	6be2                	ld	s7,24(sp)
    80002e06:	6c42                	ld	s8,16(sp)
    80002e08:	6ca2                	ld	s9,8(sp)
}
    80002e0a:	8526                	mv	a0,s1
    80002e0c:	60e6                	ld	ra,88(sp)
    80002e0e:	6446                	ld	s0,80(sp)
    80002e10:	64a6                	ld	s1,72(sp)
    80002e12:	6125                	addi	sp,sp,96
    80002e14:	8082                	ret
    brelse(bp);
    80002e16:	854a                	mv	a0,s2
    80002e18:	e13ff0ef          	jal	80002c2a <brelse>
  for(b = 0; b < sb.size; b += BPB){
    80002e1c:	015c87bb          	addw	a5,s9,s5
    80002e20:	00078a9b          	sext.w	s5,a5
    80002e24:	004b2703          	lw	a4,4(s6)
    80002e28:	04eaff63          	bgeu	s5,a4,80002e86 <balloc+0x100>
    bp = bread(dev, BBLOCK(b, sb));
    80002e2c:	41fad79b          	sraiw	a5,s5,0x1f
    80002e30:	0137d79b          	srliw	a5,a5,0x13
    80002e34:	015787bb          	addw	a5,a5,s5
    80002e38:	40d7d79b          	sraiw	a5,a5,0xd
    80002e3c:	01cb2583          	lw	a1,28(s6)
    80002e40:	9dbd                	addw	a1,a1,a5
    80002e42:	855e                	mv	a0,s7
    80002e44:	cdfff0ef          	jal	80002b22 <bread>
    80002e48:	892a                	mv	s2,a0
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e4a:	004b2503          	lw	a0,4(s6)
    80002e4e:	000a849b          	sext.w	s1,s5
    80002e52:	8762                	mv	a4,s8
    80002e54:	fca4f1e3          	bgeu	s1,a0,80002e16 <balloc+0x90>
      m = 1 << (bi % 8);
    80002e58:	00777693          	andi	a3,a4,7
    80002e5c:	00d996bb          	sllw	a3,s3,a3
      if((bp->data[bi/8] & m) == 0){  // Is block free?
    80002e60:	41f7579b          	sraiw	a5,a4,0x1f
    80002e64:	01d7d79b          	srliw	a5,a5,0x1d
    80002e68:	9fb9                	addw	a5,a5,a4
    80002e6a:	4037d79b          	sraiw	a5,a5,0x3
    80002e6e:	00f90633          	add	a2,s2,a5
    80002e72:	05864603          	lbu	a2,88(a2)
    80002e76:	00c6f5b3          	and	a1,a3,a2
    80002e7a:	d5a1                	beqz	a1,80002dc2 <balloc+0x3c>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
    80002e7c:	2705                	addiw	a4,a4,1
    80002e7e:	2485                	addiw	s1,s1,1
    80002e80:	fd471ae3          	bne	a4,s4,80002e54 <balloc+0xce>
    80002e84:	bf49                	j	80002e16 <balloc+0x90>
    80002e86:	6906                	ld	s2,64(sp)
    80002e88:	79e2                	ld	s3,56(sp)
    80002e8a:	7a42                	ld	s4,48(sp)
    80002e8c:	7aa2                	ld	s5,40(sp)
    80002e8e:	7b02                	ld	s6,32(sp)
    80002e90:	6be2                	ld	s7,24(sp)
    80002e92:	6c42                	ld	s8,16(sp)
    80002e94:	6ca2                	ld	s9,8(sp)
  printf("balloc: out of blocks\n");
    80002e96:	00004517          	auipc	a0,0x4
    80002e9a:	54a50513          	addi	a0,a0,1354 # 800073e0 <etext+0x3e0>
    80002e9e:	e5cfd0ef          	jal	800004fa <printf>
  return 0;
    80002ea2:	4481                	li	s1,0
    80002ea4:	b79d                	j	80002e0a <balloc+0x84>

0000000080002ea6 <bmap>:
// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
// returns 0 if out of disk space.
static uint
bmap(struct inode *ip, uint bn)
{
    80002ea6:	7179                	addi	sp,sp,-48
    80002ea8:	f406                	sd	ra,40(sp)
    80002eaa:	f022                	sd	s0,32(sp)
    80002eac:	ec26                	sd	s1,24(sp)
    80002eae:	e84a                	sd	s2,16(sp)
    80002eb0:	e44e                	sd	s3,8(sp)
    80002eb2:	1800                	addi	s0,sp,48
    80002eb4:	89aa                	mv	s3,a0
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
    80002eb6:	47ad                	li	a5,11
    80002eb8:	02b7e663          	bltu	a5,a1,80002ee4 <bmap+0x3e>
    if((addr = ip->addrs[bn]) == 0){
    80002ebc:	02059793          	slli	a5,a1,0x20
    80002ec0:	01e7d593          	srli	a1,a5,0x1e
    80002ec4:	00b504b3          	add	s1,a0,a1
    80002ec8:	0504a903          	lw	s2,80(s1)
    80002ecc:	06091a63          	bnez	s2,80002f40 <bmap+0x9a>
      addr = balloc(ip->dev);
    80002ed0:	4108                	lw	a0,0(a0)
    80002ed2:	eb5ff0ef          	jal	80002d86 <balloc>
    80002ed6:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002eda:	06090363          	beqz	s2,80002f40 <bmap+0x9a>
        return 0;
      ip->addrs[bn] = addr;
    80002ede:	0524a823          	sw	s2,80(s1)
    80002ee2:	a8b9                	j	80002f40 <bmap+0x9a>
    }
    return addr;
  }
  bn -= NDIRECT;
    80002ee4:	ff45849b          	addiw	s1,a1,-12
    80002ee8:	0004871b          	sext.w	a4,s1

  if(bn < NINDIRECT){
    80002eec:	0ff00793          	li	a5,255
    80002ef0:	06e7ee63          	bltu	a5,a4,80002f6c <bmap+0xc6>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0){
    80002ef4:	08052903          	lw	s2,128(a0)
    80002ef8:	00091d63          	bnez	s2,80002f12 <bmap+0x6c>
      addr = balloc(ip->dev);
    80002efc:	4108                	lw	a0,0(a0)
    80002efe:	e89ff0ef          	jal	80002d86 <balloc>
    80002f02:	0005091b          	sext.w	s2,a0
      if(addr == 0)
    80002f06:	02090d63          	beqz	s2,80002f40 <bmap+0x9a>
    80002f0a:	e052                	sd	s4,0(sp)
        return 0;
      ip->addrs[NDIRECT] = addr;
    80002f0c:	0929a023          	sw	s2,128(s3)
    80002f10:	a011                	j	80002f14 <bmap+0x6e>
    80002f12:	e052                	sd	s4,0(sp)
    }
    bp = bread(ip->dev, addr);
    80002f14:	85ca                	mv	a1,s2
    80002f16:	0009a503          	lw	a0,0(s3)
    80002f1a:	c09ff0ef          	jal	80002b22 <bread>
    80002f1e:	8a2a                	mv	s4,a0
    a = (uint*)bp->data;
    80002f20:	05850793          	addi	a5,a0,88
    if((addr = a[bn]) == 0){
    80002f24:	02049713          	slli	a4,s1,0x20
    80002f28:	01e75593          	srli	a1,a4,0x1e
    80002f2c:	00b784b3          	add	s1,a5,a1
    80002f30:	0004a903          	lw	s2,0(s1)
    80002f34:	00090e63          	beqz	s2,80002f50 <bmap+0xaa>
      if(addr){
        a[bn] = addr;
        log_write(bp);
      }
    }
    brelse(bp);
    80002f38:	8552                	mv	a0,s4
    80002f3a:	cf1ff0ef          	jal	80002c2a <brelse>
    return addr;
    80002f3e:	6a02                	ld	s4,0(sp)
  }

  panic("bmap: out of range");
}
    80002f40:	854a                	mv	a0,s2
    80002f42:	70a2                	ld	ra,40(sp)
    80002f44:	7402                	ld	s0,32(sp)
    80002f46:	64e2                	ld	s1,24(sp)
    80002f48:	6942                	ld	s2,16(sp)
    80002f4a:	69a2                	ld	s3,8(sp)
    80002f4c:	6145                	addi	sp,sp,48
    80002f4e:	8082                	ret
      addr = balloc(ip->dev);
    80002f50:	0009a503          	lw	a0,0(s3)
    80002f54:	e33ff0ef          	jal	80002d86 <balloc>
    80002f58:	0005091b          	sext.w	s2,a0
      if(addr){
    80002f5c:	fc090ee3          	beqz	s2,80002f38 <bmap+0x92>
        a[bn] = addr;
    80002f60:	0124a023          	sw	s2,0(s1)
        log_write(bp);
    80002f64:	8552                	mv	a0,s4
    80002f66:	5f7000ef          	jal	80003d5c <log_write>
    80002f6a:	b7f9                	j	80002f38 <bmap+0x92>
    80002f6c:	e052                	sd	s4,0(sp)
  panic("bmap: out of range");
    80002f6e:	00004517          	auipc	a0,0x4
    80002f72:	48a50513          	addi	a0,a0,1162 # 800073f8 <etext+0x3f8>
    80002f76:	86bfd0ef          	jal	800007e0 <panic>

0000000080002f7a <iget>:
{
    80002f7a:	7179                	addi	sp,sp,-48
    80002f7c:	f406                	sd	ra,40(sp)
    80002f7e:	f022                	sd	s0,32(sp)
    80002f80:	ec26                	sd	s1,24(sp)
    80002f82:	e84a                	sd	s2,16(sp)
    80002f84:	e44e                	sd	s3,8(sp)
    80002f86:	e052                	sd	s4,0(sp)
    80002f88:	1800                	addi	s0,sp,48
    80002f8a:	89aa                	mv	s3,a0
    80002f8c:	8a2e                	mv	s4,a1
  acquire(&itable.lock);
    80002f8e:	0001e517          	auipc	a0,0x1e
    80002f92:	98250513          	addi	a0,a0,-1662 # 80020910 <itable>
    80002f96:	c39fd0ef          	jal	80000bce <acquire>
  empty = 0;
    80002f9a:	4901                	li	s2,0
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80002f9c:	0001e497          	auipc	s1,0x1e
    80002fa0:	98c48493          	addi	s1,s1,-1652 # 80020928 <itable+0x18>
    80002fa4:	0001f697          	auipc	a3,0x1f
    80002fa8:	41468693          	addi	a3,a3,1044 # 800223b8 <log>
    80002fac:	a039                	j	80002fba <iget+0x40>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80002fae:	02090963          	beqz	s2,80002fe0 <iget+0x66>
  for(ip = &itable.inode[0]; ip < &itable.inode[NINODE]; ip++){
    80002fb2:	08848493          	addi	s1,s1,136
    80002fb6:	02d48863          	beq	s1,a3,80002fe6 <iget+0x6c>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
    80002fba:	449c                	lw	a5,8(s1)
    80002fbc:	fef059e3          	blez	a5,80002fae <iget+0x34>
    80002fc0:	4098                	lw	a4,0(s1)
    80002fc2:	ff3716e3          	bne	a4,s3,80002fae <iget+0x34>
    80002fc6:	40d8                	lw	a4,4(s1)
    80002fc8:	ff4713e3          	bne	a4,s4,80002fae <iget+0x34>
      ip->ref++;
    80002fcc:	2785                	addiw	a5,a5,1
    80002fce:	c49c                	sw	a5,8(s1)
      release(&itable.lock);
    80002fd0:	0001e517          	auipc	a0,0x1e
    80002fd4:	94050513          	addi	a0,a0,-1728 # 80020910 <itable>
    80002fd8:	c8ffd0ef          	jal	80000c66 <release>
      return ip;
    80002fdc:	8926                	mv	s2,s1
    80002fde:	a02d                	j	80003008 <iget+0x8e>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
    80002fe0:	fbe9                	bnez	a5,80002fb2 <iget+0x38>
      empty = ip;
    80002fe2:	8926                	mv	s2,s1
    80002fe4:	b7f9                	j	80002fb2 <iget+0x38>
  if(empty == 0)
    80002fe6:	02090a63          	beqz	s2,8000301a <iget+0xa0>
  ip->dev = dev;
    80002fea:	01392023          	sw	s3,0(s2)
  ip->inum = inum;
    80002fee:	01492223          	sw	s4,4(s2)
  ip->ref = 1;
    80002ff2:	4785                	li	a5,1
    80002ff4:	00f92423          	sw	a5,8(s2)
  ip->valid = 0;
    80002ff8:	04092023          	sw	zero,64(s2)
  release(&itable.lock);
    80002ffc:	0001e517          	auipc	a0,0x1e
    80003000:	91450513          	addi	a0,a0,-1772 # 80020910 <itable>
    80003004:	c63fd0ef          	jal	80000c66 <release>
}
    80003008:	854a                	mv	a0,s2
    8000300a:	70a2                	ld	ra,40(sp)
    8000300c:	7402                	ld	s0,32(sp)
    8000300e:	64e2                	ld	s1,24(sp)
    80003010:	6942                	ld	s2,16(sp)
    80003012:	69a2                	ld	s3,8(sp)
    80003014:	6a02                	ld	s4,0(sp)
    80003016:	6145                	addi	sp,sp,48
    80003018:	8082                	ret
    panic("iget: no inodes");
    8000301a:	00004517          	auipc	a0,0x4
    8000301e:	3f650513          	addi	a0,a0,1014 # 80007410 <etext+0x410>
    80003022:	fbefd0ef          	jal	800007e0 <panic>

0000000080003026 <iinit>:
{
    80003026:	7179                	addi	sp,sp,-48
    80003028:	f406                	sd	ra,40(sp)
    8000302a:	f022                	sd	s0,32(sp)
    8000302c:	ec26                	sd	s1,24(sp)
    8000302e:	e84a                	sd	s2,16(sp)
    80003030:	e44e                	sd	s3,8(sp)
    80003032:	1800                	addi	s0,sp,48
  initlock(&itable.lock, "itable");
    80003034:	00004597          	auipc	a1,0x4
    80003038:	3ec58593          	addi	a1,a1,1004 # 80007420 <etext+0x420>
    8000303c:	0001e517          	auipc	a0,0x1e
    80003040:	8d450513          	addi	a0,a0,-1836 # 80020910 <itable>
    80003044:	b0bfd0ef          	jal	80000b4e <initlock>
  for(i = 0; i < NINODE; i++) {
    80003048:	0001e497          	auipc	s1,0x1e
    8000304c:	8f048493          	addi	s1,s1,-1808 # 80020938 <itable+0x28>
    80003050:	0001f997          	auipc	s3,0x1f
    80003054:	37898993          	addi	s3,s3,888 # 800223c8 <log+0x10>
    initsleeplock(&itable.inode[i].lock, "inode");
    80003058:	00004917          	auipc	s2,0x4
    8000305c:	3d090913          	addi	s2,s2,976 # 80007428 <etext+0x428>
    80003060:	85ca                	mv	a1,s2
    80003062:	8526                	mv	a0,s1
    80003064:	5bb000ef          	jal	80003e1e <initsleeplock>
  for(i = 0; i < NINODE; i++) {
    80003068:	08848493          	addi	s1,s1,136
    8000306c:	ff349ae3          	bne	s1,s3,80003060 <iinit+0x3a>
}
    80003070:	70a2                	ld	ra,40(sp)
    80003072:	7402                	ld	s0,32(sp)
    80003074:	64e2                	ld	s1,24(sp)
    80003076:	6942                	ld	s2,16(sp)
    80003078:	69a2                	ld	s3,8(sp)
    8000307a:	6145                	addi	sp,sp,48
    8000307c:	8082                	ret

000000008000307e <ialloc>:
{
    8000307e:	7139                	addi	sp,sp,-64
    80003080:	fc06                	sd	ra,56(sp)
    80003082:	f822                	sd	s0,48(sp)
    80003084:	0080                	addi	s0,sp,64
  for(inum = 1; inum < sb.ninodes; inum++){
    80003086:	0001e717          	auipc	a4,0x1e
    8000308a:	87672703          	lw	a4,-1930(a4) # 800208fc <sb+0xc>
    8000308e:	4785                	li	a5,1
    80003090:	06e7f063          	bgeu	a5,a4,800030f0 <ialloc+0x72>
    80003094:	f426                	sd	s1,40(sp)
    80003096:	f04a                	sd	s2,32(sp)
    80003098:	ec4e                	sd	s3,24(sp)
    8000309a:	e852                	sd	s4,16(sp)
    8000309c:	e456                	sd	s5,8(sp)
    8000309e:	e05a                	sd	s6,0(sp)
    800030a0:	8aaa                	mv	s5,a0
    800030a2:	8b2e                	mv	s6,a1
    800030a4:	4905                	li	s2,1
    bp = bread(dev, IBLOCK(inum, sb));
    800030a6:	0001ea17          	auipc	s4,0x1e
    800030aa:	84aa0a13          	addi	s4,s4,-1974 # 800208f0 <sb>
    800030ae:	00495593          	srli	a1,s2,0x4
    800030b2:	018a2783          	lw	a5,24(s4)
    800030b6:	9dbd                	addw	a1,a1,a5
    800030b8:	8556                	mv	a0,s5
    800030ba:	a69ff0ef          	jal	80002b22 <bread>
    800030be:	84aa                	mv	s1,a0
    dip = (struct dinode*)bp->data + inum%IPB;
    800030c0:	05850993          	addi	s3,a0,88
    800030c4:	00f97793          	andi	a5,s2,15
    800030c8:	079a                	slli	a5,a5,0x6
    800030ca:	99be                	add	s3,s3,a5
    if(dip->type == 0){  // a free inode
    800030cc:	00099783          	lh	a5,0(s3)
    800030d0:	cb9d                	beqz	a5,80003106 <ialloc+0x88>
    brelse(bp);
    800030d2:	b59ff0ef          	jal	80002c2a <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
    800030d6:	0905                	addi	s2,s2,1
    800030d8:	00ca2703          	lw	a4,12(s4)
    800030dc:	0009079b          	sext.w	a5,s2
    800030e0:	fce7e7e3          	bltu	a5,a4,800030ae <ialloc+0x30>
    800030e4:	74a2                	ld	s1,40(sp)
    800030e6:	7902                	ld	s2,32(sp)
    800030e8:	69e2                	ld	s3,24(sp)
    800030ea:	6a42                	ld	s4,16(sp)
    800030ec:	6aa2                	ld	s5,8(sp)
    800030ee:	6b02                	ld	s6,0(sp)
  printf("ialloc: no inodes\n");
    800030f0:	00004517          	auipc	a0,0x4
    800030f4:	34050513          	addi	a0,a0,832 # 80007430 <etext+0x430>
    800030f8:	c02fd0ef          	jal	800004fa <printf>
  return 0;
    800030fc:	4501                	li	a0,0
}
    800030fe:	70e2                	ld	ra,56(sp)
    80003100:	7442                	ld	s0,48(sp)
    80003102:	6121                	addi	sp,sp,64
    80003104:	8082                	ret
      memset(dip, 0, sizeof(*dip));
    80003106:	04000613          	li	a2,64
    8000310a:	4581                	li	a1,0
    8000310c:	854e                	mv	a0,s3
    8000310e:	b95fd0ef          	jal	80000ca2 <memset>
      dip->type = type;
    80003112:	01699023          	sh	s6,0(s3)
      log_write(bp);   // mark it allocated on the disk
    80003116:	8526                	mv	a0,s1
    80003118:	445000ef          	jal	80003d5c <log_write>
      brelse(bp);
    8000311c:	8526                	mv	a0,s1
    8000311e:	b0dff0ef          	jal	80002c2a <brelse>
      return iget(dev, inum);
    80003122:	0009059b          	sext.w	a1,s2
    80003126:	8556                	mv	a0,s5
    80003128:	e53ff0ef          	jal	80002f7a <iget>
    8000312c:	74a2                	ld	s1,40(sp)
    8000312e:	7902                	ld	s2,32(sp)
    80003130:	69e2                	ld	s3,24(sp)
    80003132:	6a42                	ld	s4,16(sp)
    80003134:	6aa2                	ld	s5,8(sp)
    80003136:	6b02                	ld	s6,0(sp)
    80003138:	b7d9                	j	800030fe <ialloc+0x80>

000000008000313a <iupdate>:
{
    8000313a:	1101                	addi	sp,sp,-32
    8000313c:	ec06                	sd	ra,24(sp)
    8000313e:	e822                	sd	s0,16(sp)
    80003140:	e426                	sd	s1,8(sp)
    80003142:	e04a                	sd	s2,0(sp)
    80003144:	1000                	addi	s0,sp,32
    80003146:	84aa                	mv	s1,a0
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003148:	415c                	lw	a5,4(a0)
    8000314a:	0047d79b          	srliw	a5,a5,0x4
    8000314e:	0001d597          	auipc	a1,0x1d
    80003152:	7ba5a583          	lw	a1,1978(a1) # 80020908 <sb+0x18>
    80003156:	9dbd                	addw	a1,a1,a5
    80003158:	4108                	lw	a0,0(a0)
    8000315a:	9c9ff0ef          	jal	80002b22 <bread>
    8000315e:	892a                	mv	s2,a0
  dip = (struct dinode*)bp->data + ip->inum%IPB;
    80003160:	05850793          	addi	a5,a0,88
    80003164:	40d8                	lw	a4,4(s1)
    80003166:	8b3d                	andi	a4,a4,15
    80003168:	071a                	slli	a4,a4,0x6
    8000316a:	97ba                	add	a5,a5,a4
  dip->type = ip->type;
    8000316c:	04449703          	lh	a4,68(s1)
    80003170:	00e79023          	sh	a4,0(a5)
  dip->major = ip->major;
    80003174:	04649703          	lh	a4,70(s1)
    80003178:	00e79123          	sh	a4,2(a5)
  dip->minor = ip->minor;
    8000317c:	04849703          	lh	a4,72(s1)
    80003180:	00e79223          	sh	a4,4(a5)
  dip->nlink = ip->nlink;
    80003184:	04a49703          	lh	a4,74(s1)
    80003188:	00e79323          	sh	a4,6(a5)
  dip->size = ip->size;
    8000318c:	44f8                	lw	a4,76(s1)
    8000318e:	c798                	sw	a4,8(a5)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
    80003190:	03400613          	li	a2,52
    80003194:	05048593          	addi	a1,s1,80
    80003198:	00c78513          	addi	a0,a5,12
    8000319c:	b63fd0ef          	jal	80000cfe <memmove>
  log_write(bp);
    800031a0:	854a                	mv	a0,s2
    800031a2:	3bb000ef          	jal	80003d5c <log_write>
  brelse(bp);
    800031a6:	854a                	mv	a0,s2
    800031a8:	a83ff0ef          	jal	80002c2a <brelse>
}
    800031ac:	60e2                	ld	ra,24(sp)
    800031ae:	6442                	ld	s0,16(sp)
    800031b0:	64a2                	ld	s1,8(sp)
    800031b2:	6902                	ld	s2,0(sp)
    800031b4:	6105                	addi	sp,sp,32
    800031b6:	8082                	ret

00000000800031b8 <idup>:
{
    800031b8:	1101                	addi	sp,sp,-32
    800031ba:	ec06                	sd	ra,24(sp)
    800031bc:	e822                	sd	s0,16(sp)
    800031be:	e426                	sd	s1,8(sp)
    800031c0:	1000                	addi	s0,sp,32
    800031c2:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    800031c4:	0001d517          	auipc	a0,0x1d
    800031c8:	74c50513          	addi	a0,a0,1868 # 80020910 <itable>
    800031cc:	a03fd0ef          	jal	80000bce <acquire>
  ip->ref++;
    800031d0:	449c                	lw	a5,8(s1)
    800031d2:	2785                	addiw	a5,a5,1
    800031d4:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    800031d6:	0001d517          	auipc	a0,0x1d
    800031da:	73a50513          	addi	a0,a0,1850 # 80020910 <itable>
    800031de:	a89fd0ef          	jal	80000c66 <release>
}
    800031e2:	8526                	mv	a0,s1
    800031e4:	60e2                	ld	ra,24(sp)
    800031e6:	6442                	ld	s0,16(sp)
    800031e8:	64a2                	ld	s1,8(sp)
    800031ea:	6105                	addi	sp,sp,32
    800031ec:	8082                	ret

00000000800031ee <ilock>:
{
    800031ee:	1101                	addi	sp,sp,-32
    800031f0:	ec06                	sd	ra,24(sp)
    800031f2:	e822                	sd	s0,16(sp)
    800031f4:	e426                	sd	s1,8(sp)
    800031f6:	1000                	addi	s0,sp,32
  if(ip == 0 || ip->ref < 1)
    800031f8:	cd19                	beqz	a0,80003216 <ilock+0x28>
    800031fa:	84aa                	mv	s1,a0
    800031fc:	451c                	lw	a5,8(a0)
    800031fe:	00f05c63          	blez	a5,80003216 <ilock+0x28>
  acquiresleep(&ip->lock);
    80003202:	0541                	addi	a0,a0,16
    80003204:	451000ef          	jal	80003e54 <acquiresleep>
  if(ip->valid == 0){
    80003208:	40bc                	lw	a5,64(s1)
    8000320a:	cf89                	beqz	a5,80003224 <ilock+0x36>
}
    8000320c:	60e2                	ld	ra,24(sp)
    8000320e:	6442                	ld	s0,16(sp)
    80003210:	64a2                	ld	s1,8(sp)
    80003212:	6105                	addi	sp,sp,32
    80003214:	8082                	ret
    80003216:	e04a                	sd	s2,0(sp)
    panic("ilock");
    80003218:	00004517          	auipc	a0,0x4
    8000321c:	23050513          	addi	a0,a0,560 # 80007448 <etext+0x448>
    80003220:	dc0fd0ef          	jal	800007e0 <panic>
    80003224:	e04a                	sd	s2,0(sp)
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
    80003226:	40dc                	lw	a5,4(s1)
    80003228:	0047d79b          	srliw	a5,a5,0x4
    8000322c:	0001d597          	auipc	a1,0x1d
    80003230:	6dc5a583          	lw	a1,1756(a1) # 80020908 <sb+0x18>
    80003234:	9dbd                	addw	a1,a1,a5
    80003236:	4088                	lw	a0,0(s1)
    80003238:	8ebff0ef          	jal	80002b22 <bread>
    8000323c:	892a                	mv	s2,a0
    dip = (struct dinode*)bp->data + ip->inum%IPB;
    8000323e:	05850593          	addi	a1,a0,88
    80003242:	40dc                	lw	a5,4(s1)
    80003244:	8bbd                	andi	a5,a5,15
    80003246:	079a                	slli	a5,a5,0x6
    80003248:	95be                	add	a1,a1,a5
    ip->type = dip->type;
    8000324a:	00059783          	lh	a5,0(a1)
    8000324e:	04f49223          	sh	a5,68(s1)
    ip->major = dip->major;
    80003252:	00259783          	lh	a5,2(a1)
    80003256:	04f49323          	sh	a5,70(s1)
    ip->minor = dip->minor;
    8000325a:	00459783          	lh	a5,4(a1)
    8000325e:	04f49423          	sh	a5,72(s1)
    ip->nlink = dip->nlink;
    80003262:	00659783          	lh	a5,6(a1)
    80003266:	04f49523          	sh	a5,74(s1)
    ip->size = dip->size;
    8000326a:	459c                	lw	a5,8(a1)
    8000326c:	c4fc                	sw	a5,76(s1)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
    8000326e:	03400613          	li	a2,52
    80003272:	05b1                	addi	a1,a1,12
    80003274:	05048513          	addi	a0,s1,80
    80003278:	a87fd0ef          	jal	80000cfe <memmove>
    brelse(bp);
    8000327c:	854a                	mv	a0,s2
    8000327e:	9adff0ef          	jal	80002c2a <brelse>
    ip->valid = 1;
    80003282:	4785                	li	a5,1
    80003284:	c0bc                	sw	a5,64(s1)
    if(ip->type == 0)
    80003286:	04449783          	lh	a5,68(s1)
    8000328a:	c399                	beqz	a5,80003290 <ilock+0xa2>
    8000328c:	6902                	ld	s2,0(sp)
    8000328e:	bfbd                	j	8000320c <ilock+0x1e>
      panic("ilock: no type");
    80003290:	00004517          	auipc	a0,0x4
    80003294:	1c050513          	addi	a0,a0,448 # 80007450 <etext+0x450>
    80003298:	d48fd0ef          	jal	800007e0 <panic>

000000008000329c <iunlock>:
{
    8000329c:	1101                	addi	sp,sp,-32
    8000329e:	ec06                	sd	ra,24(sp)
    800032a0:	e822                	sd	s0,16(sp)
    800032a2:	e426                	sd	s1,8(sp)
    800032a4:	e04a                	sd	s2,0(sp)
    800032a6:	1000                	addi	s0,sp,32
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
    800032a8:	c505                	beqz	a0,800032d0 <iunlock+0x34>
    800032aa:	84aa                	mv	s1,a0
    800032ac:	01050913          	addi	s2,a0,16
    800032b0:	854a                	mv	a0,s2
    800032b2:	421000ef          	jal	80003ed2 <holdingsleep>
    800032b6:	cd09                	beqz	a0,800032d0 <iunlock+0x34>
    800032b8:	449c                	lw	a5,8(s1)
    800032ba:	00f05b63          	blez	a5,800032d0 <iunlock+0x34>
  releasesleep(&ip->lock);
    800032be:	854a                	mv	a0,s2
    800032c0:	3db000ef          	jal	80003e9a <releasesleep>
}
    800032c4:	60e2                	ld	ra,24(sp)
    800032c6:	6442                	ld	s0,16(sp)
    800032c8:	64a2                	ld	s1,8(sp)
    800032ca:	6902                	ld	s2,0(sp)
    800032cc:	6105                	addi	sp,sp,32
    800032ce:	8082                	ret
    panic("iunlock");
    800032d0:	00004517          	auipc	a0,0x4
    800032d4:	19050513          	addi	a0,a0,400 # 80007460 <etext+0x460>
    800032d8:	d08fd0ef          	jal	800007e0 <panic>

00000000800032dc <itrunc>:

// Truncate inode (discard contents).
// Caller must hold ip->lock.
void
itrunc(struct inode *ip)
{
    800032dc:	7179                	addi	sp,sp,-48
    800032de:	f406                	sd	ra,40(sp)
    800032e0:	f022                	sd	s0,32(sp)
    800032e2:	ec26                	sd	s1,24(sp)
    800032e4:	e84a                	sd	s2,16(sp)
    800032e6:	e44e                	sd	s3,8(sp)
    800032e8:	1800                	addi	s0,sp,48
    800032ea:	89aa                	mv	s3,a0
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
    800032ec:	05050493          	addi	s1,a0,80
    800032f0:	08050913          	addi	s2,a0,128
    800032f4:	a021                	j	800032fc <itrunc+0x20>
    800032f6:	0491                	addi	s1,s1,4
    800032f8:	01248b63          	beq	s1,s2,8000330e <itrunc+0x32>
    if(ip->addrs[i]){
    800032fc:	408c                	lw	a1,0(s1)
    800032fe:	dde5                	beqz	a1,800032f6 <itrunc+0x1a>
      bfree(ip->dev, ip->addrs[i]);
    80003300:	0009a503          	lw	a0,0(s3)
    80003304:	a17ff0ef          	jal	80002d1a <bfree>
      ip->addrs[i] = 0;
    80003308:	0004a023          	sw	zero,0(s1)
    8000330c:	b7ed                	j	800032f6 <itrunc+0x1a>
    }
  }

  if(ip->addrs[NDIRECT]){
    8000330e:	0809a583          	lw	a1,128(s3)
    80003312:	ed89                	bnez	a1,8000332c <itrunc+0x50>
    brelse(bp);
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
    80003314:	0409a623          	sw	zero,76(s3)
  iupdate(ip);
    80003318:	854e                	mv	a0,s3
    8000331a:	e21ff0ef          	jal	8000313a <iupdate>
}
    8000331e:	70a2                	ld	ra,40(sp)
    80003320:	7402                	ld	s0,32(sp)
    80003322:	64e2                	ld	s1,24(sp)
    80003324:	6942                	ld	s2,16(sp)
    80003326:	69a2                	ld	s3,8(sp)
    80003328:	6145                	addi	sp,sp,48
    8000332a:	8082                	ret
    8000332c:	e052                	sd	s4,0(sp)
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    8000332e:	0009a503          	lw	a0,0(s3)
    80003332:	ff0ff0ef          	jal	80002b22 <bread>
    80003336:	8a2a                	mv	s4,a0
    for(j = 0; j < NINDIRECT; j++){
    80003338:	05850493          	addi	s1,a0,88
    8000333c:	45850913          	addi	s2,a0,1112
    80003340:	a021                	j	80003348 <itrunc+0x6c>
    80003342:	0491                	addi	s1,s1,4
    80003344:	01248963          	beq	s1,s2,80003356 <itrunc+0x7a>
      if(a[j])
    80003348:	408c                	lw	a1,0(s1)
    8000334a:	dde5                	beqz	a1,80003342 <itrunc+0x66>
        bfree(ip->dev, a[j]);
    8000334c:	0009a503          	lw	a0,0(s3)
    80003350:	9cbff0ef          	jal	80002d1a <bfree>
    80003354:	b7fd                	j	80003342 <itrunc+0x66>
    brelse(bp);
    80003356:	8552                	mv	a0,s4
    80003358:	8d3ff0ef          	jal	80002c2a <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    8000335c:	0809a583          	lw	a1,128(s3)
    80003360:	0009a503          	lw	a0,0(s3)
    80003364:	9b7ff0ef          	jal	80002d1a <bfree>
    ip->addrs[NDIRECT] = 0;
    80003368:	0809a023          	sw	zero,128(s3)
    8000336c:	6a02                	ld	s4,0(sp)
    8000336e:	b75d                	j	80003314 <itrunc+0x38>

0000000080003370 <iput>:
{
    80003370:	1101                	addi	sp,sp,-32
    80003372:	ec06                	sd	ra,24(sp)
    80003374:	e822                	sd	s0,16(sp)
    80003376:	e426                	sd	s1,8(sp)
    80003378:	1000                	addi	s0,sp,32
    8000337a:	84aa                	mv	s1,a0
  acquire(&itable.lock);
    8000337c:	0001d517          	auipc	a0,0x1d
    80003380:	59450513          	addi	a0,a0,1428 # 80020910 <itable>
    80003384:	84bfd0ef          	jal	80000bce <acquire>
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    80003388:	4498                	lw	a4,8(s1)
    8000338a:	4785                	li	a5,1
    8000338c:	02f70063          	beq	a4,a5,800033ac <iput+0x3c>
  ip->ref--;
    80003390:	449c                	lw	a5,8(s1)
    80003392:	37fd                	addiw	a5,a5,-1
    80003394:	c49c                	sw	a5,8(s1)
  release(&itable.lock);
    80003396:	0001d517          	auipc	a0,0x1d
    8000339a:	57a50513          	addi	a0,a0,1402 # 80020910 <itable>
    8000339e:	8c9fd0ef          	jal	80000c66 <release>
}
    800033a2:	60e2                	ld	ra,24(sp)
    800033a4:	6442                	ld	s0,16(sp)
    800033a6:	64a2                	ld	s1,8(sp)
    800033a8:	6105                	addi	sp,sp,32
    800033aa:	8082                	ret
  if(ip->ref == 1 && ip->valid && ip->nlink == 0){
    800033ac:	40bc                	lw	a5,64(s1)
    800033ae:	d3ed                	beqz	a5,80003390 <iput+0x20>
    800033b0:	04a49783          	lh	a5,74(s1)
    800033b4:	fff1                	bnez	a5,80003390 <iput+0x20>
    800033b6:	e04a                	sd	s2,0(sp)
    acquiresleep(&ip->lock);
    800033b8:	01048913          	addi	s2,s1,16
    800033bc:	854a                	mv	a0,s2
    800033be:	297000ef          	jal	80003e54 <acquiresleep>
    release(&itable.lock);
    800033c2:	0001d517          	auipc	a0,0x1d
    800033c6:	54e50513          	addi	a0,a0,1358 # 80020910 <itable>
    800033ca:	89dfd0ef          	jal	80000c66 <release>
    itrunc(ip);
    800033ce:	8526                	mv	a0,s1
    800033d0:	f0dff0ef          	jal	800032dc <itrunc>
    ip->type = 0;
    800033d4:	04049223          	sh	zero,68(s1)
    iupdate(ip);
    800033d8:	8526                	mv	a0,s1
    800033da:	d61ff0ef          	jal	8000313a <iupdate>
    ip->valid = 0;
    800033de:	0404a023          	sw	zero,64(s1)
    releasesleep(&ip->lock);
    800033e2:	854a                	mv	a0,s2
    800033e4:	2b7000ef          	jal	80003e9a <releasesleep>
    acquire(&itable.lock);
    800033e8:	0001d517          	auipc	a0,0x1d
    800033ec:	52850513          	addi	a0,a0,1320 # 80020910 <itable>
    800033f0:	fdefd0ef          	jal	80000bce <acquire>
    800033f4:	6902                	ld	s2,0(sp)
    800033f6:	bf69                	j	80003390 <iput+0x20>

00000000800033f8 <iunlockput>:
{
    800033f8:	1101                	addi	sp,sp,-32
    800033fa:	ec06                	sd	ra,24(sp)
    800033fc:	e822                	sd	s0,16(sp)
    800033fe:	e426                	sd	s1,8(sp)
    80003400:	1000                	addi	s0,sp,32
    80003402:	84aa                	mv	s1,a0
  iunlock(ip);
    80003404:	e99ff0ef          	jal	8000329c <iunlock>
  iput(ip);
    80003408:	8526                	mv	a0,s1
    8000340a:	f67ff0ef          	jal	80003370 <iput>
}
    8000340e:	60e2                	ld	ra,24(sp)
    80003410:	6442                	ld	s0,16(sp)
    80003412:	64a2                	ld	s1,8(sp)
    80003414:	6105                	addi	sp,sp,32
    80003416:	8082                	ret

0000000080003418 <ireclaim>:
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003418:	0001d717          	auipc	a4,0x1d
    8000341c:	4e472703          	lw	a4,1252(a4) # 800208fc <sb+0xc>
    80003420:	4785                	li	a5,1
    80003422:	0ae7ff63          	bgeu	a5,a4,800034e0 <ireclaim+0xc8>
{
    80003426:	7139                	addi	sp,sp,-64
    80003428:	fc06                	sd	ra,56(sp)
    8000342a:	f822                	sd	s0,48(sp)
    8000342c:	f426                	sd	s1,40(sp)
    8000342e:	f04a                	sd	s2,32(sp)
    80003430:	ec4e                	sd	s3,24(sp)
    80003432:	e852                	sd	s4,16(sp)
    80003434:	e456                	sd	s5,8(sp)
    80003436:	e05a                	sd	s6,0(sp)
    80003438:	0080                	addi	s0,sp,64
  for (int inum = 1; inum < sb.ninodes; inum++) {
    8000343a:	4485                	li	s1,1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    8000343c:	00050a1b          	sext.w	s4,a0
    80003440:	0001da97          	auipc	s5,0x1d
    80003444:	4b0a8a93          	addi	s5,s5,1200 # 800208f0 <sb>
      printf("ireclaim: orphaned inode %d\n", inum);
    80003448:	00004b17          	auipc	s6,0x4
    8000344c:	020b0b13          	addi	s6,s6,32 # 80007468 <etext+0x468>
    80003450:	a099                	j	80003496 <ireclaim+0x7e>
    80003452:	85ce                	mv	a1,s3
    80003454:	855a                	mv	a0,s6
    80003456:	8a4fd0ef          	jal	800004fa <printf>
      ip = iget(dev, inum);
    8000345a:	85ce                	mv	a1,s3
    8000345c:	8552                	mv	a0,s4
    8000345e:	b1dff0ef          	jal	80002f7a <iget>
    80003462:	89aa                	mv	s3,a0
    brelse(bp);
    80003464:	854a                	mv	a0,s2
    80003466:	fc4ff0ef          	jal	80002c2a <brelse>
    if (ip) {
    8000346a:	00098f63          	beqz	s3,80003488 <ireclaim+0x70>
      begin_op();
    8000346e:	76a000ef          	jal	80003bd8 <begin_op>
      ilock(ip);
    80003472:	854e                	mv	a0,s3
    80003474:	d7bff0ef          	jal	800031ee <ilock>
      iunlock(ip);
    80003478:	854e                	mv	a0,s3
    8000347a:	e23ff0ef          	jal	8000329c <iunlock>
      iput(ip);
    8000347e:	854e                	mv	a0,s3
    80003480:	ef1ff0ef          	jal	80003370 <iput>
      end_op();
    80003484:	7be000ef          	jal	80003c42 <end_op>
  for (int inum = 1; inum < sb.ninodes; inum++) {
    80003488:	0485                	addi	s1,s1,1
    8000348a:	00caa703          	lw	a4,12(s5)
    8000348e:	0004879b          	sext.w	a5,s1
    80003492:	02e7fd63          	bgeu	a5,a4,800034cc <ireclaim+0xb4>
    80003496:	0004899b          	sext.w	s3,s1
    struct buf *bp = bread(dev, IBLOCK(inum, sb));
    8000349a:	0044d593          	srli	a1,s1,0x4
    8000349e:	018aa783          	lw	a5,24(s5)
    800034a2:	9dbd                	addw	a1,a1,a5
    800034a4:	8552                	mv	a0,s4
    800034a6:	e7cff0ef          	jal	80002b22 <bread>
    800034aa:	892a                	mv	s2,a0
    struct dinode *dip = (struct dinode *)bp->data + inum % IPB;
    800034ac:	05850793          	addi	a5,a0,88
    800034b0:	00f9f713          	andi	a4,s3,15
    800034b4:	071a                	slli	a4,a4,0x6
    800034b6:	97ba                	add	a5,a5,a4
    if (dip->type != 0 && dip->nlink == 0) {  // is an orphaned inode
    800034b8:	00079703          	lh	a4,0(a5)
    800034bc:	c701                	beqz	a4,800034c4 <ireclaim+0xac>
    800034be:	00679783          	lh	a5,6(a5)
    800034c2:	dbc1                	beqz	a5,80003452 <ireclaim+0x3a>
    brelse(bp);
    800034c4:	854a                	mv	a0,s2
    800034c6:	f64ff0ef          	jal	80002c2a <brelse>
    if (ip) {
    800034ca:	bf7d                	j	80003488 <ireclaim+0x70>
}
    800034cc:	70e2                	ld	ra,56(sp)
    800034ce:	7442                	ld	s0,48(sp)
    800034d0:	74a2                	ld	s1,40(sp)
    800034d2:	7902                	ld	s2,32(sp)
    800034d4:	69e2                	ld	s3,24(sp)
    800034d6:	6a42                	ld	s4,16(sp)
    800034d8:	6aa2                	ld	s5,8(sp)
    800034da:	6b02                	ld	s6,0(sp)
    800034dc:	6121                	addi	sp,sp,64
    800034de:	8082                	ret
    800034e0:	8082                	ret

00000000800034e2 <fsinit>:
fsinit(int dev) {
    800034e2:	7179                	addi	sp,sp,-48
    800034e4:	f406                	sd	ra,40(sp)
    800034e6:	f022                	sd	s0,32(sp)
    800034e8:	ec26                	sd	s1,24(sp)
    800034ea:	e84a                	sd	s2,16(sp)
    800034ec:	e44e                	sd	s3,8(sp)
    800034ee:	1800                	addi	s0,sp,48
    800034f0:	84aa                	mv	s1,a0
  bp = bread(dev, 1);
    800034f2:	4585                	li	a1,1
    800034f4:	e2eff0ef          	jal	80002b22 <bread>
    800034f8:	892a                	mv	s2,a0
  memmove(sb, bp->data, sizeof(*sb));
    800034fa:	0001d997          	auipc	s3,0x1d
    800034fe:	3f698993          	addi	s3,s3,1014 # 800208f0 <sb>
    80003502:	02000613          	li	a2,32
    80003506:	05850593          	addi	a1,a0,88
    8000350a:	854e                	mv	a0,s3
    8000350c:	ff2fd0ef          	jal	80000cfe <memmove>
  brelse(bp);
    80003510:	854a                	mv	a0,s2
    80003512:	f18ff0ef          	jal	80002c2a <brelse>
  if(sb.magic != FSMAGIC)
    80003516:	0009a703          	lw	a4,0(s3)
    8000351a:	102037b7          	lui	a5,0x10203
    8000351e:	04078793          	addi	a5,a5,64 # 10203040 <_entry-0x6fdfcfc0>
    80003522:	02f71363          	bne	a4,a5,80003548 <fsinit+0x66>
  initlog(dev, &sb);
    80003526:	0001d597          	auipc	a1,0x1d
    8000352a:	3ca58593          	addi	a1,a1,970 # 800208f0 <sb>
    8000352e:	8526                	mv	a0,s1
    80003530:	62a000ef          	jal	80003b5a <initlog>
  ireclaim(dev);
    80003534:	8526                	mv	a0,s1
    80003536:	ee3ff0ef          	jal	80003418 <ireclaim>
}
    8000353a:	70a2                	ld	ra,40(sp)
    8000353c:	7402                	ld	s0,32(sp)
    8000353e:	64e2                	ld	s1,24(sp)
    80003540:	6942                	ld	s2,16(sp)
    80003542:	69a2                	ld	s3,8(sp)
    80003544:	6145                	addi	sp,sp,48
    80003546:	8082                	ret
    panic("invalid file system");
    80003548:	00004517          	auipc	a0,0x4
    8000354c:	f4050513          	addi	a0,a0,-192 # 80007488 <etext+0x488>
    80003550:	a90fd0ef          	jal	800007e0 <panic>

0000000080003554 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
    80003554:	1141                	addi	sp,sp,-16
    80003556:	e422                	sd	s0,8(sp)
    80003558:	0800                	addi	s0,sp,16
  st->dev = ip->dev;
    8000355a:	411c                	lw	a5,0(a0)
    8000355c:	c19c                	sw	a5,0(a1)
  st->ino = ip->inum;
    8000355e:	415c                	lw	a5,4(a0)
    80003560:	c1dc                	sw	a5,4(a1)
  st->type = ip->type;
    80003562:	04451783          	lh	a5,68(a0)
    80003566:	00f59423          	sh	a5,8(a1)
  st->nlink = ip->nlink;
    8000356a:	04a51783          	lh	a5,74(a0)
    8000356e:	00f59523          	sh	a5,10(a1)
  st->size = ip->size;
    80003572:	04c56783          	lwu	a5,76(a0)
    80003576:	e99c                	sd	a5,16(a1)
}
    80003578:	6422                	ld	s0,8(sp)
    8000357a:	0141                	addi	sp,sp,16
    8000357c:	8082                	ret

000000008000357e <readi>:
readi(struct inode *ip, int user_dst, uint64 dst, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    8000357e:	457c                	lw	a5,76(a0)
    80003580:	0ed7eb63          	bltu	a5,a3,80003676 <readi+0xf8>
{
    80003584:	7159                	addi	sp,sp,-112
    80003586:	f486                	sd	ra,104(sp)
    80003588:	f0a2                	sd	s0,96(sp)
    8000358a:	eca6                	sd	s1,88(sp)
    8000358c:	e0d2                	sd	s4,64(sp)
    8000358e:	fc56                	sd	s5,56(sp)
    80003590:	f85a                	sd	s6,48(sp)
    80003592:	f45e                	sd	s7,40(sp)
    80003594:	1880                	addi	s0,sp,112
    80003596:	8b2a                	mv	s6,a0
    80003598:	8bae                	mv	s7,a1
    8000359a:	8a32                	mv	s4,a2
    8000359c:	84b6                	mv	s1,a3
    8000359e:	8aba                	mv	s5,a4
  if(off > ip->size || off + n < off)
    800035a0:	9f35                	addw	a4,a4,a3
    return 0;
    800035a2:	4501                	li	a0,0
  if(off > ip->size || off + n < off)
    800035a4:	0cd76063          	bltu	a4,a3,80003664 <readi+0xe6>
    800035a8:	e4ce                	sd	s3,72(sp)
  if(off + n > ip->size)
    800035aa:	00e7f463          	bgeu	a5,a4,800035b2 <readi+0x34>
    n = ip->size - off;
    800035ae:	40d78abb          	subw	s5,a5,a3

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    800035b2:	080a8f63          	beqz	s5,80003650 <readi+0xd2>
    800035b6:	e8ca                	sd	s2,80(sp)
    800035b8:	f062                	sd	s8,32(sp)
    800035ba:	ec66                	sd	s9,24(sp)
    800035bc:	e86a                	sd	s10,16(sp)
    800035be:	e46e                	sd	s11,8(sp)
    800035c0:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    800035c2:	40000c93          	li	s9,1024
    if(either_copyout(user_dst, dst, bp->data + (off % BSIZE), m) == -1) {
    800035c6:	5c7d                	li	s8,-1
    800035c8:	a80d                	j	800035fa <readi+0x7c>
    800035ca:	020d1d93          	slli	s11,s10,0x20
    800035ce:	020ddd93          	srli	s11,s11,0x20
    800035d2:	05890613          	addi	a2,s2,88
    800035d6:	86ee                	mv	a3,s11
    800035d8:	963a                	add	a2,a2,a4
    800035da:	85d2                	mv	a1,s4
    800035dc:	855e                	mv	a0,s7
    800035de:	c63fe0ef          	jal	80002240 <either_copyout>
    800035e2:	05850763          	beq	a0,s8,80003630 <readi+0xb2>
      brelse(bp);
      tot = -1;
      break;
    }
    brelse(bp);
    800035e6:	854a                	mv	a0,s2
    800035e8:	e42ff0ef          	jal	80002c2a <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    800035ec:	013d09bb          	addw	s3,s10,s3
    800035f0:	009d04bb          	addw	s1,s10,s1
    800035f4:	9a6e                	add	s4,s4,s11
    800035f6:	0559f763          	bgeu	s3,s5,80003644 <readi+0xc6>
    uint addr = bmap(ip, off/BSIZE);
    800035fa:	00a4d59b          	srliw	a1,s1,0xa
    800035fe:	855a                	mv	a0,s6
    80003600:	8a7ff0ef          	jal	80002ea6 <bmap>
    80003604:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    80003608:	c5b1                	beqz	a1,80003654 <readi+0xd6>
    bp = bread(ip->dev, addr);
    8000360a:	000b2503          	lw	a0,0(s6)
    8000360e:	d14ff0ef          	jal	80002b22 <bread>
    80003612:	892a                	mv	s2,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003614:	3ff4f713          	andi	a4,s1,1023
    80003618:	40ec87bb          	subw	a5,s9,a4
    8000361c:	413a86bb          	subw	a3,s5,s3
    80003620:	8d3e                	mv	s10,a5
    80003622:	2781                	sext.w	a5,a5
    80003624:	0006861b          	sext.w	a2,a3
    80003628:	faf671e3          	bgeu	a2,a5,800035ca <readi+0x4c>
    8000362c:	8d36                	mv	s10,a3
    8000362e:	bf71                	j	800035ca <readi+0x4c>
      brelse(bp);
    80003630:	854a                	mv	a0,s2
    80003632:	df8ff0ef          	jal	80002c2a <brelse>
      tot = -1;
    80003636:	59fd                	li	s3,-1
      break;
    80003638:	6946                	ld	s2,80(sp)
    8000363a:	7c02                	ld	s8,32(sp)
    8000363c:	6ce2                	ld	s9,24(sp)
    8000363e:	6d42                	ld	s10,16(sp)
    80003640:	6da2                	ld	s11,8(sp)
    80003642:	a831                	j	8000365e <readi+0xe0>
    80003644:	6946                	ld	s2,80(sp)
    80003646:	7c02                	ld	s8,32(sp)
    80003648:	6ce2                	ld	s9,24(sp)
    8000364a:	6d42                	ld	s10,16(sp)
    8000364c:	6da2                	ld	s11,8(sp)
    8000364e:	a801                	j	8000365e <readi+0xe0>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    80003650:	89d6                	mv	s3,s5
    80003652:	a031                	j	8000365e <readi+0xe0>
    80003654:	6946                	ld	s2,80(sp)
    80003656:	7c02                	ld	s8,32(sp)
    80003658:	6ce2                	ld	s9,24(sp)
    8000365a:	6d42                	ld	s10,16(sp)
    8000365c:	6da2                	ld	s11,8(sp)
  }
  return tot;
    8000365e:	0009851b          	sext.w	a0,s3
    80003662:	69a6                	ld	s3,72(sp)
}
    80003664:	70a6                	ld	ra,104(sp)
    80003666:	7406                	ld	s0,96(sp)
    80003668:	64e6                	ld	s1,88(sp)
    8000366a:	6a06                	ld	s4,64(sp)
    8000366c:	7ae2                	ld	s5,56(sp)
    8000366e:	7b42                	ld	s6,48(sp)
    80003670:	7ba2                	ld	s7,40(sp)
    80003672:	6165                	addi	sp,sp,112
    80003674:	8082                	ret
    return 0;
    80003676:	4501                	li	a0,0
}
    80003678:	8082                	ret

000000008000367a <writei>:
writei(struct inode *ip, int user_src, uint64 src, uint off, uint n)
{
  uint tot, m;
  struct buf *bp;

  if(off > ip->size || off + n < off)
    8000367a:	457c                	lw	a5,76(a0)
    8000367c:	10d7e063          	bltu	a5,a3,8000377c <writei+0x102>
{
    80003680:	7159                	addi	sp,sp,-112
    80003682:	f486                	sd	ra,104(sp)
    80003684:	f0a2                	sd	s0,96(sp)
    80003686:	e8ca                	sd	s2,80(sp)
    80003688:	e0d2                	sd	s4,64(sp)
    8000368a:	fc56                	sd	s5,56(sp)
    8000368c:	f85a                	sd	s6,48(sp)
    8000368e:	f45e                	sd	s7,40(sp)
    80003690:	1880                	addi	s0,sp,112
    80003692:	8aaa                	mv	s5,a0
    80003694:	8bae                	mv	s7,a1
    80003696:	8a32                	mv	s4,a2
    80003698:	8936                	mv	s2,a3
    8000369a:	8b3a                	mv	s6,a4
  if(off > ip->size || off + n < off)
    8000369c:	00e687bb          	addw	a5,a3,a4
    800036a0:	0ed7e063          	bltu	a5,a3,80003780 <writei+0x106>
    return -1;
  if(off + n > MAXFILE*BSIZE)
    800036a4:	00043737          	lui	a4,0x43
    800036a8:	0cf76e63          	bltu	a4,a5,80003784 <writei+0x10a>
    800036ac:	e4ce                	sd	s3,72(sp)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    800036ae:	0a0b0f63          	beqz	s6,8000376c <writei+0xf2>
    800036b2:	eca6                	sd	s1,88(sp)
    800036b4:	f062                	sd	s8,32(sp)
    800036b6:	ec66                	sd	s9,24(sp)
    800036b8:	e86a                	sd	s10,16(sp)
    800036ba:	e46e                	sd	s11,8(sp)
    800036bc:	4981                	li	s3,0
    uint addr = bmap(ip, off/BSIZE);
    if(addr == 0)
      break;
    bp = bread(ip->dev, addr);
    m = min(n - tot, BSIZE - off%BSIZE);
    800036be:	40000c93          	li	s9,1024
    if(either_copyin(bp->data + (off % BSIZE), user_src, src, m) == -1) {
    800036c2:	5c7d                	li	s8,-1
    800036c4:	a825                	j	800036fc <writei+0x82>
    800036c6:	020d1d93          	slli	s11,s10,0x20
    800036ca:	020ddd93          	srli	s11,s11,0x20
    800036ce:	05848513          	addi	a0,s1,88
    800036d2:	86ee                	mv	a3,s11
    800036d4:	8652                	mv	a2,s4
    800036d6:	85de                	mv	a1,s7
    800036d8:	953a                	add	a0,a0,a4
    800036da:	bb1fe0ef          	jal	8000228a <either_copyin>
    800036de:	05850a63          	beq	a0,s8,80003732 <writei+0xb8>
      brelse(bp);
      break;
    }
    log_write(bp);
    800036e2:	8526                	mv	a0,s1
    800036e4:	678000ef          	jal	80003d5c <log_write>
    brelse(bp);
    800036e8:	8526                	mv	a0,s1
    800036ea:	d40ff0ef          	jal	80002c2a <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    800036ee:	013d09bb          	addw	s3,s10,s3
    800036f2:	012d093b          	addw	s2,s10,s2
    800036f6:	9a6e                	add	s4,s4,s11
    800036f8:	0569f063          	bgeu	s3,s6,80003738 <writei+0xbe>
    uint addr = bmap(ip, off/BSIZE);
    800036fc:	00a9559b          	srliw	a1,s2,0xa
    80003700:	8556                	mv	a0,s5
    80003702:	fa4ff0ef          	jal	80002ea6 <bmap>
    80003706:	0005059b          	sext.w	a1,a0
    if(addr == 0)
    8000370a:	c59d                	beqz	a1,80003738 <writei+0xbe>
    bp = bread(ip->dev, addr);
    8000370c:	000aa503          	lw	a0,0(s5)
    80003710:	c12ff0ef          	jal	80002b22 <bread>
    80003714:	84aa                	mv	s1,a0
    m = min(n - tot, BSIZE - off%BSIZE);
    80003716:	3ff97713          	andi	a4,s2,1023
    8000371a:	40ec87bb          	subw	a5,s9,a4
    8000371e:	413b06bb          	subw	a3,s6,s3
    80003722:	8d3e                	mv	s10,a5
    80003724:	2781                	sext.w	a5,a5
    80003726:	0006861b          	sext.w	a2,a3
    8000372a:	f8f67ee3          	bgeu	a2,a5,800036c6 <writei+0x4c>
    8000372e:	8d36                	mv	s10,a3
    80003730:	bf59                	j	800036c6 <writei+0x4c>
      brelse(bp);
    80003732:	8526                	mv	a0,s1
    80003734:	cf6ff0ef          	jal	80002c2a <brelse>
  }

  if(off > ip->size)
    80003738:	04caa783          	lw	a5,76(s5)
    8000373c:	0327fa63          	bgeu	a5,s2,80003770 <writei+0xf6>
    ip->size = off;
    80003740:	052aa623          	sw	s2,76(s5)
    80003744:	64e6                	ld	s1,88(sp)
    80003746:	7c02                	ld	s8,32(sp)
    80003748:	6ce2                	ld	s9,24(sp)
    8000374a:	6d42                	ld	s10,16(sp)
    8000374c:	6da2                	ld	s11,8(sp)

  // write the i-node back to disk even if the size didn't change
  // because the loop above might have called bmap() and added a new
  // block to ip->addrs[].
  iupdate(ip);
    8000374e:	8556                	mv	a0,s5
    80003750:	9ebff0ef          	jal	8000313a <iupdate>

  return tot;
    80003754:	0009851b          	sext.w	a0,s3
    80003758:	69a6                	ld	s3,72(sp)
}
    8000375a:	70a6                	ld	ra,104(sp)
    8000375c:	7406                	ld	s0,96(sp)
    8000375e:	6946                	ld	s2,80(sp)
    80003760:	6a06                	ld	s4,64(sp)
    80003762:	7ae2                	ld	s5,56(sp)
    80003764:	7b42                	ld	s6,48(sp)
    80003766:	7ba2                	ld	s7,40(sp)
    80003768:	6165                	addi	sp,sp,112
    8000376a:	8082                	ret
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    8000376c:	89da                	mv	s3,s6
    8000376e:	b7c5                	j	8000374e <writei+0xd4>
    80003770:	64e6                	ld	s1,88(sp)
    80003772:	7c02                	ld	s8,32(sp)
    80003774:	6ce2                	ld	s9,24(sp)
    80003776:	6d42                	ld	s10,16(sp)
    80003778:	6da2                	ld	s11,8(sp)
    8000377a:	bfd1                	j	8000374e <writei+0xd4>
    return -1;
    8000377c:	557d                	li	a0,-1
}
    8000377e:	8082                	ret
    return -1;
    80003780:	557d                	li	a0,-1
    80003782:	bfe1                	j	8000375a <writei+0xe0>
    return -1;
    80003784:	557d                	li	a0,-1
    80003786:	bfd1                	j	8000375a <writei+0xe0>

0000000080003788 <namecmp>:

// Directories

int
namecmp(const char *s, const char *t)
{
    80003788:	1141                	addi	sp,sp,-16
    8000378a:	e406                	sd	ra,8(sp)
    8000378c:	e022                	sd	s0,0(sp)
    8000378e:	0800                	addi	s0,sp,16
  return strncmp(s, t, DIRSIZ);
    80003790:	4639                	li	a2,14
    80003792:	ddcfd0ef          	jal	80000d6e <strncmp>
}
    80003796:	60a2                	ld	ra,8(sp)
    80003798:	6402                	ld	s0,0(sp)
    8000379a:	0141                	addi	sp,sp,16
    8000379c:	8082                	ret

000000008000379e <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
    8000379e:	7139                	addi	sp,sp,-64
    800037a0:	fc06                	sd	ra,56(sp)
    800037a2:	f822                	sd	s0,48(sp)
    800037a4:	f426                	sd	s1,40(sp)
    800037a6:	f04a                	sd	s2,32(sp)
    800037a8:	ec4e                	sd	s3,24(sp)
    800037aa:	e852                	sd	s4,16(sp)
    800037ac:	0080                	addi	s0,sp,64
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
    800037ae:	04451703          	lh	a4,68(a0)
    800037b2:	4785                	li	a5,1
    800037b4:	00f71a63          	bne	a4,a5,800037c8 <dirlookup+0x2a>
    800037b8:	892a                	mv	s2,a0
    800037ba:	89ae                	mv	s3,a1
    800037bc:	8a32                	mv	s4,a2
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
    800037be:	457c                	lw	a5,76(a0)
    800037c0:	4481                	li	s1,0
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
    800037c2:	4501                	li	a0,0
  for(off = 0; off < dp->size; off += sizeof(de)){
    800037c4:	e39d                	bnez	a5,800037ea <dirlookup+0x4c>
    800037c6:	a095                	j	8000382a <dirlookup+0x8c>
    panic("dirlookup not DIR");
    800037c8:	00004517          	auipc	a0,0x4
    800037cc:	cd850513          	addi	a0,a0,-808 # 800074a0 <etext+0x4a0>
    800037d0:	810fd0ef          	jal	800007e0 <panic>
      panic("dirlookup read");
    800037d4:	00004517          	auipc	a0,0x4
    800037d8:	ce450513          	addi	a0,a0,-796 # 800074b8 <etext+0x4b8>
    800037dc:	804fd0ef          	jal	800007e0 <panic>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800037e0:	24c1                	addiw	s1,s1,16
    800037e2:	04c92783          	lw	a5,76(s2)
    800037e6:	04f4f163          	bgeu	s1,a5,80003828 <dirlookup+0x8a>
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800037ea:	4741                	li	a4,16
    800037ec:	86a6                	mv	a3,s1
    800037ee:	fc040613          	addi	a2,s0,-64
    800037f2:	4581                	li	a1,0
    800037f4:	854a                	mv	a0,s2
    800037f6:	d89ff0ef          	jal	8000357e <readi>
    800037fa:	47c1                	li	a5,16
    800037fc:	fcf51ce3          	bne	a0,a5,800037d4 <dirlookup+0x36>
    if(de.inum == 0)
    80003800:	fc045783          	lhu	a5,-64(s0)
    80003804:	dff1                	beqz	a5,800037e0 <dirlookup+0x42>
    if(namecmp(name, de.name) == 0){
    80003806:	fc240593          	addi	a1,s0,-62
    8000380a:	854e                	mv	a0,s3
    8000380c:	f7dff0ef          	jal	80003788 <namecmp>
    80003810:	f961                	bnez	a0,800037e0 <dirlookup+0x42>
      if(poff)
    80003812:	000a0463          	beqz	s4,8000381a <dirlookup+0x7c>
        *poff = off;
    80003816:	009a2023          	sw	s1,0(s4)
      return iget(dp->dev, inum);
    8000381a:	fc045583          	lhu	a1,-64(s0)
    8000381e:	00092503          	lw	a0,0(s2)
    80003822:	f58ff0ef          	jal	80002f7a <iget>
    80003826:	a011                	j	8000382a <dirlookup+0x8c>
  return 0;
    80003828:	4501                	li	a0,0
}
    8000382a:	70e2                	ld	ra,56(sp)
    8000382c:	7442                	ld	s0,48(sp)
    8000382e:	74a2                	ld	s1,40(sp)
    80003830:	7902                	ld	s2,32(sp)
    80003832:	69e2                	ld	s3,24(sp)
    80003834:	6a42                	ld	s4,16(sp)
    80003836:	6121                	addi	sp,sp,64
    80003838:	8082                	ret

000000008000383a <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
    8000383a:	711d                	addi	sp,sp,-96
    8000383c:	ec86                	sd	ra,88(sp)
    8000383e:	e8a2                	sd	s0,80(sp)
    80003840:	e4a6                	sd	s1,72(sp)
    80003842:	e0ca                	sd	s2,64(sp)
    80003844:	fc4e                	sd	s3,56(sp)
    80003846:	f852                	sd	s4,48(sp)
    80003848:	f456                	sd	s5,40(sp)
    8000384a:	f05a                	sd	s6,32(sp)
    8000384c:	ec5e                	sd	s7,24(sp)
    8000384e:	e862                	sd	s8,16(sp)
    80003850:	e466                	sd	s9,8(sp)
    80003852:	1080                	addi	s0,sp,96
    80003854:	84aa                	mv	s1,a0
    80003856:	8b2e                	mv	s6,a1
    80003858:	8ab2                	mv	s5,a2
  struct inode *ip, *next;

  if(*path == '/')
    8000385a:	00054703          	lbu	a4,0(a0)
    8000385e:	02f00793          	li	a5,47
    80003862:	00f70e63          	beq	a4,a5,8000387e <namex+0x44>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
    80003866:	874fe0ef          	jal	800018da <myproc>
    8000386a:	15053503          	ld	a0,336(a0)
    8000386e:	94bff0ef          	jal	800031b8 <idup>
    80003872:	8a2a                	mv	s4,a0
  while(*path == '/')
    80003874:	02f00913          	li	s2,47
  if(len >= DIRSIZ)
    80003878:	4c35                	li	s8,13

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
    if(ip->type != T_DIR){
    8000387a:	4b85                	li	s7,1
    8000387c:	a871                	j	80003918 <namex+0xde>
    ip = iget(ROOTDEV, ROOTINO);
    8000387e:	4585                	li	a1,1
    80003880:	4505                	li	a0,1
    80003882:	ef8ff0ef          	jal	80002f7a <iget>
    80003886:	8a2a                	mv	s4,a0
    80003888:	b7f5                	j	80003874 <namex+0x3a>
      iunlockput(ip);
    8000388a:	8552                	mv	a0,s4
    8000388c:	b6dff0ef          	jal	800033f8 <iunlockput>
      return 0;
    80003890:	4a01                	li	s4,0
  if(nameiparent){
    iput(ip);
    return 0;
  }
  return ip;
}
    80003892:	8552                	mv	a0,s4
    80003894:	60e6                	ld	ra,88(sp)
    80003896:	6446                	ld	s0,80(sp)
    80003898:	64a6                	ld	s1,72(sp)
    8000389a:	6906                	ld	s2,64(sp)
    8000389c:	79e2                	ld	s3,56(sp)
    8000389e:	7a42                	ld	s4,48(sp)
    800038a0:	7aa2                	ld	s5,40(sp)
    800038a2:	7b02                	ld	s6,32(sp)
    800038a4:	6be2                	ld	s7,24(sp)
    800038a6:	6c42                	ld	s8,16(sp)
    800038a8:	6ca2                	ld	s9,8(sp)
    800038aa:	6125                	addi	sp,sp,96
    800038ac:	8082                	ret
      iunlock(ip);
    800038ae:	8552                	mv	a0,s4
    800038b0:	9edff0ef          	jal	8000329c <iunlock>
      return ip;
    800038b4:	bff9                	j	80003892 <namex+0x58>
      iunlockput(ip);
    800038b6:	8552                	mv	a0,s4
    800038b8:	b41ff0ef          	jal	800033f8 <iunlockput>
      return 0;
    800038bc:	8a4e                	mv	s4,s3
    800038be:	bfd1                	j	80003892 <namex+0x58>
  len = path - s;
    800038c0:	40998633          	sub	a2,s3,s1
    800038c4:	00060c9b          	sext.w	s9,a2
  if(len >= DIRSIZ)
    800038c8:	099c5063          	bge	s8,s9,80003948 <namex+0x10e>
    memmove(name, s, DIRSIZ);
    800038cc:	4639                	li	a2,14
    800038ce:	85a6                	mv	a1,s1
    800038d0:	8556                	mv	a0,s5
    800038d2:	c2cfd0ef          	jal	80000cfe <memmove>
    800038d6:	84ce                	mv	s1,s3
  while(*path == '/')
    800038d8:	0004c783          	lbu	a5,0(s1)
    800038dc:	01279763          	bne	a5,s2,800038ea <namex+0xb0>
    path++;
    800038e0:	0485                	addi	s1,s1,1
  while(*path == '/')
    800038e2:	0004c783          	lbu	a5,0(s1)
    800038e6:	ff278de3          	beq	a5,s2,800038e0 <namex+0xa6>
    ilock(ip);
    800038ea:	8552                	mv	a0,s4
    800038ec:	903ff0ef          	jal	800031ee <ilock>
    if(ip->type != T_DIR){
    800038f0:	044a1783          	lh	a5,68(s4)
    800038f4:	f9779be3          	bne	a5,s7,8000388a <namex+0x50>
    if(nameiparent && *path == '\0'){
    800038f8:	000b0563          	beqz	s6,80003902 <namex+0xc8>
    800038fc:	0004c783          	lbu	a5,0(s1)
    80003900:	d7dd                	beqz	a5,800038ae <namex+0x74>
    if((next = dirlookup(ip, name, 0)) == 0){
    80003902:	4601                	li	a2,0
    80003904:	85d6                	mv	a1,s5
    80003906:	8552                	mv	a0,s4
    80003908:	e97ff0ef          	jal	8000379e <dirlookup>
    8000390c:	89aa                	mv	s3,a0
    8000390e:	d545                	beqz	a0,800038b6 <namex+0x7c>
    iunlockput(ip);
    80003910:	8552                	mv	a0,s4
    80003912:	ae7ff0ef          	jal	800033f8 <iunlockput>
    ip = next;
    80003916:	8a4e                	mv	s4,s3
  while(*path == '/')
    80003918:	0004c783          	lbu	a5,0(s1)
    8000391c:	01279763          	bne	a5,s2,8000392a <namex+0xf0>
    path++;
    80003920:	0485                	addi	s1,s1,1
  while(*path == '/')
    80003922:	0004c783          	lbu	a5,0(s1)
    80003926:	ff278de3          	beq	a5,s2,80003920 <namex+0xe6>
  if(*path == 0)
    8000392a:	cb8d                	beqz	a5,8000395c <namex+0x122>
  while(*path != '/' && *path != 0)
    8000392c:	0004c783          	lbu	a5,0(s1)
    80003930:	89a6                	mv	s3,s1
  len = path - s;
    80003932:	4c81                	li	s9,0
    80003934:	4601                	li	a2,0
  while(*path != '/' && *path != 0)
    80003936:	01278963          	beq	a5,s2,80003948 <namex+0x10e>
    8000393a:	d3d9                	beqz	a5,800038c0 <namex+0x86>
    path++;
    8000393c:	0985                	addi	s3,s3,1
  while(*path != '/' && *path != 0)
    8000393e:	0009c783          	lbu	a5,0(s3)
    80003942:	ff279ce3          	bne	a5,s2,8000393a <namex+0x100>
    80003946:	bfad                	j	800038c0 <namex+0x86>
    memmove(name, s, len);
    80003948:	2601                	sext.w	a2,a2
    8000394a:	85a6                	mv	a1,s1
    8000394c:	8556                	mv	a0,s5
    8000394e:	bb0fd0ef          	jal	80000cfe <memmove>
    name[len] = 0;
    80003952:	9cd6                	add	s9,s9,s5
    80003954:	000c8023          	sb	zero,0(s9) # 2000 <_entry-0x7fffe000>
    80003958:	84ce                	mv	s1,s3
    8000395a:	bfbd                	j	800038d8 <namex+0x9e>
  if(nameiparent){
    8000395c:	f20b0be3          	beqz	s6,80003892 <namex+0x58>
    iput(ip);
    80003960:	8552                	mv	a0,s4
    80003962:	a0fff0ef          	jal	80003370 <iput>
    return 0;
    80003966:	4a01                	li	s4,0
    80003968:	b72d                	j	80003892 <namex+0x58>

000000008000396a <dirlink>:
{
    8000396a:	7139                	addi	sp,sp,-64
    8000396c:	fc06                	sd	ra,56(sp)
    8000396e:	f822                	sd	s0,48(sp)
    80003970:	f04a                	sd	s2,32(sp)
    80003972:	ec4e                	sd	s3,24(sp)
    80003974:	e852                	sd	s4,16(sp)
    80003976:	0080                	addi	s0,sp,64
    80003978:	892a                	mv	s2,a0
    8000397a:	8a2e                	mv	s4,a1
    8000397c:	89b2                	mv	s3,a2
  if((ip = dirlookup(dp, name, 0)) != 0){
    8000397e:	4601                	li	a2,0
    80003980:	e1fff0ef          	jal	8000379e <dirlookup>
    80003984:	e535                	bnez	a0,800039f0 <dirlink+0x86>
    80003986:	f426                	sd	s1,40(sp)
  for(off = 0; off < dp->size; off += sizeof(de)){
    80003988:	04c92483          	lw	s1,76(s2)
    8000398c:	c48d                	beqz	s1,800039b6 <dirlink+0x4c>
    8000398e:	4481                	li	s1,0
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80003990:	4741                	li	a4,16
    80003992:	86a6                	mv	a3,s1
    80003994:	fc040613          	addi	a2,s0,-64
    80003998:	4581                	li	a1,0
    8000399a:	854a                	mv	a0,s2
    8000399c:	be3ff0ef          	jal	8000357e <readi>
    800039a0:	47c1                	li	a5,16
    800039a2:	04f51b63          	bne	a0,a5,800039f8 <dirlink+0x8e>
    if(de.inum == 0)
    800039a6:	fc045783          	lhu	a5,-64(s0)
    800039aa:	c791                	beqz	a5,800039b6 <dirlink+0x4c>
  for(off = 0; off < dp->size; off += sizeof(de)){
    800039ac:	24c1                	addiw	s1,s1,16
    800039ae:	04c92783          	lw	a5,76(s2)
    800039b2:	fcf4efe3          	bltu	s1,a5,80003990 <dirlink+0x26>
  strncpy(de.name, name, DIRSIZ);
    800039b6:	4639                	li	a2,14
    800039b8:	85d2                	mv	a1,s4
    800039ba:	fc240513          	addi	a0,s0,-62
    800039be:	be6fd0ef          	jal	80000da4 <strncpy>
  de.inum = inum;
    800039c2:	fd341023          	sh	s3,-64(s0)
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    800039c6:	4741                	li	a4,16
    800039c8:	86a6                	mv	a3,s1
    800039ca:	fc040613          	addi	a2,s0,-64
    800039ce:	4581                	li	a1,0
    800039d0:	854a                	mv	a0,s2
    800039d2:	ca9ff0ef          	jal	8000367a <writei>
    800039d6:	1541                	addi	a0,a0,-16
    800039d8:	00a03533          	snez	a0,a0
    800039dc:	40a00533          	neg	a0,a0
    800039e0:	74a2                	ld	s1,40(sp)
}
    800039e2:	70e2                	ld	ra,56(sp)
    800039e4:	7442                	ld	s0,48(sp)
    800039e6:	7902                	ld	s2,32(sp)
    800039e8:	69e2                	ld	s3,24(sp)
    800039ea:	6a42                	ld	s4,16(sp)
    800039ec:	6121                	addi	sp,sp,64
    800039ee:	8082                	ret
    iput(ip);
    800039f0:	981ff0ef          	jal	80003370 <iput>
    return -1;
    800039f4:	557d                	li	a0,-1
    800039f6:	b7f5                	j	800039e2 <dirlink+0x78>
      panic("dirlink read");
    800039f8:	00004517          	auipc	a0,0x4
    800039fc:	ad050513          	addi	a0,a0,-1328 # 800074c8 <etext+0x4c8>
    80003a00:	de1fc0ef          	jal	800007e0 <panic>

0000000080003a04 <namei>:

struct inode*
namei(char *path)
{
    80003a04:	1101                	addi	sp,sp,-32
    80003a06:	ec06                	sd	ra,24(sp)
    80003a08:	e822                	sd	s0,16(sp)
    80003a0a:	1000                	addi	s0,sp,32
  char name[DIRSIZ];
  return namex(path, 0, name);
    80003a0c:	fe040613          	addi	a2,s0,-32
    80003a10:	4581                	li	a1,0
    80003a12:	e29ff0ef          	jal	8000383a <namex>
}
    80003a16:	60e2                	ld	ra,24(sp)
    80003a18:	6442                	ld	s0,16(sp)
    80003a1a:	6105                	addi	sp,sp,32
    80003a1c:	8082                	ret

0000000080003a1e <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
    80003a1e:	1141                	addi	sp,sp,-16
    80003a20:	e406                	sd	ra,8(sp)
    80003a22:	e022                	sd	s0,0(sp)
    80003a24:	0800                	addi	s0,sp,16
    80003a26:	862e                	mv	a2,a1
  return namex(path, 1, name);
    80003a28:	4585                	li	a1,1
    80003a2a:	e11ff0ef          	jal	8000383a <namex>
}
    80003a2e:	60a2                	ld	ra,8(sp)
    80003a30:	6402                	ld	s0,0(sp)
    80003a32:	0141                	addi	sp,sp,16
    80003a34:	8082                	ret

0000000080003a36 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
    80003a36:	1101                	addi	sp,sp,-32
    80003a38:	ec06                	sd	ra,24(sp)
    80003a3a:	e822                	sd	s0,16(sp)
    80003a3c:	e426                	sd	s1,8(sp)
    80003a3e:	e04a                	sd	s2,0(sp)
    80003a40:	1000                	addi	s0,sp,32
  struct buf *buf = bread(log.dev, log.start);
    80003a42:	0001f917          	auipc	s2,0x1f
    80003a46:	97690913          	addi	s2,s2,-1674 # 800223b8 <log>
    80003a4a:	01892583          	lw	a1,24(s2)
    80003a4e:	02492503          	lw	a0,36(s2)
    80003a52:	8d0ff0ef          	jal	80002b22 <bread>
    80003a56:	84aa                	mv	s1,a0
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
    80003a58:	02892603          	lw	a2,40(s2)
    80003a5c:	cd30                	sw	a2,88(a0)
  for (i = 0; i < log.lh.n; i++) {
    80003a5e:	00c05f63          	blez	a2,80003a7c <write_head+0x46>
    80003a62:	0001f717          	auipc	a4,0x1f
    80003a66:	98270713          	addi	a4,a4,-1662 # 800223e4 <log+0x2c>
    80003a6a:	87aa                	mv	a5,a0
    80003a6c:	060a                	slli	a2,a2,0x2
    80003a6e:	962a                	add	a2,a2,a0
    hb->block[i] = log.lh.block[i];
    80003a70:	4314                	lw	a3,0(a4)
    80003a72:	cff4                	sw	a3,92(a5)
  for (i = 0; i < log.lh.n; i++) {
    80003a74:	0711                	addi	a4,a4,4
    80003a76:	0791                	addi	a5,a5,4
    80003a78:	fec79ce3          	bne	a5,a2,80003a70 <write_head+0x3a>
  }
  bwrite(buf);
    80003a7c:	8526                	mv	a0,s1
    80003a7e:	97aff0ef          	jal	80002bf8 <bwrite>
  brelse(buf);
    80003a82:	8526                	mv	a0,s1
    80003a84:	9a6ff0ef          	jal	80002c2a <brelse>
}
    80003a88:	60e2                	ld	ra,24(sp)
    80003a8a:	6442                	ld	s0,16(sp)
    80003a8c:	64a2                	ld	s1,8(sp)
    80003a8e:	6902                	ld	s2,0(sp)
    80003a90:	6105                	addi	sp,sp,32
    80003a92:	8082                	ret

0000000080003a94 <install_trans>:
  for (tail = 0; tail < log.lh.n; tail++) {
    80003a94:	0001f797          	auipc	a5,0x1f
    80003a98:	94c7a783          	lw	a5,-1716(a5) # 800223e0 <log+0x28>
    80003a9c:	0af05e63          	blez	a5,80003b58 <install_trans+0xc4>
{
    80003aa0:	715d                	addi	sp,sp,-80
    80003aa2:	e486                	sd	ra,72(sp)
    80003aa4:	e0a2                	sd	s0,64(sp)
    80003aa6:	fc26                	sd	s1,56(sp)
    80003aa8:	f84a                	sd	s2,48(sp)
    80003aaa:	f44e                	sd	s3,40(sp)
    80003aac:	f052                	sd	s4,32(sp)
    80003aae:	ec56                	sd	s5,24(sp)
    80003ab0:	e85a                	sd	s6,16(sp)
    80003ab2:	e45e                	sd	s7,8(sp)
    80003ab4:	0880                	addi	s0,sp,80
    80003ab6:	8b2a                	mv	s6,a0
    80003ab8:	0001fa97          	auipc	s5,0x1f
    80003abc:	92ca8a93          	addi	s5,s5,-1748 # 800223e4 <log+0x2c>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003ac0:	4981                	li	s3,0
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003ac2:	00004b97          	auipc	s7,0x4
    80003ac6:	a16b8b93          	addi	s7,s7,-1514 # 800074d8 <etext+0x4d8>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003aca:	0001fa17          	auipc	s4,0x1f
    80003ace:	8eea0a13          	addi	s4,s4,-1810 # 800223b8 <log>
    80003ad2:	a025                	j	80003afa <install_trans+0x66>
      printf("recovering tail %d dst %d\n", tail, log.lh.block[tail]);
    80003ad4:	000aa603          	lw	a2,0(s5)
    80003ad8:	85ce                	mv	a1,s3
    80003ada:	855e                	mv	a0,s7
    80003adc:	a1ffc0ef          	jal	800004fa <printf>
    80003ae0:	a839                	j	80003afe <install_trans+0x6a>
    brelse(lbuf);
    80003ae2:	854a                	mv	a0,s2
    80003ae4:	946ff0ef          	jal	80002c2a <brelse>
    brelse(dbuf);
    80003ae8:	8526                	mv	a0,s1
    80003aea:	940ff0ef          	jal	80002c2a <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003aee:	2985                	addiw	s3,s3,1
    80003af0:	0a91                	addi	s5,s5,4
    80003af2:	028a2783          	lw	a5,40(s4)
    80003af6:	04f9d663          	bge	s3,a5,80003b42 <install_trans+0xae>
    if(recovering) {
    80003afa:	fc0b1de3          	bnez	s6,80003ad4 <install_trans+0x40>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
    80003afe:	018a2583          	lw	a1,24(s4)
    80003b02:	013585bb          	addw	a1,a1,s3
    80003b06:	2585                	addiw	a1,a1,1
    80003b08:	024a2503          	lw	a0,36(s4)
    80003b0c:	816ff0ef          	jal	80002b22 <bread>
    80003b10:	892a                	mv	s2,a0
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
    80003b12:	000aa583          	lw	a1,0(s5)
    80003b16:	024a2503          	lw	a0,36(s4)
    80003b1a:	808ff0ef          	jal	80002b22 <bread>
    80003b1e:	84aa                	mv	s1,a0
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    80003b20:	40000613          	li	a2,1024
    80003b24:	05890593          	addi	a1,s2,88
    80003b28:	05850513          	addi	a0,a0,88
    80003b2c:	9d2fd0ef          	jal	80000cfe <memmove>
    bwrite(dbuf);  // write dst to disk
    80003b30:	8526                	mv	a0,s1
    80003b32:	8c6ff0ef          	jal	80002bf8 <bwrite>
    if(recovering == 0)
    80003b36:	fa0b16e3          	bnez	s6,80003ae2 <install_trans+0x4e>
      bunpin(dbuf);
    80003b3a:	8526                	mv	a0,s1
    80003b3c:	9aaff0ef          	jal	80002ce6 <bunpin>
    80003b40:	b74d                	j	80003ae2 <install_trans+0x4e>
}
    80003b42:	60a6                	ld	ra,72(sp)
    80003b44:	6406                	ld	s0,64(sp)
    80003b46:	74e2                	ld	s1,56(sp)
    80003b48:	7942                	ld	s2,48(sp)
    80003b4a:	79a2                	ld	s3,40(sp)
    80003b4c:	7a02                	ld	s4,32(sp)
    80003b4e:	6ae2                	ld	s5,24(sp)
    80003b50:	6b42                	ld	s6,16(sp)
    80003b52:	6ba2                	ld	s7,8(sp)
    80003b54:	6161                	addi	sp,sp,80
    80003b56:	8082                	ret
    80003b58:	8082                	ret

0000000080003b5a <initlog>:
{
    80003b5a:	7179                	addi	sp,sp,-48
    80003b5c:	f406                	sd	ra,40(sp)
    80003b5e:	f022                	sd	s0,32(sp)
    80003b60:	ec26                	sd	s1,24(sp)
    80003b62:	e84a                	sd	s2,16(sp)
    80003b64:	e44e                	sd	s3,8(sp)
    80003b66:	1800                	addi	s0,sp,48
    80003b68:	892a                	mv	s2,a0
    80003b6a:	89ae                	mv	s3,a1
  initlock(&log.lock, "log");
    80003b6c:	0001f497          	auipc	s1,0x1f
    80003b70:	84c48493          	addi	s1,s1,-1972 # 800223b8 <log>
    80003b74:	00004597          	auipc	a1,0x4
    80003b78:	98458593          	addi	a1,a1,-1660 # 800074f8 <etext+0x4f8>
    80003b7c:	8526                	mv	a0,s1
    80003b7e:	fd1fc0ef          	jal	80000b4e <initlock>
  log.start = sb->logstart;
    80003b82:	0149a583          	lw	a1,20(s3)
    80003b86:	cc8c                	sw	a1,24(s1)
  log.dev = dev;
    80003b88:	0324a223          	sw	s2,36(s1)
  struct buf *buf = bread(log.dev, log.start);
    80003b8c:	854a                	mv	a0,s2
    80003b8e:	f95fe0ef          	jal	80002b22 <bread>
  log.lh.n = lh->n;
    80003b92:	4d30                	lw	a2,88(a0)
    80003b94:	d490                	sw	a2,40(s1)
  for (i = 0; i < log.lh.n; i++) {
    80003b96:	00c05f63          	blez	a2,80003bb4 <initlog+0x5a>
    80003b9a:	87aa                	mv	a5,a0
    80003b9c:	0001f717          	auipc	a4,0x1f
    80003ba0:	84870713          	addi	a4,a4,-1976 # 800223e4 <log+0x2c>
    80003ba4:	060a                	slli	a2,a2,0x2
    80003ba6:	962a                	add	a2,a2,a0
    log.lh.block[i] = lh->block[i];
    80003ba8:	4ff4                	lw	a3,92(a5)
    80003baa:	c314                	sw	a3,0(a4)
  for (i = 0; i < log.lh.n; i++) {
    80003bac:	0791                	addi	a5,a5,4
    80003bae:	0711                	addi	a4,a4,4
    80003bb0:	fec79ce3          	bne	a5,a2,80003ba8 <initlog+0x4e>
  brelse(buf);
    80003bb4:	876ff0ef          	jal	80002c2a <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(1); // if committed, copy from log to disk
    80003bb8:	4505                	li	a0,1
    80003bba:	edbff0ef          	jal	80003a94 <install_trans>
  log.lh.n = 0;
    80003bbe:	0001f797          	auipc	a5,0x1f
    80003bc2:	8207a123          	sw	zero,-2014(a5) # 800223e0 <log+0x28>
  write_head(); // clear the log
    80003bc6:	e71ff0ef          	jal	80003a36 <write_head>
}
    80003bca:	70a2                	ld	ra,40(sp)
    80003bcc:	7402                	ld	s0,32(sp)
    80003bce:	64e2                	ld	s1,24(sp)
    80003bd0:	6942                	ld	s2,16(sp)
    80003bd2:	69a2                	ld	s3,8(sp)
    80003bd4:	6145                	addi	sp,sp,48
    80003bd6:	8082                	ret

0000000080003bd8 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
    80003bd8:	1101                	addi	sp,sp,-32
    80003bda:	ec06                	sd	ra,24(sp)
    80003bdc:	e822                	sd	s0,16(sp)
    80003bde:	e426                	sd	s1,8(sp)
    80003be0:	e04a                	sd	s2,0(sp)
    80003be2:	1000                	addi	s0,sp,32
  acquire(&log.lock);
    80003be4:	0001e517          	auipc	a0,0x1e
    80003be8:	7d450513          	addi	a0,a0,2004 # 800223b8 <log>
    80003bec:	fe3fc0ef          	jal	80000bce <acquire>
  while(1){
    if(log.committing){
    80003bf0:	0001e497          	auipc	s1,0x1e
    80003bf4:	7c848493          	addi	s1,s1,1992 # 800223b8 <log>
      sleep(&log, &log.lock);
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003bf8:	4979                	li	s2,30
    80003bfa:	a029                	j	80003c04 <begin_op+0x2c>
      sleep(&log, &log.lock);
    80003bfc:	85a6                	mv	a1,s1
    80003bfe:	8526                	mv	a0,s1
    80003c00:	ae4fe0ef          	jal	80001ee4 <sleep>
    if(log.committing){
    80003c04:	509c                	lw	a5,32(s1)
    80003c06:	fbfd                	bnez	a5,80003bfc <begin_op+0x24>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGBLOCKS){
    80003c08:	4cd8                	lw	a4,28(s1)
    80003c0a:	2705                	addiw	a4,a4,1
    80003c0c:	0027179b          	slliw	a5,a4,0x2
    80003c10:	9fb9                	addw	a5,a5,a4
    80003c12:	0017979b          	slliw	a5,a5,0x1
    80003c16:	5494                	lw	a3,40(s1)
    80003c18:	9fb5                	addw	a5,a5,a3
    80003c1a:	00f95763          	bge	s2,a5,80003c28 <begin_op+0x50>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    80003c1e:	85a6                	mv	a1,s1
    80003c20:	8526                	mv	a0,s1
    80003c22:	ac2fe0ef          	jal	80001ee4 <sleep>
    80003c26:	bff9                	j	80003c04 <begin_op+0x2c>
    } else {
      log.outstanding += 1;
    80003c28:	0001e517          	auipc	a0,0x1e
    80003c2c:	79050513          	addi	a0,a0,1936 # 800223b8 <log>
    80003c30:	cd58                	sw	a4,28(a0)
      release(&log.lock);
    80003c32:	834fd0ef          	jal	80000c66 <release>
      break;
    }
  }
}
    80003c36:	60e2                	ld	ra,24(sp)
    80003c38:	6442                	ld	s0,16(sp)
    80003c3a:	64a2                	ld	s1,8(sp)
    80003c3c:	6902                	ld	s2,0(sp)
    80003c3e:	6105                	addi	sp,sp,32
    80003c40:	8082                	ret

0000000080003c42 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
    80003c42:	7139                	addi	sp,sp,-64
    80003c44:	fc06                	sd	ra,56(sp)
    80003c46:	f822                	sd	s0,48(sp)
    80003c48:	f426                	sd	s1,40(sp)
    80003c4a:	f04a                	sd	s2,32(sp)
    80003c4c:	0080                	addi	s0,sp,64
  int do_commit = 0;

  acquire(&log.lock);
    80003c4e:	0001e497          	auipc	s1,0x1e
    80003c52:	76a48493          	addi	s1,s1,1898 # 800223b8 <log>
    80003c56:	8526                	mv	a0,s1
    80003c58:	f77fc0ef          	jal	80000bce <acquire>
  log.outstanding -= 1;
    80003c5c:	4cdc                	lw	a5,28(s1)
    80003c5e:	37fd                	addiw	a5,a5,-1
    80003c60:	0007891b          	sext.w	s2,a5
    80003c64:	ccdc                	sw	a5,28(s1)
  if(log.committing)
    80003c66:	509c                	lw	a5,32(s1)
    80003c68:	ef9d                	bnez	a5,80003ca6 <end_op+0x64>
    panic("log.committing");
  if(log.outstanding == 0){
    80003c6a:	04091763          	bnez	s2,80003cb8 <end_op+0x76>
    do_commit = 1;
    log.committing = 1;
    80003c6e:	0001e497          	auipc	s1,0x1e
    80003c72:	74a48493          	addi	s1,s1,1866 # 800223b8 <log>
    80003c76:	4785                	li	a5,1
    80003c78:	d09c                	sw	a5,32(s1)
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
    80003c7a:	8526                	mv	a0,s1
    80003c7c:	febfc0ef          	jal	80000c66 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
    80003c80:	549c                	lw	a5,40(s1)
    80003c82:	04f04b63          	bgtz	a5,80003cd8 <end_op+0x96>
    acquire(&log.lock);
    80003c86:	0001e497          	auipc	s1,0x1e
    80003c8a:	73248493          	addi	s1,s1,1842 # 800223b8 <log>
    80003c8e:	8526                	mv	a0,s1
    80003c90:	f3ffc0ef          	jal	80000bce <acquire>
    log.committing = 0;
    80003c94:	0204a023          	sw	zero,32(s1)
    wakeup(&log);
    80003c98:	8526                	mv	a0,s1
    80003c9a:	a96fe0ef          	jal	80001f30 <wakeup>
    release(&log.lock);
    80003c9e:	8526                	mv	a0,s1
    80003ca0:	fc7fc0ef          	jal	80000c66 <release>
}
    80003ca4:	a025                	j	80003ccc <end_op+0x8a>
    80003ca6:	ec4e                	sd	s3,24(sp)
    80003ca8:	e852                	sd	s4,16(sp)
    80003caa:	e456                	sd	s5,8(sp)
    panic("log.committing");
    80003cac:	00004517          	auipc	a0,0x4
    80003cb0:	85450513          	addi	a0,a0,-1964 # 80007500 <etext+0x500>
    80003cb4:	b2dfc0ef          	jal	800007e0 <panic>
    wakeup(&log);
    80003cb8:	0001e497          	auipc	s1,0x1e
    80003cbc:	70048493          	addi	s1,s1,1792 # 800223b8 <log>
    80003cc0:	8526                	mv	a0,s1
    80003cc2:	a6efe0ef          	jal	80001f30 <wakeup>
  release(&log.lock);
    80003cc6:	8526                	mv	a0,s1
    80003cc8:	f9ffc0ef          	jal	80000c66 <release>
}
    80003ccc:	70e2                	ld	ra,56(sp)
    80003cce:	7442                	ld	s0,48(sp)
    80003cd0:	74a2                	ld	s1,40(sp)
    80003cd2:	7902                	ld	s2,32(sp)
    80003cd4:	6121                	addi	sp,sp,64
    80003cd6:	8082                	ret
    80003cd8:	ec4e                	sd	s3,24(sp)
    80003cda:	e852                	sd	s4,16(sp)
    80003cdc:	e456                	sd	s5,8(sp)
  for (tail = 0; tail < log.lh.n; tail++) {
    80003cde:	0001ea97          	auipc	s5,0x1e
    80003ce2:	706a8a93          	addi	s5,s5,1798 # 800223e4 <log+0x2c>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
    80003ce6:	0001ea17          	auipc	s4,0x1e
    80003cea:	6d2a0a13          	addi	s4,s4,1746 # 800223b8 <log>
    80003cee:	018a2583          	lw	a1,24(s4)
    80003cf2:	012585bb          	addw	a1,a1,s2
    80003cf6:	2585                	addiw	a1,a1,1
    80003cf8:	024a2503          	lw	a0,36(s4)
    80003cfc:	e27fe0ef          	jal	80002b22 <bread>
    80003d00:	84aa                	mv	s1,a0
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
    80003d02:	000aa583          	lw	a1,0(s5)
    80003d06:	024a2503          	lw	a0,36(s4)
    80003d0a:	e19fe0ef          	jal	80002b22 <bread>
    80003d0e:	89aa                	mv	s3,a0
    memmove(to->data, from->data, BSIZE);
    80003d10:	40000613          	li	a2,1024
    80003d14:	05850593          	addi	a1,a0,88
    80003d18:	05848513          	addi	a0,s1,88
    80003d1c:	fe3fc0ef          	jal	80000cfe <memmove>
    bwrite(to);  // write the log
    80003d20:	8526                	mv	a0,s1
    80003d22:	ed7fe0ef          	jal	80002bf8 <bwrite>
    brelse(from);
    80003d26:	854e                	mv	a0,s3
    80003d28:	f03fe0ef          	jal	80002c2a <brelse>
    brelse(to);
    80003d2c:	8526                	mv	a0,s1
    80003d2e:	efdfe0ef          	jal	80002c2a <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
    80003d32:	2905                	addiw	s2,s2,1
    80003d34:	0a91                	addi	s5,s5,4
    80003d36:	028a2783          	lw	a5,40(s4)
    80003d3a:	faf94ae3          	blt	s2,a5,80003cee <end_op+0xac>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
    80003d3e:	cf9ff0ef          	jal	80003a36 <write_head>
    install_trans(0); // Now install writes to home locations
    80003d42:	4501                	li	a0,0
    80003d44:	d51ff0ef          	jal	80003a94 <install_trans>
    log.lh.n = 0;
    80003d48:	0001e797          	auipc	a5,0x1e
    80003d4c:	6807ac23          	sw	zero,1688(a5) # 800223e0 <log+0x28>
    write_head();    // Erase the transaction from the log
    80003d50:	ce7ff0ef          	jal	80003a36 <write_head>
    80003d54:	69e2                	ld	s3,24(sp)
    80003d56:	6a42                	ld	s4,16(sp)
    80003d58:	6aa2                	ld	s5,8(sp)
    80003d5a:	b735                	j	80003c86 <end_op+0x44>

0000000080003d5c <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
    80003d5c:	1101                	addi	sp,sp,-32
    80003d5e:	ec06                	sd	ra,24(sp)
    80003d60:	e822                	sd	s0,16(sp)
    80003d62:	e426                	sd	s1,8(sp)
    80003d64:	e04a                	sd	s2,0(sp)
    80003d66:	1000                	addi	s0,sp,32
    80003d68:	84aa                	mv	s1,a0
  int i;

  acquire(&log.lock);
    80003d6a:	0001e917          	auipc	s2,0x1e
    80003d6e:	64e90913          	addi	s2,s2,1614 # 800223b8 <log>
    80003d72:	854a                	mv	a0,s2
    80003d74:	e5bfc0ef          	jal	80000bce <acquire>
  if (log.lh.n >= LOGBLOCKS)
    80003d78:	02892603          	lw	a2,40(s2)
    80003d7c:	47f5                	li	a5,29
    80003d7e:	04c7cc63          	blt	a5,a2,80003dd6 <log_write+0x7a>
    panic("too big a transaction");
  if (log.outstanding < 1)
    80003d82:	0001e797          	auipc	a5,0x1e
    80003d86:	6527a783          	lw	a5,1618(a5) # 800223d4 <log+0x1c>
    80003d8a:	04f05c63          	blez	a5,80003de2 <log_write+0x86>
    panic("log_write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
    80003d8e:	4781                	li	a5,0
    80003d90:	04c05f63          	blez	a2,80003dee <log_write+0x92>
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003d94:	44cc                	lw	a1,12(s1)
    80003d96:	0001e717          	auipc	a4,0x1e
    80003d9a:	64e70713          	addi	a4,a4,1614 # 800223e4 <log+0x2c>
  for (i = 0; i < log.lh.n; i++) {
    80003d9e:	4781                	li	a5,0
    if (log.lh.block[i] == b->blockno)   // log absorption
    80003da0:	4314                	lw	a3,0(a4)
    80003da2:	04b68663          	beq	a3,a1,80003dee <log_write+0x92>
  for (i = 0; i < log.lh.n; i++) {
    80003da6:	2785                	addiw	a5,a5,1
    80003da8:	0711                	addi	a4,a4,4
    80003daa:	fef61be3          	bne	a2,a5,80003da0 <log_write+0x44>
      break;
  }
  log.lh.block[i] = b->blockno;
    80003dae:	0621                	addi	a2,a2,8
    80003db0:	060a                	slli	a2,a2,0x2
    80003db2:	0001e797          	auipc	a5,0x1e
    80003db6:	60678793          	addi	a5,a5,1542 # 800223b8 <log>
    80003dba:	97b2                	add	a5,a5,a2
    80003dbc:	44d8                	lw	a4,12(s1)
    80003dbe:	c7d8                	sw	a4,12(a5)
  if (i == log.lh.n) {  // Add new block to log?
    bpin(b);
    80003dc0:	8526                	mv	a0,s1
    80003dc2:	ef1fe0ef          	jal	80002cb2 <bpin>
    log.lh.n++;
    80003dc6:	0001e717          	auipc	a4,0x1e
    80003dca:	5f270713          	addi	a4,a4,1522 # 800223b8 <log>
    80003dce:	571c                	lw	a5,40(a4)
    80003dd0:	2785                	addiw	a5,a5,1
    80003dd2:	d71c                	sw	a5,40(a4)
    80003dd4:	a80d                	j	80003e06 <log_write+0xaa>
    panic("too big a transaction");
    80003dd6:	00003517          	auipc	a0,0x3
    80003dda:	73a50513          	addi	a0,a0,1850 # 80007510 <etext+0x510>
    80003dde:	a03fc0ef          	jal	800007e0 <panic>
    panic("log_write outside of trans");
    80003de2:	00003517          	auipc	a0,0x3
    80003de6:	74650513          	addi	a0,a0,1862 # 80007528 <etext+0x528>
    80003dea:	9f7fc0ef          	jal	800007e0 <panic>
  log.lh.block[i] = b->blockno;
    80003dee:	00878693          	addi	a3,a5,8
    80003df2:	068a                	slli	a3,a3,0x2
    80003df4:	0001e717          	auipc	a4,0x1e
    80003df8:	5c470713          	addi	a4,a4,1476 # 800223b8 <log>
    80003dfc:	9736                	add	a4,a4,a3
    80003dfe:	44d4                	lw	a3,12(s1)
    80003e00:	c754                	sw	a3,12(a4)
  if (i == log.lh.n) {  // Add new block to log?
    80003e02:	faf60fe3          	beq	a2,a5,80003dc0 <log_write+0x64>
  }
  release(&log.lock);
    80003e06:	0001e517          	auipc	a0,0x1e
    80003e0a:	5b250513          	addi	a0,a0,1458 # 800223b8 <log>
    80003e0e:	e59fc0ef          	jal	80000c66 <release>
}
    80003e12:	60e2                	ld	ra,24(sp)
    80003e14:	6442                	ld	s0,16(sp)
    80003e16:	64a2                	ld	s1,8(sp)
    80003e18:	6902                	ld	s2,0(sp)
    80003e1a:	6105                	addi	sp,sp,32
    80003e1c:	8082                	ret

0000000080003e1e <initsleeplock>:
#include "proc.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
    80003e1e:	1101                	addi	sp,sp,-32
    80003e20:	ec06                	sd	ra,24(sp)
    80003e22:	e822                	sd	s0,16(sp)
    80003e24:	e426                	sd	s1,8(sp)
    80003e26:	e04a                	sd	s2,0(sp)
    80003e28:	1000                	addi	s0,sp,32
    80003e2a:	84aa                	mv	s1,a0
    80003e2c:	892e                	mv	s2,a1
  initlock(&lk->lk, "sleep lock");
    80003e2e:	00003597          	auipc	a1,0x3
    80003e32:	71a58593          	addi	a1,a1,1818 # 80007548 <etext+0x548>
    80003e36:	0521                	addi	a0,a0,8
    80003e38:	d17fc0ef          	jal	80000b4e <initlock>
  lk->name = name;
    80003e3c:	0324b023          	sd	s2,32(s1)
  lk->locked = 0;
    80003e40:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80003e44:	0204a423          	sw	zero,40(s1)
}
    80003e48:	60e2                	ld	ra,24(sp)
    80003e4a:	6442                	ld	s0,16(sp)
    80003e4c:	64a2                	ld	s1,8(sp)
    80003e4e:	6902                	ld	s2,0(sp)
    80003e50:	6105                	addi	sp,sp,32
    80003e52:	8082                	ret

0000000080003e54 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
    80003e54:	1101                	addi	sp,sp,-32
    80003e56:	ec06                	sd	ra,24(sp)
    80003e58:	e822                	sd	s0,16(sp)
    80003e5a:	e426                	sd	s1,8(sp)
    80003e5c:	e04a                	sd	s2,0(sp)
    80003e5e:	1000                	addi	s0,sp,32
    80003e60:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80003e62:	00850913          	addi	s2,a0,8
    80003e66:	854a                	mv	a0,s2
    80003e68:	d67fc0ef          	jal	80000bce <acquire>
  while (lk->locked) {
    80003e6c:	409c                	lw	a5,0(s1)
    80003e6e:	c799                	beqz	a5,80003e7c <acquiresleep+0x28>
    sleep(lk, &lk->lk);
    80003e70:	85ca                	mv	a1,s2
    80003e72:	8526                	mv	a0,s1
    80003e74:	870fe0ef          	jal	80001ee4 <sleep>
  while (lk->locked) {
    80003e78:	409c                	lw	a5,0(s1)
    80003e7a:	fbfd                	bnez	a5,80003e70 <acquiresleep+0x1c>
  }
  lk->locked = 1;
    80003e7c:	4785                	li	a5,1
    80003e7e:	c09c                	sw	a5,0(s1)
  lk->pid = myproc()->pid;
    80003e80:	a5bfd0ef          	jal	800018da <myproc>
    80003e84:	591c                	lw	a5,48(a0)
    80003e86:	d49c                	sw	a5,40(s1)
  release(&lk->lk);
    80003e88:	854a                	mv	a0,s2
    80003e8a:	dddfc0ef          	jal	80000c66 <release>
}
    80003e8e:	60e2                	ld	ra,24(sp)
    80003e90:	6442                	ld	s0,16(sp)
    80003e92:	64a2                	ld	s1,8(sp)
    80003e94:	6902                	ld	s2,0(sp)
    80003e96:	6105                	addi	sp,sp,32
    80003e98:	8082                	ret

0000000080003e9a <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
    80003e9a:	1101                	addi	sp,sp,-32
    80003e9c:	ec06                	sd	ra,24(sp)
    80003e9e:	e822                	sd	s0,16(sp)
    80003ea0:	e426                	sd	s1,8(sp)
    80003ea2:	e04a                	sd	s2,0(sp)
    80003ea4:	1000                	addi	s0,sp,32
    80003ea6:	84aa                	mv	s1,a0
  acquire(&lk->lk);
    80003ea8:	00850913          	addi	s2,a0,8
    80003eac:	854a                	mv	a0,s2
    80003eae:	d21fc0ef          	jal	80000bce <acquire>
  lk->locked = 0;
    80003eb2:	0004a023          	sw	zero,0(s1)
  lk->pid = 0;
    80003eb6:	0204a423          	sw	zero,40(s1)
  wakeup(lk);
    80003eba:	8526                	mv	a0,s1
    80003ebc:	874fe0ef          	jal	80001f30 <wakeup>
  release(&lk->lk);
    80003ec0:	854a                	mv	a0,s2
    80003ec2:	da5fc0ef          	jal	80000c66 <release>
}
    80003ec6:	60e2                	ld	ra,24(sp)
    80003ec8:	6442                	ld	s0,16(sp)
    80003eca:	64a2                	ld	s1,8(sp)
    80003ecc:	6902                	ld	s2,0(sp)
    80003ece:	6105                	addi	sp,sp,32
    80003ed0:	8082                	ret

0000000080003ed2 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
    80003ed2:	7179                	addi	sp,sp,-48
    80003ed4:	f406                	sd	ra,40(sp)
    80003ed6:	f022                	sd	s0,32(sp)
    80003ed8:	ec26                	sd	s1,24(sp)
    80003eda:	e84a                	sd	s2,16(sp)
    80003edc:	1800                	addi	s0,sp,48
    80003ede:	84aa                	mv	s1,a0
  int r;
  
  acquire(&lk->lk);
    80003ee0:	00850913          	addi	s2,a0,8
    80003ee4:	854a                	mv	a0,s2
    80003ee6:	ce9fc0ef          	jal	80000bce <acquire>
  r = lk->locked && (lk->pid == myproc()->pid);
    80003eea:	409c                	lw	a5,0(s1)
    80003eec:	ef81                	bnez	a5,80003f04 <holdingsleep+0x32>
    80003eee:	4481                	li	s1,0
  release(&lk->lk);
    80003ef0:	854a                	mv	a0,s2
    80003ef2:	d75fc0ef          	jal	80000c66 <release>
  return r;
}
    80003ef6:	8526                	mv	a0,s1
    80003ef8:	70a2                	ld	ra,40(sp)
    80003efa:	7402                	ld	s0,32(sp)
    80003efc:	64e2                	ld	s1,24(sp)
    80003efe:	6942                	ld	s2,16(sp)
    80003f00:	6145                	addi	sp,sp,48
    80003f02:	8082                	ret
    80003f04:	e44e                	sd	s3,8(sp)
  r = lk->locked && (lk->pid == myproc()->pid);
    80003f06:	0284a983          	lw	s3,40(s1)
    80003f0a:	9d1fd0ef          	jal	800018da <myproc>
    80003f0e:	5904                	lw	s1,48(a0)
    80003f10:	413484b3          	sub	s1,s1,s3
    80003f14:	0014b493          	seqz	s1,s1
    80003f18:	69a2                	ld	s3,8(sp)
    80003f1a:	bfd9                	j	80003ef0 <holdingsleep+0x1e>

0000000080003f1c <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
    80003f1c:	1141                	addi	sp,sp,-16
    80003f1e:	e406                	sd	ra,8(sp)
    80003f20:	e022                	sd	s0,0(sp)
    80003f22:	0800                	addi	s0,sp,16
  initlock(&ftable.lock, "ftable");
    80003f24:	00003597          	auipc	a1,0x3
    80003f28:	63458593          	addi	a1,a1,1588 # 80007558 <etext+0x558>
    80003f2c:	0001e517          	auipc	a0,0x1e
    80003f30:	5d450513          	addi	a0,a0,1492 # 80022500 <ftable>
    80003f34:	c1bfc0ef          	jal	80000b4e <initlock>
}
    80003f38:	60a2                	ld	ra,8(sp)
    80003f3a:	6402                	ld	s0,0(sp)
    80003f3c:	0141                	addi	sp,sp,16
    80003f3e:	8082                	ret

0000000080003f40 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
    80003f40:	1101                	addi	sp,sp,-32
    80003f42:	ec06                	sd	ra,24(sp)
    80003f44:	e822                	sd	s0,16(sp)
    80003f46:	e426                	sd	s1,8(sp)
    80003f48:	1000                	addi	s0,sp,32
  struct file *f;

  acquire(&ftable.lock);
    80003f4a:	0001e517          	auipc	a0,0x1e
    80003f4e:	5b650513          	addi	a0,a0,1462 # 80022500 <ftable>
    80003f52:	c7dfc0ef          	jal	80000bce <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80003f56:	0001e497          	auipc	s1,0x1e
    80003f5a:	5c248493          	addi	s1,s1,1474 # 80022518 <ftable+0x18>
    80003f5e:	0001f717          	auipc	a4,0x1f
    80003f62:	55a70713          	addi	a4,a4,1370 # 800234b8 <fmtable>
    if(f->ref == 0){
    80003f66:	40dc                	lw	a5,4(s1)
    80003f68:	cf89                	beqz	a5,80003f82 <filealloc+0x42>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
    80003f6a:	02848493          	addi	s1,s1,40
    80003f6e:	fee49ce3          	bne	s1,a4,80003f66 <filealloc+0x26>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
    80003f72:	0001e517          	auipc	a0,0x1e
    80003f76:	58e50513          	addi	a0,a0,1422 # 80022500 <ftable>
    80003f7a:	cedfc0ef          	jal	80000c66 <release>
  return 0;
    80003f7e:	4481                	li	s1,0
    80003f80:	a809                	j	80003f92 <filealloc+0x52>
      f->ref = 1;
    80003f82:	4785                	li	a5,1
    80003f84:	c0dc                	sw	a5,4(s1)
      release(&ftable.lock);
    80003f86:	0001e517          	auipc	a0,0x1e
    80003f8a:	57a50513          	addi	a0,a0,1402 # 80022500 <ftable>
    80003f8e:	cd9fc0ef          	jal	80000c66 <release>
}
    80003f92:	8526                	mv	a0,s1
    80003f94:	60e2                	ld	ra,24(sp)
    80003f96:	6442                	ld	s0,16(sp)
    80003f98:	64a2                	ld	s1,8(sp)
    80003f9a:	6105                	addi	sp,sp,32
    80003f9c:	8082                	ret

0000000080003f9e <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
    80003f9e:	1101                	addi	sp,sp,-32
    80003fa0:	ec06                	sd	ra,24(sp)
    80003fa2:	e822                	sd	s0,16(sp)
    80003fa4:	e426                	sd	s1,8(sp)
    80003fa6:	1000                	addi	s0,sp,32
    80003fa8:	84aa                	mv	s1,a0
  acquire(&ftable.lock);
    80003faa:	0001e517          	auipc	a0,0x1e
    80003fae:	55650513          	addi	a0,a0,1366 # 80022500 <ftable>
    80003fb2:	c1dfc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
    80003fb6:	40dc                	lw	a5,4(s1)
    80003fb8:	02f05063          	blez	a5,80003fd8 <filedup+0x3a>
    panic("filedup");
  f->ref++;
    80003fbc:	2785                	addiw	a5,a5,1
    80003fbe:	c0dc                	sw	a5,4(s1)
  release(&ftable.lock);
    80003fc0:	0001e517          	auipc	a0,0x1e
    80003fc4:	54050513          	addi	a0,a0,1344 # 80022500 <ftable>
    80003fc8:	c9ffc0ef          	jal	80000c66 <release>
  return f;
}
    80003fcc:	8526                	mv	a0,s1
    80003fce:	60e2                	ld	ra,24(sp)
    80003fd0:	6442                	ld	s0,16(sp)
    80003fd2:	64a2                	ld	s1,8(sp)
    80003fd4:	6105                	addi	sp,sp,32
    80003fd6:	8082                	ret
    panic("filedup");
    80003fd8:	00003517          	auipc	a0,0x3
    80003fdc:	58850513          	addi	a0,a0,1416 # 80007560 <etext+0x560>
    80003fe0:	801fc0ef          	jal	800007e0 <panic>

0000000080003fe4 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
    80003fe4:	7139                	addi	sp,sp,-64
    80003fe6:	fc06                	sd	ra,56(sp)
    80003fe8:	f822                	sd	s0,48(sp)
    80003fea:	f426                	sd	s1,40(sp)
    80003fec:	0080                	addi	s0,sp,64
    80003fee:	84aa                	mv	s1,a0
  struct file ff;

  acquire(&ftable.lock);
    80003ff0:	0001e517          	auipc	a0,0x1e
    80003ff4:	51050513          	addi	a0,a0,1296 # 80022500 <ftable>
    80003ff8:	bd7fc0ef          	jal	80000bce <acquire>
  if(f->ref < 1)
    80003ffc:	40dc                	lw	a5,4(s1)
    80003ffe:	04f05a63          	blez	a5,80004052 <fileclose+0x6e>
    panic("fileclose");
  if(--f->ref > 0){
    80004002:	37fd                	addiw	a5,a5,-1
    80004004:	0007871b          	sext.w	a4,a5
    80004008:	c0dc                	sw	a5,4(s1)
    8000400a:	04e04e63          	bgtz	a4,80004066 <fileclose+0x82>
    8000400e:	f04a                	sd	s2,32(sp)
    80004010:	ec4e                	sd	s3,24(sp)
    80004012:	e852                	sd	s4,16(sp)
    80004014:	e456                	sd	s5,8(sp)
    release(&ftable.lock);
    return;
  }
  ff = *f;
    80004016:	0004a903          	lw	s2,0(s1)
    8000401a:	0094ca83          	lbu	s5,9(s1)
    8000401e:	0104ba03          	ld	s4,16(s1)
    80004022:	0184b983          	ld	s3,24(s1)
  f->ref = 0;
    80004026:	0004a223          	sw	zero,4(s1)
  f->type = FD_NONE;
    8000402a:	0004a023          	sw	zero,0(s1)
  release(&ftable.lock);
    8000402e:	0001e517          	auipc	a0,0x1e
    80004032:	4d250513          	addi	a0,a0,1234 # 80022500 <ftable>
    80004036:	c31fc0ef          	jal	80000c66 <release>

  if(ff.type == FD_PIPE){
    8000403a:	4785                	li	a5,1
    8000403c:	04f90063          	beq	s2,a5,8000407c <fileclose+0x98>
    pipeclose(ff.pipe, ff.writable);
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){
    80004040:	3979                	addiw	s2,s2,-2
    80004042:	4785                	li	a5,1
    80004044:	0527f563          	bgeu	a5,s2,8000408e <fileclose+0xaa>
    80004048:	7902                	ld	s2,32(sp)
    8000404a:	69e2                	ld	s3,24(sp)
    8000404c:	6a42                	ld	s4,16(sp)
    8000404e:	6aa2                	ld	s5,8(sp)
    80004050:	a00d                	j	80004072 <fileclose+0x8e>
    80004052:	f04a                	sd	s2,32(sp)
    80004054:	ec4e                	sd	s3,24(sp)
    80004056:	e852                	sd	s4,16(sp)
    80004058:	e456                	sd	s5,8(sp)
    panic("fileclose");
    8000405a:	00003517          	auipc	a0,0x3
    8000405e:	50e50513          	addi	a0,a0,1294 # 80007568 <etext+0x568>
    80004062:	f7efc0ef          	jal	800007e0 <panic>
    release(&ftable.lock);
    80004066:	0001e517          	auipc	a0,0x1e
    8000406a:	49a50513          	addi	a0,a0,1178 # 80022500 <ftable>
    8000406e:	bf9fc0ef          	jal	80000c66 <release>
    begin_op();
    iput(ff.ip);
    end_op();
  }
}
    80004072:	70e2                	ld	ra,56(sp)
    80004074:	7442                	ld	s0,48(sp)
    80004076:	74a2                	ld	s1,40(sp)
    80004078:	6121                	addi	sp,sp,64
    8000407a:	8082                	ret
    pipeclose(ff.pipe, ff.writable);
    8000407c:	85d6                	mv	a1,s5
    8000407e:	8552                	mv	a0,s4
    80004080:	336000ef          	jal	800043b6 <pipeclose>
    80004084:	7902                	ld	s2,32(sp)
    80004086:	69e2                	ld	s3,24(sp)
    80004088:	6a42                	ld	s4,16(sp)
    8000408a:	6aa2                	ld	s5,8(sp)
    8000408c:	b7dd                	j	80004072 <fileclose+0x8e>
    begin_op();
    8000408e:	b4bff0ef          	jal	80003bd8 <begin_op>
    iput(ff.ip);
    80004092:	854e                	mv	a0,s3
    80004094:	adcff0ef          	jal	80003370 <iput>
    end_op();
    80004098:	babff0ef          	jal	80003c42 <end_op>
    8000409c:	7902                	ld	s2,32(sp)
    8000409e:	69e2                	ld	s3,24(sp)
    800040a0:	6a42                	ld	s4,16(sp)
    800040a2:	6aa2                	ld	s5,8(sp)
    800040a4:	b7f9                	j	80004072 <fileclose+0x8e>

00000000800040a6 <filestat>:

// Get metadata about file f.
// addr is a user virtual address, pointing to a struct stat.
int
filestat(struct file *f, uint64 addr)
{
    800040a6:	715d                	addi	sp,sp,-80
    800040a8:	e486                	sd	ra,72(sp)
    800040aa:	e0a2                	sd	s0,64(sp)
    800040ac:	fc26                	sd	s1,56(sp)
    800040ae:	f44e                	sd	s3,40(sp)
    800040b0:	0880                	addi	s0,sp,80
    800040b2:	84aa                	mv	s1,a0
    800040b4:	89ae                	mv	s3,a1
  struct proc *p = myproc();
    800040b6:	825fd0ef          	jal	800018da <myproc>
  struct stat st;
  
  if(f->type == FD_INODE || f->type == FD_DEVICE){
    800040ba:	409c                	lw	a5,0(s1)
    800040bc:	37f9                	addiw	a5,a5,-2
    800040be:	4705                	li	a4,1
    800040c0:	04f76063          	bltu	a4,a5,80004100 <filestat+0x5a>
    800040c4:	f84a                	sd	s2,48(sp)
    800040c6:	892a                	mv	s2,a0
    ilock(f->ip);
    800040c8:	6c88                	ld	a0,24(s1)
    800040ca:	924ff0ef          	jal	800031ee <ilock>
    stati(f->ip, &st);
    800040ce:	fb840593          	addi	a1,s0,-72
    800040d2:	6c88                	ld	a0,24(s1)
    800040d4:	c80ff0ef          	jal	80003554 <stati>
    iunlock(f->ip);
    800040d8:	6c88                	ld	a0,24(s1)
    800040da:	9c2ff0ef          	jal	8000329c <iunlock>
    if(copyout(p->pagetable, addr, (char *)&st, sizeof(st)) < 0)
    800040de:	46e1                	li	a3,24
    800040e0:	fb840613          	addi	a2,s0,-72
    800040e4:	85ce                	mv	a1,s3
    800040e6:	05093503          	ld	a0,80(s2)
    800040ea:	d04fd0ef          	jal	800015ee <copyout>
    800040ee:	41f5551b          	sraiw	a0,a0,0x1f
    800040f2:	7942                	ld	s2,48(sp)
      return -1;
    return 0;
  }
  return -1;
}
    800040f4:	60a6                	ld	ra,72(sp)
    800040f6:	6406                	ld	s0,64(sp)
    800040f8:	74e2                	ld	s1,56(sp)
    800040fa:	79a2                	ld	s3,40(sp)
    800040fc:	6161                	addi	sp,sp,80
    800040fe:	8082                	ret
  return -1;
    80004100:	557d                	li	a0,-1
    80004102:	bfcd                	j	800040f4 <filestat+0x4e>

0000000080004104 <fileread>:

// Read from file f.
// addr is a user virtual address.
int
fileread(struct file *f, uint64 addr, int n)
{
    80004104:	7179                	addi	sp,sp,-48
    80004106:	f406                	sd	ra,40(sp)
    80004108:	f022                	sd	s0,32(sp)
    8000410a:	e84a                	sd	s2,16(sp)
    8000410c:	1800                	addi	s0,sp,48
  int r = 0;

  if(f->readable == 0)
    8000410e:	00854783          	lbu	a5,8(a0)
    80004112:	cfd1                	beqz	a5,800041ae <fileread+0xaa>
    80004114:	ec26                	sd	s1,24(sp)
    80004116:	e44e                	sd	s3,8(sp)
    80004118:	84aa                	mv	s1,a0
    8000411a:	89ae                	mv	s3,a1
    8000411c:	8932                	mv	s2,a2
    return -1;

  if(f->type == FD_PIPE){
    8000411e:	411c                	lw	a5,0(a0)
    80004120:	4705                	li	a4,1
    80004122:	04e78363          	beq	a5,a4,80004168 <fileread+0x64>
    r = piperead(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    80004126:	470d                	li	a4,3
    80004128:	04e78763          	beq	a5,a4,80004176 <fileread+0x72>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
    r = devsw[f->major].read(1, addr, n);
  } else if(f->type == FD_INODE){
    8000412c:	4709                	li	a4,2
    8000412e:	06e79a63          	bne	a5,a4,800041a2 <fileread+0x9e>
    ilock(f->ip);
    80004132:	6d08                	ld	a0,24(a0)
    80004134:	8baff0ef          	jal	800031ee <ilock>
    if((r = readi(f->ip, 1, addr, f->off, n)) > 0)
    80004138:	874a                	mv	a4,s2
    8000413a:	5094                	lw	a3,32(s1)
    8000413c:	864e                	mv	a2,s3
    8000413e:	4585                	li	a1,1
    80004140:	6c88                	ld	a0,24(s1)
    80004142:	c3cff0ef          	jal	8000357e <readi>
    80004146:	892a                	mv	s2,a0
    80004148:	00a05563          	blez	a0,80004152 <fileread+0x4e>
      f->off += r;
    8000414c:	509c                	lw	a5,32(s1)
    8000414e:	9fa9                	addw	a5,a5,a0
    80004150:	d09c                	sw	a5,32(s1)
    iunlock(f->ip);
    80004152:	6c88                	ld	a0,24(s1)
    80004154:	948ff0ef          	jal	8000329c <iunlock>
    80004158:	64e2                	ld	s1,24(sp)
    8000415a:	69a2                	ld	s3,8(sp)
  } else {
    panic("fileread");
  }

  return r;
}
    8000415c:	854a                	mv	a0,s2
    8000415e:	70a2                	ld	ra,40(sp)
    80004160:	7402                	ld	s0,32(sp)
    80004162:	6942                	ld	s2,16(sp)
    80004164:	6145                	addi	sp,sp,48
    80004166:	8082                	ret
    r = piperead(f->pipe, addr, n);
    80004168:	6908                	ld	a0,16(a0)
    8000416a:	388000ef          	jal	800044f2 <piperead>
    8000416e:	892a                	mv	s2,a0
    80004170:	64e2                	ld	s1,24(sp)
    80004172:	69a2                	ld	s3,8(sp)
    80004174:	b7e5                	j	8000415c <fileread+0x58>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
    80004176:	02451783          	lh	a5,36(a0)
    8000417a:	03079693          	slli	a3,a5,0x30
    8000417e:	92c1                	srli	a3,a3,0x30
    80004180:	4725                	li	a4,9
    80004182:	02d76863          	bltu	a4,a3,800041b2 <fileread+0xae>
    80004186:	0792                	slli	a5,a5,0x4
    80004188:	0001e717          	auipc	a4,0x1e
    8000418c:	2d870713          	addi	a4,a4,728 # 80022460 <devsw>
    80004190:	97ba                	add	a5,a5,a4
    80004192:	639c                	ld	a5,0(a5)
    80004194:	c39d                	beqz	a5,800041ba <fileread+0xb6>
    r = devsw[f->major].read(1, addr, n);
    80004196:	4505                	li	a0,1
    80004198:	9782                	jalr	a5
    8000419a:	892a                	mv	s2,a0
    8000419c:	64e2                	ld	s1,24(sp)
    8000419e:	69a2                	ld	s3,8(sp)
    800041a0:	bf75                	j	8000415c <fileread+0x58>
    panic("fileread");
    800041a2:	00003517          	auipc	a0,0x3
    800041a6:	3d650513          	addi	a0,a0,982 # 80007578 <etext+0x578>
    800041aa:	e36fc0ef          	jal	800007e0 <panic>
    return -1;
    800041ae:	597d                	li	s2,-1
    800041b0:	b775                	j	8000415c <fileread+0x58>
      return -1;
    800041b2:	597d                	li	s2,-1
    800041b4:	64e2                	ld	s1,24(sp)
    800041b6:	69a2                	ld	s3,8(sp)
    800041b8:	b755                	j	8000415c <fileread+0x58>
    800041ba:	597d                	li	s2,-1
    800041bc:	64e2                	ld	s1,24(sp)
    800041be:	69a2                	ld	s3,8(sp)
    800041c0:	bf71                	j	8000415c <fileread+0x58>

00000000800041c2 <filewrite>:
int
filewrite(struct file *f, uint64 addr, int n)
{
  int r, ret = 0;

  if(f->writable == 0)
    800041c2:	00954783          	lbu	a5,9(a0)
    800041c6:	10078b63          	beqz	a5,800042dc <filewrite+0x11a>
{
    800041ca:	715d                	addi	sp,sp,-80
    800041cc:	e486                	sd	ra,72(sp)
    800041ce:	e0a2                	sd	s0,64(sp)
    800041d0:	f84a                	sd	s2,48(sp)
    800041d2:	f052                	sd	s4,32(sp)
    800041d4:	e85a                	sd	s6,16(sp)
    800041d6:	0880                	addi	s0,sp,80
    800041d8:	892a                	mv	s2,a0
    800041da:	8b2e                	mv	s6,a1
    800041dc:	8a32                	mv	s4,a2
    return -1;

  if(f->type == FD_PIPE){
    800041de:	411c                	lw	a5,0(a0)
    800041e0:	4705                	li	a4,1
    800041e2:	02e78763          	beq	a5,a4,80004210 <filewrite+0x4e>
    ret = pipewrite(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    800041e6:	470d                	li	a4,3
    800041e8:	02e78863          	beq	a5,a4,80004218 <filewrite+0x56>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
    ret = devsw[f->major].write(1, addr, n);
  } else if(f->type == FD_INODE){
    800041ec:	4709                	li	a4,2
    800041ee:	0ce79c63          	bne	a5,a4,800042c6 <filewrite+0x104>
    800041f2:	f44e                	sd	s3,40(sp)
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * BSIZE;
    int i = 0;
    while(i < n){
    800041f4:	0ac05863          	blez	a2,800042a4 <filewrite+0xe2>
    800041f8:	fc26                	sd	s1,56(sp)
    800041fa:	ec56                	sd	s5,24(sp)
    800041fc:	e45e                	sd	s7,8(sp)
    800041fe:	e062                	sd	s8,0(sp)
    int i = 0;
    80004200:	4981                	li	s3,0
      int n1 = n - i;
      if(n1 > max)
    80004202:	6b85                	lui	s7,0x1
    80004204:	c00b8b93          	addi	s7,s7,-1024 # c00 <_entry-0x7ffff400>
    80004208:	6c05                	lui	s8,0x1
    8000420a:	c00c0c1b          	addiw	s8,s8,-1024 # c00 <_entry-0x7ffff400>
    8000420e:	a8b5                	j	8000428a <filewrite+0xc8>
    ret = pipewrite(f->pipe, addr, n);
    80004210:	6908                	ld	a0,16(a0)
    80004212:	1fc000ef          	jal	8000440e <pipewrite>
    80004216:	a04d                	j	800042b8 <filewrite+0xf6>
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
    80004218:	02451783          	lh	a5,36(a0)
    8000421c:	03079693          	slli	a3,a5,0x30
    80004220:	92c1                	srli	a3,a3,0x30
    80004222:	4725                	li	a4,9
    80004224:	0ad76e63          	bltu	a4,a3,800042e0 <filewrite+0x11e>
    80004228:	0792                	slli	a5,a5,0x4
    8000422a:	0001e717          	auipc	a4,0x1e
    8000422e:	23670713          	addi	a4,a4,566 # 80022460 <devsw>
    80004232:	97ba                	add	a5,a5,a4
    80004234:	679c                	ld	a5,8(a5)
    80004236:	c7dd                	beqz	a5,800042e4 <filewrite+0x122>
    ret = devsw[f->major].write(1, addr, n);
    80004238:	4505                	li	a0,1
    8000423a:	9782                	jalr	a5
    8000423c:	a8b5                	j	800042b8 <filewrite+0xf6>
      if(n1 > max)
    8000423e:	00048a9b          	sext.w	s5,s1
        n1 = max;

      begin_op();
    80004242:	997ff0ef          	jal	80003bd8 <begin_op>
      ilock(f->ip);
    80004246:	01893503          	ld	a0,24(s2)
    8000424a:	fa5fe0ef          	jal	800031ee <ilock>
      if ((r = writei(f->ip, 1, addr + i, f->off, n1)) > 0)
    8000424e:	8756                	mv	a4,s5
    80004250:	02092683          	lw	a3,32(s2)
    80004254:	01698633          	add	a2,s3,s6
    80004258:	4585                	li	a1,1
    8000425a:	01893503          	ld	a0,24(s2)
    8000425e:	c1cff0ef          	jal	8000367a <writei>
    80004262:	84aa                	mv	s1,a0
    80004264:	00a05763          	blez	a0,80004272 <filewrite+0xb0>
        f->off += r;
    80004268:	02092783          	lw	a5,32(s2)
    8000426c:	9fa9                	addw	a5,a5,a0
    8000426e:	02f92023          	sw	a5,32(s2)
      iunlock(f->ip);
    80004272:	01893503          	ld	a0,24(s2)
    80004276:	826ff0ef          	jal	8000329c <iunlock>
      end_op();
    8000427a:	9c9ff0ef          	jal	80003c42 <end_op>

      if(r != n1){
    8000427e:	029a9563          	bne	s5,s1,800042a8 <filewrite+0xe6>
        // error from writei
        break;
      }
      i += r;
    80004282:	013489bb          	addw	s3,s1,s3
    while(i < n){
    80004286:	0149da63          	bge	s3,s4,8000429a <filewrite+0xd8>
      int n1 = n - i;
    8000428a:	413a04bb          	subw	s1,s4,s3
      if(n1 > max)
    8000428e:	0004879b          	sext.w	a5,s1
    80004292:	fafbd6e3          	bge	s7,a5,8000423e <filewrite+0x7c>
    80004296:	84e2                	mv	s1,s8
    80004298:	b75d                	j	8000423e <filewrite+0x7c>
    8000429a:	74e2                	ld	s1,56(sp)
    8000429c:	6ae2                	ld	s5,24(sp)
    8000429e:	6ba2                	ld	s7,8(sp)
    800042a0:	6c02                	ld	s8,0(sp)
    800042a2:	a039                	j	800042b0 <filewrite+0xee>
    int i = 0;
    800042a4:	4981                	li	s3,0
    800042a6:	a029                	j	800042b0 <filewrite+0xee>
    800042a8:	74e2                	ld	s1,56(sp)
    800042aa:	6ae2                	ld	s5,24(sp)
    800042ac:	6ba2                	ld	s7,8(sp)
    800042ae:	6c02                	ld	s8,0(sp)
    }
    ret = (i == n ? n : -1);
    800042b0:	033a1c63          	bne	s4,s3,800042e8 <filewrite+0x126>
    800042b4:	8552                	mv	a0,s4
    800042b6:	79a2                	ld	s3,40(sp)
  } else {
    panic("filewrite");
  }

  return ret;
}
    800042b8:	60a6                	ld	ra,72(sp)
    800042ba:	6406                	ld	s0,64(sp)
    800042bc:	7942                	ld	s2,48(sp)
    800042be:	7a02                	ld	s4,32(sp)
    800042c0:	6b42                	ld	s6,16(sp)
    800042c2:	6161                	addi	sp,sp,80
    800042c4:	8082                	ret
    800042c6:	fc26                	sd	s1,56(sp)
    800042c8:	f44e                	sd	s3,40(sp)
    800042ca:	ec56                	sd	s5,24(sp)
    800042cc:	e45e                	sd	s7,8(sp)
    800042ce:	e062                	sd	s8,0(sp)
    panic("filewrite");
    800042d0:	00003517          	auipc	a0,0x3
    800042d4:	2b850513          	addi	a0,a0,696 # 80007588 <etext+0x588>
    800042d8:	d08fc0ef          	jal	800007e0 <panic>
    return -1;
    800042dc:	557d                	li	a0,-1
}
    800042de:	8082                	ret
      return -1;
    800042e0:	557d                	li	a0,-1
    800042e2:	bfd9                	j	800042b8 <filewrite+0xf6>
    800042e4:	557d                	li	a0,-1
    800042e6:	bfc9                	j	800042b8 <filewrite+0xf6>
    ret = (i == n ? n : -1);
    800042e8:	557d                	li	a0,-1
    800042ea:	79a2                	ld	s3,40(sp)
    800042ec:	b7f1                	j	800042b8 <filewrite+0xf6>

00000000800042ee <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
    800042ee:	7179                	addi	sp,sp,-48
    800042f0:	f406                	sd	ra,40(sp)
    800042f2:	f022                	sd	s0,32(sp)
    800042f4:	ec26                	sd	s1,24(sp)
    800042f6:	e052                	sd	s4,0(sp)
    800042f8:	1800                	addi	s0,sp,48
    800042fa:	84aa                	mv	s1,a0
    800042fc:	8a2e                	mv	s4,a1
  struct pipe *pi;

  pi = 0;
  *f0 = *f1 = 0;
    800042fe:	0005b023          	sd	zero,0(a1)
    80004302:	00053023          	sd	zero,0(a0)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    80004306:	c3bff0ef          	jal	80003f40 <filealloc>
    8000430a:	e088                	sd	a0,0(s1)
    8000430c:	c549                	beqz	a0,80004396 <pipealloc+0xa8>
    8000430e:	c33ff0ef          	jal	80003f40 <filealloc>
    80004312:	00aa3023          	sd	a0,0(s4)
    80004316:	cd25                	beqz	a0,8000438e <pipealloc+0xa0>
    80004318:	e84a                	sd	s2,16(sp)
    goto bad;
  if((pi = (struct pipe*)kalloc()) == 0)
    8000431a:	fe4fc0ef          	jal	80000afe <kalloc>
    8000431e:	892a                	mv	s2,a0
    80004320:	c12d                	beqz	a0,80004382 <pipealloc+0x94>
    80004322:	e44e                	sd	s3,8(sp)
    goto bad;
  pi->readopen = 1;
    80004324:	4985                	li	s3,1
    80004326:	23352023          	sw	s3,544(a0)
  pi->writeopen = 1;
    8000432a:	23352223          	sw	s3,548(a0)
  pi->nwrite = 0;
    8000432e:	20052e23          	sw	zero,540(a0)
  pi->nread = 0;
    80004332:	20052c23          	sw	zero,536(a0)
  initlock(&pi->lock, "pipe");
    80004336:	00003597          	auipc	a1,0x3
    8000433a:	26258593          	addi	a1,a1,610 # 80007598 <etext+0x598>
    8000433e:	811fc0ef          	jal	80000b4e <initlock>
  (*f0)->type = FD_PIPE;
    80004342:	609c                	ld	a5,0(s1)
    80004344:	0137a023          	sw	s3,0(a5)
  (*f0)->readable = 1;
    80004348:	609c                	ld	a5,0(s1)
    8000434a:	01378423          	sb	s3,8(a5)
  (*f0)->writable = 0;
    8000434e:	609c                	ld	a5,0(s1)
    80004350:	000784a3          	sb	zero,9(a5)
  (*f0)->pipe = pi;
    80004354:	609c                	ld	a5,0(s1)
    80004356:	0127b823          	sd	s2,16(a5)
  (*f1)->type = FD_PIPE;
    8000435a:	000a3783          	ld	a5,0(s4)
    8000435e:	0137a023          	sw	s3,0(a5)
  (*f1)->readable = 0;
    80004362:	000a3783          	ld	a5,0(s4)
    80004366:	00078423          	sb	zero,8(a5)
  (*f1)->writable = 1;
    8000436a:	000a3783          	ld	a5,0(s4)
    8000436e:	013784a3          	sb	s3,9(a5)
  (*f1)->pipe = pi;
    80004372:	000a3783          	ld	a5,0(s4)
    80004376:	0127b823          	sd	s2,16(a5)
  return 0;
    8000437a:	4501                	li	a0,0
    8000437c:	6942                	ld	s2,16(sp)
    8000437e:	69a2                	ld	s3,8(sp)
    80004380:	a01d                	j	800043a6 <pipealloc+0xb8>

 bad:
  if(pi)
    kfree((char*)pi);
  if(*f0)
    80004382:	6088                	ld	a0,0(s1)
    80004384:	c119                	beqz	a0,8000438a <pipealloc+0x9c>
    80004386:	6942                	ld	s2,16(sp)
    80004388:	a029                	j	80004392 <pipealloc+0xa4>
    8000438a:	6942                	ld	s2,16(sp)
    8000438c:	a029                	j	80004396 <pipealloc+0xa8>
    8000438e:	6088                	ld	a0,0(s1)
    80004390:	c10d                	beqz	a0,800043b2 <pipealloc+0xc4>
    fileclose(*f0);
    80004392:	c53ff0ef          	jal	80003fe4 <fileclose>
  if(*f1)
    80004396:	000a3783          	ld	a5,0(s4)
    fileclose(*f1);
  return -1;
    8000439a:	557d                	li	a0,-1
  if(*f1)
    8000439c:	c789                	beqz	a5,800043a6 <pipealloc+0xb8>
    fileclose(*f1);
    8000439e:	853e                	mv	a0,a5
    800043a0:	c45ff0ef          	jal	80003fe4 <fileclose>
  return -1;
    800043a4:	557d                	li	a0,-1
}
    800043a6:	70a2                	ld	ra,40(sp)
    800043a8:	7402                	ld	s0,32(sp)
    800043aa:	64e2                	ld	s1,24(sp)
    800043ac:	6a02                	ld	s4,0(sp)
    800043ae:	6145                	addi	sp,sp,48
    800043b0:	8082                	ret
  return -1;
    800043b2:	557d                	li	a0,-1
    800043b4:	bfcd                	j	800043a6 <pipealloc+0xb8>

00000000800043b6 <pipeclose>:

void
pipeclose(struct pipe *pi, int writable)
{
    800043b6:	1101                	addi	sp,sp,-32
    800043b8:	ec06                	sd	ra,24(sp)
    800043ba:	e822                	sd	s0,16(sp)
    800043bc:	e426                	sd	s1,8(sp)
    800043be:	e04a                	sd	s2,0(sp)
    800043c0:	1000                	addi	s0,sp,32
    800043c2:	84aa                	mv	s1,a0
    800043c4:	892e                	mv	s2,a1
  acquire(&pi->lock);
    800043c6:	809fc0ef          	jal	80000bce <acquire>
  if(writable){
    800043ca:	02090763          	beqz	s2,800043f8 <pipeclose+0x42>
    pi->writeopen = 0;
    800043ce:	2204a223          	sw	zero,548(s1)
    wakeup(&pi->nread);
    800043d2:	21848513          	addi	a0,s1,536
    800043d6:	b5bfd0ef          	jal	80001f30 <wakeup>
  } else {
    pi->readopen = 0;
    wakeup(&pi->nwrite);
  }
  if(pi->readopen == 0 && pi->writeopen == 0){
    800043da:	2204b783          	ld	a5,544(s1)
    800043de:	e785                	bnez	a5,80004406 <pipeclose+0x50>
    release(&pi->lock);
    800043e0:	8526                	mv	a0,s1
    800043e2:	885fc0ef          	jal	80000c66 <release>
    kfree((char*)pi);
    800043e6:	8526                	mv	a0,s1
    800043e8:	e34fc0ef          	jal	80000a1c <kfree>
  } else
    release(&pi->lock);
}
    800043ec:	60e2                	ld	ra,24(sp)
    800043ee:	6442                	ld	s0,16(sp)
    800043f0:	64a2                	ld	s1,8(sp)
    800043f2:	6902                	ld	s2,0(sp)
    800043f4:	6105                	addi	sp,sp,32
    800043f6:	8082                	ret
    pi->readopen = 0;
    800043f8:	2204a023          	sw	zero,544(s1)
    wakeup(&pi->nwrite);
    800043fc:	21c48513          	addi	a0,s1,540
    80004400:	b31fd0ef          	jal	80001f30 <wakeup>
    80004404:	bfd9                	j	800043da <pipeclose+0x24>
    release(&pi->lock);
    80004406:	8526                	mv	a0,s1
    80004408:	85ffc0ef          	jal	80000c66 <release>
}
    8000440c:	b7c5                	j	800043ec <pipeclose+0x36>

000000008000440e <pipewrite>:

int
pipewrite(struct pipe *pi, uint64 addr, int n)
{
    8000440e:	711d                	addi	sp,sp,-96
    80004410:	ec86                	sd	ra,88(sp)
    80004412:	e8a2                	sd	s0,80(sp)
    80004414:	e4a6                	sd	s1,72(sp)
    80004416:	e0ca                	sd	s2,64(sp)
    80004418:	fc4e                	sd	s3,56(sp)
    8000441a:	f852                	sd	s4,48(sp)
    8000441c:	f456                	sd	s5,40(sp)
    8000441e:	1080                	addi	s0,sp,96
    80004420:	84aa                	mv	s1,a0
    80004422:	8aae                	mv	s5,a1
    80004424:	8a32                	mv	s4,a2
  int i = 0;
  struct proc *pr = myproc();
    80004426:	cb4fd0ef          	jal	800018da <myproc>
    8000442a:	89aa                	mv	s3,a0

  acquire(&pi->lock);
    8000442c:	8526                	mv	a0,s1
    8000442e:	fa0fc0ef          	jal	80000bce <acquire>
  while(i < n){
    80004432:	0b405a63          	blez	s4,800044e6 <pipewrite+0xd8>
    80004436:	f05a                	sd	s6,32(sp)
    80004438:	ec5e                	sd	s7,24(sp)
    8000443a:	e862                	sd	s8,16(sp)
  int i = 0;
    8000443c:	4901                	li	s2,0
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
      wakeup(&pi->nread);
      sleep(&pi->nwrite, &pi->lock);
    } else {
      char ch;
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    8000443e:	5b7d                	li	s6,-1
      wakeup(&pi->nread);
    80004440:	21848c13          	addi	s8,s1,536
      sleep(&pi->nwrite, &pi->lock);
    80004444:	21c48b93          	addi	s7,s1,540
    80004448:	a81d                	j	8000447e <pipewrite+0x70>
      release(&pi->lock);
    8000444a:	8526                	mv	a0,s1
    8000444c:	81bfc0ef          	jal	80000c66 <release>
      return -1;
    80004450:	597d                	li	s2,-1
    80004452:	7b02                	ld	s6,32(sp)
    80004454:	6be2                	ld	s7,24(sp)
    80004456:	6c42                	ld	s8,16(sp)
  }
  wakeup(&pi->nread);
  release(&pi->lock);

  return i;
}
    80004458:	854a                	mv	a0,s2
    8000445a:	60e6                	ld	ra,88(sp)
    8000445c:	6446                	ld	s0,80(sp)
    8000445e:	64a6                	ld	s1,72(sp)
    80004460:	6906                	ld	s2,64(sp)
    80004462:	79e2                	ld	s3,56(sp)
    80004464:	7a42                	ld	s4,48(sp)
    80004466:	7aa2                	ld	s5,40(sp)
    80004468:	6125                	addi	sp,sp,96
    8000446a:	8082                	ret
      wakeup(&pi->nread);
    8000446c:	8562                	mv	a0,s8
    8000446e:	ac3fd0ef          	jal	80001f30 <wakeup>
      sleep(&pi->nwrite, &pi->lock);
    80004472:	85a6                	mv	a1,s1
    80004474:	855e                	mv	a0,s7
    80004476:	a6ffd0ef          	jal	80001ee4 <sleep>
  while(i < n){
    8000447a:	05495b63          	bge	s2,s4,800044d0 <pipewrite+0xc2>
    if(pi->readopen == 0 || killed(pr)){
    8000447e:	2204a783          	lw	a5,544(s1)
    80004482:	d7e1                	beqz	a5,8000444a <pipewrite+0x3c>
    80004484:	854e                	mv	a0,s3
    80004486:	c97fd0ef          	jal	8000211c <killed>
    8000448a:	f161                	bnez	a0,8000444a <pipewrite+0x3c>
    if(pi->nwrite == pi->nread + PIPESIZE){ //DOC: pipewrite-full
    8000448c:	2184a783          	lw	a5,536(s1)
    80004490:	21c4a703          	lw	a4,540(s1)
    80004494:	2007879b          	addiw	a5,a5,512
    80004498:	fcf70ae3          	beq	a4,a5,8000446c <pipewrite+0x5e>
      if(copyin(pr->pagetable, &ch, addr + i, 1) == -1)
    8000449c:	4685                	li	a3,1
    8000449e:	01590633          	add	a2,s2,s5
    800044a2:	faf40593          	addi	a1,s0,-81
    800044a6:	0509b503          	ld	a0,80(s3)
    800044aa:	a28fd0ef          	jal	800016d2 <copyin>
    800044ae:	03650e63          	beq	a0,s6,800044ea <pipewrite+0xdc>
      pi->data[pi->nwrite++ % PIPESIZE] = ch;
    800044b2:	21c4a783          	lw	a5,540(s1)
    800044b6:	0017871b          	addiw	a4,a5,1
    800044ba:	20e4ae23          	sw	a4,540(s1)
    800044be:	1ff7f793          	andi	a5,a5,511
    800044c2:	97a6                	add	a5,a5,s1
    800044c4:	faf44703          	lbu	a4,-81(s0)
    800044c8:	00e78c23          	sb	a4,24(a5)
      i++;
    800044cc:	2905                	addiw	s2,s2,1
    800044ce:	b775                	j	8000447a <pipewrite+0x6c>
    800044d0:	7b02                	ld	s6,32(sp)
    800044d2:	6be2                	ld	s7,24(sp)
    800044d4:	6c42                	ld	s8,16(sp)
  wakeup(&pi->nread);
    800044d6:	21848513          	addi	a0,s1,536
    800044da:	a57fd0ef          	jal	80001f30 <wakeup>
  release(&pi->lock);
    800044de:	8526                	mv	a0,s1
    800044e0:	f86fc0ef          	jal	80000c66 <release>
  return i;
    800044e4:	bf95                	j	80004458 <pipewrite+0x4a>
  int i = 0;
    800044e6:	4901                	li	s2,0
    800044e8:	b7fd                	j	800044d6 <pipewrite+0xc8>
    800044ea:	7b02                	ld	s6,32(sp)
    800044ec:	6be2                	ld	s7,24(sp)
    800044ee:	6c42                	ld	s8,16(sp)
    800044f0:	b7dd                	j	800044d6 <pipewrite+0xc8>

00000000800044f2 <piperead>:

int
piperead(struct pipe *pi, uint64 addr, int n)
{
    800044f2:	715d                	addi	sp,sp,-80
    800044f4:	e486                	sd	ra,72(sp)
    800044f6:	e0a2                	sd	s0,64(sp)
    800044f8:	fc26                	sd	s1,56(sp)
    800044fa:	f84a                	sd	s2,48(sp)
    800044fc:	f44e                	sd	s3,40(sp)
    800044fe:	f052                	sd	s4,32(sp)
    80004500:	ec56                	sd	s5,24(sp)
    80004502:	0880                	addi	s0,sp,80
    80004504:	84aa                	mv	s1,a0
    80004506:	892e                	mv	s2,a1
    80004508:	8ab2                	mv	s5,a2
  int i;
  struct proc *pr = myproc();
    8000450a:	bd0fd0ef          	jal	800018da <myproc>
    8000450e:	8a2a                	mv	s4,a0
  char ch;

  acquire(&pi->lock);
    80004510:	8526                	mv	a0,s1
    80004512:	ebcfc0ef          	jal	80000bce <acquire>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004516:	2184a703          	lw	a4,536(s1)
    8000451a:	21c4a783          	lw	a5,540(s1)
    if(killed(pr)){
      release(&pi->lock);
      return -1;
    }
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    8000451e:	21848993          	addi	s3,s1,536
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    80004522:	02f71563          	bne	a4,a5,8000454c <piperead+0x5a>
    80004526:	2244a783          	lw	a5,548(s1)
    8000452a:	cb85                	beqz	a5,8000455a <piperead+0x68>
    if(killed(pr)){
    8000452c:	8552                	mv	a0,s4
    8000452e:	beffd0ef          	jal	8000211c <killed>
    80004532:	ed19                	bnez	a0,80004550 <piperead+0x5e>
    sleep(&pi->nread, &pi->lock); //DOC: piperead-sleep
    80004534:	85a6                	mv	a1,s1
    80004536:	854e                	mv	a0,s3
    80004538:	9adfd0ef          	jal	80001ee4 <sleep>
  while(pi->nread == pi->nwrite && pi->writeopen){  //DOC: pipe-empty
    8000453c:	2184a703          	lw	a4,536(s1)
    80004540:	21c4a783          	lw	a5,540(s1)
    80004544:	fef701e3          	beq	a4,a5,80004526 <piperead+0x34>
    80004548:	e85a                	sd	s6,16(sp)
    8000454a:	a809                	j	8000455c <piperead+0x6a>
    8000454c:	e85a                	sd	s6,16(sp)
    8000454e:	a039                	j	8000455c <piperead+0x6a>
      release(&pi->lock);
    80004550:	8526                	mv	a0,s1
    80004552:	f14fc0ef          	jal	80000c66 <release>
      return -1;
    80004556:	59fd                	li	s3,-1
    80004558:	a8b9                	j	800045b6 <piperead+0xc4>
    8000455a:	e85a                	sd	s6,16(sp)
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    8000455c:	4981                	li	s3,0
    if(pi->nread == pi->nwrite)
      break;
    ch = pi->data[pi->nread % PIPESIZE];
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1) {
    8000455e:	5b7d                	li	s6,-1
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    80004560:	05505363          	blez	s5,800045a6 <piperead+0xb4>
    if(pi->nread == pi->nwrite)
    80004564:	2184a783          	lw	a5,536(s1)
    80004568:	21c4a703          	lw	a4,540(s1)
    8000456c:	02f70d63          	beq	a4,a5,800045a6 <piperead+0xb4>
    ch = pi->data[pi->nread % PIPESIZE];
    80004570:	1ff7f793          	andi	a5,a5,511
    80004574:	97a6                	add	a5,a5,s1
    80004576:	0187c783          	lbu	a5,24(a5)
    8000457a:	faf40fa3          	sb	a5,-65(s0)
    if(copyout(pr->pagetable, addr + i, &ch, 1) == -1) {
    8000457e:	4685                	li	a3,1
    80004580:	fbf40613          	addi	a2,s0,-65
    80004584:	85ca                	mv	a1,s2
    80004586:	050a3503          	ld	a0,80(s4)
    8000458a:	864fd0ef          	jal	800015ee <copyout>
    8000458e:	03650e63          	beq	a0,s6,800045ca <piperead+0xd8>
      if(i == 0)
        i = -1;
      break;
    }
    pi->nread++;
    80004592:	2184a783          	lw	a5,536(s1)
    80004596:	2785                	addiw	a5,a5,1
    80004598:	20f4ac23          	sw	a5,536(s1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
    8000459c:	2985                	addiw	s3,s3,1
    8000459e:	0905                	addi	s2,s2,1
    800045a0:	fd3a92e3          	bne	s5,s3,80004564 <piperead+0x72>
    800045a4:	89d6                	mv	s3,s5
  }
  wakeup(&pi->nwrite);  //DOC: piperead-wakeup
    800045a6:	21c48513          	addi	a0,s1,540
    800045aa:	987fd0ef          	jal	80001f30 <wakeup>
  release(&pi->lock);
    800045ae:	8526                	mv	a0,s1
    800045b0:	eb6fc0ef          	jal	80000c66 <release>
    800045b4:	6b42                	ld	s6,16(sp)
  return i;
}
    800045b6:	854e                	mv	a0,s3
    800045b8:	60a6                	ld	ra,72(sp)
    800045ba:	6406                	ld	s0,64(sp)
    800045bc:	74e2                	ld	s1,56(sp)
    800045be:	7942                	ld	s2,48(sp)
    800045c0:	79a2                	ld	s3,40(sp)
    800045c2:	7a02                	ld	s4,32(sp)
    800045c4:	6ae2                	ld	s5,24(sp)
    800045c6:	6161                	addi	sp,sp,80
    800045c8:	8082                	ret
      if(i == 0)
    800045ca:	fc099ee3          	bnez	s3,800045a6 <piperead+0xb4>
        i = -1;
    800045ce:	89aa                	mv	s3,a0
    800045d0:	bfd9                	j	800045a6 <piperead+0xb4>

00000000800045d2 <flags2perm>:

static int loadseg(pde_t *, uint64, struct inode *, uint, uint);

// map ELF permissions to PTE permission bits.
int flags2perm(int flags)
{
    800045d2:	1141                	addi	sp,sp,-16
    800045d4:	e422                	sd	s0,8(sp)
    800045d6:	0800                	addi	s0,sp,16
    800045d8:	87aa                	mv	a5,a0
    int perm = 0;
    if(flags & 0x1)
    800045da:	8905                	andi	a0,a0,1
    800045dc:	050e                	slli	a0,a0,0x3
      perm = PTE_X;
    if(flags & 0x2)
    800045de:	8b89                	andi	a5,a5,2
    800045e0:	c399                	beqz	a5,800045e6 <flags2perm+0x14>
      perm |= PTE_W;
    800045e2:	00456513          	ori	a0,a0,4
    return perm;
}
    800045e6:	6422                	ld	s0,8(sp)
    800045e8:	0141                	addi	sp,sp,16
    800045ea:	8082                	ret

00000000800045ec <kexec>:
//
// the implementation of the exec() system call
//
int
kexec(char *path, char **argv)
{
    800045ec:	df010113          	addi	sp,sp,-528
    800045f0:	20113423          	sd	ra,520(sp)
    800045f4:	20813023          	sd	s0,512(sp)
    800045f8:	ffa6                	sd	s1,504(sp)
    800045fa:	fbca                	sd	s2,496(sp)
    800045fc:	0c00                	addi	s0,sp,528
    800045fe:	892a                	mv	s2,a0
    80004600:	dea43c23          	sd	a0,-520(s0)
    80004604:	e0b43023          	sd	a1,-512(s0)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  struct proc *p = myproc();
    80004608:	ad2fd0ef          	jal	800018da <myproc>
    8000460c:	84aa                	mv	s1,a0

  begin_op();
    8000460e:	dcaff0ef          	jal	80003bd8 <begin_op>

  // Open the executable file.
  if((ip = namei(path)) == 0){
    80004612:	854a                	mv	a0,s2
    80004614:	bf0ff0ef          	jal	80003a04 <namei>
    80004618:	c931                	beqz	a0,8000466c <kexec+0x80>
    8000461a:	f3d2                	sd	s4,480(sp)
    8000461c:	8a2a                	mv	s4,a0
    end_op();
    return -1;
  }
  ilock(ip);
    8000461e:	bd1fe0ef          	jal	800031ee <ilock>

  // Read the ELF header.
  if(readi(ip, 0, (uint64)&elf, 0, sizeof(elf)) != sizeof(elf))
    80004622:	04000713          	li	a4,64
    80004626:	4681                	li	a3,0
    80004628:	e5040613          	addi	a2,s0,-432
    8000462c:	4581                	li	a1,0
    8000462e:	8552                	mv	a0,s4
    80004630:	f4ffe0ef          	jal	8000357e <readi>
    80004634:	04000793          	li	a5,64
    80004638:	00f51a63          	bne	a0,a5,8000464c <kexec+0x60>
    goto bad;

  // Is this really an ELF file?
  if(elf.magic != ELF_MAGIC)
    8000463c:	e5042703          	lw	a4,-432(s0)
    80004640:	464c47b7          	lui	a5,0x464c4
    80004644:	57f78793          	addi	a5,a5,1407 # 464c457f <_entry-0x39b3ba81>
    80004648:	02f70663          	beq	a4,a5,80004674 <kexec+0x88>

 bad:
  if(pagetable)
    proc_freepagetable(pagetable, sz);
  if(ip){
    iunlockput(ip);
    8000464c:	8552                	mv	a0,s4
    8000464e:	dabfe0ef          	jal	800033f8 <iunlockput>
    end_op();
    80004652:	df0ff0ef          	jal	80003c42 <end_op>
  }
  return -1;
    80004656:	557d                	li	a0,-1
    80004658:	7a1e                	ld	s4,480(sp)
}
    8000465a:	20813083          	ld	ra,520(sp)
    8000465e:	20013403          	ld	s0,512(sp)
    80004662:	74fe                	ld	s1,504(sp)
    80004664:	795e                	ld	s2,496(sp)
    80004666:	21010113          	addi	sp,sp,528
    8000466a:	8082                	ret
    end_op();
    8000466c:	dd6ff0ef          	jal	80003c42 <end_op>
    return -1;
    80004670:	557d                	li	a0,-1
    80004672:	b7e5                	j	8000465a <kexec+0x6e>
    80004674:	ebda                	sd	s6,464(sp)
  if((pagetable = proc_pagetable(p)) == 0)
    80004676:	8526                	mv	a0,s1
    80004678:	b68fd0ef          	jal	800019e0 <proc_pagetable>
    8000467c:	8b2a                	mv	s6,a0
    8000467e:	2c050b63          	beqz	a0,80004954 <kexec+0x368>
    80004682:	f7ce                	sd	s3,488(sp)
    80004684:	efd6                	sd	s5,472(sp)
    80004686:	e7de                	sd	s7,456(sp)
    80004688:	e3e2                	sd	s8,448(sp)
    8000468a:	ff66                	sd	s9,440(sp)
    8000468c:	fb6a                	sd	s10,432(sp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    8000468e:	e7042d03          	lw	s10,-400(s0)
    80004692:	e8845783          	lhu	a5,-376(s0)
    80004696:	12078963          	beqz	a5,800047c8 <kexec+0x1dc>
    8000469a:	f76e                	sd	s11,424(sp)
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    8000469c:	4901                	li	s2,0
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    8000469e:	4d81                	li	s11,0
    if(ph.vaddr % PGSIZE != 0)
    800046a0:	6c85                	lui	s9,0x1
    800046a2:	fffc8793          	addi	a5,s9,-1 # fff <_entry-0x7ffff001>
    800046a6:	def43823          	sd	a5,-528(s0)

  for(i = 0; i < sz; i += PGSIZE){
    pa = walkaddr(pagetable, va + i);
    if(pa == 0)
      panic("loadseg: address should exist");
    if(sz - i < PGSIZE)
    800046aa:	6a85                	lui	s5,0x1
    800046ac:	a085                	j	8000470c <kexec+0x120>
      panic("loadseg: address should exist");
    800046ae:	00003517          	auipc	a0,0x3
    800046b2:	ef250513          	addi	a0,a0,-270 # 800075a0 <etext+0x5a0>
    800046b6:	92afc0ef          	jal	800007e0 <panic>
    if(sz - i < PGSIZE)
    800046ba:	2481                	sext.w	s1,s1
      n = sz - i;
    else
      n = PGSIZE;
    if(readi(ip, 0, (uint64)pa, offset+i, n) != n)
    800046bc:	8726                	mv	a4,s1
    800046be:	012c06bb          	addw	a3,s8,s2
    800046c2:	4581                	li	a1,0
    800046c4:	8552                	mv	a0,s4
    800046c6:	eb9fe0ef          	jal	8000357e <readi>
    800046ca:	2501                	sext.w	a0,a0
    800046cc:	24a49a63          	bne	s1,a0,80004920 <kexec+0x334>
  for(i = 0; i < sz; i += PGSIZE){
    800046d0:	012a893b          	addw	s2,s5,s2
    800046d4:	03397363          	bgeu	s2,s3,800046fa <kexec+0x10e>
    pa = walkaddr(pagetable, va + i);
    800046d8:	02091593          	slli	a1,s2,0x20
    800046dc:	9181                	srli	a1,a1,0x20
    800046de:	95de                	add	a1,a1,s7
    800046e0:	855a                	mv	a0,s6
    800046e2:	8dbfc0ef          	jal	80000fbc <walkaddr>
    800046e6:	862a                	mv	a2,a0
    if(pa == 0)
    800046e8:	d179                	beqz	a0,800046ae <kexec+0xc2>
    if(sz - i < PGSIZE)
    800046ea:	412984bb          	subw	s1,s3,s2
    800046ee:	0004879b          	sext.w	a5,s1
    800046f2:	fcfcf4e3          	bgeu	s9,a5,800046ba <kexec+0xce>
    800046f6:	84d6                	mv	s1,s5
    800046f8:	b7c9                	j	800046ba <kexec+0xce>
    sz = sz1;
    800046fa:	e0843903          	ld	s2,-504(s0)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    800046fe:	2d85                	addiw	s11,s11,1
    80004700:	038d0d1b          	addiw	s10,s10,56 # 1038 <_entry-0x7fffefc8>
    80004704:	e8845783          	lhu	a5,-376(s0)
    80004708:	08fdd063          	bge	s11,a5,80004788 <kexec+0x19c>
    if(readi(ip, 0, (uint64)&ph, off, sizeof(ph)) != sizeof(ph))
    8000470c:	2d01                	sext.w	s10,s10
    8000470e:	03800713          	li	a4,56
    80004712:	86ea                	mv	a3,s10
    80004714:	e1840613          	addi	a2,s0,-488
    80004718:	4581                	li	a1,0
    8000471a:	8552                	mv	a0,s4
    8000471c:	e63fe0ef          	jal	8000357e <readi>
    80004720:	03800793          	li	a5,56
    80004724:	1cf51663          	bne	a0,a5,800048f0 <kexec+0x304>
    if(ph.type != ELF_PROG_LOAD)
    80004728:	e1842783          	lw	a5,-488(s0)
    8000472c:	4705                	li	a4,1
    8000472e:	fce798e3          	bne	a5,a4,800046fe <kexec+0x112>
    if(ph.memsz < ph.filesz)
    80004732:	e4043483          	ld	s1,-448(s0)
    80004736:	e3843783          	ld	a5,-456(s0)
    8000473a:	1af4ef63          	bltu	s1,a5,800048f8 <kexec+0x30c>
    if(ph.vaddr + ph.memsz < ph.vaddr)
    8000473e:	e2843783          	ld	a5,-472(s0)
    80004742:	94be                	add	s1,s1,a5
    80004744:	1af4ee63          	bltu	s1,a5,80004900 <kexec+0x314>
    if(ph.vaddr % PGSIZE != 0)
    80004748:	df043703          	ld	a4,-528(s0)
    8000474c:	8ff9                	and	a5,a5,a4
    8000474e:	1a079d63          	bnez	a5,80004908 <kexec+0x31c>
    if((sz1 = uvmalloc(pagetable, sz, ph.vaddr + ph.memsz, flags2perm(ph.flags))) == 0)
    80004752:	e1c42503          	lw	a0,-484(s0)
    80004756:	e7dff0ef          	jal	800045d2 <flags2perm>
    8000475a:	86aa                	mv	a3,a0
    8000475c:	8626                	mv	a2,s1
    8000475e:	85ca                	mv	a1,s2
    80004760:	855a                	mv	a0,s6
    80004762:	b33fc0ef          	jal	80001294 <uvmalloc>
    80004766:	e0a43423          	sd	a0,-504(s0)
    8000476a:	1a050363          	beqz	a0,80004910 <kexec+0x324>
    if(loadseg(pagetable, ph.vaddr, ip, ph.off, ph.filesz) < 0)
    8000476e:	e2843b83          	ld	s7,-472(s0)
    80004772:	e2042c03          	lw	s8,-480(s0)
    80004776:	e3842983          	lw	s3,-456(s0)
  for(i = 0; i < sz; i += PGSIZE){
    8000477a:	00098463          	beqz	s3,80004782 <kexec+0x196>
    8000477e:	4901                	li	s2,0
    80004780:	bfa1                	j	800046d8 <kexec+0xec>
    sz = sz1;
    80004782:	e0843903          	ld	s2,-504(s0)
    80004786:	bfa5                	j	800046fe <kexec+0x112>
    80004788:	7dba                	ld	s11,424(sp)
  iunlockput(ip);
    8000478a:	8552                	mv	a0,s4
    8000478c:	c6dfe0ef          	jal	800033f8 <iunlockput>
  end_op();
    80004790:	cb2ff0ef          	jal	80003c42 <end_op>
  p = myproc();
    80004794:	946fd0ef          	jal	800018da <myproc>
    80004798:	8aaa                	mv	s5,a0
  uint64 oldsz = p->sz;
    8000479a:	04853c83          	ld	s9,72(a0)
  sz = PGROUNDUP(sz);
    8000479e:	6985                	lui	s3,0x1
    800047a0:	19fd                	addi	s3,s3,-1 # fff <_entry-0x7ffff001>
    800047a2:	99ca                	add	s3,s3,s2
    800047a4:	77fd                	lui	a5,0xfffff
    800047a6:	00f9f9b3          	and	s3,s3,a5
  if((sz1 = uvmalloc(pagetable, sz, sz + (USERSTACK+1)*PGSIZE, PTE_W)) == 0)
    800047aa:	4691                	li	a3,4
    800047ac:	6609                	lui	a2,0x2
    800047ae:	964e                	add	a2,a2,s3
    800047b0:	85ce                	mv	a1,s3
    800047b2:	855a                	mv	a0,s6
    800047b4:	ae1fc0ef          	jal	80001294 <uvmalloc>
    800047b8:	892a                	mv	s2,a0
    800047ba:	e0a43423          	sd	a0,-504(s0)
    800047be:	e519                	bnez	a0,800047cc <kexec+0x1e0>
  if(pagetable)
    800047c0:	e1343423          	sd	s3,-504(s0)
    800047c4:	4a01                	li	s4,0
    800047c6:	aab1                	j	80004922 <kexec+0x336>
  uint64 argc, sz = 0, sp, ustack[MAXARG], stackbase;
    800047c8:	4901                	li	s2,0
    800047ca:	b7c1                	j	8000478a <kexec+0x19e>
  uvmclear(pagetable, sz-(USERSTACK+1)*PGSIZE);
    800047cc:	75f9                	lui	a1,0xffffe
    800047ce:	95aa                	add	a1,a1,a0
    800047d0:	855a                	mv	a0,s6
    800047d2:	c99fc0ef          	jal	8000146a <uvmclear>
  stackbase = sp - USERSTACK*PGSIZE;
    800047d6:	7bfd                	lui	s7,0xfffff
    800047d8:	9bca                	add	s7,s7,s2
  for(argc = 0; argv[argc]; argc++) {
    800047da:	e0043783          	ld	a5,-512(s0)
    800047de:	6388                	ld	a0,0(a5)
    800047e0:	cd39                	beqz	a0,8000483e <kexec+0x252>
    800047e2:	e9040993          	addi	s3,s0,-368
    800047e6:	f9040c13          	addi	s8,s0,-112
    800047ea:	4481                	li	s1,0
    sp -= strlen(argv[argc]) + 1;
    800047ec:	e26fc0ef          	jal	80000e12 <strlen>
    800047f0:	0015079b          	addiw	a5,a0,1
    800047f4:	40f907b3          	sub	a5,s2,a5
    sp -= sp % 16; // riscv sp must be 16-byte aligned
    800047f8:	ff07f913          	andi	s2,a5,-16
    if(sp < stackbase)
    800047fc:	11796e63          	bltu	s2,s7,80004918 <kexec+0x32c>
    if(copyout(pagetable, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
    80004800:	e0043d03          	ld	s10,-512(s0)
    80004804:	000d3a03          	ld	s4,0(s10)
    80004808:	8552                	mv	a0,s4
    8000480a:	e08fc0ef          	jal	80000e12 <strlen>
    8000480e:	0015069b          	addiw	a3,a0,1
    80004812:	8652                	mv	a2,s4
    80004814:	85ca                	mv	a1,s2
    80004816:	855a                	mv	a0,s6
    80004818:	dd7fc0ef          	jal	800015ee <copyout>
    8000481c:	10054063          	bltz	a0,8000491c <kexec+0x330>
    ustack[argc] = sp;
    80004820:	0129b023          	sd	s2,0(s3)
  for(argc = 0; argv[argc]; argc++) {
    80004824:	0485                	addi	s1,s1,1
    80004826:	008d0793          	addi	a5,s10,8
    8000482a:	e0f43023          	sd	a5,-512(s0)
    8000482e:	008d3503          	ld	a0,8(s10)
    80004832:	c909                	beqz	a0,80004844 <kexec+0x258>
    if(argc >= MAXARG)
    80004834:	09a1                	addi	s3,s3,8
    80004836:	fb899be3          	bne	s3,s8,800047ec <kexec+0x200>
  ip = 0;
    8000483a:	4a01                	li	s4,0
    8000483c:	a0dd                	j	80004922 <kexec+0x336>
  sp = sz;
    8000483e:	e0843903          	ld	s2,-504(s0)
  for(argc = 0; argv[argc]; argc++) {
    80004842:	4481                	li	s1,0
  ustack[argc] = 0;
    80004844:	00349793          	slli	a5,s1,0x3
    80004848:	f9078793          	addi	a5,a5,-112 # ffffffffffffef90 <end+0xffffffff7ffdb000>
    8000484c:	97a2                	add	a5,a5,s0
    8000484e:	f007b023          	sd	zero,-256(a5)
  sp -= (argc+1) * sizeof(uint64);
    80004852:	00148693          	addi	a3,s1,1
    80004856:	068e                	slli	a3,a3,0x3
    80004858:	40d90933          	sub	s2,s2,a3
  sp -= sp % 16;
    8000485c:	ff097913          	andi	s2,s2,-16
  sz = sz1;
    80004860:	e0843983          	ld	s3,-504(s0)
  if(sp < stackbase)
    80004864:	f5796ee3          	bltu	s2,s7,800047c0 <kexec+0x1d4>
  if(copyout(pagetable, sp, (char *)ustack, (argc+1)*sizeof(uint64)) < 0)
    80004868:	e9040613          	addi	a2,s0,-368
    8000486c:	85ca                	mv	a1,s2
    8000486e:	855a                	mv	a0,s6
    80004870:	d7ffc0ef          	jal	800015ee <copyout>
    80004874:	0e054263          	bltz	a0,80004958 <kexec+0x36c>
  p->trapframe->a1 = sp;
    80004878:	058ab783          	ld	a5,88(s5) # 1058 <_entry-0x7fffefa8>
    8000487c:	0727bc23          	sd	s2,120(a5)
  for(last=s=path; *s; s++)
    80004880:	df843783          	ld	a5,-520(s0)
    80004884:	0007c703          	lbu	a4,0(a5)
    80004888:	cf11                	beqz	a4,800048a4 <kexec+0x2b8>
    8000488a:	0785                	addi	a5,a5,1
    if(*s == '/')
    8000488c:	02f00693          	li	a3,47
    80004890:	a039                	j	8000489e <kexec+0x2b2>
      last = s+1;
    80004892:	def43c23          	sd	a5,-520(s0)
  for(last=s=path; *s; s++)
    80004896:	0785                	addi	a5,a5,1
    80004898:	fff7c703          	lbu	a4,-1(a5)
    8000489c:	c701                	beqz	a4,800048a4 <kexec+0x2b8>
    if(*s == '/')
    8000489e:	fed71ce3          	bne	a4,a3,80004896 <kexec+0x2aa>
    800048a2:	bfc5                	j	80004892 <kexec+0x2a6>
  safestrcpy(p->name, last, sizeof(p->name));
    800048a4:	4641                	li	a2,16
    800048a6:	df843583          	ld	a1,-520(s0)
    800048aa:	158a8513          	addi	a0,s5,344
    800048ae:	d32fc0ef          	jal	80000de0 <safestrcpy>
  oldpagetable = p->pagetable;
    800048b2:	050ab503          	ld	a0,80(s5)
  p->pagetable = pagetable;
    800048b6:	056ab823          	sd	s6,80(s5)
  p->sz = sz;
    800048ba:	e0843783          	ld	a5,-504(s0)
    800048be:	04fab423          	sd	a5,72(s5)
  p->trapframe->epc = elf.entry;  // initial program counter = ulib.c:start()
    800048c2:	058ab783          	ld	a5,88(s5)
    800048c6:	e6843703          	ld	a4,-408(s0)
    800048ca:	ef98                	sd	a4,24(a5)
  p->trapframe->sp = sp; // initial stack pointer
    800048cc:	058ab783          	ld	a5,88(s5)
    800048d0:	0327b823          	sd	s2,48(a5)
  proc_freepagetable(oldpagetable, oldsz);
    800048d4:	85e6                	mv	a1,s9
    800048d6:	98efd0ef          	jal	80001a64 <proc_freepagetable>
  return argc; // this ends up in a0, the first argument to main(argc, argv)
    800048da:	0004851b          	sext.w	a0,s1
    800048de:	79be                	ld	s3,488(sp)
    800048e0:	7a1e                	ld	s4,480(sp)
    800048e2:	6afe                	ld	s5,472(sp)
    800048e4:	6b5e                	ld	s6,464(sp)
    800048e6:	6bbe                	ld	s7,456(sp)
    800048e8:	6c1e                	ld	s8,448(sp)
    800048ea:	7cfa                	ld	s9,440(sp)
    800048ec:	7d5a                	ld	s10,432(sp)
    800048ee:	b3b5                	j	8000465a <kexec+0x6e>
    800048f0:	e1243423          	sd	s2,-504(s0)
    800048f4:	7dba                	ld	s11,424(sp)
    800048f6:	a035                	j	80004922 <kexec+0x336>
    800048f8:	e1243423          	sd	s2,-504(s0)
    800048fc:	7dba                	ld	s11,424(sp)
    800048fe:	a015                	j	80004922 <kexec+0x336>
    80004900:	e1243423          	sd	s2,-504(s0)
    80004904:	7dba                	ld	s11,424(sp)
    80004906:	a831                	j	80004922 <kexec+0x336>
    80004908:	e1243423          	sd	s2,-504(s0)
    8000490c:	7dba                	ld	s11,424(sp)
    8000490e:	a811                	j	80004922 <kexec+0x336>
    80004910:	e1243423          	sd	s2,-504(s0)
    80004914:	7dba                	ld	s11,424(sp)
    80004916:	a031                	j	80004922 <kexec+0x336>
  ip = 0;
    80004918:	4a01                	li	s4,0
    8000491a:	a021                	j	80004922 <kexec+0x336>
    8000491c:	4a01                	li	s4,0
  if(pagetable)
    8000491e:	a011                	j	80004922 <kexec+0x336>
    80004920:	7dba                	ld	s11,424(sp)
    proc_freepagetable(pagetable, sz);
    80004922:	e0843583          	ld	a1,-504(s0)
    80004926:	855a                	mv	a0,s6
    80004928:	93cfd0ef          	jal	80001a64 <proc_freepagetable>
  return -1;
    8000492c:	557d                	li	a0,-1
  if(ip){
    8000492e:	000a1b63          	bnez	s4,80004944 <kexec+0x358>
    80004932:	79be                	ld	s3,488(sp)
    80004934:	7a1e                	ld	s4,480(sp)
    80004936:	6afe                	ld	s5,472(sp)
    80004938:	6b5e                	ld	s6,464(sp)
    8000493a:	6bbe                	ld	s7,456(sp)
    8000493c:	6c1e                	ld	s8,448(sp)
    8000493e:	7cfa                	ld	s9,440(sp)
    80004940:	7d5a                	ld	s10,432(sp)
    80004942:	bb21                	j	8000465a <kexec+0x6e>
    80004944:	79be                	ld	s3,488(sp)
    80004946:	6afe                	ld	s5,472(sp)
    80004948:	6b5e                	ld	s6,464(sp)
    8000494a:	6bbe                	ld	s7,456(sp)
    8000494c:	6c1e                	ld	s8,448(sp)
    8000494e:	7cfa                	ld	s9,440(sp)
    80004950:	7d5a                	ld	s10,432(sp)
    80004952:	b9ed                	j	8000464c <kexec+0x60>
    80004954:	6b5e                	ld	s6,464(sp)
    80004956:	b9dd                	j	8000464c <kexec+0x60>
  sz = sz1;
    80004958:	e0843983          	ld	s3,-504(s0)
    8000495c:	b595                	j	800047c0 <kexec+0x1d4>

000000008000495e <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
    8000495e:	7179                	addi	sp,sp,-48
    80004960:	f406                	sd	ra,40(sp)
    80004962:	f022                	sd	s0,32(sp)
    80004964:	ec26                	sd	s1,24(sp)
    80004966:	e84a                	sd	s2,16(sp)
    80004968:	1800                	addi	s0,sp,48
    8000496a:	892e                	mv	s2,a1
    8000496c:	84b2                	mv	s1,a2
  int fd;
  struct file *f;

  argint(n, &fd);
    8000496e:	fdc40593          	addi	a1,s0,-36
    80004972:	e77fd0ef          	jal	800027e8 <argint>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
    80004976:	fdc42703          	lw	a4,-36(s0)
    8000497a:	47bd                	li	a5,15
    8000497c:	02e7e963          	bltu	a5,a4,800049ae <argfd+0x50>
    80004980:	f5bfc0ef          	jal	800018da <myproc>
    80004984:	fdc42703          	lw	a4,-36(s0)
    80004988:	01a70793          	addi	a5,a4,26
    8000498c:	078e                	slli	a5,a5,0x3
    8000498e:	953e                	add	a0,a0,a5
    80004990:	611c                	ld	a5,0(a0)
    80004992:	c385                	beqz	a5,800049b2 <argfd+0x54>
    return -1;
  if(pfd)
    80004994:	00090463          	beqz	s2,8000499c <argfd+0x3e>
    *pfd = fd;
    80004998:	00e92023          	sw	a4,0(s2)
  if(pf)
    *pf = f;
  return 0;
    8000499c:	4501                	li	a0,0
  if(pf)
    8000499e:	c091                	beqz	s1,800049a2 <argfd+0x44>
    *pf = f;
    800049a0:	e09c                	sd	a5,0(s1)
}
    800049a2:	70a2                	ld	ra,40(sp)
    800049a4:	7402                	ld	s0,32(sp)
    800049a6:	64e2                	ld	s1,24(sp)
    800049a8:	6942                	ld	s2,16(sp)
    800049aa:	6145                	addi	sp,sp,48
    800049ac:	8082                	ret
    return -1;
    800049ae:	557d                	li	a0,-1
    800049b0:	bfcd                	j	800049a2 <argfd+0x44>
    800049b2:	557d                	li	a0,-1
    800049b4:	b7fd                	j	800049a2 <argfd+0x44>

00000000800049b6 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
    800049b6:	1101                	addi	sp,sp,-32
    800049b8:	ec06                	sd	ra,24(sp)
    800049ba:	e822                	sd	s0,16(sp)
    800049bc:	e426                	sd	s1,8(sp)
    800049be:	1000                	addi	s0,sp,32
    800049c0:	84aa                	mv	s1,a0
  int fd;
  struct proc *p = myproc();
    800049c2:	f19fc0ef          	jal	800018da <myproc>
    800049c6:	862a                	mv	a2,a0

  for(fd = 0; fd < NOFILE; fd++){
    800049c8:	0d050793          	addi	a5,a0,208
    800049cc:	4501                	li	a0,0
    800049ce:	46c1                	li	a3,16
    if(p->ofile[fd] == 0){
    800049d0:	6398                	ld	a4,0(a5)
    800049d2:	cb19                	beqz	a4,800049e8 <fdalloc+0x32>
  for(fd = 0; fd < NOFILE; fd++){
    800049d4:	2505                	addiw	a0,a0,1
    800049d6:	07a1                	addi	a5,a5,8
    800049d8:	fed51ce3          	bne	a0,a3,800049d0 <fdalloc+0x1a>
      p->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
    800049dc:	557d                	li	a0,-1
}
    800049de:	60e2                	ld	ra,24(sp)
    800049e0:	6442                	ld	s0,16(sp)
    800049e2:	64a2                	ld	s1,8(sp)
    800049e4:	6105                	addi	sp,sp,32
    800049e6:	8082                	ret
      p->ofile[fd] = f;
    800049e8:	01a50793          	addi	a5,a0,26
    800049ec:	078e                	slli	a5,a5,0x3
    800049ee:	963e                	add	a2,a2,a5
    800049f0:	e204                	sd	s1,0(a2)
      return fd;
    800049f2:	b7f5                	j	800049de <fdalloc+0x28>

00000000800049f4 <create>:
  return -1;
}

static struct inode*
create(char *path, short type, short major, short minor)
{
    800049f4:	715d                	addi	sp,sp,-80
    800049f6:	e486                	sd	ra,72(sp)
    800049f8:	e0a2                	sd	s0,64(sp)
    800049fa:	fc26                	sd	s1,56(sp)
    800049fc:	f84a                	sd	s2,48(sp)
    800049fe:	f44e                	sd	s3,40(sp)
    80004a00:	ec56                	sd	s5,24(sp)
    80004a02:	e85a                	sd	s6,16(sp)
    80004a04:	0880                	addi	s0,sp,80
    80004a06:	8b2e                	mv	s6,a1
    80004a08:	89b2                	mv	s3,a2
    80004a0a:	8936                	mv	s2,a3
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
    80004a0c:	fb040593          	addi	a1,s0,-80
    80004a10:	80eff0ef          	jal	80003a1e <nameiparent>
    80004a14:	84aa                	mv	s1,a0
    80004a16:	10050a63          	beqz	a0,80004b2a <create+0x136>
    return 0;

  ilock(dp);
    80004a1a:	fd4fe0ef          	jal	800031ee <ilock>

  if((ip = dirlookup(dp, name, 0)) != 0){
    80004a1e:	4601                	li	a2,0
    80004a20:	fb040593          	addi	a1,s0,-80
    80004a24:	8526                	mv	a0,s1
    80004a26:	d79fe0ef          	jal	8000379e <dirlookup>
    80004a2a:	8aaa                	mv	s5,a0
    80004a2c:	c129                	beqz	a0,80004a6e <create+0x7a>
    iunlockput(dp);
    80004a2e:	8526                	mv	a0,s1
    80004a30:	9c9fe0ef          	jal	800033f8 <iunlockput>
    ilock(ip);
    80004a34:	8556                	mv	a0,s5
    80004a36:	fb8fe0ef          	jal	800031ee <ilock>
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))
    80004a3a:	4789                	li	a5,2
    80004a3c:	02fb1463          	bne	s6,a5,80004a64 <create+0x70>
    80004a40:	044ad783          	lhu	a5,68(s5)
    80004a44:	37f9                	addiw	a5,a5,-2
    80004a46:	17c2                	slli	a5,a5,0x30
    80004a48:	93c1                	srli	a5,a5,0x30
    80004a4a:	4705                	li	a4,1
    80004a4c:	00f76c63          	bltu	a4,a5,80004a64 <create+0x70>
  ip->nlink = 0;
  iupdate(ip);
  iunlockput(ip);
  iunlockput(dp);
  return 0;
}
    80004a50:	8556                	mv	a0,s5
    80004a52:	60a6                	ld	ra,72(sp)
    80004a54:	6406                	ld	s0,64(sp)
    80004a56:	74e2                	ld	s1,56(sp)
    80004a58:	7942                	ld	s2,48(sp)
    80004a5a:	79a2                	ld	s3,40(sp)
    80004a5c:	6ae2                	ld	s5,24(sp)
    80004a5e:	6b42                	ld	s6,16(sp)
    80004a60:	6161                	addi	sp,sp,80
    80004a62:	8082                	ret
    iunlockput(ip);
    80004a64:	8556                	mv	a0,s5
    80004a66:	993fe0ef          	jal	800033f8 <iunlockput>
    return 0;
    80004a6a:	4a81                	li	s5,0
    80004a6c:	b7d5                	j	80004a50 <create+0x5c>
    80004a6e:	f052                	sd	s4,32(sp)
  if((ip = ialloc(dp->dev, type)) == 0){
    80004a70:	85da                	mv	a1,s6
    80004a72:	4088                	lw	a0,0(s1)
    80004a74:	e0afe0ef          	jal	8000307e <ialloc>
    80004a78:	8a2a                	mv	s4,a0
    80004a7a:	cd15                	beqz	a0,80004ab6 <create+0xc2>
  ilock(ip);
    80004a7c:	f72fe0ef          	jal	800031ee <ilock>
  ip->major = major;
    80004a80:	053a1323          	sh	s3,70(s4)
  ip->minor = minor;
    80004a84:	052a1423          	sh	s2,72(s4)
  ip->nlink = 1;
    80004a88:	4905                	li	s2,1
    80004a8a:	052a1523          	sh	s2,74(s4)
  iupdate(ip);
    80004a8e:	8552                	mv	a0,s4
    80004a90:	eaafe0ef          	jal	8000313a <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
    80004a94:	032b0763          	beq	s6,s2,80004ac2 <create+0xce>
  if(dirlink(dp, name, ip->inum) < 0)
    80004a98:	004a2603          	lw	a2,4(s4)
    80004a9c:	fb040593          	addi	a1,s0,-80
    80004aa0:	8526                	mv	a0,s1
    80004aa2:	ec9fe0ef          	jal	8000396a <dirlink>
    80004aa6:	06054563          	bltz	a0,80004b10 <create+0x11c>
  iunlockput(dp);
    80004aaa:	8526                	mv	a0,s1
    80004aac:	94dfe0ef          	jal	800033f8 <iunlockput>
  return ip;
    80004ab0:	8ad2                	mv	s5,s4
    80004ab2:	7a02                	ld	s4,32(sp)
    80004ab4:	bf71                	j	80004a50 <create+0x5c>
    iunlockput(dp);
    80004ab6:	8526                	mv	a0,s1
    80004ab8:	941fe0ef          	jal	800033f8 <iunlockput>
    return 0;
    80004abc:	8ad2                	mv	s5,s4
    80004abe:	7a02                	ld	s4,32(sp)
    80004ac0:	bf41                	j	80004a50 <create+0x5c>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
    80004ac2:	004a2603          	lw	a2,4(s4)
    80004ac6:	00003597          	auipc	a1,0x3
    80004aca:	afa58593          	addi	a1,a1,-1286 # 800075c0 <etext+0x5c0>
    80004ace:	8552                	mv	a0,s4
    80004ad0:	e9bfe0ef          	jal	8000396a <dirlink>
    80004ad4:	02054e63          	bltz	a0,80004b10 <create+0x11c>
    80004ad8:	40d0                	lw	a2,4(s1)
    80004ada:	00003597          	auipc	a1,0x3
    80004ade:	aee58593          	addi	a1,a1,-1298 # 800075c8 <etext+0x5c8>
    80004ae2:	8552                	mv	a0,s4
    80004ae4:	e87fe0ef          	jal	8000396a <dirlink>
    80004ae8:	02054463          	bltz	a0,80004b10 <create+0x11c>
  if(dirlink(dp, name, ip->inum) < 0)
    80004aec:	004a2603          	lw	a2,4(s4)
    80004af0:	fb040593          	addi	a1,s0,-80
    80004af4:	8526                	mv	a0,s1
    80004af6:	e75fe0ef          	jal	8000396a <dirlink>
    80004afa:	00054b63          	bltz	a0,80004b10 <create+0x11c>
    dp->nlink++;  // for ".."
    80004afe:	04a4d783          	lhu	a5,74(s1)
    80004b02:	2785                	addiw	a5,a5,1
    80004b04:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004b08:	8526                	mv	a0,s1
    80004b0a:	e30fe0ef          	jal	8000313a <iupdate>
    80004b0e:	bf71                	j	80004aaa <create+0xb6>
  ip->nlink = 0;
    80004b10:	040a1523          	sh	zero,74(s4)
  iupdate(ip);
    80004b14:	8552                	mv	a0,s4
    80004b16:	e24fe0ef          	jal	8000313a <iupdate>
  iunlockput(ip);
    80004b1a:	8552                	mv	a0,s4
    80004b1c:	8ddfe0ef          	jal	800033f8 <iunlockput>
  iunlockput(dp);
    80004b20:	8526                	mv	a0,s1
    80004b22:	8d7fe0ef          	jal	800033f8 <iunlockput>
  return 0;
    80004b26:	7a02                	ld	s4,32(sp)
    80004b28:	b725                	j	80004a50 <create+0x5c>
    return 0;
    80004b2a:	8aaa                	mv	s5,a0
    80004b2c:	b715                	j	80004a50 <create+0x5c>

0000000080004b2e <sys_dup>:
{
    80004b2e:	7179                	addi	sp,sp,-48
    80004b30:	f406                	sd	ra,40(sp)
    80004b32:	f022                	sd	s0,32(sp)
    80004b34:	1800                	addi	s0,sp,48
  if(argfd(0, 0, &f) < 0)
    80004b36:	fd840613          	addi	a2,s0,-40
    80004b3a:	4581                	li	a1,0
    80004b3c:	4501                	li	a0,0
    80004b3e:	e21ff0ef          	jal	8000495e <argfd>
    return -1;
    80004b42:	57fd                	li	a5,-1
  if(argfd(0, 0, &f) < 0)
    80004b44:	02054363          	bltz	a0,80004b6a <sys_dup+0x3c>
    80004b48:	ec26                	sd	s1,24(sp)
    80004b4a:	e84a                	sd	s2,16(sp)
  if((fd=fdalloc(f)) < 0)
    80004b4c:	fd843903          	ld	s2,-40(s0)
    80004b50:	854a                	mv	a0,s2
    80004b52:	e65ff0ef          	jal	800049b6 <fdalloc>
    80004b56:	84aa                	mv	s1,a0
    return -1;
    80004b58:	57fd                	li	a5,-1
  if((fd=fdalloc(f)) < 0)
    80004b5a:	00054d63          	bltz	a0,80004b74 <sys_dup+0x46>
  filedup(f);
    80004b5e:	854a                	mv	a0,s2
    80004b60:	c3eff0ef          	jal	80003f9e <filedup>
  return fd;
    80004b64:	87a6                	mv	a5,s1
    80004b66:	64e2                	ld	s1,24(sp)
    80004b68:	6942                	ld	s2,16(sp)
}
    80004b6a:	853e                	mv	a0,a5
    80004b6c:	70a2                	ld	ra,40(sp)
    80004b6e:	7402                	ld	s0,32(sp)
    80004b70:	6145                	addi	sp,sp,48
    80004b72:	8082                	ret
    80004b74:	64e2                	ld	s1,24(sp)
    80004b76:	6942                	ld	s2,16(sp)
    80004b78:	bfcd                	j	80004b6a <sys_dup+0x3c>

0000000080004b7a <sys_read>:
{
    80004b7a:	7179                	addi	sp,sp,-48
    80004b7c:	f406                	sd	ra,40(sp)
    80004b7e:	f022                	sd	s0,32(sp)
    80004b80:	1800                	addi	s0,sp,48
  argaddr(1, &p);
    80004b82:	fd840593          	addi	a1,s0,-40
    80004b86:	4505                	li	a0,1
    80004b88:	c7dfd0ef          	jal	80002804 <argaddr>
  argint(2, &n);
    80004b8c:	fe440593          	addi	a1,s0,-28
    80004b90:	4509                	li	a0,2
    80004b92:	c57fd0ef          	jal	800027e8 <argint>
  if(argfd(0, 0, &f) < 0)
    80004b96:	fe840613          	addi	a2,s0,-24
    80004b9a:	4581                	li	a1,0
    80004b9c:	4501                	li	a0,0
    80004b9e:	dc1ff0ef          	jal	8000495e <argfd>
    80004ba2:	87aa                	mv	a5,a0
    return -1;
    80004ba4:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004ba6:	0007ca63          	bltz	a5,80004bba <sys_read+0x40>
  return fileread(f, p, n);
    80004baa:	fe442603          	lw	a2,-28(s0)
    80004bae:	fd843583          	ld	a1,-40(s0)
    80004bb2:	fe843503          	ld	a0,-24(s0)
    80004bb6:	d4eff0ef          	jal	80004104 <fileread>
}
    80004bba:	70a2                	ld	ra,40(sp)
    80004bbc:	7402                	ld	s0,32(sp)
    80004bbe:	6145                	addi	sp,sp,48
    80004bc0:	8082                	ret

0000000080004bc2 <sys_write>:
{
    80004bc2:	7179                	addi	sp,sp,-48
    80004bc4:	f406                	sd	ra,40(sp)
    80004bc6:	f022                	sd	s0,32(sp)
    80004bc8:	1800                	addi	s0,sp,48
  argaddr(1, &p);
    80004bca:	fd840593          	addi	a1,s0,-40
    80004bce:	4505                	li	a0,1
    80004bd0:	c35fd0ef          	jal	80002804 <argaddr>
  argint(2, &n);
    80004bd4:	fe440593          	addi	a1,s0,-28
    80004bd8:	4509                	li	a0,2
    80004bda:	c0ffd0ef          	jal	800027e8 <argint>
  if(argfd(0, 0, &f) < 0)
    80004bde:	fe840613          	addi	a2,s0,-24
    80004be2:	4581                	li	a1,0
    80004be4:	4501                	li	a0,0
    80004be6:	d79ff0ef          	jal	8000495e <argfd>
    80004bea:	87aa                	mv	a5,a0
    return -1;
    80004bec:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004bee:	0007ca63          	bltz	a5,80004c02 <sys_write+0x40>
  return filewrite(f, p, n);
    80004bf2:	fe442603          	lw	a2,-28(s0)
    80004bf6:	fd843583          	ld	a1,-40(s0)
    80004bfa:	fe843503          	ld	a0,-24(s0)
    80004bfe:	dc4ff0ef          	jal	800041c2 <filewrite>
}
    80004c02:	70a2                	ld	ra,40(sp)
    80004c04:	7402                	ld	s0,32(sp)
    80004c06:	6145                	addi	sp,sp,48
    80004c08:	8082                	ret

0000000080004c0a <sys_close>:
{
    80004c0a:	1101                	addi	sp,sp,-32
    80004c0c:	ec06                	sd	ra,24(sp)
    80004c0e:	e822                	sd	s0,16(sp)
    80004c10:	1000                	addi	s0,sp,32
  if(argfd(0, &fd, &f) < 0)
    80004c12:	fe040613          	addi	a2,s0,-32
    80004c16:	fec40593          	addi	a1,s0,-20
    80004c1a:	4501                	li	a0,0
    80004c1c:	d43ff0ef          	jal	8000495e <argfd>
    return -1;
    80004c20:	57fd                	li	a5,-1
  if(argfd(0, &fd, &f) < 0)
    80004c22:	02054063          	bltz	a0,80004c42 <sys_close+0x38>
  myproc()->ofile[fd] = 0;
    80004c26:	cb5fc0ef          	jal	800018da <myproc>
    80004c2a:	fec42783          	lw	a5,-20(s0)
    80004c2e:	07e9                	addi	a5,a5,26
    80004c30:	078e                	slli	a5,a5,0x3
    80004c32:	953e                	add	a0,a0,a5
    80004c34:	00053023          	sd	zero,0(a0)
  fileclose(f);
    80004c38:	fe043503          	ld	a0,-32(s0)
    80004c3c:	ba8ff0ef          	jal	80003fe4 <fileclose>
  return 0;
    80004c40:	4781                	li	a5,0
}
    80004c42:	853e                	mv	a0,a5
    80004c44:	60e2                	ld	ra,24(sp)
    80004c46:	6442                	ld	s0,16(sp)
    80004c48:	6105                	addi	sp,sp,32
    80004c4a:	8082                	ret

0000000080004c4c <sys_fstat>:
{
    80004c4c:	1101                	addi	sp,sp,-32
    80004c4e:	ec06                	sd	ra,24(sp)
    80004c50:	e822                	sd	s0,16(sp)
    80004c52:	1000                	addi	s0,sp,32
  argaddr(1, &st);
    80004c54:	fe040593          	addi	a1,s0,-32
    80004c58:	4505                	li	a0,1
    80004c5a:	babfd0ef          	jal	80002804 <argaddr>
  if(argfd(0, 0, &f) < 0)
    80004c5e:	fe840613          	addi	a2,s0,-24
    80004c62:	4581                	li	a1,0
    80004c64:	4501                	li	a0,0
    80004c66:	cf9ff0ef          	jal	8000495e <argfd>
    80004c6a:	87aa                	mv	a5,a0
    return -1;
    80004c6c:	557d                	li	a0,-1
  if(argfd(0, 0, &f) < 0)
    80004c6e:	0007c863          	bltz	a5,80004c7e <sys_fstat+0x32>
  return filestat(f, st);
    80004c72:	fe043583          	ld	a1,-32(s0)
    80004c76:	fe843503          	ld	a0,-24(s0)
    80004c7a:	c2cff0ef          	jal	800040a6 <filestat>
}
    80004c7e:	60e2                	ld	ra,24(sp)
    80004c80:	6442                	ld	s0,16(sp)
    80004c82:	6105                	addi	sp,sp,32
    80004c84:	8082                	ret

0000000080004c86 <sys_link>:
{
    80004c86:	7169                	addi	sp,sp,-304
    80004c88:	f606                	sd	ra,296(sp)
    80004c8a:	f222                	sd	s0,288(sp)
    80004c8c:	1a00                	addi	s0,sp,304
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004c8e:	08000613          	li	a2,128
    80004c92:	ed040593          	addi	a1,s0,-304
    80004c96:	4501                	li	a0,0
    80004c98:	b89fd0ef          	jal	80002820 <argstr>
    return -1;
    80004c9c:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004c9e:	0c054e63          	bltz	a0,80004d7a <sys_link+0xf4>
    80004ca2:	08000613          	li	a2,128
    80004ca6:	f5040593          	addi	a1,s0,-176
    80004caa:	4505                	li	a0,1
    80004cac:	b75fd0ef          	jal	80002820 <argstr>
    return -1;
    80004cb0:	57fd                	li	a5,-1
  if(argstr(0, old, MAXPATH) < 0 || argstr(1, new, MAXPATH) < 0)
    80004cb2:	0c054463          	bltz	a0,80004d7a <sys_link+0xf4>
    80004cb6:	ee26                	sd	s1,280(sp)
  begin_op();
    80004cb8:	f21fe0ef          	jal	80003bd8 <begin_op>
  if((ip = namei(old)) == 0){
    80004cbc:	ed040513          	addi	a0,s0,-304
    80004cc0:	d45fe0ef          	jal	80003a04 <namei>
    80004cc4:	84aa                	mv	s1,a0
    80004cc6:	c53d                	beqz	a0,80004d34 <sys_link+0xae>
  ilock(ip);
    80004cc8:	d26fe0ef          	jal	800031ee <ilock>
  if(ip->type == T_DIR){
    80004ccc:	04449703          	lh	a4,68(s1)
    80004cd0:	4785                	li	a5,1
    80004cd2:	06f70663          	beq	a4,a5,80004d3e <sys_link+0xb8>
    80004cd6:	ea4a                	sd	s2,272(sp)
  ip->nlink++;
    80004cd8:	04a4d783          	lhu	a5,74(s1)
    80004cdc:	2785                	addiw	a5,a5,1
    80004cde:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004ce2:	8526                	mv	a0,s1
    80004ce4:	c56fe0ef          	jal	8000313a <iupdate>
  iunlock(ip);
    80004ce8:	8526                	mv	a0,s1
    80004cea:	db2fe0ef          	jal	8000329c <iunlock>
  if((dp = nameiparent(new, name)) == 0)
    80004cee:	fd040593          	addi	a1,s0,-48
    80004cf2:	f5040513          	addi	a0,s0,-176
    80004cf6:	d29fe0ef          	jal	80003a1e <nameiparent>
    80004cfa:	892a                	mv	s2,a0
    80004cfc:	cd21                	beqz	a0,80004d54 <sys_link+0xce>
  ilock(dp);
    80004cfe:	cf0fe0ef          	jal	800031ee <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
    80004d02:	00092703          	lw	a4,0(s2)
    80004d06:	409c                	lw	a5,0(s1)
    80004d08:	04f71363          	bne	a4,a5,80004d4e <sys_link+0xc8>
    80004d0c:	40d0                	lw	a2,4(s1)
    80004d0e:	fd040593          	addi	a1,s0,-48
    80004d12:	854a                	mv	a0,s2
    80004d14:	c57fe0ef          	jal	8000396a <dirlink>
    80004d18:	02054b63          	bltz	a0,80004d4e <sys_link+0xc8>
  iunlockput(dp);
    80004d1c:	854a                	mv	a0,s2
    80004d1e:	edafe0ef          	jal	800033f8 <iunlockput>
  iput(ip);
    80004d22:	8526                	mv	a0,s1
    80004d24:	e4cfe0ef          	jal	80003370 <iput>
  end_op();
    80004d28:	f1bfe0ef          	jal	80003c42 <end_op>
  return 0;
    80004d2c:	4781                	li	a5,0
    80004d2e:	64f2                	ld	s1,280(sp)
    80004d30:	6952                	ld	s2,272(sp)
    80004d32:	a0a1                	j	80004d7a <sys_link+0xf4>
    end_op();
    80004d34:	f0ffe0ef          	jal	80003c42 <end_op>
    return -1;
    80004d38:	57fd                	li	a5,-1
    80004d3a:	64f2                	ld	s1,280(sp)
    80004d3c:	a83d                	j	80004d7a <sys_link+0xf4>
    iunlockput(ip);
    80004d3e:	8526                	mv	a0,s1
    80004d40:	eb8fe0ef          	jal	800033f8 <iunlockput>
    end_op();
    80004d44:	efffe0ef          	jal	80003c42 <end_op>
    return -1;
    80004d48:	57fd                	li	a5,-1
    80004d4a:	64f2                	ld	s1,280(sp)
    80004d4c:	a03d                	j	80004d7a <sys_link+0xf4>
    iunlockput(dp);
    80004d4e:	854a                	mv	a0,s2
    80004d50:	ea8fe0ef          	jal	800033f8 <iunlockput>
  ilock(ip);
    80004d54:	8526                	mv	a0,s1
    80004d56:	c98fe0ef          	jal	800031ee <ilock>
  ip->nlink--;
    80004d5a:	04a4d783          	lhu	a5,74(s1)
    80004d5e:	37fd                	addiw	a5,a5,-1
    80004d60:	04f49523          	sh	a5,74(s1)
  iupdate(ip);
    80004d64:	8526                	mv	a0,s1
    80004d66:	bd4fe0ef          	jal	8000313a <iupdate>
  iunlockput(ip);
    80004d6a:	8526                	mv	a0,s1
    80004d6c:	e8cfe0ef          	jal	800033f8 <iunlockput>
  end_op();
    80004d70:	ed3fe0ef          	jal	80003c42 <end_op>
  return -1;
    80004d74:	57fd                	li	a5,-1
    80004d76:	64f2                	ld	s1,280(sp)
    80004d78:	6952                	ld	s2,272(sp)
}
    80004d7a:	853e                	mv	a0,a5
    80004d7c:	70b2                	ld	ra,296(sp)
    80004d7e:	7412                	ld	s0,288(sp)
    80004d80:	6155                	addi	sp,sp,304
    80004d82:	8082                	ret

0000000080004d84 <sys_unlink>:
{
    80004d84:	7151                	addi	sp,sp,-240
    80004d86:	f586                	sd	ra,232(sp)
    80004d88:	f1a2                	sd	s0,224(sp)
    80004d8a:	1980                	addi	s0,sp,240
  if(argstr(0, path, MAXPATH) < 0)
    80004d8c:	08000613          	li	a2,128
    80004d90:	f3040593          	addi	a1,s0,-208
    80004d94:	4501                	li	a0,0
    80004d96:	a8bfd0ef          	jal	80002820 <argstr>
    80004d9a:	16054063          	bltz	a0,80004efa <sys_unlink+0x176>
    80004d9e:	eda6                	sd	s1,216(sp)
  begin_op();
    80004da0:	e39fe0ef          	jal	80003bd8 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
    80004da4:	fb040593          	addi	a1,s0,-80
    80004da8:	f3040513          	addi	a0,s0,-208
    80004dac:	c73fe0ef          	jal	80003a1e <nameiparent>
    80004db0:	84aa                	mv	s1,a0
    80004db2:	c945                	beqz	a0,80004e62 <sys_unlink+0xde>
  ilock(dp);
    80004db4:	c3afe0ef          	jal	800031ee <ilock>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    80004db8:	00003597          	auipc	a1,0x3
    80004dbc:	80858593          	addi	a1,a1,-2040 # 800075c0 <etext+0x5c0>
    80004dc0:	fb040513          	addi	a0,s0,-80
    80004dc4:	9c5fe0ef          	jal	80003788 <namecmp>
    80004dc8:	10050e63          	beqz	a0,80004ee4 <sys_unlink+0x160>
    80004dcc:	00002597          	auipc	a1,0x2
    80004dd0:	7fc58593          	addi	a1,a1,2044 # 800075c8 <etext+0x5c8>
    80004dd4:	fb040513          	addi	a0,s0,-80
    80004dd8:	9b1fe0ef          	jal	80003788 <namecmp>
    80004ddc:	10050463          	beqz	a0,80004ee4 <sys_unlink+0x160>
    80004de0:	e9ca                	sd	s2,208(sp)
  if((ip = dirlookup(dp, name, &off)) == 0)
    80004de2:	f2c40613          	addi	a2,s0,-212
    80004de6:	fb040593          	addi	a1,s0,-80
    80004dea:	8526                	mv	a0,s1
    80004dec:	9b3fe0ef          	jal	8000379e <dirlookup>
    80004df0:	892a                	mv	s2,a0
    80004df2:	0e050863          	beqz	a0,80004ee2 <sys_unlink+0x15e>
  ilock(ip);
    80004df6:	bf8fe0ef          	jal	800031ee <ilock>
  if(ip->nlink < 1)
    80004dfa:	04a91783          	lh	a5,74(s2)
    80004dfe:	06f05763          	blez	a5,80004e6c <sys_unlink+0xe8>
  if(ip->type == T_DIR && !isdirempty(ip)){
    80004e02:	04491703          	lh	a4,68(s2)
    80004e06:	4785                	li	a5,1
    80004e08:	06f70963          	beq	a4,a5,80004e7a <sys_unlink+0xf6>
  memset(&de, 0, sizeof(de));
    80004e0c:	4641                	li	a2,16
    80004e0e:	4581                	li	a1,0
    80004e10:	fc040513          	addi	a0,s0,-64
    80004e14:	e8ffb0ef          	jal	80000ca2 <memset>
  if(writei(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004e18:	4741                	li	a4,16
    80004e1a:	f2c42683          	lw	a3,-212(s0)
    80004e1e:	fc040613          	addi	a2,s0,-64
    80004e22:	4581                	li	a1,0
    80004e24:	8526                	mv	a0,s1
    80004e26:	855fe0ef          	jal	8000367a <writei>
    80004e2a:	47c1                	li	a5,16
    80004e2c:	08f51b63          	bne	a0,a5,80004ec2 <sys_unlink+0x13e>
  if(ip->type == T_DIR){
    80004e30:	04491703          	lh	a4,68(s2)
    80004e34:	4785                	li	a5,1
    80004e36:	08f70d63          	beq	a4,a5,80004ed0 <sys_unlink+0x14c>
  iunlockput(dp);
    80004e3a:	8526                	mv	a0,s1
    80004e3c:	dbcfe0ef          	jal	800033f8 <iunlockput>
  ip->nlink--;
    80004e40:	04a95783          	lhu	a5,74(s2)
    80004e44:	37fd                	addiw	a5,a5,-1
    80004e46:	04f91523          	sh	a5,74(s2)
  iupdate(ip);
    80004e4a:	854a                	mv	a0,s2
    80004e4c:	aeefe0ef          	jal	8000313a <iupdate>
  iunlockput(ip);
    80004e50:	854a                	mv	a0,s2
    80004e52:	da6fe0ef          	jal	800033f8 <iunlockput>
  end_op();
    80004e56:	dedfe0ef          	jal	80003c42 <end_op>
  return 0;
    80004e5a:	4501                	li	a0,0
    80004e5c:	64ee                	ld	s1,216(sp)
    80004e5e:	694e                	ld	s2,208(sp)
    80004e60:	a849                	j	80004ef2 <sys_unlink+0x16e>
    end_op();
    80004e62:	de1fe0ef          	jal	80003c42 <end_op>
    return -1;
    80004e66:	557d                	li	a0,-1
    80004e68:	64ee                	ld	s1,216(sp)
    80004e6a:	a061                	j	80004ef2 <sys_unlink+0x16e>
    80004e6c:	e5ce                	sd	s3,200(sp)
    panic("unlink: nlink < 1");
    80004e6e:	00002517          	auipc	a0,0x2
    80004e72:	76250513          	addi	a0,a0,1890 # 800075d0 <etext+0x5d0>
    80004e76:	96bfb0ef          	jal	800007e0 <panic>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80004e7a:	04c92703          	lw	a4,76(s2)
    80004e7e:	02000793          	li	a5,32
    80004e82:	f8e7f5e3          	bgeu	a5,a4,80004e0c <sys_unlink+0x88>
    80004e86:	e5ce                	sd	s3,200(sp)
    80004e88:	02000993          	li	s3,32
    if(readi(dp, 0, (uint64)&de, off, sizeof(de)) != sizeof(de))
    80004e8c:	4741                	li	a4,16
    80004e8e:	86ce                	mv	a3,s3
    80004e90:	f1840613          	addi	a2,s0,-232
    80004e94:	4581                	li	a1,0
    80004e96:	854a                	mv	a0,s2
    80004e98:	ee6fe0ef          	jal	8000357e <readi>
    80004e9c:	47c1                	li	a5,16
    80004e9e:	00f51c63          	bne	a0,a5,80004eb6 <sys_unlink+0x132>
    if(de.inum != 0)
    80004ea2:	f1845783          	lhu	a5,-232(s0)
    80004ea6:	efa1                	bnez	a5,80004efe <sys_unlink+0x17a>
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    80004ea8:	29c1                	addiw	s3,s3,16
    80004eaa:	04c92783          	lw	a5,76(s2)
    80004eae:	fcf9efe3          	bltu	s3,a5,80004e8c <sys_unlink+0x108>
    80004eb2:	69ae                	ld	s3,200(sp)
    80004eb4:	bfa1                	j	80004e0c <sys_unlink+0x88>
      panic("isdirempty: readi");
    80004eb6:	00002517          	auipc	a0,0x2
    80004eba:	73250513          	addi	a0,a0,1842 # 800075e8 <etext+0x5e8>
    80004ebe:	923fb0ef          	jal	800007e0 <panic>
    80004ec2:	e5ce                	sd	s3,200(sp)
    panic("unlink: writei");
    80004ec4:	00002517          	auipc	a0,0x2
    80004ec8:	73c50513          	addi	a0,a0,1852 # 80007600 <etext+0x600>
    80004ecc:	915fb0ef          	jal	800007e0 <panic>
    dp->nlink--;
    80004ed0:	04a4d783          	lhu	a5,74(s1)
    80004ed4:	37fd                	addiw	a5,a5,-1
    80004ed6:	04f49523          	sh	a5,74(s1)
    iupdate(dp);
    80004eda:	8526                	mv	a0,s1
    80004edc:	a5efe0ef          	jal	8000313a <iupdate>
    80004ee0:	bfa9                	j	80004e3a <sys_unlink+0xb6>
    80004ee2:	694e                	ld	s2,208(sp)
  iunlockput(dp);
    80004ee4:	8526                	mv	a0,s1
    80004ee6:	d12fe0ef          	jal	800033f8 <iunlockput>
  end_op();
    80004eea:	d59fe0ef          	jal	80003c42 <end_op>
  return -1;
    80004eee:	557d                	li	a0,-1
    80004ef0:	64ee                	ld	s1,216(sp)
}
    80004ef2:	70ae                	ld	ra,232(sp)
    80004ef4:	740e                	ld	s0,224(sp)
    80004ef6:	616d                	addi	sp,sp,240
    80004ef8:	8082                	ret
    return -1;
    80004efa:	557d                	li	a0,-1
    80004efc:	bfdd                	j	80004ef2 <sys_unlink+0x16e>
    iunlockput(ip);
    80004efe:	854a                	mv	a0,s2
    80004f00:	cf8fe0ef          	jal	800033f8 <iunlockput>
    goto bad;
    80004f04:	694e                	ld	s2,208(sp)
    80004f06:	69ae                	ld	s3,200(sp)
    80004f08:	bff1                	j	80004ee4 <sys_unlink+0x160>

0000000080004f0a <sys_open>:

uint64
sys_open(void)
{
    80004f0a:	7131                	addi	sp,sp,-192
    80004f0c:	fd06                	sd	ra,184(sp)
    80004f0e:	f922                	sd	s0,176(sp)
    80004f10:	0180                	addi	s0,sp,192
  int fd, omode;
  struct file *f;
  struct inode *ip;
  int n;

  argint(1, &omode);
    80004f12:	f4c40593          	addi	a1,s0,-180
    80004f16:	4505                	li	a0,1
    80004f18:	8d1fd0ef          	jal	800027e8 <argint>
  if((n = argstr(0, path, MAXPATH)) < 0)
    80004f1c:	08000613          	li	a2,128
    80004f20:	f5040593          	addi	a1,s0,-176
    80004f24:	4501                	li	a0,0
    80004f26:	8fbfd0ef          	jal	80002820 <argstr>
    80004f2a:	87aa                	mv	a5,a0
    return -1;
    80004f2c:	557d                	li	a0,-1
  if((n = argstr(0, path, MAXPATH)) < 0)
    80004f2e:	0a07c263          	bltz	a5,80004fd2 <sys_open+0xc8>
    80004f32:	f526                	sd	s1,168(sp)

  begin_op();
    80004f34:	ca5fe0ef          	jal	80003bd8 <begin_op>

  if(omode & O_CREATE){
    80004f38:	f4c42783          	lw	a5,-180(s0)
    80004f3c:	2007f793          	andi	a5,a5,512
    80004f40:	c3d5                	beqz	a5,80004fe4 <sys_open+0xda>
    ip = create(path, T_FILE, 0, 0);
    80004f42:	4681                	li	a3,0
    80004f44:	4601                	li	a2,0
    80004f46:	4589                	li	a1,2
    80004f48:	f5040513          	addi	a0,s0,-176
    80004f4c:	aa9ff0ef          	jal	800049f4 <create>
    80004f50:	84aa                	mv	s1,a0
    if(ip == 0){
    80004f52:	c541                	beqz	a0,80004fda <sys_open+0xd0>
      end_op();
      return -1;
    }
  }

  if(ip->type == T_DEVICE && (ip->major < 0 || ip->major >= NDEV)){
    80004f54:	04449703          	lh	a4,68(s1)
    80004f58:	478d                	li	a5,3
    80004f5a:	00f71763          	bne	a4,a5,80004f68 <sys_open+0x5e>
    80004f5e:	0464d703          	lhu	a4,70(s1)
    80004f62:	47a5                	li	a5,9
    80004f64:	0ae7ed63          	bltu	a5,a4,8000501e <sys_open+0x114>
    80004f68:	f14a                	sd	s2,160(sp)
    iunlockput(ip);
    end_op();
    return -1;
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
    80004f6a:	fd7fe0ef          	jal	80003f40 <filealloc>
    80004f6e:	892a                	mv	s2,a0
    80004f70:	c179                	beqz	a0,80005036 <sys_open+0x12c>
    80004f72:	ed4e                	sd	s3,152(sp)
    80004f74:	a43ff0ef          	jal	800049b6 <fdalloc>
    80004f78:	89aa                	mv	s3,a0
    80004f7a:	0a054a63          	bltz	a0,8000502e <sys_open+0x124>
    iunlockput(ip);
    end_op();
    return -1;
  }

  if(ip->type == T_DEVICE){
    80004f7e:	04449703          	lh	a4,68(s1)
    80004f82:	478d                	li	a5,3
    80004f84:	0cf70263          	beq	a4,a5,80005048 <sys_open+0x13e>
    f->type = FD_DEVICE;
    f->major = ip->major;
  } else {
    f->type = FD_INODE;
    80004f88:	4789                	li	a5,2
    80004f8a:	00f92023          	sw	a5,0(s2)
    f->off = 0;
    80004f8e:	02092023          	sw	zero,32(s2)
  }
  f->ip = ip;
    80004f92:	00993c23          	sd	s1,24(s2)
  f->readable = !(omode & O_WRONLY);
    80004f96:	f4c42783          	lw	a5,-180(s0)
    80004f9a:	0017c713          	xori	a4,a5,1
    80004f9e:	8b05                	andi	a4,a4,1
    80004fa0:	00e90423          	sb	a4,8(s2)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    80004fa4:	0037f713          	andi	a4,a5,3
    80004fa8:	00e03733          	snez	a4,a4
    80004fac:	00e904a3          	sb	a4,9(s2)

  if((omode & O_TRUNC) && ip->type == T_FILE){
    80004fb0:	4007f793          	andi	a5,a5,1024
    80004fb4:	c791                	beqz	a5,80004fc0 <sys_open+0xb6>
    80004fb6:	04449703          	lh	a4,68(s1)
    80004fba:	4789                	li	a5,2
    80004fbc:	08f70d63          	beq	a4,a5,80005056 <sys_open+0x14c>
    itrunc(ip);
  }

  iunlock(ip);
    80004fc0:	8526                	mv	a0,s1
    80004fc2:	adafe0ef          	jal	8000329c <iunlock>
  end_op();
    80004fc6:	c7dfe0ef          	jal	80003c42 <end_op>

  return fd;
    80004fca:	854e                	mv	a0,s3
    80004fcc:	74aa                	ld	s1,168(sp)
    80004fce:	790a                	ld	s2,160(sp)
    80004fd0:	69ea                	ld	s3,152(sp)
}
    80004fd2:	70ea                	ld	ra,184(sp)
    80004fd4:	744a                	ld	s0,176(sp)
    80004fd6:	6129                	addi	sp,sp,192
    80004fd8:	8082                	ret
      end_op();
    80004fda:	c69fe0ef          	jal	80003c42 <end_op>
      return -1;
    80004fde:	557d                	li	a0,-1
    80004fe0:	74aa                	ld	s1,168(sp)
    80004fe2:	bfc5                	j	80004fd2 <sys_open+0xc8>
    if((ip = namei(path)) == 0){
    80004fe4:	f5040513          	addi	a0,s0,-176
    80004fe8:	a1dfe0ef          	jal	80003a04 <namei>
    80004fec:	84aa                	mv	s1,a0
    80004fee:	c11d                	beqz	a0,80005014 <sys_open+0x10a>
    ilock(ip);
    80004ff0:	9fefe0ef          	jal	800031ee <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
    80004ff4:	04449703          	lh	a4,68(s1)
    80004ff8:	4785                	li	a5,1
    80004ffa:	f4f71de3          	bne	a4,a5,80004f54 <sys_open+0x4a>
    80004ffe:	f4c42783          	lw	a5,-180(s0)
    80005002:	d3bd                	beqz	a5,80004f68 <sys_open+0x5e>
      iunlockput(ip);
    80005004:	8526                	mv	a0,s1
    80005006:	bf2fe0ef          	jal	800033f8 <iunlockput>
      end_op();
    8000500a:	c39fe0ef          	jal	80003c42 <end_op>
      return -1;
    8000500e:	557d                	li	a0,-1
    80005010:	74aa                	ld	s1,168(sp)
    80005012:	b7c1                	j	80004fd2 <sys_open+0xc8>
      end_op();
    80005014:	c2ffe0ef          	jal	80003c42 <end_op>
      return -1;
    80005018:	557d                	li	a0,-1
    8000501a:	74aa                	ld	s1,168(sp)
    8000501c:	bf5d                	j	80004fd2 <sys_open+0xc8>
    iunlockput(ip);
    8000501e:	8526                	mv	a0,s1
    80005020:	bd8fe0ef          	jal	800033f8 <iunlockput>
    end_op();
    80005024:	c1ffe0ef          	jal	80003c42 <end_op>
    return -1;
    80005028:	557d                	li	a0,-1
    8000502a:	74aa                	ld	s1,168(sp)
    8000502c:	b75d                	j	80004fd2 <sys_open+0xc8>
      fileclose(f);
    8000502e:	854a                	mv	a0,s2
    80005030:	fb5fe0ef          	jal	80003fe4 <fileclose>
    80005034:	69ea                	ld	s3,152(sp)
    iunlockput(ip);
    80005036:	8526                	mv	a0,s1
    80005038:	bc0fe0ef          	jal	800033f8 <iunlockput>
    end_op();
    8000503c:	c07fe0ef          	jal	80003c42 <end_op>
    return -1;
    80005040:	557d                	li	a0,-1
    80005042:	74aa                	ld	s1,168(sp)
    80005044:	790a                	ld	s2,160(sp)
    80005046:	b771                	j	80004fd2 <sys_open+0xc8>
    f->type = FD_DEVICE;
    80005048:	00f92023          	sw	a5,0(s2)
    f->major = ip->major;
    8000504c:	04649783          	lh	a5,70(s1)
    80005050:	02f91223          	sh	a5,36(s2)
    80005054:	bf3d                	j	80004f92 <sys_open+0x88>
    itrunc(ip);
    80005056:	8526                	mv	a0,s1
    80005058:	a84fe0ef          	jal	800032dc <itrunc>
    8000505c:	b795                	j	80004fc0 <sys_open+0xb6>

000000008000505e <sys_mkdir>:

uint64
sys_mkdir(void)
{
    8000505e:	7175                	addi	sp,sp,-144
    80005060:	e506                	sd	ra,136(sp)
    80005062:	e122                	sd	s0,128(sp)
    80005064:	0900                	addi	s0,sp,144
  char path[MAXPATH];
  struct inode *ip;

  begin_op();
    80005066:	b73fe0ef          	jal	80003bd8 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
    8000506a:	08000613          	li	a2,128
    8000506e:	f7040593          	addi	a1,s0,-144
    80005072:	4501                	li	a0,0
    80005074:	facfd0ef          	jal	80002820 <argstr>
    80005078:	02054363          	bltz	a0,8000509e <sys_mkdir+0x40>
    8000507c:	4681                	li	a3,0
    8000507e:	4601                	li	a2,0
    80005080:	4585                	li	a1,1
    80005082:	f7040513          	addi	a0,s0,-144
    80005086:	96fff0ef          	jal	800049f4 <create>
    8000508a:	c911                	beqz	a0,8000509e <sys_mkdir+0x40>
    end_op();
    return -1;
  }
  iunlockput(ip);
    8000508c:	b6cfe0ef          	jal	800033f8 <iunlockput>
  end_op();
    80005090:	bb3fe0ef          	jal	80003c42 <end_op>
  return 0;
    80005094:	4501                	li	a0,0
}
    80005096:	60aa                	ld	ra,136(sp)
    80005098:	640a                	ld	s0,128(sp)
    8000509a:	6149                	addi	sp,sp,144
    8000509c:	8082                	ret
    end_op();
    8000509e:	ba5fe0ef          	jal	80003c42 <end_op>
    return -1;
    800050a2:	557d                	li	a0,-1
    800050a4:	bfcd                	j	80005096 <sys_mkdir+0x38>

00000000800050a6 <sys_mknod>:

uint64
sys_mknod(void)
{
    800050a6:	7135                	addi	sp,sp,-160
    800050a8:	ed06                	sd	ra,152(sp)
    800050aa:	e922                	sd	s0,144(sp)
    800050ac:	1100                	addi	s0,sp,160
  struct inode *ip;
  char path[MAXPATH];
  int major, minor;

  begin_op();
    800050ae:	b2bfe0ef          	jal	80003bd8 <begin_op>
  argint(1, &major);
    800050b2:	f6c40593          	addi	a1,s0,-148
    800050b6:	4505                	li	a0,1
    800050b8:	f30fd0ef          	jal	800027e8 <argint>
  argint(2, &minor);
    800050bc:	f6840593          	addi	a1,s0,-152
    800050c0:	4509                	li	a0,2
    800050c2:	f26fd0ef          	jal	800027e8 <argint>
  if((argstr(0, path, MAXPATH)) < 0 ||
    800050c6:	08000613          	li	a2,128
    800050ca:	f7040593          	addi	a1,s0,-144
    800050ce:	4501                	li	a0,0
    800050d0:	f50fd0ef          	jal	80002820 <argstr>
    800050d4:	02054563          	bltz	a0,800050fe <sys_mknod+0x58>
     (ip = create(path, T_DEVICE, major, minor)) == 0){
    800050d8:	f6841683          	lh	a3,-152(s0)
    800050dc:	f6c41603          	lh	a2,-148(s0)
    800050e0:	458d                	li	a1,3
    800050e2:	f7040513          	addi	a0,s0,-144
    800050e6:	90fff0ef          	jal	800049f4 <create>
  if((argstr(0, path, MAXPATH)) < 0 ||
    800050ea:	c911                	beqz	a0,800050fe <sys_mknod+0x58>
    end_op();
    return -1;
  }
  iunlockput(ip);
    800050ec:	b0cfe0ef          	jal	800033f8 <iunlockput>
  end_op();
    800050f0:	b53fe0ef          	jal	80003c42 <end_op>
  return 0;
    800050f4:	4501                	li	a0,0
}
    800050f6:	60ea                	ld	ra,152(sp)
    800050f8:	644a                	ld	s0,144(sp)
    800050fa:	610d                	addi	sp,sp,160
    800050fc:	8082                	ret
    end_op();
    800050fe:	b45fe0ef          	jal	80003c42 <end_op>
    return -1;
    80005102:	557d                	li	a0,-1
    80005104:	bfcd                	j	800050f6 <sys_mknod+0x50>

0000000080005106 <sys_chdir>:

uint64
sys_chdir(void)
{
    80005106:	7135                	addi	sp,sp,-160
    80005108:	ed06                	sd	ra,152(sp)
    8000510a:	e922                	sd	s0,144(sp)
    8000510c:	e14a                	sd	s2,128(sp)
    8000510e:	1100                	addi	s0,sp,160
  char path[MAXPATH];
  struct inode *ip;
  struct proc *p = myproc();
    80005110:	fcafc0ef          	jal	800018da <myproc>
    80005114:	892a                	mv	s2,a0
  
  begin_op();
    80005116:	ac3fe0ef          	jal	80003bd8 <begin_op>
  if(argstr(0, path, MAXPATH) < 0 || (ip = namei(path)) == 0){
    8000511a:	08000613          	li	a2,128
    8000511e:	f6040593          	addi	a1,s0,-160
    80005122:	4501                	li	a0,0
    80005124:	efcfd0ef          	jal	80002820 <argstr>
    80005128:	04054363          	bltz	a0,8000516e <sys_chdir+0x68>
    8000512c:	e526                	sd	s1,136(sp)
    8000512e:	f6040513          	addi	a0,s0,-160
    80005132:	8d3fe0ef          	jal	80003a04 <namei>
    80005136:	84aa                	mv	s1,a0
    80005138:	c915                	beqz	a0,8000516c <sys_chdir+0x66>
    end_op();
    return -1;
  }
  ilock(ip);
    8000513a:	8b4fe0ef          	jal	800031ee <ilock>
  if(ip->type != T_DIR){
    8000513e:	04449703          	lh	a4,68(s1)
    80005142:	4785                	li	a5,1
    80005144:	02f71963          	bne	a4,a5,80005176 <sys_chdir+0x70>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
    80005148:	8526                	mv	a0,s1
    8000514a:	952fe0ef          	jal	8000329c <iunlock>
  iput(p->cwd);
    8000514e:	15093503          	ld	a0,336(s2)
    80005152:	a1efe0ef          	jal	80003370 <iput>
  end_op();
    80005156:	aedfe0ef          	jal	80003c42 <end_op>
  p->cwd = ip;
    8000515a:	14993823          	sd	s1,336(s2)
  return 0;
    8000515e:	4501                	li	a0,0
    80005160:	64aa                	ld	s1,136(sp)
}
    80005162:	60ea                	ld	ra,152(sp)
    80005164:	644a                	ld	s0,144(sp)
    80005166:	690a                	ld	s2,128(sp)
    80005168:	610d                	addi	sp,sp,160
    8000516a:	8082                	ret
    8000516c:	64aa                	ld	s1,136(sp)
    end_op();
    8000516e:	ad5fe0ef          	jal	80003c42 <end_op>
    return -1;
    80005172:	557d                	li	a0,-1
    80005174:	b7fd                	j	80005162 <sys_chdir+0x5c>
    iunlockput(ip);
    80005176:	8526                	mv	a0,s1
    80005178:	a80fe0ef          	jal	800033f8 <iunlockput>
    end_op();
    8000517c:	ac7fe0ef          	jal	80003c42 <end_op>
    return -1;
    80005180:	557d                	li	a0,-1
    80005182:	64aa                	ld	s1,136(sp)
    80005184:	bff9                	j	80005162 <sys_chdir+0x5c>

0000000080005186 <sys_exec>:

uint64
sys_exec(void)
{
    80005186:	7121                	addi	sp,sp,-448
    80005188:	ff06                	sd	ra,440(sp)
    8000518a:	fb22                	sd	s0,432(sp)
    8000518c:	0380                	addi	s0,sp,448
  char path[MAXPATH], *argv[MAXARG];
  int i;
  uint64 uargv, uarg;

  argaddr(1, &uargv);
    8000518e:	e4840593          	addi	a1,s0,-440
    80005192:	4505                	li	a0,1
    80005194:	e70fd0ef          	jal	80002804 <argaddr>
  if(argstr(0, path, MAXPATH) < 0) {
    80005198:	08000613          	li	a2,128
    8000519c:	f5040593          	addi	a1,s0,-176
    800051a0:	4501                	li	a0,0
    800051a2:	e7efd0ef          	jal	80002820 <argstr>
    800051a6:	87aa                	mv	a5,a0
    return -1;
    800051a8:	557d                	li	a0,-1
  if(argstr(0, path, MAXPATH) < 0) {
    800051aa:	0c07c463          	bltz	a5,80005272 <sys_exec+0xec>
    800051ae:	f726                	sd	s1,424(sp)
    800051b0:	f34a                	sd	s2,416(sp)
    800051b2:	ef4e                	sd	s3,408(sp)
    800051b4:	eb52                	sd	s4,400(sp)
  }
  memset(argv, 0, sizeof(argv));
    800051b6:	10000613          	li	a2,256
    800051ba:	4581                	li	a1,0
    800051bc:	e5040513          	addi	a0,s0,-432
    800051c0:	ae3fb0ef          	jal	80000ca2 <memset>
  for(i=0;; i++){
    if(i >= NELEM(argv)){
    800051c4:	e5040493          	addi	s1,s0,-432
  memset(argv, 0, sizeof(argv));
    800051c8:	89a6                	mv	s3,s1
    800051ca:	4901                	li	s2,0
    if(i >= NELEM(argv)){
    800051cc:	02000a13          	li	s4,32
      goto bad;
    }
    if(fetchaddr(uargv+sizeof(uint64)*i, (uint64*)&uarg) < 0){
    800051d0:	00391513          	slli	a0,s2,0x3
    800051d4:	e4040593          	addi	a1,s0,-448
    800051d8:	e4843783          	ld	a5,-440(s0)
    800051dc:	953e                	add	a0,a0,a5
    800051de:	d80fd0ef          	jal	8000275e <fetchaddr>
    800051e2:	02054663          	bltz	a0,8000520e <sys_exec+0x88>
      goto bad;
    }
    if(uarg == 0){
    800051e6:	e4043783          	ld	a5,-448(s0)
    800051ea:	c3a9                	beqz	a5,8000522c <sys_exec+0xa6>
      argv[i] = 0;
      break;
    }
    argv[i] = kalloc();
    800051ec:	913fb0ef          	jal	80000afe <kalloc>
    800051f0:	85aa                	mv	a1,a0
    800051f2:	00a9b023          	sd	a0,0(s3)
    if(argv[i] == 0)
    800051f6:	cd01                	beqz	a0,8000520e <sys_exec+0x88>
      goto bad;
    if(fetchstr(uarg, argv[i], PGSIZE) < 0)
    800051f8:	6605                	lui	a2,0x1
    800051fa:	e4043503          	ld	a0,-448(s0)
    800051fe:	daafd0ef          	jal	800027a8 <fetchstr>
    80005202:	00054663          	bltz	a0,8000520e <sys_exec+0x88>
    if(i >= NELEM(argv)){
    80005206:	0905                	addi	s2,s2,1
    80005208:	09a1                	addi	s3,s3,8
    8000520a:	fd4913e3          	bne	s2,s4,800051d0 <sys_exec+0x4a>
    kfree(argv[i]);

  return ret;

 bad:
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000520e:	f5040913          	addi	s2,s0,-176
    80005212:	6088                	ld	a0,0(s1)
    80005214:	c931                	beqz	a0,80005268 <sys_exec+0xe2>
    kfree(argv[i]);
    80005216:	807fb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000521a:	04a1                	addi	s1,s1,8
    8000521c:	ff249be3          	bne	s1,s2,80005212 <sys_exec+0x8c>
  return -1;
    80005220:	557d                	li	a0,-1
    80005222:	74ba                	ld	s1,424(sp)
    80005224:	791a                	ld	s2,416(sp)
    80005226:	69fa                	ld	s3,408(sp)
    80005228:	6a5a                	ld	s4,400(sp)
    8000522a:	a0a1                	j	80005272 <sys_exec+0xec>
      argv[i] = 0;
    8000522c:	0009079b          	sext.w	a5,s2
    80005230:	078e                	slli	a5,a5,0x3
    80005232:	fd078793          	addi	a5,a5,-48
    80005236:	97a2                	add	a5,a5,s0
    80005238:	e807b023          	sd	zero,-384(a5)
  int ret = kexec(path, argv);
    8000523c:	e5040593          	addi	a1,s0,-432
    80005240:	f5040513          	addi	a0,s0,-176
    80005244:	ba8ff0ef          	jal	800045ec <kexec>
    80005248:	892a                	mv	s2,a0
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    8000524a:	f5040993          	addi	s3,s0,-176
    8000524e:	6088                	ld	a0,0(s1)
    80005250:	c511                	beqz	a0,8000525c <sys_exec+0xd6>
    kfree(argv[i]);
    80005252:	fcafb0ef          	jal	80000a1c <kfree>
  for(i = 0; i < NELEM(argv) && argv[i] != 0; i++)
    80005256:	04a1                	addi	s1,s1,8
    80005258:	ff349be3          	bne	s1,s3,8000524e <sys_exec+0xc8>
  return ret;
    8000525c:	854a                	mv	a0,s2
    8000525e:	74ba                	ld	s1,424(sp)
    80005260:	791a                	ld	s2,416(sp)
    80005262:	69fa                	ld	s3,408(sp)
    80005264:	6a5a                	ld	s4,400(sp)
    80005266:	a031                	j	80005272 <sys_exec+0xec>
  return -1;
    80005268:	557d                	li	a0,-1
    8000526a:	74ba                	ld	s1,424(sp)
    8000526c:	791a                	ld	s2,416(sp)
    8000526e:	69fa                	ld	s3,408(sp)
    80005270:	6a5a                	ld	s4,400(sp)
}
    80005272:	70fa                	ld	ra,440(sp)
    80005274:	745a                	ld	s0,432(sp)
    80005276:	6139                	addi	sp,sp,448
    80005278:	8082                	ret

000000008000527a <sys_pipe>:

uint64
sys_pipe(void)
{
    8000527a:	7139                	addi	sp,sp,-64
    8000527c:	fc06                	sd	ra,56(sp)
    8000527e:	f822                	sd	s0,48(sp)
    80005280:	f426                	sd	s1,40(sp)
    80005282:	0080                	addi	s0,sp,64
  uint64 fdarray; // user pointer to array of two integers
  struct file *rf, *wf;
  int fd0, fd1;
  struct proc *p = myproc();
    80005284:	e56fc0ef          	jal	800018da <myproc>
    80005288:	84aa                	mv	s1,a0

  argaddr(0, &fdarray);
    8000528a:	fd840593          	addi	a1,s0,-40
    8000528e:	4501                	li	a0,0
    80005290:	d74fd0ef          	jal	80002804 <argaddr>
  if(pipealloc(&rf, &wf) < 0)
    80005294:	fc840593          	addi	a1,s0,-56
    80005298:	fd040513          	addi	a0,s0,-48
    8000529c:	852ff0ef          	jal	800042ee <pipealloc>
    return -1;
    800052a0:	57fd                	li	a5,-1
  if(pipealloc(&rf, &wf) < 0)
    800052a2:	0a054463          	bltz	a0,8000534a <sys_pipe+0xd0>
  fd0 = -1;
    800052a6:	fcf42223          	sw	a5,-60(s0)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
    800052aa:	fd043503          	ld	a0,-48(s0)
    800052ae:	f08ff0ef          	jal	800049b6 <fdalloc>
    800052b2:	fca42223          	sw	a0,-60(s0)
    800052b6:	08054163          	bltz	a0,80005338 <sys_pipe+0xbe>
    800052ba:	fc843503          	ld	a0,-56(s0)
    800052be:	ef8ff0ef          	jal	800049b6 <fdalloc>
    800052c2:	fca42023          	sw	a0,-64(s0)
    800052c6:	06054063          	bltz	a0,80005326 <sys_pipe+0xac>
      p->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    800052ca:	4691                	li	a3,4
    800052cc:	fc440613          	addi	a2,s0,-60
    800052d0:	fd843583          	ld	a1,-40(s0)
    800052d4:	68a8                	ld	a0,80(s1)
    800052d6:	b18fc0ef          	jal	800015ee <copyout>
    800052da:	00054e63          	bltz	a0,800052f6 <sys_pipe+0x7c>
     copyout(p->pagetable, fdarray+sizeof(fd0), (char *)&fd1, sizeof(fd1)) < 0){
    800052de:	4691                	li	a3,4
    800052e0:	fc040613          	addi	a2,s0,-64
    800052e4:	fd843583          	ld	a1,-40(s0)
    800052e8:	0591                	addi	a1,a1,4
    800052ea:	68a8                	ld	a0,80(s1)
    800052ec:	b02fc0ef          	jal	800015ee <copyout>
    p->ofile[fd1] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  return 0;
    800052f0:	4781                	li	a5,0
  if(copyout(p->pagetable, fdarray, (char*)&fd0, sizeof(fd0)) < 0 ||
    800052f2:	04055c63          	bgez	a0,8000534a <sys_pipe+0xd0>
    p->ofile[fd0] = 0;
    800052f6:	fc442783          	lw	a5,-60(s0)
    800052fa:	07e9                	addi	a5,a5,26
    800052fc:	078e                	slli	a5,a5,0x3
    800052fe:	97a6                	add	a5,a5,s1
    80005300:	0007b023          	sd	zero,0(a5)
    p->ofile[fd1] = 0;
    80005304:	fc042783          	lw	a5,-64(s0)
    80005308:	07e9                	addi	a5,a5,26
    8000530a:	078e                	slli	a5,a5,0x3
    8000530c:	94be                	add	s1,s1,a5
    8000530e:	0004b023          	sd	zero,0(s1)
    fileclose(rf);
    80005312:	fd043503          	ld	a0,-48(s0)
    80005316:	ccffe0ef          	jal	80003fe4 <fileclose>
    fileclose(wf);
    8000531a:	fc843503          	ld	a0,-56(s0)
    8000531e:	cc7fe0ef          	jal	80003fe4 <fileclose>
    return -1;
    80005322:	57fd                	li	a5,-1
    80005324:	a01d                	j	8000534a <sys_pipe+0xd0>
    if(fd0 >= 0)
    80005326:	fc442783          	lw	a5,-60(s0)
    8000532a:	0007c763          	bltz	a5,80005338 <sys_pipe+0xbe>
      p->ofile[fd0] = 0;
    8000532e:	07e9                	addi	a5,a5,26
    80005330:	078e                	slli	a5,a5,0x3
    80005332:	97a6                	add	a5,a5,s1
    80005334:	0007b023          	sd	zero,0(a5)
    fileclose(rf);
    80005338:	fd043503          	ld	a0,-48(s0)
    8000533c:	ca9fe0ef          	jal	80003fe4 <fileclose>
    fileclose(wf);
    80005340:	fc843503          	ld	a0,-56(s0)
    80005344:	ca1fe0ef          	jal	80003fe4 <fileclose>
    return -1;
    80005348:	57fd                	li	a5,-1
}
    8000534a:	853e                	mv	a0,a5
    8000534c:	70e2                	ld	ra,56(sp)
    8000534e:	7442                	ld	s0,48(sp)
    80005350:	74a2                	ld	s1,40(sp)
    80005352:	6121                	addi	sp,sp,64
    80005354:	8082                	ret

0000000080005356 <sys_fmlock>:

uint64
sys_fmlock(void)
{
    80005356:	711d                	addi	sp,sp,-96
    80005358:	ec86                	sd	ra,88(sp)
    8000535a:	e8a2                	sd	s0,80(sp)
    8000535c:	1080                	addi	s0,sp,96
  char name[64];
  int mode;

  if(argstr(0, name, sizeof(name)) < 0)
    8000535e:	04000613          	li	a2,64
    80005362:	fb040593          	addi	a1,s0,-80
    80005366:	4501                	li	a0,0
    80005368:	cb8fd0ef          	jal	80002820 <argstr>
    8000536c:	87aa                	mv	a5,a0
    return -1;
    8000536e:	557d                	li	a0,-1
  if(argstr(0, name, sizeof(name)) < 0)
    80005370:	0007cd63          	bltz	a5,8000538a <sys_fmlock+0x34>
  argint(1, &mode);
    80005374:	fac40593          	addi	a1,s0,-84
    80005378:	4505                	li	a0,1
    8000537a:	c6efd0ef          	jal	800027e8 <argint>
  return fm_acquire(name, mode);
    8000537e:	fac42583          	lw	a1,-84(s0)
    80005382:	fb040513          	addi	a0,s0,-80
    80005386:	160000ef          	jal	800054e6 <fm_acquire>
}
    8000538a:	60e6                	ld	ra,88(sp)
    8000538c:	6446                	ld	s0,80(sp)
    8000538e:	6125                	addi	sp,sp,96
    80005390:	8082                	ret

0000000080005392 <sys_fmunlock>:

uint64
sys_fmunlock(void)
{
    80005392:	711d                	addi	sp,sp,-96
    80005394:	ec86                	sd	ra,88(sp)
    80005396:	e8a2                	sd	s0,80(sp)
    80005398:	1080                	addi	s0,sp,96
  char name[64];
  int mode;

  if(argstr(0, name, sizeof(name)) < 0)
    8000539a:	04000613          	li	a2,64
    8000539e:	fb040593          	addi	a1,s0,-80
    800053a2:	4501                	li	a0,0
    800053a4:	c7cfd0ef          	jal	80002820 <argstr>
    800053a8:	87aa                	mv	a5,a0
    return -1;
    800053aa:	557d                	li	a0,-1
  if(argstr(0, name, sizeof(name)) < 0)
    800053ac:	0007cd63          	bltz	a5,800053c6 <sys_fmunlock+0x34>
  argint(1, &mode);
    800053b0:	fac40593          	addi	a1,s0,-84
    800053b4:	4505                	li	a0,1
    800053b6:	c32fd0ef          	jal	800027e8 <argint>
  return fm_release(name, mode);
    800053ba:	fac42583          	lw	a1,-84(s0)
    800053be:	fb040513          	addi	a0,s0,-80
    800053c2:	244000ef          	jal	80005606 <fm_release>
}
    800053c6:	60e6                	ld	ra,88(sp)
    800053c8:	6446                	ld	s0,80(sp)
    800053ca:	6125                	addi	sp,sp,96
    800053cc:	8082                	ret
	...

00000000800053d0 <kernelvec>:
.globl kerneltrap
.globl kernelvec
.align 4
kernelvec:
        # make room to save registers.
        addi sp, sp, -256
    800053d0:	7111                	addi	sp,sp,-256

        # save caller-saved registers.
        sd ra, 0(sp)
    800053d2:	e006                	sd	ra,0(sp)
        # sd sp, 8(sp)
        sd gp, 16(sp)
    800053d4:	e80e                	sd	gp,16(sp)
        sd tp, 24(sp)
    800053d6:	ec12                	sd	tp,24(sp)
        sd t0, 32(sp)
    800053d8:	f016                	sd	t0,32(sp)
        sd t1, 40(sp)
    800053da:	f41a                	sd	t1,40(sp)
        sd t2, 48(sp)
    800053dc:	f81e                	sd	t2,48(sp)
        sd a0, 72(sp)
    800053de:	e4aa                	sd	a0,72(sp)
        sd a1, 80(sp)
    800053e0:	e8ae                	sd	a1,80(sp)
        sd a2, 88(sp)
    800053e2:	ecb2                	sd	a2,88(sp)
        sd a3, 96(sp)
    800053e4:	f0b6                	sd	a3,96(sp)
        sd a4, 104(sp)
    800053e6:	f4ba                	sd	a4,104(sp)
        sd a5, 112(sp)
    800053e8:	f8be                	sd	a5,112(sp)
        sd a6, 120(sp)
    800053ea:	fcc2                	sd	a6,120(sp)
        sd a7, 128(sp)
    800053ec:	e146                	sd	a7,128(sp)
        sd t3, 216(sp)
    800053ee:	edf2                	sd	t3,216(sp)
        sd t4, 224(sp)
    800053f0:	f1f6                	sd	t4,224(sp)
        sd t5, 232(sp)
    800053f2:	f5fa                	sd	t5,232(sp)
        sd t6, 240(sp)
    800053f4:	f9fe                	sd	t6,240(sp)

        # call the C trap handler in trap.c
        call kerneltrap
    800053f6:	a78fd0ef          	jal	8000266e <kerneltrap>

        # restore registers.
        ld ra, 0(sp)
    800053fa:	6082                	ld	ra,0(sp)
        # ld sp, 8(sp)
        ld gp, 16(sp)
    800053fc:	61c2                	ld	gp,16(sp)
        # not tp (contains hartid), in case we moved CPUs
        ld t0, 32(sp)
    800053fe:	7282                	ld	t0,32(sp)
        ld t1, 40(sp)
    80005400:	7322                	ld	t1,40(sp)
        ld t2, 48(sp)
    80005402:	73c2                	ld	t2,48(sp)
        ld a0, 72(sp)
    80005404:	6526                	ld	a0,72(sp)
        ld a1, 80(sp)
    80005406:	65c6                	ld	a1,80(sp)
        ld a2, 88(sp)
    80005408:	6666                	ld	a2,88(sp)
        ld a3, 96(sp)
    8000540a:	7686                	ld	a3,96(sp)
        ld a4, 104(sp)
    8000540c:	7726                	ld	a4,104(sp)
        ld a5, 112(sp)
    8000540e:	77c6                	ld	a5,112(sp)
        ld a6, 120(sp)
    80005410:	7866                	ld	a6,120(sp)
        ld a7, 128(sp)
    80005412:	688a                	ld	a7,128(sp)
        ld t3, 216(sp)
    80005414:	6e6e                	ld	t3,216(sp)
        ld t4, 224(sp)
    80005416:	7e8e                	ld	t4,224(sp)
        ld t5, 232(sp)
    80005418:	7f2e                	ld	t5,232(sp)
        ld t6, 240(sp)
    8000541a:	7fce                	ld	t6,240(sp)

        addi sp, sp, 256
    8000541c:	6111                	addi	sp,sp,256

        # return to whatever we were doing in the kernel.
        sret
    8000541e:	10200073          	sret
	...

000000008000542e <plicinit>:
// the riscv Platform Level Interrupt Controller (PLIC).
//

void
plicinit(void)
{
    8000542e:	1141                	addi	sp,sp,-16
    80005430:	e422                	sd	s0,8(sp)
    80005432:	0800                	addi	s0,sp,16
  // set desired IRQ priorities non-zero (otherwise disabled).
  *(uint32*)(PLIC + UART0_IRQ*4) = 1;
    80005434:	0c0007b7          	lui	a5,0xc000
    80005438:	4705                	li	a4,1
    8000543a:	d798                	sw	a4,40(a5)
  *(uint32*)(PLIC + VIRTIO0_IRQ*4) = 1;
    8000543c:	0c0007b7          	lui	a5,0xc000
    80005440:	c3d8                	sw	a4,4(a5)
}
    80005442:	6422                	ld	s0,8(sp)
    80005444:	0141                	addi	sp,sp,16
    80005446:	8082                	ret

0000000080005448 <plicinithart>:

void
plicinithart(void)
{
    80005448:	1141                	addi	sp,sp,-16
    8000544a:	e406                	sd	ra,8(sp)
    8000544c:	e022                	sd	s0,0(sp)
    8000544e:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005450:	c5efc0ef          	jal	800018ae <cpuid>
  
  // set enable bits for this hart's S-mode
  // for the uart and virtio disk.
  *(uint32*)PLIC_SENABLE(hart) = (1 << UART0_IRQ) | (1 << VIRTIO0_IRQ);
    80005454:	0085171b          	slliw	a4,a0,0x8
    80005458:	0c0027b7          	lui	a5,0xc002
    8000545c:	97ba                	add	a5,a5,a4
    8000545e:	40200713          	li	a4,1026
    80005462:	08e7a023          	sw	a4,128(a5) # c002080 <_entry-0x73ffdf80>

  // set this hart's S-mode priority threshold to 0.
  *(uint32*)PLIC_SPRIORITY(hart) = 0;
    80005466:	00d5151b          	slliw	a0,a0,0xd
    8000546a:	0c2017b7          	lui	a5,0xc201
    8000546e:	97aa                	add	a5,a5,a0
    80005470:	0007a023          	sw	zero,0(a5) # c201000 <_entry-0x73dff000>
}
    80005474:	60a2                	ld	ra,8(sp)
    80005476:	6402                	ld	s0,0(sp)
    80005478:	0141                	addi	sp,sp,16
    8000547a:	8082                	ret

000000008000547c <plic_claim>:

// ask the PLIC what interrupt we should serve.
int
plic_claim(void)
{
    8000547c:	1141                	addi	sp,sp,-16
    8000547e:	e406                	sd	ra,8(sp)
    80005480:	e022                	sd	s0,0(sp)
    80005482:	0800                	addi	s0,sp,16
  int hart = cpuid();
    80005484:	c2afc0ef          	jal	800018ae <cpuid>
  int irq = *(uint32*)PLIC_SCLAIM(hart);
    80005488:	00d5151b          	slliw	a0,a0,0xd
    8000548c:	0c2017b7          	lui	a5,0xc201
    80005490:	97aa                	add	a5,a5,a0
  return irq;
}
    80005492:	43c8                	lw	a0,4(a5)
    80005494:	60a2                	ld	ra,8(sp)
    80005496:	6402                	ld	s0,0(sp)
    80005498:	0141                	addi	sp,sp,16
    8000549a:	8082                	ret

000000008000549c <plic_complete>:

// tell the PLIC we've served this IRQ.
void
plic_complete(int irq)
{
    8000549c:	1101                	addi	sp,sp,-32
    8000549e:	ec06                	sd	ra,24(sp)
    800054a0:	e822                	sd	s0,16(sp)
    800054a2:	e426                	sd	s1,8(sp)
    800054a4:	1000                	addi	s0,sp,32
    800054a6:	84aa                	mv	s1,a0
  int hart = cpuid();
    800054a8:	c06fc0ef          	jal	800018ae <cpuid>
  *(uint32*)PLIC_SCLAIM(hart) = irq;
    800054ac:	00d5151b          	slliw	a0,a0,0xd
    800054b0:	0c2017b7          	lui	a5,0xc201
    800054b4:	97aa                	add	a5,a5,a0
    800054b6:	c3c4                	sw	s1,4(a5)
}
    800054b8:	60e2                	ld	ra,24(sp)
    800054ba:	6442                	ld	s0,16(sp)
    800054bc:	64a2                	ld	s1,8(sp)
    800054be:	6105                	addi	sp,sp,32
    800054c0:	8082                	ret

00000000800054c2 <fmlockinit>:
  return freee;
}

void
fmlockinit(void)
{
    800054c2:	1141                	addi	sp,sp,-16
    800054c4:	e406                	sd	ra,8(sp)
    800054c6:	e022                	sd	s0,0(sp)
    800054c8:	0800                	addi	s0,sp,16
  initlock(&fmtable.lock, "fmtable");
    800054ca:	00002597          	auipc	a1,0x2
    800054ce:	14658593          	addi	a1,a1,326 # 80007610 <etext+0x610>
    800054d2:	0001e517          	auipc	a0,0x1e
    800054d6:	fe650513          	addi	a0,a0,-26 # 800234b8 <fmtable>
    800054da:	e74fb0ef          	jal	80000b4e <initlock>
}
    800054de:	60a2                	ld	ra,8(sp)
    800054e0:	6402                	ld	s0,0(sp)
    800054e2:	0141                	addi	sp,sp,16
    800054e4:	8082                	ret

00000000800054e6 <fm_acquire>:

int
fm_acquire(char *name, int mode)
{
    800054e6:	7159                	addi	sp,sp,-112
    800054e8:	f486                	sd	ra,104(sp)
    800054ea:	f0a2                	sd	s0,96(sp)
    800054ec:	eca6                	sd	s1,88(sp)
    800054ee:	e8ca                	sd	s2,80(sp)
    800054f0:	e4ce                	sd	s3,72(sp)
    800054f2:	e0d2                	sd	s4,64(sp)
    800054f4:	fc56                	sd	s5,56(sp)
    800054f6:	f85a                	sd	s6,48(sp)
    800054f8:	f45e                	sd	s7,40(sp)
    800054fa:	f062                	sd	s8,32(sp)
    800054fc:	ec66                	sd	s9,24(sp)
    800054fe:	e86a                	sd	s10,16(sp)
    80005500:	e46e                	sd	s11,8(sp)
    80005502:	1880                	addi	s0,sp,112
    80005504:	8a2a                	mv	s4,a0
    80005506:	8bae                	mv	s7,a1
  struct fmentry *e;

  acquire(&fmtable.lock);
    80005508:	0001e517          	auipc	a0,0x1e
    8000550c:	fb050513          	addi	a0,a0,-80 # 800234b8 <fmtable>
    80005510:	ebefb0ef          	jal	80000bce <acquire>
  struct fmentry *freee = 0;
    80005514:	4a81                	li	s5,0
  for(i = 0; i < FM_MAX_LOCKS; i++){
    80005516:	02000993          	li	s3,32
    freee->used = 1;
    8000551a:	4c85                	li	s9,1
      return &fmtable.e[i];
    8000551c:	04c00c13          	li	s8,76
    80005520:	0001eb17          	auipc	s6,0x1e
    80005524:	f98b0b13          	addi	s6,s6,-104 # 800234b8 <fmtable>
    80005528:	a069                	j	800055b2 <fm_acquire+0xcc>
    if(fmtable.e[i].used && strncmp(fmtable.e[i].name, name, FM_NAME_MAX) == 0)
    8000552a:	04000613          	li	a2,64
    8000552e:	85d2                	mv	a1,s4
    80005530:	00448513          	addi	a0,s1,4
    80005534:	83bfb0ef          	jal	80000d6e <strncmp>
    80005538:	cd19                	beqz	a0,80005556 <fm_acquire+0x70>
    if(!fmtable.e[i].used && freee == 0)
    8000553a:	409c                	lw	a5,0(s1)
    8000553c:	cb89                	beqz	a5,8000554e <fm_acquire+0x68>
  for(i = 0; i < FM_MAX_LOCKS; i++){
    8000553e:	2905                	addiw	s2,s2,1
    80005540:	04c48493          	addi	s1,s1,76
    80005544:	05390063          	beq	s2,s3,80005584 <fm_acquire+0x9e>
    if(fmtable.e[i].used && strncmp(fmtable.e[i].name, name, FM_NAME_MAX) == 0)
    80005548:	8d26                	mv	s10,s1
    8000554a:	409c                	lw	a5,0(s1)
    8000554c:	fff9                	bnez	a5,8000552a <fm_acquire+0x44>
    if(!fmtable.e[i].used && freee == 0)
    8000554e:	fe0d98e3          	bnez	s11,8000553e <fm_acquire+0x58>
      freee = &fmtable.e[i];
    80005552:	8dea                	mv	s11,s10
    80005554:	b7ed                	j	8000553e <fm_acquire+0x58>
      return &fmtable.e[i];
    80005556:	03890933          	mul	s2,s2,s8
    8000555a:	0961                	addi	s2,s2,24
    8000555c:	01690db3          	add	s11,s2,s6
    if(e == 0){
      release(&fmtable.lock);
      return -1;
    }

    if(mode == FM_READ){
    80005560:	040b8263          	beqz	s7,800055a4 <fm_acquire+0xbe>
        e->readers++;
        release(&fmtable.lock);
        return 0;
      }
    } else {
      if(e->writer == 0 && e->readers == 0){
    80005564:	048da783          	lw	a5,72(s11)
    80005568:	e3a9                	bnez	a5,800055aa <fm_acquire+0xc4>
    8000556a:	044da483          	lw	s1,68(s11)
    8000556e:	ec95                	bnez	s1,800055aa <fm_acquire+0xc4>
        e->writer = 1;
    80005570:	4785                	li	a5,1
    80005572:	04fda423          	sw	a5,72(s11)
        release(&fmtable.lock);
    80005576:	0001e517          	auipc	a0,0x1e
    8000557a:	f4250513          	addi	a0,a0,-190 # 800234b8 <fmtable>
    8000557e:	ee8fb0ef          	jal	80000c66 <release>
        return 0;
    80005582:	a095                	j	800055e6 <fm_acquire+0x100>
  if(freee){
    80005584:	040d8a63          	beqz	s11,800055d8 <fm_acquire+0xf2>
    freee->used = 1;
    80005588:	019da023          	sw	s9,0(s11)
    freee->readers = 0;
    8000558c:	040da223          	sw	zero,68(s11)
    freee->writer = 0;
    80005590:	040da423          	sw	zero,72(s11)
    safestrcpy(freee->name, name, FM_NAME_MAX);
    80005594:	04000613          	li	a2,64
    80005598:	85d2                	mv	a1,s4
    8000559a:	004d8513          	addi	a0,s11,4
    8000559e:	843fb0ef          	jal	80000de0 <safestrcpy>
    800055a2:	bf7d                	j	80005560 <fm_acquire+0x7a>
      if(e->writer == 0){
    800055a4:	048da483          	lw	s1,72(s11)
    800055a8:	cc81                	beqz	s1,800055c0 <fm_acquire+0xda>
      }
    }

    sleep(e, &fmtable.lock);
    800055aa:	85da                	mv	a1,s6
    800055ac:	856e                	mv	a0,s11
    800055ae:	937fc0ef          	jal	80001ee4 <sleep>
  for(i = 0; i < FM_MAX_LOCKS; i++){
    800055b2:	0001e497          	auipc	s1,0x1e
    800055b6:	f1e48493          	addi	s1,s1,-226 # 800234d0 <fmtable+0x18>
  struct fmentry *freee = 0;
    800055ba:	8dd6                	mv	s11,s5
  for(i = 0; i < FM_MAX_LOCKS; i++){
    800055bc:	8956                	mv	s2,s5
    800055be:	b769                	j	80005548 <fm_acquire+0x62>
        e->readers++;
    800055c0:	044da783          	lw	a5,68(s11)
    800055c4:	2785                	addiw	a5,a5,1 # c201001 <_entry-0x73dfefff>
    800055c6:	04fda223          	sw	a5,68(s11)
        release(&fmtable.lock);
    800055ca:	0001e517          	auipc	a0,0x1e
    800055ce:	eee50513          	addi	a0,a0,-274 # 800234b8 <fmtable>
    800055d2:	e94fb0ef          	jal	80000c66 <release>
        return 0;
    800055d6:	a801                	j	800055e6 <fm_acquire+0x100>
      release(&fmtable.lock);
    800055d8:	0001e517          	auipc	a0,0x1e
    800055dc:	ee050513          	addi	a0,a0,-288 # 800234b8 <fmtable>
    800055e0:	e86fb0ef          	jal	80000c66 <release>
      return -1;
    800055e4:	54fd                	li	s1,-1
  }
}
    800055e6:	8526                	mv	a0,s1
    800055e8:	70a6                	ld	ra,104(sp)
    800055ea:	7406                	ld	s0,96(sp)
    800055ec:	64e6                	ld	s1,88(sp)
    800055ee:	6946                	ld	s2,80(sp)
    800055f0:	69a6                	ld	s3,72(sp)
    800055f2:	6a06                	ld	s4,64(sp)
    800055f4:	7ae2                	ld	s5,56(sp)
    800055f6:	7b42                	ld	s6,48(sp)
    800055f8:	7ba2                	ld	s7,40(sp)
    800055fa:	7c02                	ld	s8,32(sp)
    800055fc:	6ce2                	ld	s9,24(sp)
    800055fe:	6d42                	ld	s10,16(sp)
    80005600:	6da2                	ld	s11,8(sp)
    80005602:	6165                	addi	sp,sp,112
    80005604:	8082                	ret

0000000080005606 <fm_release>:

int
fm_release(char *name, int mode)
{
    80005606:	7139                	addi	sp,sp,-64
    80005608:	fc06                	sd	ra,56(sp)
    8000560a:	f822                	sd	s0,48(sp)
    8000560c:	f426                	sd	s1,40(sp)
    8000560e:	f04a                	sd	s2,32(sp)
    80005610:	ec4e                	sd	s3,24(sp)
    80005612:	e852                	sd	s4,16(sp)
    80005614:	e456                	sd	s5,8(sp)
    80005616:	e05a                	sd	s6,0(sp)
    80005618:	0080                	addi	s0,sp,64
    8000561a:	8aaa                	mv	s5,a0
    8000561c:	8b2e                	mv	s6,a1
  struct fmentry *e;
  void *chan;

  acquire(&fmtable.lock);
    8000561e:	0001e517          	auipc	a0,0x1e
    80005622:	e9a50513          	addi	a0,a0,-358 # 800234b8 <fmtable>
    80005626:	da8fb0ef          	jal	80000bce <acquire>
  for(i = 0; i < FM_MAX_LOCKS; i++){
    8000562a:	0001e497          	auipc	s1,0x1e
    8000562e:	eaa48493          	addi	s1,s1,-342 # 800234d4 <fmtable+0x1c>
    80005632:	4901                	li	s2,0
    80005634:	02000a13          	li	s4,32
    80005638:	a031                	j	80005644 <fm_release+0x3e>
    8000563a:	2905                	addiw	s2,s2,1
    8000563c:	04c48493          	addi	s1,s1,76
    80005640:	07490e63          	beq	s2,s4,800056bc <fm_release+0xb6>
    if(fmtable.e[i].used && strncmp(fmtable.e[i].name, name, FM_NAME_MAX) == 0)
    80005644:	ffc4a783          	lw	a5,-4(s1)
    80005648:	dbed                	beqz	a5,8000563a <fm_release+0x34>
    8000564a:	04000613          	li	a2,64
    8000564e:	85d6                	mv	a1,s5
    80005650:	8526                	mv	a0,s1
    80005652:	f1cfb0ef          	jal	80000d6e <strncmp>
    80005656:	89aa                	mv	s3,a0
    80005658:	f16d                	bnez	a0,8000563a <fm_release+0x34>
      return &fmtable.e[i];
    8000565a:	04c00513          	li	a0,76
    8000565e:	02a90533          	mul	a0,s2,a0
    80005662:	0001e797          	auipc	a5,0x1e
    80005666:	e6e78793          	addi	a5,a5,-402 # 800234d0 <fmtable+0x18>
    8000566a:	953e                	add	a0,a0,a5
  if(e == 0){
    release(&fmtable.lock);
    return -1;
  }

  if(mode == FM_READ){
    8000566c:	060b0063          	beqz	s6,800056cc <fm_release+0xc6>
      release(&fmtable.lock);
      return -1;
    }
    e->readers--;
  } else {
    if(e->writer == 0){
    80005670:	04c00713          	li	a4,76
    80005674:	02e90733          	mul	a4,s2,a4
    80005678:	0001e797          	auipc	a5,0x1e
    8000567c:	e4078793          	addi	a5,a5,-448 # 800234b8 <fmtable>
    80005680:	97ba                	add	a5,a5,a4
    80005682:	53bc                	lw	a5,96(a5)
    80005684:	cbcd                	beqz	a5,80005736 <fm_release+0x130>
      release(&fmtable.lock);
      return -1;
    }
    e->writer = 0;
    80005686:	04c00713          	li	a4,76
    8000568a:	02e90733          	mul	a4,s2,a4
    8000568e:	0001e797          	auipc	a5,0x1e
    80005692:	e2a78793          	addi	a5,a5,-470 # 800234b8 <fmtable>
    80005696:	97ba                	add	a5,a5,a4
    80005698:	0607a023          	sw	zero,96(a5)
  }

  chan = e;
  if(e->readers == 0 && e->writer == 0){
    8000569c:	4ffc                	lw	a5,92(a5)
    8000569e:	e3ad                	bnez	a5,80005700 <fm_release+0xfa>
    e->used = 0;
    800056a0:	04c00793          	li	a5,76
    800056a4:	02f90933          	mul	s2,s2,a5
    800056a8:	0001e797          	auipc	a5,0x1e
    800056ac:	e1078793          	addi	a5,a5,-496 # 800234b8 <fmtable>
    800056b0:	97ca                	add	a5,a5,s2
    800056b2:	0007ac23          	sw	zero,24(a5)
    e->name[0] = 0;
    800056b6:	00078e23          	sb	zero,28(a5)
    800056ba:	a099                	j	80005700 <fm_release+0xfa>
    release(&fmtable.lock);
    800056bc:	0001e517          	auipc	a0,0x1e
    800056c0:	dfc50513          	addi	a0,a0,-516 # 800234b8 <fmtable>
    800056c4:	da2fb0ef          	jal	80000c66 <release>
    return -1;
    800056c8:	59fd                	li	s3,-1
    800056ca:	a099                	j	80005710 <fm_release+0x10a>
    if(e->readers <= 0){
    800056cc:	04c00713          	li	a4,76
    800056d0:	02e90733          	mul	a4,s2,a4
    800056d4:	0001e797          	auipc	a5,0x1e
    800056d8:	de478793          	addi	a5,a5,-540 # 800234b8 <fmtable>
    800056dc:	97ba                	add	a5,a5,a4
    800056de:	4ffc                	lw	a5,92(a5)
    800056e0:	04f05363          	blez	a5,80005726 <fm_release+0x120>
    e->readers--;
    800056e4:	37fd                	addiw	a5,a5,-1
    800056e6:	0007861b          	sext.w	a2,a5
    800056ea:	04c00693          	li	a3,76
    800056ee:	02d906b3          	mul	a3,s2,a3
    800056f2:	0001e717          	auipc	a4,0x1e
    800056f6:	dc670713          	addi	a4,a4,-570 # 800234b8 <fmtable>
    800056fa:	9736                	add	a4,a4,a3
    800056fc:	cf7c                	sw	a5,92(a4)
  if(e->readers == 0 && e->writer == 0){
    800056fe:	c621                	beqz	a2,80005746 <fm_release+0x140>
  }

  wakeup(chan);
    80005700:	831fc0ef          	jal	80001f30 <wakeup>
  release(&fmtable.lock);
    80005704:	0001e517          	auipc	a0,0x1e
    80005708:	db450513          	addi	a0,a0,-588 # 800234b8 <fmtable>
    8000570c:	d5afb0ef          	jal	80000c66 <release>
  return 0;
}
    80005710:	854e                	mv	a0,s3
    80005712:	70e2                	ld	ra,56(sp)
    80005714:	7442                	ld	s0,48(sp)
    80005716:	74a2                	ld	s1,40(sp)
    80005718:	7902                	ld	s2,32(sp)
    8000571a:	69e2                	ld	s3,24(sp)
    8000571c:	6a42                	ld	s4,16(sp)
    8000571e:	6aa2                	ld	s5,8(sp)
    80005720:	6b02                	ld	s6,0(sp)
    80005722:	6121                	addi	sp,sp,64
    80005724:	8082                	ret
      release(&fmtable.lock);
    80005726:	0001e517          	auipc	a0,0x1e
    8000572a:	d9250513          	addi	a0,a0,-622 # 800234b8 <fmtable>
    8000572e:	d38fb0ef          	jal	80000c66 <release>
      return -1;
    80005732:	59fd                	li	s3,-1
    80005734:	bff1                	j	80005710 <fm_release+0x10a>
      release(&fmtable.lock);
    80005736:	0001e517          	auipc	a0,0x1e
    8000573a:	d8250513          	addi	a0,a0,-638 # 800234b8 <fmtable>
    8000573e:	d28fb0ef          	jal	80000c66 <release>
      return -1;
    80005742:	59fd                	li	s3,-1
    80005744:	b7f1                	j	80005710 <fm_release+0x10a>
  if(e->readers == 0 && e->writer == 0){
    80005746:	04c00713          	li	a4,76
    8000574a:	02e90733          	mul	a4,s2,a4
    8000574e:	0001e797          	auipc	a5,0x1e
    80005752:	d6a78793          	addi	a5,a5,-662 # 800234b8 <fmtable>
    80005756:	97ba                	add	a5,a5,a4
    80005758:	53bc                	lw	a5,96(a5)
    8000575a:	f3dd                	bnez	a5,80005700 <fm_release+0xfa>
    8000575c:	b791                	j	800056a0 <fm_release+0x9a>

000000008000575e <free_desc>:
}

// mark a descriptor as free.
static void
free_desc(int i)
{
    8000575e:	1141                	addi	sp,sp,-16
    80005760:	e406                	sd	ra,8(sp)
    80005762:	e022                	sd	s0,0(sp)
    80005764:	0800                	addi	s0,sp,16
  if(i >= NUM)
    80005766:	479d                	li	a5,7
    80005768:	04a7ca63          	blt	a5,a0,800057bc <free_desc+0x5e>
    panic("free_desc 1");
  if(disk.free[i])
    8000576c:	0001e797          	auipc	a5,0x1e
    80005770:	6e478793          	addi	a5,a5,1764 # 80023e50 <disk>
    80005774:	97aa                	add	a5,a5,a0
    80005776:	0187c783          	lbu	a5,24(a5)
    8000577a:	e7b9                	bnez	a5,800057c8 <free_desc+0x6a>
    panic("free_desc 2");
  disk.desc[i].addr = 0;
    8000577c:	00451693          	slli	a3,a0,0x4
    80005780:	0001e797          	auipc	a5,0x1e
    80005784:	6d078793          	addi	a5,a5,1744 # 80023e50 <disk>
    80005788:	6398                	ld	a4,0(a5)
    8000578a:	9736                	add	a4,a4,a3
    8000578c:	00073023          	sd	zero,0(a4)
  disk.desc[i].len = 0;
    80005790:	6398                	ld	a4,0(a5)
    80005792:	9736                	add	a4,a4,a3
    80005794:	00072423          	sw	zero,8(a4)
  disk.desc[i].flags = 0;
    80005798:	00071623          	sh	zero,12(a4)
  disk.desc[i].next = 0;
    8000579c:	00071723          	sh	zero,14(a4)
  disk.free[i] = 1;
    800057a0:	97aa                	add	a5,a5,a0
    800057a2:	4705                	li	a4,1
    800057a4:	00e78c23          	sb	a4,24(a5)
  wakeup(&disk.free[0]);
    800057a8:	0001e517          	auipc	a0,0x1e
    800057ac:	6c050513          	addi	a0,a0,1728 # 80023e68 <disk+0x18>
    800057b0:	f80fc0ef          	jal	80001f30 <wakeup>
}
    800057b4:	60a2                	ld	ra,8(sp)
    800057b6:	6402                	ld	s0,0(sp)
    800057b8:	0141                	addi	sp,sp,16
    800057ba:	8082                	ret
    panic("free_desc 1");
    800057bc:	00002517          	auipc	a0,0x2
    800057c0:	e5c50513          	addi	a0,a0,-420 # 80007618 <etext+0x618>
    800057c4:	81cfb0ef          	jal	800007e0 <panic>
    panic("free_desc 2");
    800057c8:	00002517          	auipc	a0,0x2
    800057cc:	e6050513          	addi	a0,a0,-416 # 80007628 <etext+0x628>
    800057d0:	810fb0ef          	jal	800007e0 <panic>

00000000800057d4 <virtio_disk_init>:
{
    800057d4:	1101                	addi	sp,sp,-32
    800057d6:	ec06                	sd	ra,24(sp)
    800057d8:	e822                	sd	s0,16(sp)
    800057da:	e426                	sd	s1,8(sp)
    800057dc:	e04a                	sd	s2,0(sp)
    800057de:	1000                	addi	s0,sp,32
  initlock(&disk.vdisk_lock, "virtio_disk");
    800057e0:	00002597          	auipc	a1,0x2
    800057e4:	e5858593          	addi	a1,a1,-424 # 80007638 <etext+0x638>
    800057e8:	0001e517          	auipc	a0,0x1e
    800057ec:	79050513          	addi	a0,a0,1936 # 80023f78 <disk+0x128>
    800057f0:	b5efb0ef          	jal	80000b4e <initlock>
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    800057f4:	100017b7          	lui	a5,0x10001
    800057f8:	4398                	lw	a4,0(a5)
    800057fa:	2701                	sext.w	a4,a4
    800057fc:	747277b7          	lui	a5,0x74727
    80005800:	97678793          	addi	a5,a5,-1674 # 74726976 <_entry-0xb8d968a>
    80005804:	18f71063          	bne	a4,a5,80005984 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    80005808:	100017b7          	lui	a5,0x10001
    8000580c:	0791                	addi	a5,a5,4 # 10001004 <_entry-0x6fffeffc>
    8000580e:	439c                	lw	a5,0(a5)
    80005810:	2781                	sext.w	a5,a5
  if(*R(VIRTIO_MMIO_MAGIC_VALUE) != 0x74726976 ||
    80005812:	4709                	li	a4,2
    80005814:	16e79863          	bne	a5,a4,80005984 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    80005818:	100017b7          	lui	a5,0x10001
    8000581c:	07a1                	addi	a5,a5,8 # 10001008 <_entry-0x6fffeff8>
    8000581e:	439c                	lw	a5,0(a5)
    80005820:	2781                	sext.w	a5,a5
     *R(VIRTIO_MMIO_VERSION) != 2 ||
    80005822:	16e79163          	bne	a5,a4,80005984 <virtio_disk_init+0x1b0>
     *R(VIRTIO_MMIO_VENDOR_ID) != 0x554d4551){
    80005826:	100017b7          	lui	a5,0x10001
    8000582a:	47d8                	lw	a4,12(a5)
    8000582c:	2701                	sext.w	a4,a4
     *R(VIRTIO_MMIO_DEVICE_ID) != 2 ||
    8000582e:	554d47b7          	lui	a5,0x554d4
    80005832:	55178793          	addi	a5,a5,1361 # 554d4551 <_entry-0x2ab2baaf>
    80005836:	14f71763          	bne	a4,a5,80005984 <virtio_disk_init+0x1b0>
  *R(VIRTIO_MMIO_STATUS) = status;
    8000583a:	100017b7          	lui	a5,0x10001
    8000583e:	0607a823          	sw	zero,112(a5) # 10001070 <_entry-0x6fffef90>
  *R(VIRTIO_MMIO_STATUS) = status;
    80005842:	4705                	li	a4,1
    80005844:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80005846:	470d                	li	a4,3
    80005848:	dbb8                	sw	a4,112(a5)
  uint64 features = *R(VIRTIO_MMIO_DEVICE_FEATURES);
    8000584a:	10001737          	lui	a4,0x10001
    8000584e:	4b14                	lw	a3,16(a4)
  features &= ~(1 << VIRTIO_RING_F_INDIRECT_DESC);
    80005850:	c7ffe737          	lui	a4,0xc7ffe
    80005854:	75f70713          	addi	a4,a4,1887 # ffffffffc7ffe75f <end+0xffffffff47fda7cf>
  *R(VIRTIO_MMIO_DRIVER_FEATURES) = features;
    80005858:	8ef9                	and	a3,a3,a4
    8000585a:	10001737          	lui	a4,0x10001
    8000585e:	d314                	sw	a3,32(a4)
  *R(VIRTIO_MMIO_STATUS) = status;
    80005860:	472d                	li	a4,11
    80005862:	dbb8                	sw	a4,112(a5)
  *R(VIRTIO_MMIO_STATUS) = status;
    80005864:	07078793          	addi	a5,a5,112
  status = *R(VIRTIO_MMIO_STATUS);
    80005868:	439c                	lw	a5,0(a5)
    8000586a:	0007891b          	sext.w	s2,a5
  if(!(status & VIRTIO_CONFIG_S_FEATURES_OK))
    8000586e:	8ba1                	andi	a5,a5,8
    80005870:	12078063          	beqz	a5,80005990 <virtio_disk_init+0x1bc>
  *R(VIRTIO_MMIO_QUEUE_SEL) = 0;
    80005874:	100017b7          	lui	a5,0x10001
    80005878:	0207a823          	sw	zero,48(a5) # 10001030 <_entry-0x6fffefd0>
  if(*R(VIRTIO_MMIO_QUEUE_READY))
    8000587c:	100017b7          	lui	a5,0x10001
    80005880:	04478793          	addi	a5,a5,68 # 10001044 <_entry-0x6fffefbc>
    80005884:	439c                	lw	a5,0(a5)
    80005886:	2781                	sext.w	a5,a5
    80005888:	10079a63          	bnez	a5,8000599c <virtio_disk_init+0x1c8>
  uint32 max = *R(VIRTIO_MMIO_QUEUE_NUM_MAX);
    8000588c:	100017b7          	lui	a5,0x10001
    80005890:	03478793          	addi	a5,a5,52 # 10001034 <_entry-0x6fffefcc>
    80005894:	439c                	lw	a5,0(a5)
    80005896:	2781                	sext.w	a5,a5
  if(max == 0)
    80005898:	10078863          	beqz	a5,800059a8 <virtio_disk_init+0x1d4>
  if(max < NUM)
    8000589c:	471d                	li	a4,7
    8000589e:	10f77b63          	bgeu	a4,a5,800059b4 <virtio_disk_init+0x1e0>
  disk.desc = kalloc();
    800058a2:	a5cfb0ef          	jal	80000afe <kalloc>
    800058a6:	0001e497          	auipc	s1,0x1e
    800058aa:	5aa48493          	addi	s1,s1,1450 # 80023e50 <disk>
    800058ae:	e088                	sd	a0,0(s1)
  disk.avail = kalloc();
    800058b0:	a4efb0ef          	jal	80000afe <kalloc>
    800058b4:	e488                	sd	a0,8(s1)
  disk.used = kalloc();
    800058b6:	a48fb0ef          	jal	80000afe <kalloc>
    800058ba:	87aa                	mv	a5,a0
    800058bc:	e888                	sd	a0,16(s1)
  if(!disk.desc || !disk.avail || !disk.used)
    800058be:	6088                	ld	a0,0(s1)
    800058c0:	10050063          	beqz	a0,800059c0 <virtio_disk_init+0x1ec>
    800058c4:	0001e717          	auipc	a4,0x1e
    800058c8:	59473703          	ld	a4,1428(a4) # 80023e58 <disk+0x8>
    800058cc:	0e070a63          	beqz	a4,800059c0 <virtio_disk_init+0x1ec>
    800058d0:	0e078863          	beqz	a5,800059c0 <virtio_disk_init+0x1ec>
  memset(disk.desc, 0, PGSIZE);
    800058d4:	6605                	lui	a2,0x1
    800058d6:	4581                	li	a1,0
    800058d8:	bcafb0ef          	jal	80000ca2 <memset>
  memset(disk.avail, 0, PGSIZE);
    800058dc:	0001e497          	auipc	s1,0x1e
    800058e0:	57448493          	addi	s1,s1,1396 # 80023e50 <disk>
    800058e4:	6605                	lui	a2,0x1
    800058e6:	4581                	li	a1,0
    800058e8:	6488                	ld	a0,8(s1)
    800058ea:	bb8fb0ef          	jal	80000ca2 <memset>
  memset(disk.used, 0, PGSIZE);
    800058ee:	6605                	lui	a2,0x1
    800058f0:	4581                	li	a1,0
    800058f2:	6888                	ld	a0,16(s1)
    800058f4:	baefb0ef          	jal	80000ca2 <memset>
  *R(VIRTIO_MMIO_QUEUE_NUM) = NUM;
    800058f8:	100017b7          	lui	a5,0x10001
    800058fc:	4721                	li	a4,8
    800058fe:	df98                	sw	a4,56(a5)
  *R(VIRTIO_MMIO_QUEUE_DESC_LOW) = (uint64)disk.desc;
    80005900:	4098                	lw	a4,0(s1)
    80005902:	100017b7          	lui	a5,0x10001
    80005906:	08e7a023          	sw	a4,128(a5) # 10001080 <_entry-0x6fffef80>
  *R(VIRTIO_MMIO_QUEUE_DESC_HIGH) = (uint64)disk.desc >> 32;
    8000590a:	40d8                	lw	a4,4(s1)
    8000590c:	100017b7          	lui	a5,0x10001
    80005910:	08e7a223          	sw	a4,132(a5) # 10001084 <_entry-0x6fffef7c>
  *R(VIRTIO_MMIO_DRIVER_DESC_LOW) = (uint64)disk.avail;
    80005914:	649c                	ld	a5,8(s1)
    80005916:	0007869b          	sext.w	a3,a5
    8000591a:	10001737          	lui	a4,0x10001
    8000591e:	08d72823          	sw	a3,144(a4) # 10001090 <_entry-0x6fffef70>
  *R(VIRTIO_MMIO_DRIVER_DESC_HIGH) = (uint64)disk.avail >> 32;
    80005922:	9781                	srai	a5,a5,0x20
    80005924:	10001737          	lui	a4,0x10001
    80005928:	08f72a23          	sw	a5,148(a4) # 10001094 <_entry-0x6fffef6c>
  *R(VIRTIO_MMIO_DEVICE_DESC_LOW) = (uint64)disk.used;
    8000592c:	689c                	ld	a5,16(s1)
    8000592e:	0007869b          	sext.w	a3,a5
    80005932:	10001737          	lui	a4,0x10001
    80005936:	0ad72023          	sw	a3,160(a4) # 100010a0 <_entry-0x6fffef60>
  *R(VIRTIO_MMIO_DEVICE_DESC_HIGH) = (uint64)disk.used >> 32;
    8000593a:	9781                	srai	a5,a5,0x20
    8000593c:	10001737          	lui	a4,0x10001
    80005940:	0af72223          	sw	a5,164(a4) # 100010a4 <_entry-0x6fffef5c>
  *R(VIRTIO_MMIO_QUEUE_READY) = 0x1;
    80005944:	10001737          	lui	a4,0x10001
    80005948:	4785                	li	a5,1
    8000594a:	c37c                	sw	a5,68(a4)
    disk.free[i] = 1;
    8000594c:	00f48c23          	sb	a5,24(s1)
    80005950:	00f48ca3          	sb	a5,25(s1)
    80005954:	00f48d23          	sb	a5,26(s1)
    80005958:	00f48da3          	sb	a5,27(s1)
    8000595c:	00f48e23          	sb	a5,28(s1)
    80005960:	00f48ea3          	sb	a5,29(s1)
    80005964:	00f48f23          	sb	a5,30(s1)
    80005968:	00f48fa3          	sb	a5,31(s1)
  status |= VIRTIO_CONFIG_S_DRIVER_OK;
    8000596c:	00496913          	ori	s2,s2,4
  *R(VIRTIO_MMIO_STATUS) = status;
    80005970:	100017b7          	lui	a5,0x10001
    80005974:	0727a823          	sw	s2,112(a5) # 10001070 <_entry-0x6fffef90>
}
    80005978:	60e2                	ld	ra,24(sp)
    8000597a:	6442                	ld	s0,16(sp)
    8000597c:	64a2                	ld	s1,8(sp)
    8000597e:	6902                	ld	s2,0(sp)
    80005980:	6105                	addi	sp,sp,32
    80005982:	8082                	ret
    panic("could not find virtio disk");
    80005984:	00002517          	auipc	a0,0x2
    80005988:	cc450513          	addi	a0,a0,-828 # 80007648 <etext+0x648>
    8000598c:	e55fa0ef          	jal	800007e0 <panic>
    panic("virtio disk FEATURES_OK unset");
    80005990:	00002517          	auipc	a0,0x2
    80005994:	cd850513          	addi	a0,a0,-808 # 80007668 <etext+0x668>
    80005998:	e49fa0ef          	jal	800007e0 <panic>
    panic("virtio disk should not be ready");
    8000599c:	00002517          	auipc	a0,0x2
    800059a0:	cec50513          	addi	a0,a0,-788 # 80007688 <etext+0x688>
    800059a4:	e3dfa0ef          	jal	800007e0 <panic>
    panic("virtio disk has no queue 0");
    800059a8:	00002517          	auipc	a0,0x2
    800059ac:	d0050513          	addi	a0,a0,-768 # 800076a8 <etext+0x6a8>
    800059b0:	e31fa0ef          	jal	800007e0 <panic>
    panic("virtio disk max queue too short");
    800059b4:	00002517          	auipc	a0,0x2
    800059b8:	d1450513          	addi	a0,a0,-748 # 800076c8 <etext+0x6c8>
    800059bc:	e25fa0ef          	jal	800007e0 <panic>
    panic("virtio disk kalloc");
    800059c0:	00002517          	auipc	a0,0x2
    800059c4:	d2850513          	addi	a0,a0,-728 # 800076e8 <etext+0x6e8>
    800059c8:	e19fa0ef          	jal	800007e0 <panic>

00000000800059cc <virtio_disk_rw>:
  return 0;
}

void
virtio_disk_rw(struct buf *b, int write)
{
    800059cc:	7159                	addi	sp,sp,-112
    800059ce:	f486                	sd	ra,104(sp)
    800059d0:	f0a2                	sd	s0,96(sp)
    800059d2:	eca6                	sd	s1,88(sp)
    800059d4:	e8ca                	sd	s2,80(sp)
    800059d6:	e4ce                	sd	s3,72(sp)
    800059d8:	e0d2                	sd	s4,64(sp)
    800059da:	fc56                	sd	s5,56(sp)
    800059dc:	f85a                	sd	s6,48(sp)
    800059de:	f45e                	sd	s7,40(sp)
    800059e0:	f062                	sd	s8,32(sp)
    800059e2:	ec66                	sd	s9,24(sp)
    800059e4:	1880                	addi	s0,sp,112
    800059e6:	8a2a                	mv	s4,a0
    800059e8:	8bae                	mv	s7,a1
  uint64 sector = b->blockno * (BSIZE / 512);
    800059ea:	00c52c83          	lw	s9,12(a0)
    800059ee:	001c9c9b          	slliw	s9,s9,0x1
    800059f2:	1c82                	slli	s9,s9,0x20
    800059f4:	020cdc93          	srli	s9,s9,0x20

  acquire(&disk.vdisk_lock);
    800059f8:	0001e517          	auipc	a0,0x1e
    800059fc:	58050513          	addi	a0,a0,1408 # 80023f78 <disk+0x128>
    80005a00:	9cefb0ef          	jal	80000bce <acquire>
  for(int i = 0; i < 3; i++){
    80005a04:	4981                	li	s3,0
  for(int i = 0; i < NUM; i++){
    80005a06:	44a1                	li	s1,8
      disk.free[i] = 0;
    80005a08:	0001eb17          	auipc	s6,0x1e
    80005a0c:	448b0b13          	addi	s6,s6,1096 # 80023e50 <disk>
  for(int i = 0; i < 3; i++){
    80005a10:	4a8d                	li	s5,3
  int idx[3];
  while(1){
    if(alloc3_desc(idx) == 0) {
      break;
    }
    sleep(&disk.free[0], &disk.vdisk_lock);
    80005a12:	0001ec17          	auipc	s8,0x1e
    80005a16:	566c0c13          	addi	s8,s8,1382 # 80023f78 <disk+0x128>
    80005a1a:	a8b9                	j	80005a78 <virtio_disk_rw+0xac>
      disk.free[i] = 0;
    80005a1c:	00fb0733          	add	a4,s6,a5
    80005a20:	00070c23          	sb	zero,24(a4) # 10001018 <_entry-0x6fffefe8>
    idx[i] = alloc_desc();
    80005a24:	c19c                	sw	a5,0(a1)
    if(idx[i] < 0){
    80005a26:	0207c563          	bltz	a5,80005a50 <virtio_disk_rw+0x84>
  for(int i = 0; i < 3; i++){
    80005a2a:	2905                	addiw	s2,s2,1
    80005a2c:	0611                	addi	a2,a2,4 # 1004 <_entry-0x7fffeffc>
    80005a2e:	05590963          	beq	s2,s5,80005a80 <virtio_disk_rw+0xb4>
    idx[i] = alloc_desc();
    80005a32:	85b2                	mv	a1,a2
  for(int i = 0; i < NUM; i++){
    80005a34:	0001e717          	auipc	a4,0x1e
    80005a38:	41c70713          	addi	a4,a4,1052 # 80023e50 <disk>
    80005a3c:	87ce                	mv	a5,s3
    if(disk.free[i]){
    80005a3e:	01874683          	lbu	a3,24(a4)
    80005a42:	fee9                	bnez	a3,80005a1c <virtio_disk_rw+0x50>
  for(int i = 0; i < NUM; i++){
    80005a44:	2785                	addiw	a5,a5,1
    80005a46:	0705                	addi	a4,a4,1
    80005a48:	fe979be3          	bne	a5,s1,80005a3e <virtio_disk_rw+0x72>
    idx[i] = alloc_desc();
    80005a4c:	57fd                	li	a5,-1
    80005a4e:	c19c                	sw	a5,0(a1)
      for(int j = 0; j < i; j++)
    80005a50:	01205d63          	blez	s2,80005a6a <virtio_disk_rw+0x9e>
        free_desc(idx[j]);
    80005a54:	f9042503          	lw	a0,-112(s0)
    80005a58:	d07ff0ef          	jal	8000575e <free_desc>
      for(int j = 0; j < i; j++)
    80005a5c:	4785                	li	a5,1
    80005a5e:	0127d663          	bge	a5,s2,80005a6a <virtio_disk_rw+0x9e>
        free_desc(idx[j]);
    80005a62:	f9442503          	lw	a0,-108(s0)
    80005a66:	cf9ff0ef          	jal	8000575e <free_desc>
    sleep(&disk.free[0], &disk.vdisk_lock);
    80005a6a:	85e2                	mv	a1,s8
    80005a6c:	0001e517          	auipc	a0,0x1e
    80005a70:	3fc50513          	addi	a0,a0,1020 # 80023e68 <disk+0x18>
    80005a74:	c70fc0ef          	jal	80001ee4 <sleep>
  for(int i = 0; i < 3; i++){
    80005a78:	f9040613          	addi	a2,s0,-112
    80005a7c:	894e                	mv	s2,s3
    80005a7e:	bf55                	j	80005a32 <virtio_disk_rw+0x66>
  }

  // format the three descriptors.
  // qemu's virtio-blk.c reads them.

  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    80005a80:	f9042503          	lw	a0,-112(s0)
    80005a84:	00451693          	slli	a3,a0,0x4

  if(write)
    80005a88:	0001e797          	auipc	a5,0x1e
    80005a8c:	3c878793          	addi	a5,a5,968 # 80023e50 <disk>
    80005a90:	00a50713          	addi	a4,a0,10
    80005a94:	0712                	slli	a4,a4,0x4
    80005a96:	973e                	add	a4,a4,a5
    80005a98:	01703633          	snez	a2,s7
    80005a9c:	c710                	sw	a2,8(a4)
    buf0->type = VIRTIO_BLK_T_OUT; // write the disk
  else
    buf0->type = VIRTIO_BLK_T_IN; // read the disk
  buf0->reserved = 0;
    80005a9e:	00072623          	sw	zero,12(a4)
  buf0->sector = sector;
    80005aa2:	01973823          	sd	s9,16(a4)

  disk.desc[idx[0]].addr = (uint64) buf0;
    80005aa6:	6398                	ld	a4,0(a5)
    80005aa8:	9736                	add	a4,a4,a3
  struct virtio_blk_req *buf0 = &disk.ops[idx[0]];
    80005aaa:	0a868613          	addi	a2,a3,168
    80005aae:	963e                	add	a2,a2,a5
  disk.desc[idx[0]].addr = (uint64) buf0;
    80005ab0:	e310                	sd	a2,0(a4)
  disk.desc[idx[0]].len = sizeof(struct virtio_blk_req);
    80005ab2:	6390                	ld	a2,0(a5)
    80005ab4:	00d605b3          	add	a1,a2,a3
    80005ab8:	4741                	li	a4,16
    80005aba:	c598                	sw	a4,8(a1)
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
    80005abc:	4805                	li	a6,1
    80005abe:	01059623          	sh	a6,12(a1)
  disk.desc[idx[0]].next = idx[1];
    80005ac2:	f9442703          	lw	a4,-108(s0)
    80005ac6:	00e59723          	sh	a4,14(a1)

  disk.desc[idx[1]].addr = (uint64) b->data;
    80005aca:	0712                	slli	a4,a4,0x4
    80005acc:	963a                	add	a2,a2,a4
    80005ace:	058a0593          	addi	a1,s4,88
    80005ad2:	e20c                	sd	a1,0(a2)
  disk.desc[idx[1]].len = BSIZE;
    80005ad4:	0007b883          	ld	a7,0(a5)
    80005ad8:	9746                	add	a4,a4,a7
    80005ada:	40000613          	li	a2,1024
    80005ade:	c710                	sw	a2,8(a4)
  if(write)
    80005ae0:	001bb613          	seqz	a2,s7
    80005ae4:	0016161b          	slliw	a2,a2,0x1
    disk.desc[idx[1]].flags = 0; // device reads b->data
  else
    disk.desc[idx[1]].flags = VRING_DESC_F_WRITE; // device writes b->data
  disk.desc[idx[1]].flags |= VRING_DESC_F_NEXT;
    80005ae8:	00166613          	ori	a2,a2,1
    80005aec:	00c71623          	sh	a2,12(a4)
  disk.desc[idx[1]].next = idx[2];
    80005af0:	f9842583          	lw	a1,-104(s0)
    80005af4:	00b71723          	sh	a1,14(a4)

  disk.info[idx[0]].status = 0xff; // device writes 0 on success
    80005af8:	00250613          	addi	a2,a0,2
    80005afc:	0612                	slli	a2,a2,0x4
    80005afe:	963e                	add	a2,a2,a5
    80005b00:	577d                	li	a4,-1
    80005b02:	00e60823          	sb	a4,16(a2)
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status;
    80005b06:	0592                	slli	a1,a1,0x4
    80005b08:	98ae                	add	a7,a7,a1
    80005b0a:	03068713          	addi	a4,a3,48
    80005b0e:	973e                	add	a4,a4,a5
    80005b10:	00e8b023          	sd	a4,0(a7)
  disk.desc[idx[2]].len = 1;
    80005b14:	6398                	ld	a4,0(a5)
    80005b16:	972e                	add	a4,a4,a1
    80005b18:	01072423          	sw	a6,8(a4)
  disk.desc[idx[2]].flags = VRING_DESC_F_WRITE; // device writes the status
    80005b1c:	4689                	li	a3,2
    80005b1e:	00d71623          	sh	a3,12(a4)
  disk.desc[idx[2]].next = 0;
    80005b22:	00071723          	sh	zero,14(a4)

  // record struct buf for virtio_disk_intr().
  b->disk = 1;
    80005b26:	010a2223          	sw	a6,4(s4)
  disk.info[idx[0]].b = b;
    80005b2a:	01463423          	sd	s4,8(a2)

  // tell the device the first index in our chain of descriptors.
  disk.avail->ring[disk.avail->idx % NUM] = idx[0];
    80005b2e:	6794                	ld	a3,8(a5)
    80005b30:	0026d703          	lhu	a4,2(a3)
    80005b34:	8b1d                	andi	a4,a4,7
    80005b36:	0706                	slli	a4,a4,0x1
    80005b38:	96ba                	add	a3,a3,a4
    80005b3a:	00a69223          	sh	a0,4(a3)

  __sync_synchronize();
    80005b3e:	0330000f          	fence	rw,rw

  // tell the device another avail ring entry is available.
  disk.avail->idx += 1; // not % NUM ...
    80005b42:	6798                	ld	a4,8(a5)
    80005b44:	00275783          	lhu	a5,2(a4)
    80005b48:	2785                	addiw	a5,a5,1
    80005b4a:	00f71123          	sh	a5,2(a4)

  __sync_synchronize();
    80005b4e:	0330000f          	fence	rw,rw

  *R(VIRTIO_MMIO_QUEUE_NOTIFY) = 0; // value is queue number
    80005b52:	100017b7          	lui	a5,0x10001
    80005b56:	0407a823          	sw	zero,80(a5) # 10001050 <_entry-0x6fffefb0>

  // Wait for virtio_disk_intr() to say request has finished.
  while(b->disk == 1) {
    80005b5a:	004a2783          	lw	a5,4(s4)
    sleep(b, &disk.vdisk_lock);
    80005b5e:	0001e917          	auipc	s2,0x1e
    80005b62:	41a90913          	addi	s2,s2,1050 # 80023f78 <disk+0x128>
  while(b->disk == 1) {
    80005b66:	4485                	li	s1,1
    80005b68:	01079a63          	bne	a5,a6,80005b7c <virtio_disk_rw+0x1b0>
    sleep(b, &disk.vdisk_lock);
    80005b6c:	85ca                	mv	a1,s2
    80005b6e:	8552                	mv	a0,s4
    80005b70:	b74fc0ef          	jal	80001ee4 <sleep>
  while(b->disk == 1) {
    80005b74:	004a2783          	lw	a5,4(s4)
    80005b78:	fe978ae3          	beq	a5,s1,80005b6c <virtio_disk_rw+0x1a0>
  }

  disk.info[idx[0]].b = 0;
    80005b7c:	f9042903          	lw	s2,-112(s0)
    80005b80:	00290713          	addi	a4,s2,2
    80005b84:	0712                	slli	a4,a4,0x4
    80005b86:	0001e797          	auipc	a5,0x1e
    80005b8a:	2ca78793          	addi	a5,a5,714 # 80023e50 <disk>
    80005b8e:	97ba                	add	a5,a5,a4
    80005b90:	0007b423          	sd	zero,8(a5)
    int flag = disk.desc[i].flags;
    80005b94:	0001e997          	auipc	s3,0x1e
    80005b98:	2bc98993          	addi	s3,s3,700 # 80023e50 <disk>
    80005b9c:	00491713          	slli	a4,s2,0x4
    80005ba0:	0009b783          	ld	a5,0(s3)
    80005ba4:	97ba                	add	a5,a5,a4
    80005ba6:	00c7d483          	lhu	s1,12(a5)
    int nxt = disk.desc[i].next;
    80005baa:	854a                	mv	a0,s2
    80005bac:	00e7d903          	lhu	s2,14(a5)
    free_desc(i);
    80005bb0:	bafff0ef          	jal	8000575e <free_desc>
    if(flag & VRING_DESC_F_NEXT)
    80005bb4:	8885                	andi	s1,s1,1
    80005bb6:	f0fd                	bnez	s1,80005b9c <virtio_disk_rw+0x1d0>
  free_chain(idx[0]);

  release(&disk.vdisk_lock);
    80005bb8:	0001e517          	auipc	a0,0x1e
    80005bbc:	3c050513          	addi	a0,a0,960 # 80023f78 <disk+0x128>
    80005bc0:	8a6fb0ef          	jal	80000c66 <release>
}
    80005bc4:	70a6                	ld	ra,104(sp)
    80005bc6:	7406                	ld	s0,96(sp)
    80005bc8:	64e6                	ld	s1,88(sp)
    80005bca:	6946                	ld	s2,80(sp)
    80005bcc:	69a6                	ld	s3,72(sp)
    80005bce:	6a06                	ld	s4,64(sp)
    80005bd0:	7ae2                	ld	s5,56(sp)
    80005bd2:	7b42                	ld	s6,48(sp)
    80005bd4:	7ba2                	ld	s7,40(sp)
    80005bd6:	7c02                	ld	s8,32(sp)
    80005bd8:	6ce2                	ld	s9,24(sp)
    80005bda:	6165                	addi	sp,sp,112
    80005bdc:	8082                	ret

0000000080005bde <virtio_disk_intr>:

void
virtio_disk_intr()
{
    80005bde:	1101                	addi	sp,sp,-32
    80005be0:	ec06                	sd	ra,24(sp)
    80005be2:	e822                	sd	s0,16(sp)
    80005be4:	e426                	sd	s1,8(sp)
    80005be6:	1000                	addi	s0,sp,32
  acquire(&disk.vdisk_lock);
    80005be8:	0001e497          	auipc	s1,0x1e
    80005bec:	26848493          	addi	s1,s1,616 # 80023e50 <disk>
    80005bf0:	0001e517          	auipc	a0,0x1e
    80005bf4:	38850513          	addi	a0,a0,904 # 80023f78 <disk+0x128>
    80005bf8:	fd7fa0ef          	jal	80000bce <acquire>
  // we've seen this interrupt, which the following line does.
  // this may race with the device writing new entries to
  // the "used" ring, in which case we may process the new
  // completion entries in this interrupt, and have nothing to do
  // in the next interrupt, which is harmless.
  *R(VIRTIO_MMIO_INTERRUPT_ACK) = *R(VIRTIO_MMIO_INTERRUPT_STATUS) & 0x3;
    80005bfc:	100017b7          	lui	a5,0x10001
    80005c00:	53b8                	lw	a4,96(a5)
    80005c02:	8b0d                	andi	a4,a4,3
    80005c04:	100017b7          	lui	a5,0x10001
    80005c08:	d3f8                	sw	a4,100(a5)

  __sync_synchronize();
    80005c0a:	0330000f          	fence	rw,rw

  // the device increments disk.used->idx when it
  // adds an entry to the used ring.

  while(disk.used_idx != disk.used->idx){
    80005c0e:	689c                	ld	a5,16(s1)
    80005c10:	0204d703          	lhu	a4,32(s1)
    80005c14:	0027d783          	lhu	a5,2(a5) # 10001002 <_entry-0x6fffeffe>
    80005c18:	04f70663          	beq	a4,a5,80005c64 <virtio_disk_intr+0x86>
    __sync_synchronize();
    80005c1c:	0330000f          	fence	rw,rw
    int id = disk.used->ring[disk.used_idx % NUM].id;
    80005c20:	6898                	ld	a4,16(s1)
    80005c22:	0204d783          	lhu	a5,32(s1)
    80005c26:	8b9d                	andi	a5,a5,7
    80005c28:	078e                	slli	a5,a5,0x3
    80005c2a:	97ba                	add	a5,a5,a4
    80005c2c:	43dc                	lw	a5,4(a5)

    if(disk.info[id].status != 0)
    80005c2e:	00278713          	addi	a4,a5,2
    80005c32:	0712                	slli	a4,a4,0x4
    80005c34:	9726                	add	a4,a4,s1
    80005c36:	01074703          	lbu	a4,16(a4)
    80005c3a:	e321                	bnez	a4,80005c7a <virtio_disk_intr+0x9c>
      panic("virtio_disk_intr status");

    struct buf *b = disk.info[id].b;
    80005c3c:	0789                	addi	a5,a5,2
    80005c3e:	0792                	slli	a5,a5,0x4
    80005c40:	97a6                	add	a5,a5,s1
    80005c42:	6788                	ld	a0,8(a5)
    b->disk = 0;   // disk is done with buf
    80005c44:	00052223          	sw	zero,4(a0)
    wakeup(b);
    80005c48:	ae8fc0ef          	jal	80001f30 <wakeup>

    disk.used_idx += 1;
    80005c4c:	0204d783          	lhu	a5,32(s1)
    80005c50:	2785                	addiw	a5,a5,1
    80005c52:	17c2                	slli	a5,a5,0x30
    80005c54:	93c1                	srli	a5,a5,0x30
    80005c56:	02f49023          	sh	a5,32(s1)
  while(disk.used_idx != disk.used->idx){
    80005c5a:	6898                	ld	a4,16(s1)
    80005c5c:	00275703          	lhu	a4,2(a4)
    80005c60:	faf71ee3          	bne	a4,a5,80005c1c <virtio_disk_intr+0x3e>
  }

  release(&disk.vdisk_lock);
    80005c64:	0001e517          	auipc	a0,0x1e
    80005c68:	31450513          	addi	a0,a0,788 # 80023f78 <disk+0x128>
    80005c6c:	ffbfa0ef          	jal	80000c66 <release>
}
    80005c70:	60e2                	ld	ra,24(sp)
    80005c72:	6442                	ld	s0,16(sp)
    80005c74:	64a2                	ld	s1,8(sp)
    80005c76:	6105                	addi	sp,sp,32
    80005c78:	8082                	ret
      panic("virtio_disk_intr status");
    80005c7a:	00002517          	auipc	a0,0x2
    80005c7e:	a8650513          	addi	a0,a0,-1402 # 80007700 <etext+0x700>
    80005c82:	b5ffa0ef          	jal	800007e0 <panic>
	...

0000000080006000 <_trampoline>:
    80006000:	14051073          	csrw	sscratch,a0
    80006004:	02000537          	lui	a0,0x2000
    80006008:	357d                	addiw	a0,a0,-1 # 1ffffff <_entry-0x7e000001>
    8000600a:	0536                	slli	a0,a0,0xd
    8000600c:	02153423          	sd	ra,40(a0)
    80006010:	02253823          	sd	sp,48(a0)
    80006014:	02353c23          	sd	gp,56(a0)
    80006018:	04453023          	sd	tp,64(a0)
    8000601c:	04553423          	sd	t0,72(a0)
    80006020:	04653823          	sd	t1,80(a0)
    80006024:	04753c23          	sd	t2,88(a0)
    80006028:	f120                	sd	s0,96(a0)
    8000602a:	f524                	sd	s1,104(a0)
    8000602c:	fd2c                	sd	a1,120(a0)
    8000602e:	e150                	sd	a2,128(a0)
    80006030:	e554                	sd	a3,136(a0)
    80006032:	e958                	sd	a4,144(a0)
    80006034:	ed5c                	sd	a5,152(a0)
    80006036:	0b053023          	sd	a6,160(a0)
    8000603a:	0b153423          	sd	a7,168(a0)
    8000603e:	0b253823          	sd	s2,176(a0)
    80006042:	0b353c23          	sd	s3,184(a0)
    80006046:	0d453023          	sd	s4,192(a0)
    8000604a:	0d553423          	sd	s5,200(a0)
    8000604e:	0d653823          	sd	s6,208(a0)
    80006052:	0d753c23          	sd	s7,216(a0)
    80006056:	0f853023          	sd	s8,224(a0)
    8000605a:	0f953423          	sd	s9,232(a0)
    8000605e:	0fa53823          	sd	s10,240(a0)
    80006062:	0fb53c23          	sd	s11,248(a0)
    80006066:	11c53023          	sd	t3,256(a0)
    8000606a:	11d53423          	sd	t4,264(a0)
    8000606e:	11e53823          	sd	t5,272(a0)
    80006072:	11f53c23          	sd	t6,280(a0)
    80006076:	140022f3          	csrr	t0,sscratch
    8000607a:	06553823          	sd	t0,112(a0)
    8000607e:	00853103          	ld	sp,8(a0)
    80006082:	02053203          	ld	tp,32(a0)
    80006086:	01053283          	ld	t0,16(a0)
    8000608a:	00053303          	ld	t1,0(a0)
    8000608e:	12000073          	sfence.vma
    80006092:	18031073          	csrw	satp,t1
    80006096:	12000073          	sfence.vma
    8000609a:	9282                	jalr	t0

000000008000609c <userret>:
    8000609c:	12000073          	sfence.vma
    800060a0:	18051073          	csrw	satp,a0
    800060a4:	12000073          	sfence.vma
    800060a8:	02000537          	lui	a0,0x2000
    800060ac:	357d                	addiw	a0,a0,-1 # 1ffffff <_entry-0x7e000001>
    800060ae:	0536                	slli	a0,a0,0xd
    800060b0:	02853083          	ld	ra,40(a0)
    800060b4:	03053103          	ld	sp,48(a0)
    800060b8:	03853183          	ld	gp,56(a0)
    800060bc:	04053203          	ld	tp,64(a0)
    800060c0:	04853283          	ld	t0,72(a0)
    800060c4:	05053303          	ld	t1,80(a0)
    800060c8:	05853383          	ld	t2,88(a0)
    800060cc:	7120                	ld	s0,96(a0)
    800060ce:	7524                	ld	s1,104(a0)
    800060d0:	7d2c                	ld	a1,120(a0)
    800060d2:	6150                	ld	a2,128(a0)
    800060d4:	6554                	ld	a3,136(a0)
    800060d6:	6958                	ld	a4,144(a0)
    800060d8:	6d5c                	ld	a5,152(a0)
    800060da:	0a053803          	ld	a6,160(a0)
    800060de:	0a853883          	ld	a7,168(a0)
    800060e2:	0b053903          	ld	s2,176(a0)
    800060e6:	0b853983          	ld	s3,184(a0)
    800060ea:	0c053a03          	ld	s4,192(a0)
    800060ee:	0c853a83          	ld	s5,200(a0)
    800060f2:	0d053b03          	ld	s6,208(a0)
    800060f6:	0d853b83          	ld	s7,216(a0)
    800060fa:	0e053c03          	ld	s8,224(a0)
    800060fe:	0e853c83          	ld	s9,232(a0)
    80006102:	0f053d03          	ld	s10,240(a0)
    80006106:	0f853d83          	ld	s11,248(a0)
    8000610a:	10053e03          	ld	t3,256(a0)
    8000610e:	10853e83          	ld	t4,264(a0)
    80006112:	11053f03          	ld	t5,272(a0)
    80006116:	11853f83          	ld	t6,280(a0)
    8000611a:	7928                	ld	a0,112(a0)
    8000611c:	10200073          	sret
	...
